...
def start(argv):
    ...
    set_env("COLLECTOR_RUNSERVER_PORT", str(server_port))
    os.environ["RECIPE_PORT"] = str(server_port)

    binary_path = yatest.common.binary_path("payplatform/fes/collector/cmd/runserver/runserver")
    logging.info('Binary path: ' + binary_path)
    execution = yatest.common.execute(
        [binary_path, "-c", source_path() + "/payplatform/fes/collector/config/dev.yaml"],
        stdout=open(yatest.common.output_path() + '/runserver.out', 'w'),
        stderr=open(yatest.common.output_path() + '/runserver.err', 'w'),
        wait=False,
    )

    with open("collector_runserver.pid", "w") as f:
        f.write(str(execution.process.pid))


def stop(argv):
    with open("collector_runserver.pid") as f:
        pid = f.read()
