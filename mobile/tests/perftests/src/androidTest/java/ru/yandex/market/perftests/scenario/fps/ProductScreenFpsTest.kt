...
import ru.yandex.market.perftests.dsl.getProductRootRecycler
...
@PerfTest(
    ...
    description = "ProductScreenFpsTest",
    timeoutSeconds = Constants.LONG_TIMEOUT,
    owners = [Owners.DMTRIY_DOGAEV]
)
@RunWith(PerfTestJUnit4Runner::class)
class ProductScreenFpsTest : MarketBasePerfTest() {

    override fun scenario() {
        startMainActivity()
        waitMediaCarousel()
        findProductImageInCmsRootRecycler().clickAndWaitForNewWindow()

        waitProductRootRecycler()

        waitBeforeMeasure()

        val recycler = getProductRootRecycler()
        repeat(SCROLL_COUNT) {
            recycler.scrollForward()
        }
        waitFrameMetrics("product_screen_rv")
    }
