//
//  DeleteBackupPresenterTest.swift
//  BackupTests
//
...
internal final class DeleteBackupPresenterTest: XCTestCase {
    func testStartAndRerender() {
        let render = MockRender()
        let networkPerformer = MockNetworkPerformer()
        let router = MockRouter()
        let delegate = Delegate()
        let presenter = DeleteBackupPresenter(render: render, networkPerformer: networkPerformer, router: router)
    ...
    func testCorrectOverriding() {
        ...
        XCTAssertEqual(router.error, .deleteBackup)
    ...
    func testDealocating() {
        ...
        let router = MockRouter()
        let delegate = Delegate()
        var presenter: DeleteBackupPresenter? = DeleteBackupPresenter(render: render, networkPerformer: networkPerformer, router: router)
        presenter?.delegate = delegate
        presenter?.start()
...
private extension DeleteBackupPresenterTest {
    final class MockRouter: DeleteBackupRouting {
    ...
    final class Delegate: DeleteBackupPresenterDelegate {
        var result: Bool?

        func deleteBackupPresenter(_ deleteBackupPresenter: DeleteBackupPresenter, didFinishDeletingWithResult result: Bool) {
            self.result = result
        }
    }

    final class MockNetworkPerformer: DeleteBackupNetworkPerformer {
        ...
        func deleteBackup(completion: @escaping (Result<Void>) -> Void) {
