...
import static com.yandex.mail.util.OperationsConst.ForMessageView.SECOND_FAKE_TEST_MESSAGE_IN_INBOX_IMPORTANT;
import static com.yandex.mail.util.OperationsConst.ForMessageView.SECOND_FAKE_TEST_MESSAGE_IN_INBOX_USER_LABEL;
import static kotlin.collections.CollectionsKt.listOf;

public class MessageViewToastsFakeDataRule extends FakeAccountRule {

    public MessageViewToastsFakeDataRule(@NonNull String login) {
        super(login);
    }
    ...
    public void initialize(@NonNull AccountWrapper account) {
        ...
        account.addMessages(
                account.newReadMessage(account.getInboxFolder())
                        ...
                        .build(),
                account.newReadMessage(account.getInboxFolder())
                        .subjText(FAKE_TEST_MESSAGE_IN_INBOX_IMPORTANT)
                        .labels(listOf(account.getImportantLabel()))
                        .build(),
                account.newReadMessage(account.getInboxFolder())
                        .subjText(FAKE_TEST_MESSAGE_IN_INBOX_USER_LABEL)
                        .labels(listOf(userLabel))
                        .build(),
                account.newReadMessage(account.getInboxFolder())
                        .subjText(SECOND_FAKE_TEST_MESSAGE_IN_INBOX_IMPORTANT)
                        .labels(listOf(account.getImportantLabel()))
                        .build(),
                account.newReadMessage(account.getInboxFolder())
                        .subjText(SECOND_FAKE_TEST_MESSAGE_IN_INBOX_USER_LABEL)
                        .labels(listOf(userLabel))
                        .build(),
                account.newReadMessage(account.getInboxFolder())
