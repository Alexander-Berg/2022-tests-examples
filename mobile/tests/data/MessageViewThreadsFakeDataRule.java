package com.yandex.mail.tests.data;

import com.yandex.mail.fakeserver.AccountWrapper;
import com.yandex.mail.rules.FakeAccountRule;

import androidx.annotation.NonNull;

import static com.yandex.mail.util.FoldersAndLabelsConst.LABEL_NAME;
import static com.yandex.mail.util.FoldersAndLabelsConst.USERS_FOLDER_MEDUZA;
import static com.yandex.mail.util.OperationsConst.ForMessageView.FAKE_TEST_THREAD_IN_INBOX;
import static com.yandex.mail.util.OperationsConst.ForMessageView.MESSAGES_NUMBER_IN_THREAD;
import static com.yandex.mail.util.OperationsConst.ForMessageView.SECOND_FAKE_TEST_THREAD_IN_INBOX;
import static kotlin.collections.CollectionsKt.map;
import static kotlin.ranges.RangesKt.until;

public class MessageViewThreadsFakeDataRule extends FakeAccountRule {

    public MessageViewThreadsFakeDataRule(@NonNull String login) {
        super(login);
    }

    @Override
    public void initialize(@NonNull AccountWrapper account) {
        account.addThreads(
                account.newThread(
                        map(until(0, MESSAGES_NUMBER_IN_THREAD),
                                i -> account
                                        .newUnreadMessage(account.getInboxFolder())
                                        .subjText(FAKE_TEST_THREAD_IN_INBOX)
                        )
                ).build(),
                account.newThread(
                        map(until(0, MESSAGES_NUMBER_IN_THREAD),
                                i -> account
                                        .newUnreadMessage(account.getInboxFolder())
                                        .subjText(SECOND_FAKE_TEST_THREAD_IN_INBOX)
                        )
                ).build()
        );

        account.addFolders(account.newFolder(USERS_FOLDER_MEDUZA).build());
        account.addLabels(account.newLabel(LABEL_NAME).build());
    }
}
