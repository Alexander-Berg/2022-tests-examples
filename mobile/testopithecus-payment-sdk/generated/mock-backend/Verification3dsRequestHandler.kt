...
public open class Verification3dsRequestHandler(private val trustModel: MockTrustModel): HttpRequestHandler {
    open override fun handleRequest(request: HttpRequest): HttpResponse {
        if (request.url.startsWith("/web/redirect_3ds")) {
            ...
            val purchaseToken = url!!.getQueryParameter("purchase_token")
            val code = url!!.getQueryParameter("3ds_code")
            if (code == null || purchaseToken == null) {
                return HttpResponseBuilder().setCode(500).build()
            }
            val result = this.trustModel.provide3ds(purchaseToken, code)
