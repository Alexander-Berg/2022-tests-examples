...
public class ListItemMoveUpTriggerTest {
    ...
    private static final int MOCK_POSITION = 1;

    private static final int MOCK_POSITION_DESTINATION = 5;
    ...
    @Test
    public void testListPositionOrderOnMoveUp() throws IOException {
    ...
    @Test
    public void testFirstToLast() throws IOException {
        fillDatabase(databaseManager, MOCK_CONTEXT, MOCK_DATABASE_ID, "database_snapshot.json");

        final long internalFieldId = getInternalFieldId();
        final long parentId = getParentId(internalFieldId);

        final int moveFrom = 0;
        final int moveTo = getSize(internalFieldId, parentId) - 1;
        final String expectedValue = getValue(internalFieldId, parentId, moveFrom);
