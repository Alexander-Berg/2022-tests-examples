...
import com.yandex.datasync.internal.operation.local.LocalUpdateTitleOperation;
...
public class RemoteUpdateTitleOperationTest {
    ...
    private static final String MOCK_DATABASE_TITLE = "title";
    ...
    public void testRun() throws IOException {
        ...
        final Operation localOperation = new LocalUpdateTitleOperation(MOCK_CONTEXT,
                                                                       MOCK_DATABASE_ID,
                                                                       NEW_MOCK_DATABASE_TITLE,
                                                                       databaseManager);
        localOperation.run();

        final Operation apiOperation = new RemoteUpdateTitleOperation(MOCK_CONTEXT,
                                                                      api,
                                                                      databaseManager,
                                                                      observable);
        apiOperation.run();

        final String selection = DatabaseDescriptor.Databases.Rows.TITLE + "=?";
        String selectionArgs[] = {NEW_MOCK_DATABASE_TITLE};

        try (final Cursor cursor = databaseWrapper.query(TABLE_NAME, selection, selectionArgs)) {
            assertThat(cursor.getCount(), is(1));
            cursor.moveToFirst();
            final boolean synced = cursor.isNull(cursor.getColumnIndex(SYNCED));
            assertTrue(synced);
        }

        selectionArgs = new String[]{MOCK_DATABASE_TITLE};
