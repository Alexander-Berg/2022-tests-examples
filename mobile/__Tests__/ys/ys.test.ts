...
import { objectToMap } from '../../../common/__tests__/__helpers__/utils'
...
describe(floorDouble, () => {
  it('should round down Double', () => {
    expect(floorDouble(1.1)).toBe(1.0)
  })
})

describe(randomDouble, () => {
  it('should return pseudorandom number between 0 and 1', () => {
    expect(randomDouble()).toBeGreaterThanOrEqual(0)
    expect(randomDouble()).toBeLessThan(1)
  })
})
...
describe(iterableToArray, () => {
  it('should convert Iterable to Array', () => {
    expect(iterableToArray(objectToMap({ a: 1, b: 2 }).keys())).toStrictEqual(['a', 'b'])
    expect(iterableToArray(objectToMap({}).values())).toStrictEqual([])
  })
})

describe(iterableToSet, () => {
  it('should convert Iterable to Set', () => {
    expect(iterableToSet(objectToMap({ a: 1, b: 2 }).keys())).toStrictEqual(new Set(['a', 'b']))
    expect(iterableToSet(objectToMap({}).values())).toStrictEqual(new Set())
  })
})
