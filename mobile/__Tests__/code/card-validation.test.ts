import { CardPaymentSystem } from '../../code/busilogics/card-payment-system'
import {
  CardBinRange,
  CardBinRangeBuilder,
  CardValidationError,
  ...
  EndlessCardExpirationDateValidator,
  ...
  describe('default validators', () => {
    ...
    describe(LengthCardNumberValidator, () => {
      ...
      it('should create with CardType', () => {
        const validator = LengthCardNumberValidator.withPaymentSystem(CardPaymentSystem.MasterCard)
        expect(validator.validate(CardField.number('5100000000000000'))).toBeNull()
      })
    ...
    describe(EndlessCardExpirationDateValidator, () => {
      it('should validate only month', () => {
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('05', '22'))).toBeNull()
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('12', '21'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('13', '22'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('01', '99'))).toBeNull()
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('00', '99'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('wrong', '22'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('01', '-1'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('01', '111'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('01', 'wrong'))).toBe(
          CardValidationError.default,
        )
        expect(new EndlessCardExpirationDateValidator().validate(CardField.expirationDate('01', 'no'))).toBe(
          CardValidationError.default,
        )
      })
    })
    ...
    describe(LengthCardCvnValidator, () => {
      ...
      it('should create with CardType', () => {
        const validator = LengthCardCvnValidator.withPaymentSystem(CardPaymentSystem.MasterCard)
...
describe(CardBinRangeBuilder, () => {
  it('should throw error if provided configuration is incorrect', () => {
    ...
    expect(() => new CardBinRangeBuilder().addRange('0', '1')).toThrow(InvalidArgumentError)
    expect(() => binRangeBuilder.addRange('5200000', '53000000')).toThrow(InvalidArgumentError)
    expect(() => binRangeBuilder.addRange('53999999', '52000000')).toThrow(InvalidArgumentError)
  })
})
