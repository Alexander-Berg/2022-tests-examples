import { JSONItemFromJSON } from '../../../../common/__tests__/__helpers__/json-helpers'
import { AuthMethods } from '../../../code/models/auth-methods'
import { CardNetworks } from '../../../code/models/card-networks'
import { CountryCodes } from '../../../code/models/country-codes'
import { CurrencyCodes } from '../../../code/models/currency-codes'
import { Merchant } from '../../../code/models/merchant'
import { Order } from '../../../code/models/order'
import { OrderItem } from '../../../code/models/order-item'
import { OrderItemQuantity } from '../../../code/models/order-item-quantity'
import { OrderItemTypes } from '../../../code/models/order-item-types'
import { OrderTotal } from '../../../code/models/order-total'
import { PaymentMethod } from '../../../code/models/payment-method'
import { PaymentMethodTypes } from '../../../code/models/payment-method-types'
import { PaymentSheet } from '../../../code/models/payment-sheet'

describe(PaymentSheet, () => {
  it('should be serializable to JSON with version', () => {
    const data = new PaymentSheet(
      new Merchant('MerchantID', 'MerchantName'),
      new Order('OrderID', new OrderTotal('100.00', 'Total'), [
        new OrderItem('Item1', '70.00', OrderItemTypes.Pickup, new OrderItemQuantity('2', 'quantityLabel1')),
        new OrderItem('Item2', '30.00', OrderItemTypes.Shipping, new OrderItemQuantity('4', 'quantityLabel2')),
      ]),
      CurrencyCodes.rub,
      CountryCodes.ru,
      [
        new PaymentMethod(
          [AuthMethods.cloudToken, AuthMethods.panOnly],
          PaymentMethodTypes.card,
          'Gateway1',
          [CardNetworks.amex, CardNetworks.discover],
          'GatewayMerchant1',
        ),
        new PaymentMethod(
          [AuthMethods.panOnly],
          PaymentMethodTypes.card,
          'Gateway2',
          [CardNetworks.visa],
          'GatewayMerchant2',
        ),
      ],
    )
    const json = data.toJSON(true)
    expect(json).toStrictEqual(
      JSONItemFromJSON({
        version: 2,
        country_code: 'RU',
        currency_code: 'RUB',
        merchant: {
          id: 'MerchantID',
          name: 'MerchantName',
        },
        order: {
          id: 'OrderID',
          total: {
            amount: '100.00',
            label: 'Total',
          },
          items: [
            {
              label: 'Item1',
              amount: '70.00',
              type: 'PICKUP',
              quantity: {
                count: '2',
                label: 'quantityLabel1',
              },
            },
            {
              label: 'Item2',
              amount: '30.00',
              type: 'SHIPPING',
              quantity: {
                count: '4',
                label: 'quantityLabel2',
              },
            },
          ],
        },
        payment_methods: [
          {
            allowed_auth_methods: ['CLOUD_TOKEN', 'PAN_ONLY'],
            type: 'CARD',
            gateway: 'Gateway1',
            allowed_card_networks: ['AMEX', 'DISCOVER'],
            gateway_merchant_id: 'GatewayMerchant1',
          },
          {
            allowed_auth_methods: ['PAN_ONLY'],
            type: 'CARD',
            gateway: 'Gateway2',
            allowed_card_networks: ['VISA'],
            gateway_merchant_id: 'GatewayMerchant2',
          },
        ],
      }),
    )
  })
  it('should be serializable to JSON without version', () => {
    const data = new PaymentSheet(
      new Merchant('MerchantID', 'MerchantName'),
      new Order('OrderID', new OrderTotal('100.00', 'Total'), [
        new OrderItem('Item1', '100.00', OrderItemTypes.Pickup, new OrderItemQuantity('2', 'quantityLabel1')),
        new OrderItem('Item2', '50.00', OrderItemTypes.Shipping, new OrderItemQuantity('4', 'quantityLabel2')),
      ]),
      CurrencyCodes.rub,
      CountryCodes.ru,
      [
        new PaymentMethod(
          [AuthMethods.cloudToken, AuthMethods.panOnly],
          PaymentMethodTypes.card,
          'Gateway1',
          [CardNetworks.amex, CardNetworks.discover],
          'GatewayMerchant1',
        ),
        new PaymentMethod(
          [AuthMethods.panOnly],
          PaymentMethodTypes.card,
          'Gateway2',
          [CardNetworks.visa],
          'GatewayMerchant2',
        ),
      ],
    )
    const json = data.toJSON(false)
    expect(json).toStrictEqual(
      JSONItemFromJSON({
        country_code: 'RU',
        currency_code: 'RUB',
        merchant: {
          id: 'MerchantID',
          name: 'MerchantName',
        },
        order: {
          id: 'OrderID',
          total: {
            amount: '100.00',
            label: 'Total',
          },
          items: [
            {
              label: 'Item1',
              amount: '100.00',
              type: 'PICKUP',
              quantity: {
                count: '2',
                label: 'quantityLabel1',
              },
            },
            {
              label: 'Item2',
              amount: '50.00',
              type: 'SHIPPING',
              quantity: {
                count: '4',
                label: 'quantityLabel2',
              },
            },
          ],
        },
        payment_methods: [
          {
            allowed_auth_methods: ['CLOUD_TOKEN', 'PAN_ONLY'],
            type: 'CARD',
            gateway: 'Gateway1',
            allowed_card_networks: ['AMEX', 'DISCOVER'],
            gateway_merchant_id: 'GatewayMerchant1',
          },
          {
            allowed_auth_methods: ['PAN_ONLY'],
            type: 'CARD',
            gateway: 'Gateway2',
            allowed_card_networks: ['VISA'],
            gateway_merchant_id: 'GatewayMerchant2',
          },
        ],
      }),
    )
  })
})
