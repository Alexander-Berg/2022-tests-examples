import { JSONItemFromJSON } from '../../../../../common/__tests__/__helpers__/json-helpers'
import { APIError } from '../../../../code/network/yandex-pay-backend/api-errors'
import { RegisterPushTokenResponse } from '../../../../code/network/yandex-pay-backend/register-push-token-response'

describe(RegisterPushTokenResponse, () => {
  it('should be deserializable from JSON response', () => {
    const response = RegisterPushTokenResponse.fromJSONItem(
      JSONItemFromJSON({
        status: 'success',
        code: 200,
        data: {},
      }),
    )
    expect(response.isError()).toBe(false)
    expect(response.getValue()).toStrictEqual(new RegisterPushTokenResponse('success', 200))
  })
  it('should deserialize error if status is not success', () => {
    const response = RegisterPushTokenResponse.fromJSONItem(
      JSONItemFromJSON({
        status: 'fail',
        code: 501,
        data: {},
      }),
    )
    expect(response.isError()).toBe(true)
    expect(response.getError()).toStrictEqual(new APIError('fail', 501, null, 'RegisterPushToken'))
  })
})
