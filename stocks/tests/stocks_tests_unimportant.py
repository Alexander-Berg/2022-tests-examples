...
if __name__ == "__main__":
    if not locker.try_lock():
        print(
            'PASSIVE-CHECK:{0};{1};{2}'.format('stocks_tests_unimportant', '1', 'Cannot create lock - second instance is running'))
        sys.exit(1)

    suite = unittest.TestSuite()
    testloader = unittest.TestLoader()
    suite.addTest(testloader.loadTestsFromTestCase(OldquotesTest))
    ...
    message = 'Tests R:{0}, E:{1}/{2}. {3}'.format(result.testsRun, len(result.errors), len(result.failures),
                                                   ...
                                                   error_description)
    print('PASSIVE-CHECK:{0};{1};{2}'.format('stocks_tests_unimportant', failure, message))
