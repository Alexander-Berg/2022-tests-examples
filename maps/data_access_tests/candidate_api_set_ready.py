...
@pytest.mark.asyncio
async def test_set_candidate_ready(mongo: MongoFixture,
    ...
    async with await mongo.async_client.start_session() as session:
        candidate_id = await ReleaseCandidateApi(session).set_candidate_ready(
            service_config=service_config,
            task_id='12345',
    ...
    assert candidate.release_spec == release_spec
    assert candidate.progress.start_time is not None
    assert candidate.progress.end_time is not None
    assert candidate.progress.status == ReleaseCandidateStatus.READY
...
async def test_set_bad_candidate_ready(mongo: MongoFixture,
    ...
    with pytest.raises(Exception, match=case.expected_error):
        async with await mongo.async_client.start_session() as session:
            await ReleaseCandidateApi(session).set_candidate_ready(
                service_config=service_config,
