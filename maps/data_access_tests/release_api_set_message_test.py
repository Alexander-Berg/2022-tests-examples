...
@pytest.mark.asyncio
async def test_set_message(mongo: MongoFixture,
    ...
    async with await mongo.async_client.start_session() as session:
        release_id = await ReleaseApi(session).set_release_message(
            service_config=service_config,
            major_version=2,
            minor_version=1,
    ...
    assert release.message == 'some message'


@pytest.mark.asyncio
async def test_set_message_for_nonexistent_release(mongo: MongoFixture,
                                                   service_config_factory: ServiceConfigFactory) -> None:
    service_config = await service_config_factory(name='maps-fake-service')

    with pytest.raises(Exception, match=r'No release v1\.1 for maps-fake-service found'):
        async with await mongo.async_client.start_session() as session:
            await ReleaseApi(session).set_release_message(
                service_config=service_config,
                major_version=1,
                minor_version=1,
...
@pytest.mark.asyncio
async def test_drop_message(mongo: MongoFixture,
                            release_factory: ReleaseFactory,
                            service_config_factory: ServiceConfigFactory) -> None:
    service_config = await service_config_factory(name='maps-fake-service')
    ...
    assert release.message is None
