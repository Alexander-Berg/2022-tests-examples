...
public class MockSpeaker implements LocalizedSpeaker, TextToSpeech.OnInitListener {
    ...
    public void say(LocalizedPhrase phrase) {
        String result = phrase.text();
        if (callback != null) {
            callback.onSay((phrase.phraseFlags().getHasToponyms() ? "has toponym: " : "") + (phrase.phraseFlags().getHasCustomAnnotations() ? "has custom: " : "") + result);
        }
        if (ttsIsAvailable) {
