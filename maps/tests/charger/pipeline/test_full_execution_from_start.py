...
def setup_normalized_table(ch_client):
    setup_normalized_db(
        ...
        (
            ...
            # 4444
            (4444, 86555),  # 2 events in time scope
            (4444, 86777),
        ),
    )
...
def setup_charged_table(ch_client):
    setup_charged_db(
        ...
        (
            ...
            # 4444
            (4444, 75599, Decimal(5)),  # yesterday (by local time)
            (4444, 75600, Decimal(5)),  # today (by local time)
        ),
    )
...
def mock_collected_data(
    ...
    adv_store_message = CampaignForChargerList(
        campaigns=[
            ...
            CampaignForCharger(
                ...
                cost=Money(value=50000000),
                budget=Money(value=2000000),
                # max limit: 2 events per day
                daily_budget=Money(value=100000),
    ...
    billing_message = OrdersStatInfo(
        orders_info=[
            OrderStatInfo(order_id=567382, balance="15"),
...
async def test_full_execution_from_start(pipeline_from_start, ch_client):
    ...
    expected_events = [
        ...
        for args in (
            ...
            # 4444
            # Day limit = 2 events, there is 1 charged event
            # today (by local time) => +1 charged one (the oldest)
            (4444, 86777, Decimal(5)),
        )
    ]
