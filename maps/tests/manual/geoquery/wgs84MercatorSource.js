var wgs84MercatorSource = function (ymaps, myMap, that) {
    ...
    // util.Promise
    // geoCoder
    that.promisePoints = ymaps.geocode("Санкт-Петербург, ул. Яхтенная 4к3");
    ...
    that.geoXMLObject = ymaps.geoXml.load('https://maps.yandex.ru/export/usermaps/93jfWjoXws37exPmKH-OFIuj3IQduHal/');
    ...
    that.geoQueryResult = ymaps.geoQuery(that.geoObjectPlacemark).add(that.geoObjectPolyline)
        ...
        .add(that.objectPlacemark).add(that.objectPolyline).add(that.objectCircle).add(that.objectPolygon).add(that.objectRectangle)
        .add(that.stringPlacemark).add(that.stringPolyline).add(that.stringCircle).add(that.stringPolygon).add(that.stringRectangle)
        .add(that.geometryArray).add(that.objectCollection)/*.add(that.promisePoints.geoObjects).add(that.promisePolyline)*/.add(that.geoXMLObject);
};
