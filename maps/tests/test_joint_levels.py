...
from maps.geoq.hypotheses.flats.lib import joint_levels

from data_joint_levels import (
...
def test_prepare_exact_entrance_hypothesis():
    ...
    job = cluster.job()
    joint_levels.prepare(job, '')

    hypothesis = []
    job.local_run(
        sources={
            'entrance_level_flat_range': prepare_source(ENTRANCE_LEVEL_FLAT_RANGE),
