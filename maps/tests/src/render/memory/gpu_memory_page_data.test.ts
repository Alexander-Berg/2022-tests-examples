...
import {
    BufferData, GpuMemoryPageData,
    IndexBufferData,
    VertexBufferData
} from '../../../../src/vector_render_engine/render/memory/gpu_memory_page_data';
...
describe('GpuMemoryPageData', () => {
    it('should write indices relative to current mesh properly', () => {
        ...
        expect(data.indexBuffer.asUint16Array()).to.be.deep.equal(new Uint16Array([0, 1, 2]));
        ...
        expect(data.indexBuffer.asUint16Array()).to.be.deep.equal(new Uint16Array([0, 1, 2, 3, 4, 5]));
    });
    ...
    describe('BufferData', () => {
        ...
        it('should write multiple meshes', () => {
            ...
            expect(bufferData.endMesh()).to.be.deep.equal({byteOffset: 0, byteSize: 6});

            bufferData.writeWord(4);
            bufferData.writeWord(5);
            expect(bufferData.endMesh()).to.be.deep.equal({byteOffset: 6, byteSize: 4});

            expect(new Int16Array(bufferData.data, 0, 5)).to.be.deep.equal(new Int16Array([1, 2, 3, 4, 5]));
        });
        ...
        it('should reset its cursor', () => {
            ...
            expect(new Int16Array(bufferData.data, 0, 3)).to.be.deep.equal(new Int16Array([4, 5, 6]));
        });
        ...
        it('should transfer data of current mesh', () => {
            ...
            expect(sourceBufferData.endMesh()).to.be.deep.equal({byteOffset: 6, byteSize: 4});
            expect(new Int16Array(sourceBufferData.data, 0, 5)).to.be.deep.equal(new Int16Array([1, 2, 3, 66, 77]));

            targetBufferData.writeWord(6);
            targetBufferData.writeWord(7);
            targetBufferData.writeWord(8);
            expect(targetBufferData.endMesh()).to.be.deep.equal({byteOffset: 0, byteSize: 10});
            expect(new Int16Array(targetBufferData.data, 0, 5)).to.be.deep.equal(new Int16Array([4, 5, 6, 7, 8]));
        });
    });
    ...
    describe('VertexBufferData', () => {
        it ('should update currentMeshBaseIndex', () => {
            ...
            expect(bufferData.currentMeshBaseIndex).to.be.equal(0);
            ...
            bufferData.pushUint32(2);
            bufferData.endMesh();
            expect(bufferData.currentMeshBaseIndex).to.be.equal(2);
    ...
    describe('IndexBufferData', () => {
        it ('should update indices during transferring data to another buffer data', () => {
            ...
            expect(new Uint16Array(targetBufferData.data, 0, 3)).to.be.deep.equal(new Uint16Array([0, 1, 2]));
        });
    });
