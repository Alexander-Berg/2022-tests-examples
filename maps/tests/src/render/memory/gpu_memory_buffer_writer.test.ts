...
import {
    GpuMemoryBufferWriter,
    VertexWriter
} from '../../../../src/vector_render_engine/render/memory/gpu_memory_buffer_writer';
import Vector2 from '../../../../src/vector_render_engine/math/vector2';
import {GpuMemoryPageLocation} from '../../../../src/vector_render_engine/render/memory/gpu_memory_page_location';
...
describe('GpuMemoryBufferWriter', () => {
    it('should request new page data if there is no room for the next mesh', () => {
        ...
        writer.endMesh();
        expect(memory_pages.length).to.be.equal(1);
        ...
        writer.endMesh();
        expect(memory_pages.length).to.be.equal(2);

        writer.endMesh();
        writer.writePrimitive([], 6);
        expect(memory_pages.length).to.be.equal(3);
    });
