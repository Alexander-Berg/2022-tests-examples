...
async def test_start_raises_on_start_timeout(consumer_mock, mocker):
    mocker.patch(
        "maps_adv.geosmb.clients.logbroker.logbroker.TopicReader.START_TIMEOUT",  # noqa
        0,
    )
...
async def test_stop_raises_on_stop_timeout(consumer_mock, mocker):
    mocker.patch(
        "maps_adv.geosmb.clients.logbroker.logbroker.TopicReader.STOP_TIMEOUT",
        0,
    )
    consumer_mock.start.coro.return_value = init_response
...
async def test_read_batch_raises_on_read_timeout(consumer_mock, mocker):
    consumer_mock.start.coro.return_value = init_response
    consumer_mock.next_event.coro.side_effect = lambda: asyncio.sleep(1)

    async with TopicReader(consumer_mock) as reader:
        with pytest.raises(LogbrokerReadTimeout):
            [msg async for msg in reader.read_batch(1, read_timeout=0)]
