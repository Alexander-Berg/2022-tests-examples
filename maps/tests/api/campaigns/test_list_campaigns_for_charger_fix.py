...
url = "/campaigns/charger/fix/?active_at={}"
...
async def test_campaign_returned_if_active(api, factory):
    ...
    got = await api.get(
        ...
        decode_as=CampaignForChargerFixList,
        expected_status=200,
    )

    assert got == CampaignForChargerFixList(
        campaigns=[
            CampaignForChargerFix(
                campaign_id=campaign_id,
                order_id=10,
                cost=Money(value=10000000),
                time_interval=Fix.TimeIntervalEnum.MONTHLY,
                timezone="Europe/Moscow",
            )
        ]
    )
...
async def test_not_fix_campaigns_not_returned(api, factory, billing_kwargs):
    await factory.create_campaign(
        start_datetime=dt("2019-01-01 00:00:00"),
        end_datetime=dt("2020-01-01 00:00:00"),
        publication_envs=[PublicationEnvEnum.PRODUCTION],
...
async def test_returns_error_if_active_at_not_provided(api, factory):
    ...
    await api.get("/campaigns/charger/fix/", expected_status=400)
...
async def test_campaign_discounts_not_respected(api, factory):
    campaign_id = (
        await factory.create_campaign(
            fix=DAILY_FIX,
    ...
    assert got.campaigns[0].cost == Money(value=100000)
