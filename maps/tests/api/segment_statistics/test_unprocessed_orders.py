...
async def test_returns_statistics(api, add_client):
    ...
    got = await api.post(
        ...
        proto=segments_pb2.SegmentStatisticsInput(
            biz_id=123, segment_type=segments_pb2.SegmentType.UNPROCESSED_ORDERS
        ),
...
async def test_slot_includes_clients_with_resolved_events_if_they_has_unresolved(
    api, factory, add_client, resolution
):
...
async def test_slot_not_includes_event_from_future(api, factory):
...
async def test_counts_clients_not_events(api, add_client, factory):
    client_id = await add_client(dt("2020-04-05 00:00:00"))
    await factory.create_order_event(
        client_id=client_id,
        event_type=OrderEvent.CREATED,
