...
async def test_returns_statistics(api, add_client):
    await add_client(dt("2020-04-10 15:00:00"), talk_duration=0)
    await add_client(dt("2020-09-05 23:00:00"), talk_duration=0)

    got = await api.post(
        url,
        proto=segments_pb2.SegmentStatisticsInput(
            biz_id=123, segment_type=segments_pb2.SegmentType.MISSED_LAST_CALL
        ),
...
async def test_not_includes_client_with_call_duration(api, add_client, factory):
    client_id = await add_client(dt("2020-04-10 15:00:00"), talk_duration=0)
    await factory.create_call_event(
        client_id=client_id,
...
async def test_includes_client_with_long_old_call(api, add_client, factory):
    client_id = await add_client(dt("2020-04-10 15:00:00"), talk_duration=0)
    await factory.create_call_event(
        client_id=client_id,
        event_type=CallEvent.FINISHED,
...
async def test_not_includes_client_with_last_unfinished_call(api, add_client, factory):
    client_id = await add_client(dt("2020-04-10 15:00:00"), talk_duration=0)
    await factory.create_call_event(
        client_id=client_id,
