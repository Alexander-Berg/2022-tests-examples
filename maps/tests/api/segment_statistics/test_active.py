...
def add_client(factory):
    async def _add_client(on_dt: datetime):
...
async def test_returns_statistics(api, add_client):
    ...
    got = await api.post(
        ...
        proto=segments_pb2.SegmentStatisticsInput(
            biz_id=123, segment_type=segments_pb2.SegmentType.ACTIVE
        ),
...
async def test_counts_call_events(api, factory):
    ...
    assert got.statistics[0].size == 1
