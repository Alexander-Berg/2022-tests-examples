...
async def test_segments_as_disloyal_if_review_rating_is_under_4(
    ...
    assert parse_pb_segment_size(got, SegmentType.DISLOYAL) == 1


async def test_does_not_segments_loyal_if_disloyal(factory, api):
    ...
    assert got.total_leads == 1
    assert parse_pb_segment_size(got, SegmentType.LOYAL) == 0
...
async def test_counts_leads_in_segment_correctly(factory, api):
    ...
    assert parse_pb_segment_size(got, SegmentType.DISLOYAL) == 3
