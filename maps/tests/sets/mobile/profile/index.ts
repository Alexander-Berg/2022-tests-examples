...
const shouldBeHiddenSelector = [
    ...
    cssSelectors.actionBar.prefix.businessCard + ' ' + cssSelectors.actionBar.shareButton
].join(', ');
...
describe('Профиль организации.', () => {
    describe('Обычный.', () => {
        it('Отсутствуют элементы', async function () {
            ...
            await this.browser.scrollIntoView(getSelectorByText('ул. Льва Толстого, 16'));
            await this.browser.waitAndClick(getSelectorByText('ул. Льва Толстого, 16'));
            ...
            await this.browser.waitForNotAppear(cssSelectors.search.form.input);
            await this.browser.waitForNotAppear(cssSelectors.shortcutsCarousel.view);
        });
    ...
    describe('С параметром no_similar=true', () => {
        it('Отсутствуют элементы', async function () {
        ...
        it('Отсутствует реклама в маршруте', async function () {
            await this.browser.openPage('/1788984433?no_similar=true', {
                application: 'profile',
                mockGeolocation: [55.734238, 37.589355, 10]
            });
            await this.browser.waitAndClick(
                cssSelectors.actionBar.prefix.businessCard + ' ' + cssSelectors.actionBar.routeButton
            );
            await this.browser.waitForNotAppear(cssSelectors.bannerView);
