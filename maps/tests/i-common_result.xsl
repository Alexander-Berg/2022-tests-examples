...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:lego="https://lego.yandex-team.ru" xmlns:x="http://www.yandex.ru/xscript" xmlns:str="http://exslt.org/strings" xmlns:exsl="http://exslt.org/common" xmlns:func="http://exslt.org/functions" exclude-result-prefixes=" lego x str exsl func " extension-element-prefixes=" func " version="1.0">
...
<xsl:param name="media" select="''"/>
    ...
    <xsl:for-each select="str:split($css, ',')">
        ...
        <xsl:variable name="href-with-proto">
            <xsl:if test="starts-with($href-original, '//')"><xsl:value-of select="$lego:params/lego:css-protocol"/>:</xsl:if><xsl:value-of select="$href-original"/>
        </xsl:variable>
...
<xsl:param name="build" select="$lego:params/lego:build"/>
    ...
    <xsl:for-each select="str:split($js, ',')">
        <script type="text/javascript" charset="{$charset}" src="{normalize-space(.)}{$build-string}"/>
    </xsl:for-each>
</xsl:template>
...
<xsl:template match="     lego:index |     lego:yandexuid |     lego:passport-host |     lego:pass-host |     lego:passport-msg |     lego:lego-static-host |     lego:social-host |     lego:lego-path |     lego:retpath     " mode="lego:params-param">
    <xsl:text>,</xsl:text>
