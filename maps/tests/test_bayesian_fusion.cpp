#include <maps/factory/libs/image/bayesian_fusion.h>
...
Y_UNIT_TEST_SUITE(bayesian_fusion_should) {
...
Y_UNIT_TEST(scale_down_and_fuse_back)
{
    ...
    ImageCovariance<4, float> stats(SkipRows::LastBandZero);
    vrt.ForEachBlock<float>({}, [&](FloatImageBase& img, auto&&) { stats(img); });

    TDataset resDs = CreateTiff(pathFused).like(pan).setBands(3);
    BayesianFusion<3> fuse(stats.result().cov());
    ...
    EXPECT_LE(resImg.normDifference(srcImg), 700);
    EXPECT_LE(resImg.meanAbsDifference(srcImg), 3);
    EXPECT_LE(resImg.maxAbsDifference(srcImg), 30);
}
