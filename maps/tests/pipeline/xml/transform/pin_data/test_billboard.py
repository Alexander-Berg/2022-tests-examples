...
async def test_will_transform_campaign_type_billboard_as_expected(
    ...
    avatars = avatars_batch(dict(pin=ImageType.PIN, banner=ImageType.BANNER))
    campaign = dict(
        id=campaign_id,
        pages=["p1", "p2"],
        places=["bb:1", "bb:2"],
        ...
        creatives={
            CreativeType.BILLBOARD: dict(images=[avatars["pin"].avatar]),
...
async def test_returns_empty_result_if_billboard_is_not_supported_on_platform(
    platform, config
):
    campaign = dict(campaign_type=CampaignType.BILLBOARD, platforms=[platform])
    result = await pin_data_transform(campaign, None, config.INSTANCE_TAG_CTYPE)
