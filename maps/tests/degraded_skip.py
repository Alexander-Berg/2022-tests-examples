def test_degraded_skip(mongo, coordinator):
    ...
    hash_1 = coordinator.upload(
        'pkg-a', '1.0', 'gen-ab1', now=0, disk_usage=10, tvm_id=1)
    ...
    require_torrent_time(hash_2, 3595, shard_2)
    require_torrent_time(hash_1, 3595, ['storage11'])
    hash_3 = coordinator.upload(
        'pkg-a', '3.0', 'gen-ab1', now=3600, disk_usage=20, tvm_id=1)
    require_torrent_time_absent(hash_3, 3605, shard_1)
    require_torrent_time(hash_3, 3605, shard_2)
