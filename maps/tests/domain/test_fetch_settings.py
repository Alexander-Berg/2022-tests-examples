...
async def test_returns_dm_data(domain, dm):
    dm.fetch_settings.coro.return_value = dict(
        emails=["email@ya.ru"],
        phone=900,
    ...
    got = await domain.fetch_settings(biz_id=123)

    dm.create_default_settings.assert_not_called()
    assert got == dict(
        emails=["email@ya.ru"],
        phone=900,
        notifications=dict(notify_kek=True, notify_meow=False),
...
async def test_does_not_create_default_settings_if_there_are_any(domain, dm):
    ...
    await domain.fetch_settings(biz_id=123)

    dm.create_default_settings.assert_not_called()


async def test_returns_default_settings_if_were_not_any(domain, dm):
    dm.fetch_settings.coro.side_effect = UnknownBizId()
    dm.create_default_settings.coro.return_value = dict(
        emails=["meow@ya.ru"],
        phone=900,
...
async def test_saves_default_settings_if_were_not_any(domain, dm):
    ...
    dm.create_default_settings.coro.assert_called_with(
        biz_id=123, emails=None, phone=None
    )
