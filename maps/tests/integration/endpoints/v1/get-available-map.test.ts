...
export const main: EndpointSuiteDefinition = (context) => {
    describe('GET /v1/available_maps/{sid}', () => {
        const validateResponseSchema = createResponseSchemaValidator({path: '/v1/available_maps/{sid}'});
        ...
        describe('authentication', () => {
            it('should return 401 for request without tickets', async () => {
                const res = await context.server.request('/v1/available_maps/sid-1');
        ...
        it('should return 404 if map does not exist', async () => {
            const res = await context.server.request('/v1/available_maps/sid-404', {
                headers: {
        ...
        describe('getting map without user ticket', () => {
            it('should return 404 if request private map', async () => {
                const res = await context.server.request('/v1/available_maps/sid-2', {
                    headers: {
                        'X-Ya-Service-Ticket': SELF_SERVICE_TICKET
                    },
                    json: true
            ...
            it('should return public map without objects', async () => {
                const res = await context.server.request('/v1/available_maps/sid-3', {
                    headers: {
                        'X-Ya-Service-Ticket': SELF_SERVICE_TICKET
                    },
                    json: true
                ...
                expect(res.body.maps[0].userRole).to.be.undefined;
            });

            it('should return public map with objects', async () => {
                await context.db.loadFixtures([
                    {
                        table: 'objects',
                        rows: [
            ...
            describe('sanitizing', () => {
                ...
                it('should not sanitize map and objects by default', async () => {
                    const res = await context.server.request('/v1/available_maps/sid-4', {
                        headers: {
                            'X-Ya-Service-Ticket': SELF_SERVICE_TICKET
                        },
                        json: true
        ...
        describe('getting map with user ticket', () => {
            it('should return 404 if user has not access to map', async () => {
                await context.db.loadFixtures([
                    {
                        table: 'maps',
                        rows: [
            ...
            it('should return private map without objects', async () => {
                const res = await context.server.request('/v1/available_maps/sid-2', {
                    headers: {
                        'X-Ya-Service-Ticket': SELF_SERVICE_TICKET,
                        'X-Ya-User-Ticket': UID1_USER_TICKET
                ...
                expect(res.body.maps[0].geoObjects.features).to.have.length(0, 'Invalid number of objects');
                expect(res.body.maps[0].userRole).to.equal('administrator', 'Invalid user role');
            });

            it('should return private map with objects', async () => {
                await context.db.loadFixtures([
                    {
                        table: 'objects',
                        rows: [
