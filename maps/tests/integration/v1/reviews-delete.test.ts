...
describe('POST /v1/reviews/delete', () => {
    ...
    beforeEach(() => {
        nock(ugcSearch)
            .post(`/v1/orgs/${org_id.right}/delete-my-review?review_id=${defaultBody.data.review_id}`)
            .reply(200, {});

        nock(ugcSearch)
            .post(`/v1/orgs/${org_id.wrong}/delete-my-review?review_id=${defaultBody.data.review_id}`)
            .reply(404);
    });
    ...
    it('should pass success response', async () => {
        const response = await client.post(`${url}/v1/reviews/delete`);
        expect(response.statusCode).toEqual(200);
        validator.validateResponse('post', '/v1/reviews/delete')(response);
    });
