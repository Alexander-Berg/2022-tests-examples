...
import {createStoryRequest, simpleCreateStory} from '../../utils/v1/request-utils';
...
import {StoryDbRecord, DbData} from '../../../app/types/db';

describe('post /v1/create_story', () => {
    prepareTests();
    ...
    describe('success cases', () => {
        ...
        it('should successfully create a story for several business ids', async () => {
            ...
            nockSnippetsOnPut([BIZ_IDS[0], BIZ_IDS[1]]);
            nockModeration(story.data);
...
function validateFailedRequestWithOpenUrlButton(
    extension: Partial<StoriesIntTypes.StoryScreenOpenUrlButton['openUrlButton']>,
...
function validateFailedRequestWithScreen(
    extension: Partial<StoriesIntTypes.StoryScreen>,
    expectedError: ExpectedError
): Promise<void> {
...
async function validateFailedRequest(
    extension: Partial<StoriesIntTypes.StoryCreateRequest>,
    expectedError: ExpectedError
): Promise<void> {
    const story = getTestStory(BIZ_IDS[0]);
