...
const URL_PREFIX = '/v1/user_account';
...
describe(URL_PREFIX, () => {
    ...
    describe('GET routes with Authorization', () => {
        ...
        it('should return error if backend is unavailable', async () => {
            for (let i = 0; i < routes.length; i++) {
                ...
                nock(userAccountHost).get(`/v1${routes[i]}`).query(true).reply(403);
        ...
        it('should return success', async () => {
            for (let i = 0; i < routes.length; i++) {
                blackboxService.reply(200, blackboxValidResponse);
                nock(userAccountHost).get(`/v1${routes[i]}`).query(true).reply(200);
    ...
    describe('POST routes with Authorization', () => {
        const routes = [
            ...
            '/reviews/delete',
            '/photo/delete',
            '/public_account/edit_access'
        ];

        it('should return error if backend is unavailable', async () => {
            for (let i = 0; i < routes.length; i++) {
                blackboxService.reply(200, blackboxValidResponse);
                nock(userAccountHost).post(`/v1${routes[i]}`).reply(403);
        ...
        it('should return success', async () => {
            for (let i = 0; i < routes.length; i++) {
                blackboxService.reply(200, blackboxValidResponse);
                nock(userAccountHost).post(`/v1${routes[i]}`).reply(200);
    ...
    describe('Public Account routes without Authorization', () => {
        const routes = [
            '/public_account',
            '/public_account/reviews',
            '/public_account/photos'
        ];

        it('should return error if backend is unavailable', async () => {
            for (let i = 0; i < routes.length; i++) {
                nock(userAccountHost).get(`/v1${routes[i]}`).reply(403);
        ...
        it('should return success', async () => {
            for (let i = 0; i < routes.length; i++) {
                nock(userAccountHost).get(`/v1${routes[i]}`).reply(200);
