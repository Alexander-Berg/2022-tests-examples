...
describe('/keyserv/settings', () => {
    ...
    beforeAll(async () => {
        ...
        keyserv = nock(keyservHost)
            .get(`/2.x/`)
            .query({
                action: SetKeyActions.RESTRICTION,
                key: 'sample_key',
                name: 'maxDrawArea',
                value: 32
            });
    ...
    it('should return 200 for request to keyserv', async () => {
        ...
        const response = await client.post(`${url}/api/keys/set_restriction`, {
            form: {
                key: 'sample_key',
                restrictionName: 'maxDrawArea',
                restrictionValue: 32
            }
        });
    ...
    describe('error validation && parsing', () => {
        ...
        it('should return 400 after validation', async () => {
            const response = await client.post(`${url}/api/keys/set_restriction`, {
                form: {
                    key: 'sample_key',
                    restrictionName: 'Wrong Restriction',
                    restrictionValue: 'hahah'
                }
            });
