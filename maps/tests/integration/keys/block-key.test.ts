...
import {KeyState, BlockReasons} from 'server/routers/keys/keyserv-types';
...
describe('/keyserv/settings', () => {
    ...
    beforeAll(async () => {
        ...
        keyserv = nock(keyservHost)
            .get(`/2.x/`)
            .query({
                action: KeyState.BLOCK,
                aid: '1234',
                key: 'sample_key',
                reason: BlockReasons.BY_ADMIN,
    ...
    it('should return 200 for request to keyserv', async () => {
        ...
        const response = await client.post(`${url}/api/keys/block_key`, {
            form: {
                key: 'sample_key',
                description: 'Custom',
                customDescription: 'sample_description'
            }
        });
