...
def test_success():
    ...
    validator_build = autostart.Build(
        full_id=autostart.BuildId(module_name="bicycle_graph_validator", id=1),
    ...
    deploy_starters.deploy_if_validated_instantly(validator_build, build_manager)
...
def test_old_release():
    ...
    build_manager = autostart.BuildManager([source_build, validator_build, other_deploy_build], MODULE_TRAITS)
