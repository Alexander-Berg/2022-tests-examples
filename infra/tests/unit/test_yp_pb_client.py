...
def _make_value_payload(obj):
    ...
    rv.protobuf = obj.SerializeToString()
    return rv
...
def test_select_object_ids(yp_proto_client):
    metas = [data_model.TPod().meta, data_model.TPod().meta, data_model.TPod().meta]
    metas[0].id = "id1"
    metas[1].id = "id2"
    metas[2].id = "id3"
    ...
    r = resp1.results.add()
    r.value_payloads.extend([_make_value_payload(metas[0])])
    r = resp1.results.add()
    r.value_payloads.extend([_make_value_payload(metas[1])])
    resp2 = object_service_pb2.TRspSelectObjects()
    resp2.timestamp = 1
    resp2.continuation_token = 'any-token'
    r = resp2.results.add()
    r.value_payloads.extend([_make_value_payload(metas[2])])
    yp_proto_client.stub.SelectObjects.side_effect = [resp1, resp2]
    ...
    expected_req1.selector.paths.extend(['/meta'])
    expected_req1.format = object_service_pb2.PF_PROTOBUF
    ...
    selected_ids = yp_proto_client.select_object_ids(object_type=data_model.OT_POD,
                                                     object_class=yp.data_model.TPod,
                                                     batch_size=batch_size)

    assert selected_ids == ids
    calls = yp_proto_client.stub.method_calls
    assert len(calls) == 2
    assert calls[0] == mock.call.SelectObjects(expected_req1)
    assert calls[1] == mock.call.SelectObjects(expected_req2)


def test_create_pod_set(yp_proto_client):
    rs_id = 'rs-id'
    ...
    yp_proto_client.create_pod_set(ps=expected_ps,
    ...
    yp_proto_client.stub.CreateObject.assert_called_once_with(expected_req)


def test_remove_pod_set(yp_proto_client):
    ps = data_model.TPodSet()
    ps.meta.id = 'ps'
    tid = 'tid'

    resp = object_service_pb2.TRspRemoveObjects()
    yp_proto_client.stub.RemoveObject.side_effect = [resp]

    yp_proto_client.remove_pod_set(ps.meta.id, transaction_id=tid)
    ...
    assert yp_proto_client.stub.method_calls[-1] == mock.call.RemoveObject(expected_req)
...
def test_get_pod_set_ignore(yp_proto_client):
    ...
    yp_proto_client.stub.GetObject.side_effect = [resp1]

    expected_req = _make_expected_req_get_object(data_model.OT_POD_SET)

    o = yp_proto_client.get_pod_set_ignore('id', selectors=['/meta'])
    assert o == obj
    assert yp_proto_client.stub.method_calls[-1] == mock.call.GetObject(expected_req)


def test_get_replica_set_ignore(yp_proto_client):
    ...
    o = yp_proto_client.get_replica_set_ignore('id', selectors=['/meta'])
    assert o == obj
    assert yp_proto_client.stub.method_calls[-1] == mock.call.GetObject(expected_req)


def test_get_multi_cluster_replica_set_ignore(yp_proto_client):
    ...
    o = yp_proto_client.get_multi_cluster_replica_set_ignore('id', selectors=['/meta'])
    assert o == obj
    assert yp_proto_client.stub.method_calls[-1] == mock.call.GetObject(expected_req)


def test_get_replica_set(yp_proto_client):
    obj = data_model.TReplicaSet()
    obj.meta.id = 'id'
    ...
    rs = yp_proto_client.get_replica_set('id', selectors=['/meta'], timestamp=100)
    assert rs == obj
    assert yp_proto_client.stub.method_calls[-1] == mock.call.GetObject(expected_req)


def test_make_and_load_object_attrs(yp_proto_client):
    selectors = [
        "/meta",
        "/spec",
        "/status"
    ...
    value_payloads = [
        _make_value_payload(fake_obj.meta),
        _make_value_payload(fake_obj.spec),
        _make_value_payload(fake_obj.status)
    ]
    result = yp_proto_client.loader.load_object(data_model.TPod, selectors, value_payloads)
    ...
    assert result.status.agent.iss_payload == fake_obj.status.agent.iss_payload
    assert result.status.generation_number == fake_obj.status.generation_number

    assert result == yp_proto_client.loader.load_object_attrs(data_model.TPod, paths, value_payloads)


def test_pod_set_exists(yp_proto_client):
    ps = data_model.TPodSet()
    ps.meta.id = 'ps'
    ...
    yp_proto_client.stub.GetObject.side_effect = [resp, resp2]

    assert yp_proto_client.pod_set_exists('ps')
    ...
    assert not yp_proto_client.pod_set_exists('fake_ps')


def test_generate_timestamp(yp_proto_client):
    resp = object_service_pb2.TRspGenerateTimestamp()
    resp.timestamp = 179
    yp_proto_client.stub.GenerateTimestamp.side_effect = [resp]

    ts = yp_proto_client.generate_timestamp()

    assert ts == 179
    assert yp_proto_client.stub.method_calls[-1] == mock.call.GenerateTimestamp(object_service_pb2.TReqGenerateTimestamp())


def test_start_and_commit_transaction(yp_proto_client):
    resp = object_service_pb2.TRspStartTransaction()
    resp.start_timestamp = 179
    resp.transaction_id = 'tr'
    yp_proto_client.stub.StartTransaction.side_effect = [resp]

    t = yp_proto_client.start_transaction()

    assert t == ('tr', 179)
    assert yp_proto_client.stub.method_calls[-1] == mock.call.StartTransaction(object_service_pb2.TReqStartTransaction())

    resp = object_service_pb2.TRspCommitTransaction()
    resp.commit_timestamp = 200
    yp_proto_client.stub.CommitTransaction.side_effect = [resp]

    expected_req = object_service_pb2.TReqCommitTransaction()
    expected_req.transaction_id = t[0]

    yp_proto_client.commit_transaction(t[0])
    assert yp_proto_client.stub.method_calls[-1] == mock.call.CommitTransaction(expected_req)


def test_create_pods(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    p2 = data_model.TPod()
    p2.meta.id = 'p2'
    ...
    yp_proto_client.stub.CreateObjects.side_effect = [resp]

    yp_proto_client.create_pods([p1, p2], 'tr')
    ...
    assert yp_proto_client.stub.method_calls[-1] == mock.call.CreateObjects(expected_req)


def test_remove_pods(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    p2 = data_model.TPod()
    p2.meta.id = 'p2'
    ...
    yp_proto_client.remove_pods([p1.meta.id, p2.meta.id], 'tr')
    ...
    assert yp_proto_client.stub.method_calls[-1] == mock.call.RemoveObjects(expected_req)
...
def test_update_pod_set(yp_proto_client):
    ps = data_model.TPodSet()
    ...
    yp_proto_client.update_pod_set('ps-id', ps)
    assert yp_proto_client.stub.method_calls[-1] == mock.call.UpdateObject(expected_req)


def test_update_replica_set_status(yp_proto_client):
    resp = object_service_pb2.TRspUpdateObject()
    yp_proto_client.stub.UpdateObject.side_effect = [resp]
    value = data_model.TReplicaSetStatus()

    expected_req = _make_expected_req_update_object(data_model.OT_REPLICA_SET, '/status', value)

    yp_proto_client.update_replica_set_status('id', value)
    assert yp_proto_client.stub.method_calls[-1] == mock.call.UpdateObject(expected_req)


def test_update_replica_set_annotations(yp_proto_client):
    resp = object_service_pb2.TRspUpdateObject()
    yp_proto_client.stub.UpdateObject.side_effect = [resp]
    ...
    yp_proto_client.update_replica_set_annotations('id', value, 'tr')
    assert yp_proto_client.stub.method_calls[-1] == mock.call.UpdateObject(expected_req)


def test_update_multi_cluster_replica_set_status(yp_proto_client):
    resp = object_service_pb2.TRspUpdateObject()
    yp_proto_client.stub.UpdateObject.side_effect = [resp]
    ...
    yp_proto_client.update_multi_cluster_replica_set_status('id', value)
    assert yp_proto_client.stub.method_calls[-1] == mock.call.UpdateObject(expected_req)
...
def test_update_pods(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    ...
    yp_proto_client.update_pods(pods, batch_size=2)
    expected_req = object_service_pb2.TReqUpdateObjects()
    for pod in pods:
        _add_pod_to_update_pods_request(expected_req, pod)
    assert yp_proto_client.stub.method_calls[0] == mock.call.UpdateObjects(expected_req)

    yp_proto_client.stub.reset_mock()
    # Case 2: update pods in 2 batches concurrently
    yp_proto_client.update_pods(pods, batch_size=1)
    expected_reqs = []
    for pod in pods:
        req = object_service_pb2.TReqUpdateObjects()
        _add_pod_to_update_pods_request(req, pod)
        expected_reqs.append(req)
    assert yp_proto_client.stub.method_calls[0] == mock.call.UpdateObjects(expected_reqs[0])
    assert yp_proto_client.stub.method_calls[1] == mock.call.UpdateObjects(expected_reqs[1])
    ...
    yp_proto_client.stub.UpdateObjects.side_effect = update_pods_side_effect
...
def test_update_pods_acknowledge_eviction(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    p2 = data_model.TPod()
    p2.meta.id = 'p2'
    ...
    yp_proto_client.stub.UpdateObjects.side_effect = [resp]

    yp_proto_client.update_pods_acknowledge_eviction(pod_ids, msg)
    ...
    assert yp_proto_client.stub.method_calls[-1] == mock.call.UpdateObjects(expected_req)


def test_update_pods_target_state_to_removed(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    p2 = data_model.TPod()
    p2.meta.id = 'p2'
    ...
    yp_proto_client.update_pods_target_state(pod_ids, target_state)
...
def test_update_pods_target_state_to_active(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    p2 = data_model.TPod()
    p2.meta.id = 'p2'
...
def test_safe_update_pods(yp_proto_client):
    p1 = data_model.TPod()
    p1.meta.id = 'p1'
    p2 = data_model.TPod()
    p2.meta.id = 'p2'
    ...
    resp = yp_proto_client.safe_update_pods(pods)
    assert resp == []
