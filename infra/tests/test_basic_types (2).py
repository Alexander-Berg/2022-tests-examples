...
def test_bool_invalid_type():
    error = pytest.raises(InvalidTypeError, lambda: _validate(0, Bool())).value
...
if PY2:
    def test_integer_long():
        _validate(long(1), Integer())  # noqa


def test_integer_invalid_type():
    error = pytest.raises(InvalidTypeError, lambda: _validate(True, Integer())).value
...
def test_float_invalid_type():
    error = pytest.raises(InvalidTypeError, lambda: _validate(1, Float())).value
...
def test_string_invalid_type():
    error = pytest.raises(InvalidTypeError, lambda: _validate(b"bytes", String())).value
...
def test_choices_invalid_value():
    error = pytest.raises(InvalidValueError, lambda: _validate("c", String(choices=("a", "b")))).value
