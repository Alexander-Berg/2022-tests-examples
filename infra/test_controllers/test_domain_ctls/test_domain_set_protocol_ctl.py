...
class DomainOperationCtl(BaseDomainOperationCtl):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._runners['set_protocol'].processing_interval = self.PROCESSING_INTERVAL
...
def create_ctl_with_cert_order(cache, zk_storage):
    pb = create_set_protocol_pb_with_cert_order(cache, zk_storage)
...
def create_ctl_with_cert_ref(cache, zk_storage):
    pb = create_set_protocol_pb_with_cert_ref(cache, zk_storage)
    ctl = DomainOperationCtl(NS_ID, DOMAIN_ID)
    ctl._pb = pb
    return ctl


def create_ctl_with_http(cache, zk_storage):
    pb = create_set_protocol_pb_with_http(cache, zk_storage)
    ctl = DomainOperationCtl(NS_ID, DOMAIN_ID)
    ctl._pb = pb
    return ctl
...
def create_set_protocol_pb_with_cert_order(cache, zk_storage):
    ...
    domain_op_pb.order.content.set_protocol.cert_order.content.public_key_algorithm_id = 'ec'
    domain_op_pb.order.content.set_protocol.secondary_cert_order.content.common_name = 'test.common.name'
    domain_op_pb.order.content.set_protocol.secondary_cert_order.content.subject_alternative_names.extend(
        ['test1.common.name', 'test2.common.name'])
    domain_op_pb.order.content.set_protocol.secondary_cert_order.content.abc_service_id = 999
    domain_op_pb.order.content.set_protocol.secondary_cert_order.content.ca_name = 'Internal'
    domain_op_pb.order.content.set_protocol.secondary_cert_order.content.public_key_algorithm_id = 'rsa'
...
@pytest.mark.parametrize('create_ctl,start_state', [
    (create_ctl_with_cert_ref, 'STARTED'),
    (create_ctl_with_cert_ref, 'CHECKING_CERT_INFO'),
    (create_ctl_with_cert_ref, 'CREATING_CERT_ORDER'),
    (create_ctl_with_http, 'STARTED'),
    (create_ctl_with_http, 'CHECKING_CERT_INFO'),
    (create_ctl_with_http, 'CREATING_CERT_ORDER'),
])
def test_cancel_without_cert_order(caplog, cache, zk_storage, ctx, create_ctl, start_state):
...
@pytest.mark.parametrize('create_ctl', [
    create_ctl_with_cert_ref,
    create_ctl_with_http,
])
def test_cancel_unsupported_without_order(caplog, cache, zk_storage, ctx, create_ctl):
    ctl = create_ctl(cache, zk_storage)
    ctl._pb.order.status.status = 'IN_PROGRESS'
    ctl._pb.order.progress.state.id = 'CANCELLING'
    ctl._pb.order.cancelled.value = True
...
def test_transitions_ctl_without_cert_order(caplog, cache, zk_storage, ctx, create_ctl, domain):
    ctl = create_ctl(cache, zk_storage)
    ctl._pb.order.progress.state.id = 'STARTED'
    update_domain_op(cache, zk_storage, ctl._pb, check=lambda pb: pb.order.progress.state.id == 'STARTED')
