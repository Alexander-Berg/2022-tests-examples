...
public class CloudEmailNotificationChannelTest {
    private NotifyClientStub notifyClient;
    private CloudAuthClientStub iamStub;
    ...
    private CloudEmailNotificationChannelFactory factory;
    private CloudEmailNotification notification;
    private NotificationChannel notificationChannel;

    @Before
    public void setUp() {
        notifyClient = new NotifyClientStub();
        iamStub = new CloudAuthClientStub();
        ...
        factory = new CloudEmailNotificationChannelFactory(notifyClient, authClient, graphLoader);
        notification = CloudEmailNotification.newBuilder()
    ...
    public void testMessageWithNoData() {
        ...
        CloudEmailNotificationChannel.Payload payload = (CloudEmailNotificationChannel.Payload) notifyClient.getOutboxEmail(0).data;
    ...
    public void emptyCrossDc() {
        ...
        var factory = new CloudEmailNotificationChannelFactory(notifyClient, authClient, graphLoader);
        notification = CloudEmailNotification.newBuilder()
            .setRecipient("bfbldg1mouscbh8gcnmq")
            .setNotifyAboutStatus(EvaluationStatus.Code.ALARM, EvaluationStatus.Code.OK)
            .setId("notification-channel")
    ...
    public void testTransformation() {
        ThresholdAlert alert = AlertTestSupport.randomCloudThresholdAlert().toBuilder()
                .setSelectors("project=solomon, cluster=production, service=test, host=solomon-dev-*")
                .setTransformations("series_sum(derivative(input{}))")
                .setPeriod(Duration.ofMinutes(15))
                .setDelaySeconds(60)
                .setPredicateRule(PredicateRule.onThreshold(30)
                        .withComparison(Compare.GT)
        ...
        for (int i = 0; i < 5; i++) {
            int size = 100;
            AggrGraphDataArrayList data = new AggrGraphDataArrayList(StockpileColumns.minColumnSet(MetricType.DGAUGE), size);

            var point = new AggrPoint();
            point.columnSet = data.columnSetMask();
            for (point.tsMillis = begin.toEpochMilli(); point.tsMillis < now.toEpochMilli(); point.tsMillis += 15_000) {
                point.valueNum = (1 + i*i) * point.tsMillis / 1e3d;
                data.addRecord(point);
            }

            solomon.addMetric(commonLabels.add("host", "solomon-dev-" + i), data);
        }
        ...
        assertEquals("", line0.alias);
        assertEquals(line0.labels, commonLabels.toMap());
        assertEquals(line0.values.length, line0.timestampsMillis.length);
        assertEquals("ALARM", line0.status);
        assertEquals(5 + 1 + 4 + 9 + 16, line0.value, 1e-6);
    }
    ...
    public void noPermissions() {
        iamStub.drop("bfbldg1mouscbh8gcnmq", Permission.DATA_READ.getSlug());
        ...
        Event event = eval(alert, state);
        var status = join(notificationChannel.send(Instant.EPOCH, event));

        assertThat(status.getCode(), equalTo(NotificationStatus.PERMISSION_DENIED.getCode()));
    }
