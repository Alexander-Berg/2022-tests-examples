...
public class IGaugeTimeSeriesStreamTest {
    private final static int MASK = StockpileColumn.TS.mask() | StockpileColumn.LONG_VALUE.mask();
    ...
    public void setUp() throws Exception {
        out = CompressStreamFactory.createOutputStream(MetricType.IGAUGE, MASK);
    ...
    @Test
    public void longMax() {
        AggrPoint source = lpoint(System.currentTimeMillis(), Long.MAX_VALUE);
        appendPoints(source);
        AggrPoint result = readNextPoint();
        assertEquals(source, result);
    }
    ...
    @Test
    public void longMin() {
        AggrPoint source = lpoint(System.currentTimeMillis(), Long.MIN_VALUE);
        appendPoints(source);
        AggrPoint result = readNextPoint();
        assertEquals(source, result);
    }
    ...
    private AggrPoint readNextPoint() {
        if (in == null || !in.hasNext()) {
            in = CompressStreamFactory.createInputStream(MetricType.IGAUGE, MASK, out.getCompressedData());
        }
