 ...
 */
public class DelegateNotificationChannelTest {
    private Notification def;
    private NotificationChannelStub channelStub;
    private DelegateNotificationChannel delegator;

    @Before
    public void setUp() throws Exception {
        def = NotificationTestSupport.randomNotification();
        channelStub = new NotificationChannelStub(def);
        delegator = new DelegateNotificationChannel(channelStub);
    }

    @Test
    public void skipReportMetricsWhenNoOneSend() {
        String result = getMetrics();
        assertThat(result, Matchers.isEmptyString());
    }

    @Test
    public void skipStatusesWithZeroCount() {
        channelStub.predefineStatuses(NotificationStatus.SUCCESS);
    ...
    public void countingStatusAsWell() {
        for (NotificationStatus.Code code : NotificationStatus.Code.values()) {
            ...
            assertThat(getMetrics(), containsString(expected));
        }
    }
