...
from balancer.test.util.predef.handler.server.http import SimpleConfig, SimpleHandler, State, DummyConfig,\
    ChunkedConfig, BrokenConfig, CloseConfig
...
@ids_parametrize('cacher_config', INVALID_CACHER_CONFIGS)
def test_invalid_cacher(ctx, req, backend_resp, cacher_config):
    """
    Balancer must try to get response from cacher and must try to put backend response to cacher
    even if cacher is broken
    """
...
@ids_parametrize('req', CACHE_REQUESTS)
def test_backend_timeout(ctx, req):
    """
    If backend sent response line + headers then balancer must send all received data to cacher
    even if backend timed out while sending response content
    """
    backend_resp = http.response.ok(headers={'Cache-Control': 'public, max-age=3600'}, data=CHUNKED_DATA)
    ctx.start_backend(SimpleConfig(response=http.response.not_found()), name='cacher')
    ctx.start_backend(ChunkedConfig(response=backend_resp, chunk_timeout=3))
    ctx.start_balancer(CachalotConfig())

    ctx.perform_request_xfail(req)

    ctx.cacher.state.get_request()
    cacher_put_req = ctx.cacher.state.read_errors.get().raw_message

    asserts.method(cacher_put_req, 'PUT')
    asserts.path(cacher_put_req, req.request_line.path)
    assert_cache_headers(cacher_put_req, backend_resp)
    # TODO: assert partial content
...
def test_invalid_backend(ctx, req, backend_config):
    """
    Balancer must not try to send backend response to cacher if backend
    sent invalid response line + headers
    """
    ctx.start_backend(SimpleConfig(response=http.response.not_found()), name='cacher')
    ctx.start_backend(backend_config)
    ctx.start_balancer(CachalotConfig())
...
@ids_parametrize(
    'backend_resp',
    [
        ('http10', http.response.raw_ok(version='HTTP/1.0', headers={'Cache-Control': 'public, max-age=300'}, data=CACHER_DATA)),
        ('http11', http.response.raw_ok(headers={'Cache-Control': 'public, max-age=300'}, data=CACHER_DATA)),
    ],
)
def test_backend_no_content_headers(ctx, backend_resp):
    """
    If backend response doesn't have Content-Length/Transfer-Encoding: chunked headers
    then balancer must send it to cacher with Transfer-Encoding: chunked
    """
    ctx.start_backend(SimpleConfig(response=http.response.not_found()), name='cacher')
    ctx.start_backend(CloseConfig(response=backend_resp))
    ctx.start_balancer(CachalotConfig(cacher_timeout=60, backend_timeout=60))

    ctx.perform_request(http.request.get(KEY, version='HTTP/1.0'))
    ...
    for run in Multirun():
        with run:
            log = ctx.manager.fs.read_file(ctx.balancer.config.accesslog)
            assert 'cacher' in log
    cacher_put_req = ctx.cacher.state.get_raw_request()
    asserts.method(cacher_put_req, 'PUT')
    asserts.path(cacher_put_req, KEY)
    assert_cache_headers(cacher_put_req, backend_resp)
    asserts.is_chunked(cacher_put_req)
    asserts.content(cacher_put_req, backend_resp.data.content)
