...
from ads_pytorch.core.runner import BaseRunnerCallback
from ads_pytorch.yt.online_learning import OnlineLearningTrainer, OnlineLearningModelCallbacksHandler


class FailureCallback(BaseRunnerCallback):
    def __init__(self, max_call_count: int):
        super(FailureCallback, self).__init__()
        self._call_counter = 0
        self._max_call_count = max_call_count

    async def __call__(self, model: BaseParameterServerModule, optimizer: ParameterServerOptimizer,
                       loss: torch.nn.modules.loss._Loss, uri: Any) -> None:
        self._call_counter += 1
        if self._call_counter >= self._max_call_count:
            raise RuntimeError
...
class PatchWithFailureTrainer(OnlineLearningTrainer):
    def update_callbacks(
        self,
        callbacks_info: OnlineLearningModelCallbacksHandler,
        train_tables,
        worker_pool,
        thread_pool,
        downloader_pool
    ):
        # Fail in the middle of evaluation according to our config
        callbacks_info.others.append(FailureCallback(8))
