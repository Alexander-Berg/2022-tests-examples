...
REGISTER_MAPPER(TPageScorerMapper<TSTModel>);


class TPageTransportTestSTHash : public TTestBase {
public:
    void SetUp() override {
        ...
        DumpSTModel(table, dumpFileName);
        TComputer<TSTModel> computer(MakeHolder<TSTModel>(dumpFileName));
        ...
        Client->Map(
                spec,
                new TPageScorerMapper<TSTModel>(std::move(computer), VectorVersion)
        );
...
private:
    UNIT_TEST_SUITE(TPageTransportTestSTHash);
    ...
    TString dumpFileName = "dump";


};
UNIT_TEST_SUITE_REGISTRATION(TPageTransportTestSTHash);
