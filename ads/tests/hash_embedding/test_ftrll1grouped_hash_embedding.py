...
HASH_TYPE = "ftrll1grouped"


@pytest.mark.parametrize("hash_type", [HASH_TYPE])
@pytest.mark.parametrize('dim', [1, 3, 100])
def test_empty_hash_lookup_ftrll1grouped(dim, hash_type):
    ...
    assert torch.all(torch.eq(item.z_ftrl, torch.FloatTensor([0] * dim)))
    assert torch.all(torch.eq(item.n_ftrl, torch.FloatTensor([0] * dim)))
    assert item.ttl_counter == 0

@pytest.mark.parametrize("hash_type", [HASH_TYPE])
@pytest.mark.parametrize('dim', [1, 3, 100])
def test_hash_insert_lookup_ftrll1grouped(dim, hash_type):
    hash_table = HashEmbedding(create_hash_table(hash_type, dim))
    item = hash_table.lookup_item(0)
    assert item.empty()

    item.w = torch.FloatTensor([4] * dim)
    item.z_ftrl = torch.FloatTensor([3] * dim)

    # Check for no side-effects
    item2 = hash_table.lookup_item(0)
    assert not torch.all(torch.eq(item2.w, torch.FloatTensor([4] * dim)))
    assert not torch.all(torch.eq(item2.z_ftrl, torch.FloatTensor([3] * dim)))

    hash_table.insert_item(0, item)
    item2 = hash_table.lookup_item(0)
    assert torch.all(torch.eq(item2.w, torch.FloatTensor([4] * dim)))
    assert torch.all(torch.eq(item2.z_ftrl, torch.FloatTensor([3] * dim)))
...
def test_serialize_model(dim, buffer_size, hash_type):
    ...
    item.z_ftrl = torch.FloatTensor([1.0] * dim)
    item.n_ftrl = torch.FloatTensor([2.0] * dim)
    hash_table.insert_item(123, item)
    item = create_hash_table_item(hash_type, dim)
    item.z_ftrl = torch.FloatTensor([3.0] * dim)
    ...
    assert torch.all(torch.eq(item.z_ftrl, torch.FloatTensor([1.0] * dim)))
    assert torch.all(torch.eq(item.n_ftrl, torch.FloatTensor([2.0] * dim)))

    item = hash_table2.lookup_item(321)
    assert torch.all(torch.eq(item.z_ftrl, torch.FloatTensor([3.0] * dim)))
