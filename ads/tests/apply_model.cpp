...
namespace {
    ...
    Y_UNIT_TEST_SUITE(ApplyModelTests) {
        Y_UNIT_TEST(LoadModelTest) {
            NPytorchTransport2::TDeepEmbeddingModel userModel(
                NPytorchTransport2::LoadDeepEmbeddingModelFromDisk("./user")
            );
            NPytorchTransport2::TDeepEmbeddingModel documentModel(
                NPytorchTransport2::LoadDeepEmbeddingModelFromDisk("./document")
            );
            Y_UNUSED(userModel);
            Y_UNUSED(documentModel);
        }

        Y_UNIT_TEST(MakeUserTensorsTest) {
            ...
            userEntity.MakeInputs(
                {*profileBuilder.GetProfile()}, {123},
                catFeatures, realvalueFeatures
            );
        }

        Y_UNIT_TEST(ApplyUserModelDummyInputTest) {
            NPytorchTransport2::TDeepEmbeddingModel userModel(
                NPytorchTransport2::LoadDeepEmbeddingModelFromDisk("./user")
            );
            ...
            userEntity.MakeInputs(
                {*profileBuilder.GetProfile(), *profileBuilder.GetProfile()}, {123, 456},
                catFeatures, realvalueFeatures
            );
        ...
        Y_UNIT_TEST(ApplyDocumentModelDummyInputTest) {
            NPytorchTransport2::TDeepEmbeddingModel docModel(
                NPytorchTransport2::LoadDeepEmbeddingModelFromDisk("./document")
            );
            ...
            NSearchTsar::TDocument doc1(tokenizer, docData), doc2(tokenizer, docData);

            THashMap<TString, NPytorchTransport2::TCategoricalInputs> catFeatures;
            THashMap<TString, NPytorchTransport2::TRealvalueInputs> realvalueFeatures;
            NSearchTsar::MakeDocumentbatch({doc1, doc2}, catFeatures, realvalueFeatures);

            auto result = docModel.Forward(catFeatures, realvalueFeatures);
            Y_UNUSED(result);
        }
    }
}
