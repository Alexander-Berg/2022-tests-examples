...
def test_page_resource(links, yt_stuff):
    ...
    with AdvMachineRobot(links, work_path, output_path, resources_path, yt_stuff=yt_stuff) as local_robot:
        local_robot.cm.call_target_async("offer_retargeting.run_all", timer=15 * 60)
        local_robot.wait_targets(["offer_retargeting.run_all", "offer_retargeting.upload_and_release_dict"])
