...
from ads.ml_engine.tests_tasks.test_expand import get_ymls, is_not_graveyard, list_dumps_mock, is_testable_task
...
learn_tasks_path = yatest.common.source_path('ads/ml_engine/tasks')
folders_to_test = [d for d in os.listdir(learn_tasks_path)
                   if os.path.isdir(os.path.join(learn_tasks_path, d))]

ymls = get_ymls(learn_tasks_path, folders_to_test, is_not_graveyard)
yml_names = [yml.split('ml_engine/tasks/')[1] for yml in ymls]
