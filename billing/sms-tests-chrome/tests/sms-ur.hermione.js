...
describe('tests', () => {
    it('выставление овердрафтного счета c подтверждением по смс, юрик', async function () {
        const { browser } = this;
        ...
        const [, request_id] = await browser.ybRun('test_request_ur_with_overdraft', {
            login
        });
        ...
        await browser.ybAssertView(
            'paystep выставление овердрафтного счета c подтверждением по смс, юрик, модальное окно подтверждение',
            elements.page,
            {
                hideElements: [...hideElements, '.yb-cart-row__eid', elements.mainButtons.submit]
            }
        );
        ...
        await browser.ybAssertView(
            'paystep выставление овердрафтного счета c подтверждением по смс, юрик, модальное окно с success',
            elements.page,
            {
                hideElements: [
                    ...hideElements,
    ...
    it('выставление кредитного счета c подтверждением по смс, юрик', async function () {
        const { browser } = this;
        ...
        const [, , , request_id] = await browser.ybRun('test_request_fictive_pa_ur', {
            login
        });
        await browser.ybUrl('user', `paystep.xml?request_id=${request_id}`);
        ...
        await browser.ybAssertView(
            'paystep выставление кредитного счета c подтверждением по смс, юрик, модальное окно подтверждение',
            elements.page,
            {
                hideElements: [...hideElements, '.yb-cart-row__eid', elements.mainButtons.submit]
            }
        ...
        await browser.ybAssertView(
            'выставление кредитного счета через смс, юрик, страница счета',
            elements.page,
