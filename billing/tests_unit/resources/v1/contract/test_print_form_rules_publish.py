...
@pytest.mark.smoke
class TestCasePrintFormRulesPublish(TestCaseApiAppBase):
    BASE_API = u'/v1/contract/print-form-rules/publish'

    @pytest.mark.parametrize('ctype', ['GENERAL', 'SPENDABLE', 'DISTRIBUTION'])
    def test_print_form_rules_publish(self, admin_role, ctype):
        role = ob.create_role(
            self.test_session,
        ...
        rule_params = {
            ...
            'branch': 'test',
            'rule_type': 'contract',
            'content': Rule(Terminal('6', '6', '6')).encode(),
        ...
        res = self.test_client.secure_post_json(
            self.BASE_API,
            data={'external_id': rule.external_id, 'force': False},
        )

        hm.assert_that(res.status_code, hm.equal_to(http.OK))
        hm.assert_that(rule.hidden, hm.equal_to(0))
        hm.assert_that(
            res.json['data'],
    ...
    def test_print_form_rules_publish_check_perm(self, admin_role):
    ...
    def test_print_form_rules_publish_error(self, admin_role):
        role = ob.create_role(
            self.test_session,
            cst.PermissionCode.PRINT_TEMPLATE_RULES_EDIT,
        )
    ...
    def test_print_form_rules_publish_error_force(self, admin_role):
        role = ob.create_role(
            self.test_session,
            cst.PermissionCode.PRINT_TEMPLATE_RULES_EDIT,
        )
        ...
        res = self.test_client.secure_post_json(
            self.BASE_API,
            data={'external_id': rule.external_id, 'force': True},
        )
