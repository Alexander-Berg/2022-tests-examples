...
@pytest.mark.smoke
class TestCasePrintFormRulesList(TestCaseApiAppBase):
    BASE_API = u'/v1/contract/print-form-rules/list'

    def test_print_form_rules_list(self, admin_role):
        role = ob.create_role(
            self.test_session,
        ...
        res = self.test_client.get(
            self.BASE_API,
            params={'rule_type': 'contract', 'ctype': 'DISTRIBUTION', 'is_published': False},
        )
        ...
        hm.assert_that(
            res.json['data'],
            hm.has_entries(
                items=hm.contains(hm.has_entries(rule_params)),
            ),
        )

    def test_print_form_rules_list_check_perm(self, admin_role):
