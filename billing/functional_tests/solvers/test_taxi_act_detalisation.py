...
from autodasha.solver_cl import TaxiActDetalisation, ParseException
...
SPENDABLE_CORPTAXI_TYPE = 81
SPENDABLE_PROMO_TYPE = 87
...
def _fake_init(self, *args, **kwargs):
    super(TaxiActDetalisation, self).__init__(*args, **kwargs)
    self.ro_session = self.session


class AbstractMockTestCase(case_utils.AbstractMockTestCase):
    _default_lines = {
        'summary': 'Детализация актов и заголовков по договорам такси',
        'contracts_eids': 'Список договоров: %s',
        'date_from': 'С какой даты: %s',
        'date_to': 'По какую дату: %s',
        'trunc_period_to_months': 'Округлить период до полных месяцев: %s',
...
class InvalidFormCase(AbstractFailingMockTestCase):
    ...
    def get_data(self, mock_manager):
        ...
        lines = [
            ...
            self._get_default_line(trunc_period_to_months='True'),
            self._get_default_line(split_period_by_months='True'),
        ]
...
class NoTruncPeriodCase(AbstractFailingMockTestCase):
    ...
    def get_result(self):
        return 'Не указан признак "Округлить период до полных месяцев".'
...
class IncorrectFormatTruncPeriodCase(AbstractFailingMockTestCase):
    ...
    def get_data(self, mock_manager):
        ...
        lines = [
            ...
            self._get_default_line(date_from='2017-02-01'),
            self._get_default_line(date_to='2017-04-01'),
            self._get_default_line(trunc_period_to_months='TrueFalse'),
            self._get_default_line(split_period_by_months='True')
        ]
        return self._get_default_line('summary'), lines

    def get_result(self):
        return 'Признак "Округлить период до полных месяцев" ' \
               'должен иметь значение True или False.'
...
class IncorrectFormatSplitPeriodCase(AbstractFailingMockTestCase):
    ...
    def get_data(self, mock_manager):
        ...
        lines = [
            ...
            self._get_default_line(date_to='2017-04-01'),
            self._get_default_line(trunc_period_to_months='True'),
            self._get_default_line(split_period_by_months='TrueFalse')
        ]
        return self._get_default_line('summary'), lines
...
@mock.patch('autodasha.solver_cl.taxi_act_detalisation.TaxiActDetalisation.__init__', _fake_init)
...
def test_parse_failing(mock_queue_object, mock_issue_data):
    ...
    solver = TaxiActDetalisation(mock_queue_object, issue)
...
class NoDatesFilledCase(AbstractGoodMockTestCase):
    ...
    def get_data(self, mock_manager):
        ...
        lines = [
            ...
            self._get_default_line(trunc_period_to_months='True'),
            self._get_default_line(split_period_by_months='False'),
        ]
        return self._get_default_line('summary'), lines
...
class AbstractDBTestCase(case_utils.AbstractDBTestCase):
    ...
    _summary = 'Детализация актов и заголовков по договорам такси'
    _description = '''
Список договоров: {contracts_eids}
С какой даты: {date_from}
По какую дату: {date_to}
Округлить период до полных месяцев: {trunc_period_to_months}
...
'''
    ...
    def gen_mock_query_row_v_partner_taxi_completion(self, contract_id, external_id, **kwargs):
        query_template = \
"""
select
  date'{dt:%Y-%m-%d}' dt,
...
"""
        ...
        params = ut.Struct(dt=dt_now, contract_id=contract_id, external_id=external_id,
                           payment_type='cash', commission_sum=10, promocode_sum=5,
                           subsidy_sum=40, currency='RUR')
        params.update(kwargs)
        return query_template.format(**params)

    def gen_mock_query_from_rows(self, rows):
        all_raws_query = '\nunion all\n'.join(rows)
        ...
        result_query_template = \
            ...
            """
        return result_query_template.format(all_raws_query=all_raws_query)

    def generate_v_partner_taxi_completions_mock_query(self, contract=None, **kwargs):
        if not contract:
            contract_id = 9999999999
            external_id = 'За500поэхали/брат'
        else:
            contract_id = contract.id
            external_id = contract.external_id

        rows = [self.gen_mock_query_row_v_partner_taxi_completion(contract_id, external_id, **kwargs) for _ in xrange(1, 6)]

        return self.gen_mock_query_from_rows(rows)
    ...
    def _get_data(self, session):
        self.contracts_data.append(
            ut.Struct(contract=ut.Struct(external_id='Неуловимый/Джо'),
                      ...
                      filename_template='commission_{contract.external_id}__{self.date_from:%Y-%m-%d}__{self.date_to:%Y-%m-%d}.xls'
                      )
        )
        self.date_from = None
        self.date_to = None
        self.split_period_by_months = False
        self.trunc_period_to_months = True
...
class Firm1AndNotSignedCase(AbstractDBTestCase):
    _representation = 'firm_1_and_not_signed_case'
    ...
    def _get_data(self, session):
        ...
        self.trunc_period_to_months = True
        self.split_period_by_months = True
    ...
    def get_result(self):
        res = RequiredResult(
            ...
            messages=[
             'Договор {self.contracts_data[0].contract.external_id} с фирмой "ООО Яндекс". '
             'Для данных условий детализация не найдена. В случае ошибки обратитесь в поддержку '
             'через ((https://wiki.yandex-team.ru/otdelsoprovozhdeniya/dokument/avtomatformi/ общую форму)).'.format(self=self),
             'Договор {self.contracts_data[1].contract.external_id} не подписан. '
             'В случае ошибки уточните данные '
             'и заполните форму ещё раз или обратитесь в поддержку через '
...
class GreatSuccessCase(AbstractDBTestCase):
    ...
    def _get_data(self, session):
        ...
        self.contracts_data.append(ut.Struct(contract=general_contract,
                                             report_type='детализация комиссии',
                                             filename_template='commission_{contract.external_id}__{date_from:%Y-%m-%d}__{date_to:%Y-%m-%d}.xls')
                                   )

        self.contracts_data.append(ut.Struct(contract=general_contract,
                                             report_type='детализация выплат безналичных заказов (отчета агента)',
                                             filename_template='agent_report_{contract.external_id}__{date_from:%Y-%m-%d}__{date_to:%Y-%m-%d}.xls')
                                   )
        corp_contract = db_utils.create_spendable_contract(session, client, person, None,
                                                           SPENDABLE_CORPTAXI_TYPE, on_dt=dt.datetime(2017, 1, 1),
                                                           external_id='РАС-666', is_offer=0)

        self.contracts_data.append(ut.Struct(contract=corp_contract,
                                             report_type='детализация корпоративных заказов',
                                             filename_template='corporate_{contract.external_id}__{date_from:%Y-%m-%d}__{date_to:%Y-%m-%d}.xls')
                                   )
        subsidy_contract = db_utils.create_spendable_contract(session, client, person, 137,
                                                              None, on_dt=dt.datetime(2017, 1, 1),
                                                              link_contract_id=general_contract.id,
                                                              is_offer=1)

        self.contracts_data.append(ut.Struct(contract=subsidy_contract,
                                             report_type='детализация прочих выплат (субсидии)',
                                             filename_template='subsidy_{contract.external_id}__{date_from:%Y-%m-%d}__{date_to:%Y-%m-%d}.xls')
                                   )
        ...
        for split_period_start, split_period_end in self.split_periods:
            db_utils.create_thirdparty_transaction(session, general_contract, 124,
                                                   dt=split_period_start)
            db_utils.create_thirdparty_transaction(session, corp_contract, 135,
                                                   dt=split_period_start)
            db_utils.create_thirdparty_transaction(session, subsidy_contract, 137,
                                                   dt=split_period_start)
    ...
    def get_result(self):
        ...
        for split_period_start, split_period_end in self.split_periods:
            for row in self.contracts_data:
                if row.report_type == 'детализация комиссии':
                    #  Наличие мат.вьюхи в bo.v_partner_taxi_completion не дает рисовать данные, мокаем запрос
                    mock_query_rows.append(
                        self.gen_mock_query_row_v_partner_taxi_completion(row.contract.id, row.contract.external_id, dt=split_period_start)
                    )
                attach_name = transliterate(row.filename_template.format(contract=row.contract,
                                                                         date_from=split_period_start, date_to=split_period_end).replace('/', '-'))
            ...
            if mock_query_rows:
                res.patch_data = (ut.Struct(
                    obj='autodasha.solver_cl.CommissionReportProcessor.query',
                    value=self.gen_mock_query_from_rows(mock_query_rows)
                    ),
                )
        return res
...
class NoDataForPeriodCase(AbstractDBTestCase):
    ...
    def _get_data(self, session):
        ...
        corp_contract = db_utils.create_spendable_contract(session, client, person, 135,
                                                            on_dt=dt.datetime(2017, 1, 1))
...
class InvalidContractTypeCase(AbstractDBTestCase):
    ...
    def _get_data(self, session):
        ...
        corp_contract = db_utils.create_spendable_contract(session, client, person, 135, None,
                                                           on_dt=dt.datetime(2017, 1, 1))
        ...
        for split_period_start, split_period_end in self.split_periods:
            db_utils.create_thirdparty_transaction(session, general_contract, 124, dt=split_period_start)
            db_utils.create_thirdparty_transaction(session, corp_contract, 135, dt=split_period_start)
        return set([row.contract for row in self.contracts_data]), self.date_from, self.date_to, \
                    self.trunc_period_to_months, self.split_period_by_months
...
class OverCountQueryFailedCase(AbstractDBTestCase):
    ...
    def _get_data(self, session):
        self.split_periods = [(dt.datetime(2017, 2, 1), dt.datetime(2017, 4, 1)),]
    ...
    def get_result(self):
        ...
        for split_period_start, split_period_end in self.split_periods:
            for row in self.contracts_data:
                ...
                message = res.get_contract_line_message(
                    ...
                    date_from=self.date_from, date_to=self.date_to,
                    comment=(
                        'Результат выполнения данного запроса превысил 1000000 строк. '
        ...
        res.patch_data = (ut.Struct(
            ...
            value=self.gen_mock_query_from_rows(mock_query_rows)
                          ...
                          ut.Struct(
            obj='autodasha.solver_cl.CommissionReportProcessor.count_rows',
...
class StartDtGreaterThenNowCase(AbstractDBTestCase):
    ...
    def _get_data(self, session):
        ...
        self.trunc_period_to_months = False
        self.split_period_by_months = False
    ...
    def get_result(self):
        res = RequiredResult(transition=IssueTransitions.wont_fix,
                             messages=['Дата начала детализации должна быть меньше текущего месяца '
                                       'при выбранной опции "Округлить период до полных месяцев"'])
        return res
...
class NoTruncPeriodCurrentMonthSuccessCase(AbstractDBTestCase):
    ...
    def _get_data(self, session):
        ...
        db_utils.create_thirdparty_transaction(session, corp_contract, 135, dt=am.end_dt)
        self.date_from = am.end_dt
    ...
    def get_result(self):
        ...
        for row in self.contracts_data:
            attach_name = transliterate(row.filename_template.format(contract=row.contract,
                                                                     date_from=row.date_from, date_to=row.date_to).replace('/', '-'))
            res.attachments_names.append(attach_name)
            message = res.get_contract_line_message(contract=row.contract, report_type=row.report_type,
...
def test_db(session, issue_data):
    ...
    solver = TaxiActDetalisation(queue_object, st_issue)
    res = solver.solve()
    session.flush()
