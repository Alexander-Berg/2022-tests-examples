        ...
        describe(`${personType} ${partner}`, () => {
            describe('валидация', () => {
                ...
                it('ОГРН, почтовый индекс', async function () {
                    ...
                    await setValue(browser, { ...details.legalAddressPostcode, value: '12345' });
                    await browser.click(elements.btnSubmit);
                    await browser.ybWaitForInvisible(elements.suggestSpin);
                    await browser.waitForVisible(elements.detailError('legalAddressPostcode'));
                    await browser.ybAssertView(
                        `поле Юр. адрес - почтовый индекс, значение несоответствующего формата, длина`,
                        elements.detail('legalAddressPostcode'),
                        assertViewOpts
                    );

                    await setValue(browser, { ...details.legalAddressPostcode, value: '12345x' });
                    await browser.click(elements.btnSubmit);
                    await browser.ybWaitForInvisible(elements.suggestSpin);
                    await browser.waitForVisible(elements.detailError('legalAddressPostcode'));
                    await browser.ybAssertView(
                        `поле Юр. адрес - почтовый индекс, значение несоответствующего формата, цифры`,
                        elements.detail('legalAddressPostcode'),
                        assertViewOpts
                    );
                    ...
                    await setValue(browser, { ...details.postcode, value: '12345' });
                    await browser.click(elements.btnSubmit);
                    await browser.ybWaitForInvisible(elements.suggestSpin);
                    await browser.waitForVisible(elements.detailError('postcode'));
                    await browser.ybAssertView(
                        `поле Почт. адрес по справ. - почтовый индекс, значение несоответствующего формата, длина`,
                        elements.detail('postcode'),
                        assertViewOpts
                    );

                    await setValue(browser, { ...details.postcode, value: '12345x' });
                    await browser.click(elements.btnSubmit);
                    await browser.ybWaitForInvisible(elements.suggestSpin);
                    await browser.waitForVisible(elements.detailError('postcode'));
                    await browser.ybAssertView(
                        `поле Почт. адрес по справ. - почтовый индекс, значение несоответствующего формата, цифры`,
                        elements.detail('postcode'),
                        assertViewOpts
                    );
                });
