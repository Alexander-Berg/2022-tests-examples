...
def test_redirect_for_persons(muzzle_logic):
    state_obj = get_state_obj()
    request_obj = get_request_obj()

    state_obj.setParam('balance_user_persons', '1')

    res = muzzle_logic.redirect_for_persons_experiment(state_obj, request_obj)

    assert res.tag == 'redirect'
    assert res.text == 'new-persons.xml?'
