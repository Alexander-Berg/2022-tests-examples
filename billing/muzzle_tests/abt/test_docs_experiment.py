    ...
    httpretty.register_uri(
        ...
        **{
            ...
            'X-Yandex-ExpFlags': b64encode(json.dumps([{
                'HANDLER': 'BALANCE',
                'CONTEXT': {'MAIN': {'BALANCE': {'balance_user_docs': 'true'}}},
                'TESTID': ['123', '234'],
            }])),
        }
    )
...
def test_check_docs_experiment_with_active_experiment(muzzle_logic, session, obj_creator, obj_type):
    ...
    assert abt_experiment.flags.get_props() == ['balance_user_docs']
...
@pytest.mark.usefixtures('httpretty_enabled_fixture')
# в experiment.py назывался test_check_docs_experiment_without_active_experiment
def test_check_docs_experiment_without_experiment(muzzle_logic, session, obj_creator, obj_type):
