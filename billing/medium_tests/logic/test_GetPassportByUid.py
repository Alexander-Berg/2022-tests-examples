...
def test_get_passport_empty_passport(xmlrpcserver, session):
    ...
    assert xmlrpcserver.GetPassportByUid(-1, passport.passport_id, {'LimitedClientIds': 1}) == expected_result
...
def test_get_passport_w_client(xmlrpcserver, session):
    ...
    expected_result = {'IsMain': 0,
                       ...
                       'Uid': passport.passport_id,
                       'ClientId': client.id}
    assert xmlrpcserver.GetPassportByUid(-1, passport.passport_id) == expected_result
...
def test_get_passport_w_limited_client(xmlrpcserver, session, relations):
    ...
    assert xmlrpcserver.GetPassportByUid(-1, passport.passport_id, relations) == expected
...
def test_get_passport_dead(xmlrpcserver, session, dead):
    ...
    expected_result = {'IsMain': 0,
                       ...
                       'ClientId': client.id,
                       'Dead': dead}
    assert xmlrpcserver.GetPassportByUid(-1, passport.passport_id, 0, True) == expected_result
