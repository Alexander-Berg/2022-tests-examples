...
def test_check_request_payment_http_error(session, medium_xmlrpc, status_code):
    def get_response():
    ...
    httpretty.register_uri(
        httpretty.POST,
        'https://' + SIMPLE_API_URL,
        get_response(),
        status=status_code)

    with pytest.raises(Fault) as exc_info:
        medium_xmlrpc.CheckRequestPayment(0, {'TransactionID': 143, 'ServiceID': 23})
    ...
    if status_code == 404:
        ...
        error_text = 'Error in connecting to trust api: <ProtocolError for {}: 502 Bad Gateway>'.format(SIMPLE_API_URL)
