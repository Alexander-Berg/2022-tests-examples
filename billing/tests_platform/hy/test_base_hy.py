            ...
            row_count = self.session.execute(
                sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                    sa.and_(
                        ...
                        base_rewards.c.contract_eid == 'C-{}'.format(test['contract_id']),
                        base_rewards.c.from_dt == from_dt,
                        base_rewards.c.till_dt == till_dt,
                        base_rewards.c.currency == 'RUR',
                        base_rewards.c.discount_type == CommType.Direct.value,
                        base_rewards.c.reward_type == RewardType.HalfYear,
                        base_rewards.c.turnover_to_charge == test['amt'],
                        base_rewards.c.reward_to_charge == test['reward'],
                        base_rewards.c.turnover_to_pay == test['amt'],
                        base_rewards.c.turnover_to_pay_w_nds == test['amt_w_nds'],
            ...
            row_count = self.session.execute(
                sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                    sa.and_(
                        ...
                        base_rewards.c.till_dt == till_dt,
                        base_rewards.c.currency == 'RUR',
                        base_rewards.c.discount_type == test['dt'],
                        base_rewards.c.reward_type == RewardType.HalfYear,
                        base_rewards.c.turnover_to_charge == test['amt'],
                        base_rewards.c.reward_to_charge == test['reward'],
                        base_rewards.c.turnover_to_pay == test['amt'],
    ...
    def test_media_common_cons(self):
        ...
        tests = [
            dict(
                ...
                amt=Decimal('10_000'),
                amt_w_nds=Decimal('10_000') * Decimal('1.2'),
                reward=Decimal('10_000') * Decimal('0.04'),
            ),
            dict(
                contract_id=self.contract_id1,
            ...
            dict(
                ...
                reward=Decimal('30_000') * Decimal('0.04'),
            ),
            dict(
                contract_id=self.contract_id1,
                ...
                reward=Decimal('40_000') * Decimal('0.04'),
            ),
        ]
...
class TestMultipleCommTypesHY(TestBase):
    ...
    def test_multiple_comm_types(self):
        ...
        tests = [
            dict(
                contract_id=self.contract_id1,
                dt=CommType.Direct.value,
                amt=Decimal('20_000'),
                amt_w_nds=Decimal('20_000') * Decimal('1.2'),
                reward=Decimal('20_000') * Decimal('0.01'),
            ),
            dict(
                contract_id=self.contract_id1,
