        ...
        session.execute(
            ...
            [
                act_q_ext(
                    ...
                    commission_type=Scale.BaseLight.value,
                    amt_q=1_000_001,
                    amt_prev_q=0,
                ),
                act_q_ext(
...
class TestSightCorrectRewardsFromActs(TestBase):
    ...
    def setup_fixtures(cls, session):
        now = datetime.now()
        q_start_from_dt, q_start_till_dt = cls.get_previous_q_first_month_ranges(now)
        q_mid_from_dt, q_mid_till_dt = cls.get_previous_q_mid_month_ranges(now)
