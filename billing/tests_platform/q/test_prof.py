    ...
    def test_correct_rewards_from_acts(self):
        ...
        contract_count = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    prof_rewards.c.contract_id == self.contract_id1,
                    prof_rewards.c.discount_type == CommType.Directory.value,
                    prof_rewards.c.reward_type == RewardType.Quarter,
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 1_000_001 * Decimal('0.22'),
                    prof_rewards.c.reward_to_pay == 1_000_001 * Decimal('0.22'),
                    prof_rewards.c.turnover_to_charge == 1_000_001,
                    prof_rewards.c.turnover_to_pay == 1_000_001,
                )
            )
        ).scalar()
        ...
        contract_count = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    prof_rewards.c.contract_id == self.contract_id2,
                    prof_rewards.c.discount_type == CommType.Directory.value,
                    prof_rewards.c.reward_type == RewardType.Quarter,
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 1_000_000 * Decimal('0.12'),
                    prof_rewards.c.reward_to_pay == 1_000_000 * Decimal('0.12'),
                    prof_rewards.c.turnover_to_charge == 1_000_000,
                    prof_rewards.c.turnover_to_pay == 1_000_000,
                )
            )
        ).scalar()
        ...
        # записи с нулевой премией не сохраняются в базе
        contract_count = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    prof_rewards.c.contract_id == self.contract_id3,
                    prof_rewards.c.discount_type == CommType.Directory.value,
                    prof_rewards.c.reward_type == RewardType.Quarter,
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 0,
                    prof_rewards.c.reward_to_pay == 0,
                    prof_rewards.c.turnover_to_charge == 100_000,
                    prof_rewards.c.turnover_to_pay == 100_000,
                )
            )
        ).scalar()
    ...
    def test_correct_rewards_from_acts(self):
        ...
        contract_count = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    prof_rewards.c.contract_id == self.contract_id1,
                    prof_rewards.c.discount_type == CommType.Sight.value,
                    prof_rewards.c.reward_type == RewardType.Quarter,
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 2_500_001 * Decimal('0.14'),
                    prof_rewards.c.reward_to_pay == 2_500_001 * Decimal('0.14'),
                    prof_rewards.c.turnover_to_charge == 2_500_001,
                    prof_rewards.c.turnover_to_pay == 2_500_001,
                )
            )
        ).scalar()
        ...
        contract_count = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 1_000_001 * Decimal('0.12'),
                    prof_rewards.c.reward_to_pay == 1_000_001 * Decimal('0.12'),
                    prof_rewards.c.turnover_to_charge == 1_000_001,
                    prof_rewards.c.turnover_to_pay == 1_000_001,
                )
            )
        ...
        contract_count = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 30,
                    prof_rewards.c.reward_to_pay == 30,
                    prof_rewards.c.turnover_to_charge == 300,
                    prof_rewards.c.turnover_to_pay == 300,
                )
            )
        ).scalar()
    ...
    def test_correct_rewards_from_acts(self):
        ...
        for index, test_dict in enumerate(expected_data):
            ...
            cnt = self.session.execute(
                sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                    sa.and_(
                        prof_rewards.c.contract_id == test_dict['contract_id'],
                        prof_rewards.c.discount_type == CommType.Dzen.value,
                        prof_rewards.c.reward_type == RewardType.Quarter,
                        prof_rewards.c.from_dt == q_start_from_dt,
                        prof_rewards.c.till_dt == q_last_till_dt,
                        prof_rewards.c.reward_to_charge == test_dict['reward'],
                        prof_rewards.c.reward_to_pay == test_dict['reward'],
                        prof_rewards.c.turnover_to_charge == test_dict['turnover'],
                        prof_rewards.c.turnover_to_pay == test_dict['turnover'],
                    )
                )
            ).scalar()
            ...
            cnt = self.session.execute(
                sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                    sa.and_(
                        ...
                        prof_rewards.c.turnover_to_charge == test['turnover'],
                        prof_rewards.c.turnover_to_pay == test['turnover'],
                    )
                )
            ).scalar()
    ...
    def test_media_no_last_year_acts(self):
        ...
        cnt = self.session.execute(
            sa.select([sa.func.count(prof_rewards.c.contract_id)]).where(
                sa.and_(
                    prof_rewards.c.contract_id == self.contract_id1,
                    prof_rewards.c.discount_type == CommType.Media.value,
                    prof_rewards.c.reward_type == RewardType.Quarter,
                    prof_rewards.c.from_dt == q_start_from_dt,
                    prof_rewards.c.till_dt == q_last_till_dt,
                    prof_rewards.c.reward_to_charge == 0,
                    prof_rewards.c.reward_to_pay == 0,
                    prof_rewards.c.turnover_to_charge == 120,
                    prof_rewards.c.turnover_to_pay == 120,
                    prof_rewards.c.turnover_to_pay_w_nds == 120 * Decimal('1.2'),
                )
            )
        ).scalar()
