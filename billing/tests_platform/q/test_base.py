"""
    Копия тестов из test_base_light с измененными шкалами
"""
...
def round_decimal(d: Decimal, rounding=ROUND_HALF_EVEN):
    return d.quantize(Decimal('1.00'), rounding=rounding)
...
def act_q_ext(contract_id, agency_id, discount_type, commission_type, amt_q, amt_prev_q, amt=0, failed=0, failed_bok=0):
    ...
    res = {'contract_eid': f'C-{contract_id}', 'amt_w_nds': amt * 10, 'till_dt': get_quarter_last_day(from_dt)}
    res.update(locals())
    res.pop('res')
    return res
...
class TestDirectoryCorrectRewardsFromActs(TestBase):
    ...
    def test_correct_rewards_from_acts(self):
        ...
        contract_id1_count = self.session.execute(
            sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    base_rewards.c.turnover_to_pay_w_nds == 1_000_001 * 10,
                )
            )
        ).scalar()

        self.assertEqual(contract_id1_count, 1)

        # contract_id2, оборот = 10^6, премия - 12%
        contract_id2_count = self.session.execute(
            sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    base_rewards.c.turnover_to_pay_w_nds == 1_000_000 * 10,
                )
            )
        ).scalar()

        self.assertEqual(contract_id2_count, 1)

        # contract_id3, оборот = 10^5, премия - 0%
        contract_id3_count = self.session.execute(
            sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    base_rewards.c.turnover_to_pay_w_nds == 100_000 * 10,
                )
            )
        ).scalar()

        self.assertEqual(contract_id3_count, 1)
        ...
        contract_id1_count = self.session.execute(
            sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    base_rewards.c.turnover_to_pay_w_nds == 2_500_001 * 10,
                )
            )
        ).scalar()
        ...
        contract_id1_count = self.session.execute(
            sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    base_rewards.c.turnover_to_pay_w_nds == 300 * 10,
                )
            )
        ).scalar()
            ...
            cnt = self.session.execute(
                sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                    sa.and_(
                        ...
                        base_rewards.c.turnover_to_pay_w_nds == test_dict['turnover'] * 10,
                    )
                )
            ).scalar()
    ...
    def test_video_rewards(self):
        ...
        expected_data = [
            ...
            dict(
                ...
                turnover=65,
                count=1,
                reward=round_decimal(65 * Decimal('0.015'), rounding=ROUND_HALF_DOWN),
            ),
            # agency_id6
            ...
            cnt = self.session.execute(
                sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                    sa.and_(
                        ...
                        base_rewards.c.turnover_to_pay_w_nds == test['turnover'] * 10,
                    )
                )
            ).scalar()
        ...
        cnt = self.session.execute(
            sa.select([sa.func.count(base_rewards.c.contract_id)]).where(
                sa.and_(
                    ...
                    base_rewards.c.turnover_to_pay_w_nds == 120 * 10,
                )
            )
        ).scalar()
...
class TestGrowthRounding(TestBase):
    """
    https://st.yandex-team.ru/BALANCE-31435
    Проверяет округление для video_growth и media_growth в YQL расчете
    """
