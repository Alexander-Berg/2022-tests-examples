...
class Resource(Object):
    ...
    def __getattr__(self, key):
        ...
        if key in collection._injected_methods:
            ...
            def injected_method(*args, **kws):
                return method(collection, self, *args, **kws)
...
class Reference(Object):
    ...
    def _dereference(self):
        if self._target is None:
            self._target = self._connection.get(self._path)
        return self._target
...
class SeekablePaginatedList(PaginatedList):
    ...
    def get_page(self, number):
        plist = self._get_page_data(expand(self._seek_page, {'page': number}))
