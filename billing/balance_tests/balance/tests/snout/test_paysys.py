...
@pytest.mark.smoke
def test_paysys_list_handle(get_free_user):
    """
    :example:
        https://snout.greed-tm.paysys.yandex.ru/v1/paysys/list
    """
    user = get_free_user()
    UserSteps.set_role(user, role_id=0)
    steps.pull_handle_and_check_result(Handles.PAYSYS, user=user)
