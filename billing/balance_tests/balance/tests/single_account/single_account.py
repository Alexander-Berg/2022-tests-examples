...
from btestlib.constants import User, Users, Passports
from balance.balance_steps import passport_steps
...
steps.ClientSteps.link(invoice_owner, 'api-trust-testuser-2')
...
direct_soid = service_order_id

context = Contexts.GEO_RUB_CONTEXT

service_order_id = steps.OrderSteps.next_id(context.service.id)
steps.OrderSteps.create(order_owner, service_order_id, service_id=context.service.id, product_id=context.product.id,
                        params={'AgencyID': agency_id, 'ManagerUID': context.manager.uid})

geo_soid = service_order_id

test_user = User(842521503, 'api-trust-testuser-2', 'Qwerty123!')
session = passport_steps.auth_session(test_user, Passports.PROD)

# Получаем csrf-токен
resp = snout_steps.CartSteps.get_item_list(session, 7)
_csrf = resp.cookies['_csrf']

snout_steps.CartSteps.post_item_add(session, 7, direct_soid, 54, None, _csrf)
snout_steps.CartSteps.post_item_add(session, 37, geo_soid, 55, None, _csrf)

resp = snout_steps.CartSteps.get_item_list(session, 7)
direct_cart_item = resp.json()['data']['items'][0]['id']
resp = snout_steps.CartSteps.get_item_list(session, 37)
geo_cart_item = resp.json()['data']['items'][0]['id']

snout_steps.CartSteps.post_create_request(session, 7, [geo_cart_item, direct_cart_item])
