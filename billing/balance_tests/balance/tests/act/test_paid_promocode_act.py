...
SERVICE_ID = 7
PRODUCT_ID = 1475
PAYSYS_ID = 101127  # payment_method: paid_promocode, firm: 1


@reporter.feature(Features.TO_UNIT)
def test_paid_promocode_act():
    ...
    act_type, external_id = act['type'], act['external_id']
    ...
    act_oebs = db.oebs().execute(
        ...
        WHERE trx_number = :external_id
        ''',
        {'external_id': external_id}
    )
    utils.check_that(act_oebs, hamcrest.equal_to([]))
    utils.check_that(act_type, hamcrest.equal_to('internal'))
