...
PAGES_MAP = {
    Services.PRACTICUM_SPENDABLE.id: [
        Pages.PRACTICUM_1,
        Pages.PRACTICUM_FLOW_1,
        Pages.PRACTICUM_MATH_MID_1,
    ],
}

PAGES_COEF = {
    Pages.PRACTICUM_1: D('1.2'),
    Pages.PRACTICUM_FLOW_1: D('1.3'),
    Pages.PRACTICUM_MATH_MID_1: D('1.4'),
}


@pytest.mark.parametrize('context, selfemployed', [
    (PRACTICUM_RU_CONTEXT_SPENDABLE.new(person_type=PersonTypes.PH, bank=Paysyses.BANK_PH_RUB), 1),
    (PRACTICUM_RU_CONTEXT_SPENDABLE, 0)
])
def test_practicum_spendable_act(context, selfemployed):
...
@pytest.mark.parametrize('context, start_dt, completions_dt, act_dt',
                         [
                             (PRACTICUM_RU_CONTEXT_SPENDABLE, prev_quarter_start_dt, prev_quarter_start_dt,
                              prev_quarter_end_dt),
                             (PRACTICUM_RU_CONTEXT_SPENDABLE, prev_quarter_start_dt, prev_quarter_last_month_start_dt,
                              prev_quarter_end_dt),
                         ],
                         ids=[
...
@pytest.mark.parametrize('context', [PRACTICUM_RU_CONTEXT_SPENDABLE],
                         ids=['PRACTICUM_RU_CONTEXT_SPENDABLE'])
