...
__author__ = ['a-vasin', 'vorobyov-as']
...
from btestlib.constants import PaymentType, PaysysType, TransactionType, Services
from btestlib.data.simpleapi_defaults import DEFAULT_PRICE, DEFAULT_COMMISSION_CATEGORY, DEFAULT_FEE, \
...
from btestlib.data.partner_contexts import EVENTS_TICKETS_CONTEXT

pytestmark = [
    reporter.feature(Features.TRUST, Features.PAYMENT, Features.EVENTS_TICKETS),
...
PROMOCODE_EXPORT_ERROR = 'TrustPayment({}) delayed: waiting for appearing composite part in t_thirdparty_transactions completely'
PAYMENT_DELAY_ERROR = 'TrustPayment({}) delayed: No payout_ready_dt for row '


SERVICES = [Services.EVENTS_TICKETS, ]
CONTEXT = [EVENTS_TICKETS_CONTEXT, ]

parametrize_context = pytest.mark.parametrize('context', CONTEXT, ids=lambda x: x.name)
...
def create_ids_for_payment_event_tickets(context):
...
@parametrize_context
def test_tickets_payment_promocode(context):
    client_id, person_id, contract_id, product, product_fee = create_ids_for_payment_event_tickets(context)

    promocode_id = promo_creating(context.service,
                                  promo_status=Promocode.Status.active,
                                  promo_amount=DEFAULT_PROMOCODE_AMOUNT)

    payment_id, trust_payment_id, _, _, _ = create_tickets_payment(context, product,
                                                                   product_fee=product_fee,
                                                                   promocode_id=promocode_id
                                                                   )

    # формируем шаблон для сравнения
    expected_payment = create_expected_payment_data(context, contract_id, client_id, payment_id, person_id, trust_payment_id,
                                                    trust_payment_id, promocode_amount=DEFAULT_PROMOCODE_AMOUNT)

    expected_fee = create_expected_fee_data(context, contract_id, client_id, payment_id, person_id, trust_payment_id,
                                            trust_payment_id)
...
@reporter.feature(Features.PROMOCODE)
@parametrize_context
def test_tickets_payment_covered_payment_with_promocode(context):
    ...
    payment_id, trust_payment_id, _, _, _ = \
        create_tickets_payment(context, product,
                               product_fee=product_fee,
                               promocode_id=promocode_id)

    # формируем шаблон для сравнения
    expected_fee = create_expected_fee_data(context, contract_id, client_id, payment_id, person_id, trust_payment_id,
                                            trust_payment_id, fee_amount=DEFAULT_PRICE + DEFAULT_FEE - promo_amount)
    ...
    expected_promo_fee = create_expected_fee_data(context, contract_id, client_id, promocode_payment_id,
                                                  person_id, promocode_trust_payment_id, promocode_trust_payment_id,
...
# тест на рефанд со сбором после платежа при использовании промокода
@reporter.feature(Features.PROMOCODE)
@parametrize_context
def test_ticket_promocode_refund_with_2_rows(context):
    ...
    expected_fee = create_expected_refund_fee_data(context, contract_id, client_id, payment_id, person_id,
                                                   trust_refund_id, trust_payment_id)
...
# тест на рефанд без сбора после платежа при использовании промокода
@reporter.feature(Features.PROMOCODE)
@parametrize_context
...
@reporter.feature(Features.PROMOCODE)
@parametrize_context
def test_tickets_export_delay_error_promocode(context):
    client_id, person_id, contract_id, product, product_fee = create_ids_for_payment_event_tickets(context)
    ...
    _, _, _, trust_payment_id = steps.SimpleApi.create_tickets_payment(context.service, product,
                                                                       product_fee=product_fee,
                                                                       promocode_id=promocode_id)
    ...
    expected_fee = create_expected_fee_data(context, contract_id, client_id, payment_id,
                                            person_id,
                                            trust_payment_id, trust_payment_id)
...
# Некоторые тесты оставляю тест без мока траста т.к здесь в основном тестируется логика траста
...
# @parametrize_context
# def test_tickets_payment_discount(context, mock_simple_api):
#     client_id, person_id, contract_id, product, product_fee = create_ids_for_payment_event_tickets(context)
#
#     payment_id, trust_payment_id, _, _, _ = create_tickets_payment(context, product,
#                                                                    product_fee=product_fee,
...
#                                                            discount_coefficient=DISCOUNT_COEFFICIENT)
#
#     expected_fee = create_expected_refund_fee_data(context, contract_id, client_id, payment_id,
...
# @reporter.feature(Features.MASTERCARD_DISCOUNT)
# @parametrize_context
# def test_ticket_discount_refund_with_1_row(context, mock_simple_api):
#     client_id, person_id, contract_id, product, product_fee = create_ids_for_payment_event_tickets(context)
#
#     payment_id, trust_payment_id, service_order_id_product, _, purchase_token = \
#         create_tickets_payment(context, product,
...
# @parametrize_context
# def test_tickets_payment_covered_payment_and_fee_with_promocode(context):
...
#     client_id_1, person_id_1, contract_id_1, product_1, product_fee_1 = create_ids_for_payment_event_tickets(context)
#     client_id_2, person_id_2, contract_id_2, product_2, product_fee_2 = create_ids_for_payment_event_tickets(context)
#
#     promocode_id = promo_creating(context.service, [context.service],
#                                   promo_status=Promocode.Status.active,
#                                   promo_amount=DEFAULT_PROMOCODE_AMOUNT)
#
#     purchase_token, service_order_id_fees, service_order_id_products, trust_payment_id = \
#         create_multiple_tickets_payment(context, [product_1, product_2],
#                                         product_fees=[product_fee_1, product_fee_2],
#                                         promocode_id=promocode_id)
#
#     payment_id = steps.SimpleApi.get_payment_by_trust_payment_id(trust_payment_id)['id']
#
...
#                                      trust_payment_id, payment_type=PaymentType.DIRECT_CARD,
#                                      paysys_type_cc=PaysysType.MONEY, promocode_amount=DEFAULT_PROMOCODE_AMOUNT / 2),
#         create_expected_payment_data(context, contract_id_2, client_id_2, payment_id, person_id_2, trust_payment_id,
#                                      trust_payment_id, payment_type=PaymentType.DIRECT_CARD,
#                                      paysys_type_cc=PaysysType.MONEY, promocode_amount=DEFAULT_PROMOCODE_AMOUNT / 2),
#         create_expected_fee_data(context, contract_id_1, client_id_1, payment_id, person_id_1, trust_payment_id,
#                                  trust_payment_id),
#         create_expected_fee_data(context, contract_id_2, client_id_2, payment_id, person_id_2, trust_payment_id,
#                                  trust_payment_id)
#     ]
#
...
#                                             trust_refund_id, trust_payment_id,
#                                             promocode_amount=DEFAULT_PROMOCODE_AMOUNT / 2),
#         create_expected_refund_fee_data(context, contract_id_1, client_id_1, payment_id, person_id_1, trust_refund_id,
#                                         trust_payment_id),
#         create_expected_refund_fee_data(context, contract_id_2, client_id_2, payment_id, person_id_2, trust_refund_id,
#                                         trust_payment_id)
#     ]
#
...
#         promo_trust_refund_id, promo_refund_id = steps.SimpleApi.create_refund(context.service, service_order_id,
#                                                                                promocode_trust_payment_id,
...
# @pytest.mark.smoke
# @reporter.feature(Features.TRUST_LOGIC, Features.PROMOCODE, Features.MASTERCARD_DISCOUNT)
# @parametrize_context
# def test_tickets_payment_promocode_with_discount(context):
#     client_id, person_id, contract_id, product, product_fee = create_ids_for_payment_event_tickets(context)
#
#     promocode_id = promo_creating(context.service,
#                                   promo_status=Promocode.Status.active,
...
def create_common_expected_data(context, contract_id, partner_id, payment_id, person_id, trust_id, trust_payment_id,
    ...
    return steps.SimpleApi.create_expected_tpt_row(
        ...
        payment_id, trust_id=trust_id, paysys_type_cc=paysys_type_cc, payment_type=payment_type,
...
def create_expected_payment_data(context, contract_id, partner_id, payment_id, person_id, trust_id, trust_payment_id,
                                 ...
                                 payment_type=None, paysys_type_cc=PaysysType.MONEY,
...
def promo_creating(service, services=None, promo_status=Promocode.Status.active, promo_amount=DEFAULT_PROMOCODE_AMOUNT):
    SimpleapiEnvironment.switch_param(dbname=TrustDbNames.BS_ORACLE, xmlrpc_url=TrustApiUrls.XMLRPC_ORA)
    promocode_id = simpleapi_steps.process_promocode_creating(service, services, promo_status,
                                                              promo_amount=promo_amount)
    SimpleapiEnvironment.switch_param(dbname=TrustDbNames.BS_PG, xmlrpc_url=TrustApiUrls.XMLRPC_PG)
    return promocode_id


def create_tickets_payment(context, product, **params):
    payment_id, trust_payment_id, service_order_id_product, service_order_id_fee, purchase_token = (
        steps.SimpleApi.create_tickets_payment_with_id(context.service, product, **params)
    )

    return payment_id, trust_payment_id, service_order_id_product, service_order_id_fee, purchase_token


def create_multiple_tickets_payment(context, products, **params):
    purchase_token, service_order_id_fees, service_order_id_products, trust_payment_id = (
        steps.SimpleApi.create_multiple_tickets_payment(context.service, products, **params)
    )
