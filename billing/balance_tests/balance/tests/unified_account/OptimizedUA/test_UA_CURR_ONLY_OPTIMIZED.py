...
pytest.mark.skip(reason='BALANCE-20958')
...
def creating_order(order_params, client_id, person_id, service_id, product_id, msr):
    ...
    if 'Parent' in order_params:
        ...
        steps.OrderSteps.make_optimized_force(order_id)
...
def creating_currency_order_group(order_params_list):
...
from_parent_with_money_to_child_ehough_funds = [
    {'Parent': True, 'with_payment': D('100.4545'), 'expected_cons_qty': [{'current_qty': D('100.4545')}]}
    , {'with_shipment': D('50.123'), 'expected_cons_qty': []}
    , {'with_shipment': D('50.3315'), 'expected_cons_qty': []}
]

from_parent_with_money_to_child_not_ehough_funds = [
    {'Parent': True, 'with_payment': D('434.55'), 'expected_cons_qty': [{'current_qty': D('434.55')}]}
    , {'with_shipment': D('323.43'), 'expected_cons_qty': []}
    , {'with_shipment': D('111.33'), 'expected_cons_qty': []}
]
...
from_child_to_parent_with_money = [
    {'Parent': True, 'with_payment': D('100.33'), 'expected_cons_qty': [
        ...
        , {'current_qty': D('4545.4455')}
        , {'current_qty': D('54.5456')}
    ]
     }
...
def test_creating_currency_order_group(order_params):
    order_params_list = creating_currency_order_group(order_params)
...
if __name__ == "__main__":
    pytest.main("test_UA_CURR_ONLY.py -v")
