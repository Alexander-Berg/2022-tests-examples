...
def making_branches(cons_ships_on_fish_order):
    ...
    steps.OrderSteps.make_optimized(main_order)
    ...
    return currency_orders_list, main_order
...
def test_making_branches(cons_ships_on_fish_order):
    ...
    for item in currency_orders_list:
        ships_sum += item['ships_qty']
    consume = db.get_consumes_by_order(item)[-1]
    utils.check_that(consume['consume_qty'], hamcrest.equal_to(ships_sum))
