...
def making_branches(cons_ships_on_fish_order):
    ...
    return currency_orders_list, fish_orders_list, main_order, client_id


@pytest.mark.parametrize('cons_ships_on_fish_order', [(
        {'cons_qty': D('100'), 'ships_qty': D('20.666666'), 'ships_on_cur_copy': D('10.2'),
         'expected_cons_qty': D('10.2')}
        , {'cons_qty': D('100'), 'ships_qty': D('31.555555'), 'ships_on_cur_copy': D('3213.13'),
           'expected_cons_qty': D('3213.13')}
)])
def test_making_branches_1(cons_ships_on_fish_order):
    currency_orders_list, fish_orders_list, _, client_id = making_branches(cons_ships_on_fish_order)
    ...
    db.balance().execute(
        '''UPDATE (SELECT * FROM t_order WHERE service_order_id = :service_order_id AND service_id = 7)
           SET UNMODERATED = 1''',
        {'service_order_id': fish_service_order_id})
