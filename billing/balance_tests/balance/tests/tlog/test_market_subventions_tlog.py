...
ROOT_PATH = YTDefaultPath.MARKET_SUBVENTIONS_YT_ROOT_PATH
...
def test_default_processing_chain():
    ...
    transaction_ids = steps.PartnerSteps.generate_subvention_transaction_log_ids(n=5)
    compls = [
          prepare_row(transaction_ids[0], 'bcds123'),
          prepare_row(transaction_ids[1], 'bcds123', amount=DEFAULT_AMOUNT*D('1.5'), product=PaymentType.PVZ_DROPOFF),
          ...
          prepare_row(transaction_ids[3], uuid.uuid1().hex, client_id=23456, amount=DEFAULT_AMOUNT * D('0.7'),
                      service_id=Services.BLUE_MARKET_SUBSIDY.id, product=PaymentType.SUBSIDY,
                      currency='USD'),
    ...
    with reporter.step(u"Закрываем предыдущий ресурс (prev_date - 1 день) и процессим текущий prev_date"):
        ...
        inserted_payments = \
            steps.PartnerSteps.get_partner_payment_stat_with_export(PVZ_RU_CONTEXT_SPENDABLE.service.id,
                                                                    transaction_ids[:1])
    ...
    with reporter.step(u"Процессим ресурс cur_date"):
        ...
        inserted_payments = \
            steps.PartnerSteps.get_partner_payment_stat_with_export(
                [PVZ_RU_CONTEXT_SPENDABLE.service.id, Services.BLUE_MARKET_SUBSIDY.id,], transaction_ids[1:])
        utils.check_that(inserted_payments, contains_dicts_with_entries(expected_data),
                         step=u'Сравним полученные платежи с ожидаемыми')
...
def test_flags():
    ...
    compls = [
        prepare_row(transaction_ids[0], uuid.uuid1().hex),
        prepare_row(transaction_ids[1], uuid.uuid1().hex, amount=DEFAULT_AMOUNT * D('1.5'), product=PaymentType.PVZ_DROPOFF),
        prepare_row(transaction_ids[2], uuid.uuid1().hex, amount=DEFAULT_AMOUNT * D('0.5'),
                    orig_transaction_id=transaction_ids[0], transaction_type=TransactionType.REFUND.name),
    ]
...
def prepare_row(
        transaction_id, service_transaction_id=None, client_id=DEFAULT_CLIENT, amount=DEFAULT_AMOUNT,
        service_id=PVZ_RU_CONTEXT_SPENDABLE.service.id, payload=None, event_time=BASE_DATE, detailed_product=None,
        transaction_time=BASE_DATE, product=PaymentType.PVZ_REWARD, transaction_type=TransactionType.PAYMENT.name,
        currency=PVZ_RU_CONTEXT_SPENDABLE.currency.iso_code, orig_transaction_id=None,
        ignore_in_balance=False, due=BASE_DATE, clid=None):
...
def prepare_yt_row(**kwargs):
    return {
        ...
        'due': steps.CommonSteps.format_dt_msk2utc(kwargs.get('due')),
...
def prepare_comparison_row(**kwargs):
    return {
        ...
        'transaction_dt': kwargs.get('transaction_time') and kwargs.get('transaction_time').replace(microsecond=0),
        'price': kwargs['amount'],
        'transaction_type': kwargs.get('transaction_type'),
        'currency': kwargs.get('currency'),
        'extra_str_0': kwargs.get('service_transaction_id'),
