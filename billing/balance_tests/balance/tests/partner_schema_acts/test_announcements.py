...
def test_announcement_acts_wo_data():
...
def test_announcement_acts():
    """Проверяем генерацию актов для Яндекс.Объявлений"""
    ...
    create_payments(context, client_id, person_id, contract_id,
                    AMOUNT, coef=2, dt=first_month_start_dt.replace(day=9))
    steps.CommonPartnerSteps.generate_partner_acts_fair_and_export(client_id, contract_id, first_month_end_dt)

    create_payments(context, client_id, person_id, contract_id,
                    AMOUNT, coef=3, dt=first_month_start_dt.replace(day=9))
        ...
        create_partner_contract(context,
                                ...
                                partner_integration_params={
                                    ...
                                    'link_integration_to_client_args': {
                                        'integration_cc': 'announcement_payments',
                                        'configuration_cc': 'announcement_payments',
                                    },
...
def test_announcement_acts_interim():
    """Проверяем генерацию актов для Яндекс.Объявлений с генерацией промежуточного акта"""

    context = ANNOUNCEMENT_CONTEXT
    client_id, person_id, contract_id = create_contract(context, first_month_start_dt)
    create_payments(context, client_id, person_id, contract_id, AMOUNT, coef=2, dt=datetime.datetime.now())

    act_dt = datetime.datetime.now()
    steps.CommonPartnerSteps.generate_interim_partner_act(contract_id, context.service.id, date=act_dt)

    expected_act_data = [
        steps.CommonData.create_expected_act_data(2 * AMOUNT, nullify_time_of_date(act_dt), context=context),
    ]
    act_data = steps.ActsSteps.get_act_data_by_client(client_id)
    utils.check_that(act_data, contains_dicts_with_entries(expected_act_data),
                     u'Сравниваем данные из акта с шаблоном')
