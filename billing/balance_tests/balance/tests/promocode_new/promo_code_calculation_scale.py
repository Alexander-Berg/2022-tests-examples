...
from promocode_commons import DIRECT_YANDEX_FIRM_RUB, DIRECT_KZ_FIRM_KZU, DIRECT_BEL_FIRM_BYN_QUASI, \
    ...
    fill_calc_params_fixed_sum, create_request, is_request_with_promocode, is_invoice_with_promocode, \
    check_invoice_consumes_discount, DIRECT_YANDEX_FIRM_FISH, DIRECT_YANDEX_FIRM_FISH_NON_RES, \
    DIRECT_KZ_FIRM_FISH, create_invoice, check_invoice_rows_discount, NOW, create_order, fill_calc_params_scale

SUM_BONUS = 5.4
QTY = Decimal('10')
QTY_FIRST_ORDER = Decimal('7.5')
QTY_SECOND_ORDER = Decimal('3.5')


@pytest.mark.parametrize('context, scale_value', [
    (DIRECT_YANDEX_FIRM_RUB, 8.4788),
    (DIRECT_KZ_FIRM_KZU, 8.933),
    (DIRECT_BEL_FIRM_BYN_QUASI, 10.004),
    (DIRECT_KZ_FIRM_KZU_QUASI, 10.004),
    (DIRECT_YANDEX_FIRM_FISH, 254.2415),
    (DIRECT_YANDEX_FIRM_FISH_NON_RES, 254.23),
    (DIRECT_KZ_FIRM_FISH, 1050)
])
@pytest.mark.parametrize('adjust_quantity', [True])
def test_scale_apply_on_pay(context, scale_value, adjust_quantity):
    ...
    calc_params = fill_calc_params_scale(currency=context.currency.iso_code, scale_points=[(scale_value, SUM_BONUS)],
                                         adjust_quantity=adjust_quantity,
                                         apply_on_create=False)
    promocode_id, promocode_code = create_and_reserve_promocode(calc_class_name=PromocodeClass.SCALE,
                                                                client_id=client_id,
...
@pytest.mark.parametrize('adjust_quantity', [True])
def test_fixed_sum_apply_on_create_adjust_quantity(context, scale_value, adjust_quantity):
    client_id = steps.ClientSteps.create()
    if context.product.type == ProductTypes.MONEY:
        steps.ClientSteps.migrate_to_currency(client_id=client_id, currency_convert_type='COPY', dt=DT_1_DAY_BEFORE,
                                              currency=context.currency.iso_code, region_id=context.region.id
