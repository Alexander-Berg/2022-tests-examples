...
from btestlib.constants import Products, Firms, Paysyses, PersonTypes, Currencies, Regions, PromocodeClass
...
def create_and_reserve_promocode(client_id, firm_id=1, is_global_unique=0, start_dt=None, end_dt=None,
                                 new_clients_only=0,
                                 valid_until_paid=1, currency=None, multicurrency_minimal_qty=None, minimal_qty=None,
                                 services=None,
    ...
    multicurrency_bonus = PROMOCODE_CURRENCY_BONUS if currency else None
    ...
    calc_params = steps.PromocodeSteps.fill_calc_params(promocode_type=PromocodeClass.LEGACY_PROMO, middle_dt=middle_dt,
                                                        bonus1=bonus1, bonus2=bonus2, minimal_qty=minimal_qty,
                                                        currency=currency, multicurrency_bonus1=multicurrency_bonus,
                                                        multicurrency_bonus2=multicurrency_bonus,
                                                        multicurrency_minimal_qty=multicurrency_minimal_qty,
                                                        discount_pct=0
                                                        )
    promo = steps.PromocodeSteps.create_new(start_dt=start_dt, end_dt=end_dt,
                                            ...
                                            reservation_days=None, service_ids=services)[0]

    promocode_id, promocode_code = promo['id'], promo['code']
    promo_code_code = db.get_promocode_by_id(promocode_id)[0]['code']
    steps.PromocodeSteps.reserve(client_id, promocode_id)
    if services:
        steps.PromocodeSteps.set_services(promocode_id, services)
    return promocode_id, promo_code_code
...
@pytest.mark.parametrize('context, params', [
    ...
    (DIRECT_YANDEX_FIRM_RUB, {'minimal_qty_currency': 8.4788, 'is_with_discount': True}),
    (DIRECT_YANDEX_FIRM_RUB, {'minimal_qty_currency': 8.4789, 'is_with_discount': False}),
...
def test_promo_code_minimal_qty_currency_check(context, params):
    ...
    promocode_id, promocode_code = create_and_reserve_promocode(client_id=client_id,
                                                                ...
                                                                currency=context.currency.iso_code,
                                                                minimal_qty=0,
                                                                multicurrency_minimal_qty=params[
                                                                    'minimal_qty_currency'],
                                                                bonus1=0, bonus2=0)

    request_id, _ = create_request(context.service.id, client_id, context.product.id, promocode_code)
    ...
    steps.PromocodeSteps.check_invoice_is_with_discount(invoice_id, bonus=PROMOCODE_CURRENCY_BONUS,
                                                        ...
                                                        qty=QTY,
                                                        nds=FIRM_NDS_MAP[context.firm.id],
...
@pytest.mark.parametrize('context, params', [
    ...
    # (DIRECT_YANDEX_FIRM_FISH, {'minimal_qty': 10.000000, 'is_with_discount': True}),
    # (DIRECT_YANDEX_FIRM_FISH, {'minimal_qty': 10.000001, 'is_with_discount': False}),
    ...
    # (DIRECT_YANDEX_FIRM_FISH_NON_RES, {'minimal_qty': 10, 'is_with_discount': True}),
    # (DIRECT_YANDEX_FIRM_FISH_NON_RES, {'minimal_qty': 10.000001, 'is_with_discount': True}),
    (DIRECT_YANDEX_FIRM_FISH_NON_RES, {'minimal_qty': 10.00177, 'is_with_discount': True}),
    (DIRECT_YANDEX_FIRM_FISH_NON_RES, {'minimal_qty': 10.00178, 'is_with_discount': False}),
