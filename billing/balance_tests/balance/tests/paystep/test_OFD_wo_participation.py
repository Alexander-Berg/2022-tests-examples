...
BANK_UR_OFD_PAYSYS_ID = 1003


# 507922 - подневный продукт, единица измерения Days
# 507931 - рублёвый продукт, Bucks
# подневный для предоплаты, прямой и АБУ, рублёвый - для постоплаты с крупными клиентами


@pytest.mark.parametrize('p', [
    # Промокод ОФД пока в ветке
    utils.aDict({'region_id': None,
                 ...
                 'paysys_id': Paysyses.PROMOCODE_OFD.id}),
    # По оферте
    utils.aDict({'region_id': None,
                 'agency_creator': lambda region_id: None,
                 'person_type': 'ur',
    ...
    pytest.mark.smoke(utils.aDict({'region_id': None,
                                   'agency_creator': lambda region_id: steps.ClientSteps.create(
                                       {'IS_AGENCY': 1, 'REGION_ID': region_id}),
                                   'person_type': 'ur',
...
def test_smoke_OFD(p):
    ...
    client_id = steps.ClientSteps.create({'REGION_ID': p.region_id, 'AGENCY_ID': agency_id})
    ...
    if p.get('invoice_person_type', None):
        person_id = steps.PersonSteps.create(client_id, p.invoice_person_type)
