...
LOGIN_ORDERS_SEARCH = User(1118982306, 'yndx-static-balance-1')
...
def test_create_objects_for_asessors_orders_xml():
    client_id = steps.ClientSteps.create()

    steps.UserSteps.link_user_and_client(LOGIN_ORDERS_SEARCH, client_id)
    ...
    steps.CampaignsSteps.do_campaigns(Services.DIRECT.id, service_order_id_direct,
                                      {Products.DIRECT_FISH.type.code: 100}, 0, dt)
    ...
    dt_1 = datetime.now() - relativedelta(days=5)
    service_order_id_direct = steps.OrderSteps.next_id(service_id=Services.DIRECT.id)
    order_id = steps.OrderSteps.create(client_id=client_id, service_order_id=service_order_id_direct,
                                       product_id=Products.DIRECT_FISH.id,
                                       service_id=Services.DIRECT.id,
    ...
    steps.InvoiceSteps.pay(invoice_id, payment_dt=dt)
    steps.CampaignsSteps.do_campaigns(Services.DIRECT.id, service_order_id_direct,
                                      {Products.DIRECT_FISH.type.code: 250}, 0, dt)
    ...
    for num in xrange(1, 5):
        ...
        orders_list = [
            {'ServiceID': Services.MARKET.id, 'ServiceOrderID': service_order_id_direct, 'Qty': 100 * num,
             'BeginDT': dt}
        ]
        request_id = steps.RequestSteps.create(client_id=client_id, orders_list=orders_list,
                                               additional_params=dict(InvoiceDesireDT=dt))
        invoice_id, _, _ = steps.InvoiceSteps.create(request_id=request_id, person_id=person_id_1,
                                                     ...
                                                     credit=0, contract_id=None, overdraft=0, endbuyer_id=None)
        steps.InvoiceSteps.pay(invoice_id, payment_dt=dt)
        steps.CampaignsSteps.do_campaigns(Services.MARKET.id, service_order_id_direct,
                                          {Products.MARKET.type.code: 100 * num}, 0, dt)
        steps.ActsSteps.generate(client_id, force=1, date=dt)
    ...
    steps.InvoiceSteps.pay(invoice_id, payment_dt=dt)
    steps.CampaignsSteps.do_campaigns(Services.MARKET.id, service_order_id_direct, {Products.MARKET.type.code: 100}, 0,
                                      dt)
    steps.ActsSteps.generate(client_id, force=1, date=dt)
    ...
    for num in xrange(1, 2):
        ...
        orders_list = [
            {'ServiceID': Services.MEDIA_70.id, 'ServiceOrderID': service_order_id_direct, 'Qty': 100 * num,
             'BeginDT': dt}
        ]
        request_id = steps.RequestSteps.create(client_id=client_id, orders_list=orders_list,
                                               additional_params=dict(InvoiceDesireDT=dt))
        invoice_id, _, _ = steps.InvoiceSteps.create(request_id=request_id, person_id=person_id_2,
                                                     ...
                                                     credit=0, contract_id=None, overdraft=0, endbuyer_id=None)
        steps.InvoiceSteps.pay(invoice_id, payment_dt=dt)
        steps.CampaignsSteps.do_campaigns(Services.MEDIA_70.id, service_order_id_direct,
                                          {Products.MEDIA_2.type.code: 100 * num}, 0, dt)
        steps.ActsSteps.generate(client_id, force=1, date=dt)
