...
def test_inc_smoke(data_cache, scenario):
    # Распаковываем словарь параметров в локальную область видимости
    # locals().update(scenario.dict)
    # update_locals(scenario.dict)
    d = scenario

    region_id, agency_creator, person_type = d.region_id, d.agency_creator, d.person_type
    contract_type, contract_params, paysys_id, credit = d.contract_type, d.contract_params, d.paysys_id, d.credit
    product = DIRECT_PRODUCT

    cached_list = ['client_id', 'agency_id', 'order_owner', 'invoice_owner', 'person_id', 'contract_id']
    with utils.CachedData(data_cache, cached_list) as c:
        ...
        agency_id = agency_creator(region_id)
        ...
        default_contract_params.update(contract_params)
        contract_id, _ = steps.ContractSteps.create_contract_new(contract_type, default_contract_params)
