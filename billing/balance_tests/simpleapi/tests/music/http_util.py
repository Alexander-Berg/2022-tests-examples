            ...
            for res in socket.getaddrinfo(self.host, self.port, 0,
                ...
                try:
                    ...
                    if self.debuglevel > 0:
                        reporter.log("connect: (%s, %s)" % (self.host, self.port))
                    self.sock.connect(sa)
                except socket.error, msg:
                    if self.debuglevel > 0:
                        reporter.log('connect fail:', (self.host, self.port))
                    if self.sock:
                        self.sock.close()
                    self.sock = None
                    continue
...
def http_post(url_parts, params, timeout=None,
    ...
    dbg('Sending %s request to [%s]://[%s]:[%d][%s]: %s body: %s' %
        (method.lower(), proto, host, port, path, params_dump, body))
    if method == 'POST':
        if not body:
            body = params_str
    else:
