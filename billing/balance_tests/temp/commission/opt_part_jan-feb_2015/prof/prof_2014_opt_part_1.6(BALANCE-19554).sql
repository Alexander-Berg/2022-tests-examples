...
--select * from s_comm_prof_2014_skv_src;
,
...
contract_id,     contract_eid,
       ...
       reward_to_charge,                            -- к начислению
       delkredere_to_charge,                        -- к начислению (делькредере)
       dkv_to_charge,                               -- ДКВ к начислению
       turnover_to_pay_w_nds,
       turnover_to_pay,                             -- оборот к перечислению
       reward_to_pay,                               -- к перечислению
       delkredere_to_pay,                           -- к перечислению (делькредере)
       dkv_to_pay                                   -- ДКВ к перечислению
  from s_opt_prof_2014
...
-- BALANCE-19554: получаем из старого расчета только те интервалы,
     ...
     where (date'2015-01-01' between opt.from_dt and opt.till_dt
        ...
        and opt.reward_type in (2, 10)


),

s_mix_skv_and_payments as (select contract_id,     contract_eid,
--BALANCE-19557
       date'2015-01-01'                         as from_dt,
       date'2015-03-01' - 1/24/60/60            as till_dt,
       nds,           
       currency,
       discount_type,
       reward_type,
...
--       abs(round(sum(reward_to_pay)     , 2)) as qwe
  from s_pre
...
having (
    -- BALANCE-19759
    abs(round(sum(turnover_to_charge), 2)) != 0 or
    abs(round(sum(reward_to_charge)  , 2)) != 0 or
    abs(round(sum(turnover_to_pay)   , 2)) != 0 or
    abs(round(sum(reward_to_pay)     , 2)) != 0
       )
)

--select * from s_mix_skv_and_payments;
,

-- BALANCE-19885
-- Акты считаем как: 10% от оборота
s_mix_to_charge as (
    select b.contract_eid, b.contract_id,
           b.currency, b.nds,
            case
      -- Если ЛС, то показываем его отдельной строкой
       when B.DISCOUNT_TYPE = 7 and NVL(a.IS_LOYAL, 0) = 1 then 71
     -- Остальное показываем "как есть"
     else B.DISCOUNT_TYPE
    end                                             as discount_type,
           date'2015-01-01'                                as from_dt,
           date'2015-03-01' - 1/24/60/60                   as till_dt,
           201                                             as reward_type,
           sum(a.amount-a.amount_nds-a.amount_nsp)      as turnover_to_charge,
           sum(a.amount-a.amount_nds-a.amount_nsp)*.1   as reward_to_charge
      from s_base       b
      join xxxx_new_comm_contract_basic    a  on a.invoice_id = b.invoice_id
                                              and a.hidden < 4
                                              and a.act_dt between date'2015-01-01'
                                                           and date'2015-03-01'-1/24/60/60

    where b.commission_type = 48
       and b.contract_till_dt > date'2014-03-01'
       and b.is_opt = 1
     group by b.contract_eid, b.contract_id, b.currency, b.nds,
  case
 when b.discount_type = 7 and nvl(a.is_loyal, 0) = 1 then 71
  else b.discount_type
  end
...
select distinct s."CONTRACT_ID",s."CONTRACT_EID",s."FROM_DT",s."TILL_DT",s."NDS",s."CURRENCY",s."DISCOUNT_TYPE",s."REWARD_TYPE",s."TURNOVER_TO_CHARGE",s."REWARD_TO_CHARGE",s."DELKREDERE_TO_CHARGE",s."DKV_TO_CHARGE",s."TURNOVER_TO_PAY_W_NDS",s."TURNOVER_TO_PAY",s."REWARD_TO_PAY",s."DELKREDERE_TO_PAY",s."DKV_TO_PAY"
  from (
    ...
    select contract_id,     contract_eid,
           ...
           0                    as delkredere_to_pay,
           0                    as dkv_to_pay
      from s_mix_to_charge
    )  s
    where reward_type = 210
order by contract_id , from_dt,till_dt, nds, currency, turnover_to_charge,
reward_to_charge,delkredere_to_charge,dkv_to_charge,turnover_to_pay_w_nds,turnover_to_pay,
reward_to_pay,delkredere_to_pay,dkv_to_pay;
