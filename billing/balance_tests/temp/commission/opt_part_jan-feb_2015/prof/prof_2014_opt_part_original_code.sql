  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."V_OPT_PROF_2014" ("CONTRACT_ID", "CONTRACT_EID", "FROM_DT", "TILL_DT", "NDS", "CURRENCY", "DISCOUNT_TYPE", "REWARD_TYPE", "TURNOVER_TO_CHARGE", "REWARD_TO_CHARGE", "DELKREDERE_TO_CHARGE", "DKV_TO_CHARGE", "TURNOVER_TO_PAY_W_NDS", "TURNOVER_TO_PAY", "REWARD_TO_PAY", "DELKREDERE_TO_PAY", "DKV_TO_PAY") AS 
  with
--
-- опорные даты
--
...
s_comm_prof_2014_acts_new_wolc as (
 select "CONTRACT_EID","CONTRACT_ID","INVOICE_EID","INVOICE_ID","INVOICE_DT","CONTRACT_FROM_DT","CONTRACT_TILL_DT","CURRENCY","NDS","PAYMENT_TYPE","DISCOUNT_TYPE","LOYAL_CLIENTS","CLIENT_ID","ACT_DT","FROM_DT","TILL_DT","AMT_W_NDS","AMT"
      from bo.v_opt_prof_2014_acts d
...
s_kv as (
    select d.contract_id, contract_eid,
      ...
      from bo.v_opt_prof_2014_acts_newwolc        d
        -- BALANCE-16952
      left outer
      join bo.v_opt_prof_2014_kv_over_70 f on f.contract_id = d.contract_id
        ...
        select b.*,
          ...
          from (
            select b.contract_eid, b.contract_id,
              ...
              from bo.v_opt_prof_2014_acts_region   b
...
s_lc_by_clients as (
    select d.contract_id, d.contract_eid,
      ...
      from (
            ...
            -- s_lc_acts вместо bo.v_comm_prof_2014_acts_newwolc
            select contract_id, contract_eid,
                   -- BALANCE-16000:
...
s_lc_src as (
    select contract_id, contract_eid, payment_type,
      ...
      from bo.v_opt_prof_2014_lc_src d
...
-- Акты, которые относятся к ЛК, у которых закончились программы.
    ...
    select d.*,
      ...
      from (
        select d.contract_eid, d.contract_id, d.from_dt, d.till_dt,
               ...
               sum(amt_rub)                     as amt
          from bo.v_opt_prof_2014_skv_src d
         group by d.contract_eid, d.contract_id, d.from_dt, d.till_dt
           ) d
),
-- ----------------------------------------------------------------------------
...
s_pre as (
    select opt.contract_id,
      ...
      left outer
        ...
        -- чтобы вычесть
      join bo.v_commission_prof_reward_2013     com on com.contract_id = opt.contract_id
