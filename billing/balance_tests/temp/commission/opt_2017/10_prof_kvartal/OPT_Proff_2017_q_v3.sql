...
-- Подписанные доп.соглашения с периодом действия
, s_cltrls as (
    select d.*, 
           start_dt                                                 as from_dt,
           nvl(
...
, s_attrs_src as (
    select d.value_num, 
		   d.key_num, 
           d.contract_id   as contract2_id, 
           d.code, 
           c.from_dt, 
           c.till_dt
      from bo.xxxx_contract_signed_attr 	d
      join s_cltrls                          c on c.collateral_id = d.collateral_id
                                             and c.contract_id = d.contract_id
                                             and c.code = d.code
)
-- BALANCE-25122: исключаемые из расчета аг-ва
, s_excluded_agencies as (
    select contract2_id as contract_id, key_num as agency_id, from_dt, till_dt
      from s_attrs_src
     ...
     where d.commission_type in (2)
      ...
      and not exists (
             select 1
              ...
              where q.till_dt between e.from_dt and e.till_dt
...
--select * From s_opt_2015_prof_q order by agency_id, contract_id;
,
s_q_by_month as (
