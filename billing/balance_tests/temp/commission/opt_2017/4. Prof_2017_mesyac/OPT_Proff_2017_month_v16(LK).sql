with
counting_date as (select date'2017-09-03' as dt from dual)
...
s_payments_src as 
(
select d."CONTRACT_EID",d."CONTRACT_ID",d."INVOICE_EID",d."INVOICE_ID",d."INVOICE_DT", d. "INVOICE_TYPE", d."CONTRACT_FROM_DT",
    ...
    select d.contract_id, contract_eid,
           ...
           sum((d.amt_to_pay_2016 + decode(f.failed,    -- BALANCE-25801
             ...
             decode(d.discount_type,
                    ...
                    0.08)
                +
                 (d.amt_to_pay_2017 + decode(f.failed,   -- BALANCE-25801
                                    0, d.amt_to_charge_2017, 0)) * case
                                    when d.discount_type in (36) then 0.15
                                    when d.discount_type in (1, 2, 3) then 0.13
                                    when d.discount_type in (7) then
                                        case
     ...
     group by d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.payment_type,
              d.discount_type, d.currency, d.nds, d.is_2017, d.do_not_fill_src
     union all
...
-- ЛК
s_nls as (
    select substr(value, 1, 1) as dot_delimeter
      from nls_session_parameters
     where parameter = 'NLS_NUMERIC_CHARACTERS'
)

--select  *from s_nls;
, s_lc_target_history as (
select d.*, sum(month_plan) over(partition by contract_id, lc_from_dt) as month_plan_ttl
  from (
    select s.contract_id,
           ...
           s.till_dt,
           add_months(to_date(
                regexp_substr(s.value_str, '\d{4}-\d{2}-\d{2}'),
                'YYYY-MM-DD')+1, -6)                            as lc_from_dt,
           to_date(
                regexp_substr(s.value_str, '\d{4}-\d{2}-\d{2}')||' 23:59:59',
                'YYYY-MM-DD HH24:MI:SS')                        as lc_end_dt,
           to_number(
                replace(
                    regexp_substr(
                        substr(value_str, instr(value_str, 'turnover')),
                        '\d+(\.\d{2})?'),
                    '.',
                    n.dot_delimeter))                          as month_plan
      from s_attrs_src          s
     cross join s_nls           n
     where s.code = 'LOYAL_CLIENTS'
       and s.value_str != 'null'
       and s.from_dt >= date'2017-03-01'
        ) d
)

--select * from s_lc_target_history;
...
, s_acts_lc as (
    select b.contract_eid,
      ...
      join xxxx_new_comm_contract_basic     xxx on b.invoice_id = xxx.invoice_id
                                              and xxx.hidden <4  
											  and xxx.is_loyal = 1
											  and xxx.act_dt >= date'2017-03-01'
											                                            
      join xxxx_currency_rate              cr on cr.cc = b.currency
                                              and cr.rate_dt = trunc(xxx.act_dt)
     where b.commission_type = 2
        -- только договора текущего фин.года
       and b.contract_till_dt > date'2017-03-01'
     group by b.contract_eid, b.contract_id, trunc(xxx.act_dt, 'MM'), xxx.client_id
)
...
, s_kv01_lc_bok as (
    select d.*,
           ...
           case
            ...
            then
                case
                when ratio_1m_ago >= 0.7 and ratio >= 0.7
                then 1
                else 0
                end
            else 0
        ...
        select d.*,
          ...
          from (
            select contract_id, from_dt, till_dt,
                   sum(amt)                 as amt,
                   sum(amt_lc)              as amt_lc,
                   sum(amt_lc) / sum(amt)   as ratio
              from (
                select contract_id, from_dt, till_dt, amt, amt as amt_lc
                  from s_acts_lc
                 union all
                select contract_id, from_dt, till_dt, sum(amt) as amt, 0 as amt_lc
                  from s_acts
...
--select * from s_kv01_lc_bok;
--select * from s_acts_lc;
--select  *from s_lc_target_history;
, s_kv01_lc_src as (
    -- 1. ЛК, которые закончились в прошлом месяце
    -- 2. все акты за период действия программы
    -- 3. Месяц окончания ДС ЛК — есть период, за который платим
    -- 4. БОК
    select a.from_dt, a.till_dt, h.lc_end_dt,
           a.contract_id, a.contract_eid, month_plan_ttl,
           sum(a.amt_rub)           as amt_rub,
           sum(a.amt_w_nds_rub)     as amt_w_nds_rub,
           sum(a.amt*0.08)          as reward
      from s_acts_lc            a
      join s_lc_target_history  h on h.contract_id = a.contract_id
                                  -- ищем версию допника на конец месяца
                                 and a.till_dt between h.from_dt and h.till_dt
                                  -- только те акты, которые попадают в период
                                  -- действия ЛК
                                 and a.from_dt between h.lc_from_dt and h.lc_end_dt
                                  -- допник закончился в прошлом месяце
                                 and h.lc_end_dt between add_months(trunc((select dt from counting_date), 'MM'), -1)
                                                     and trunc((select dt from counting_date), 'MM') - 1/24/60/60
                                  -- только субклиенты из ДС ЛК
                                 and a.client_id = h.client_id
      join s_kv01_lc_bok        b on b.contract_id = a.contract_id
                                 and b.from_dt = a.from_dt
                                 and b.failed = 0
    group by a.from_dt, a.till_dt, h.lc_end_dt, a.contract_id, a.contract_eid, h.month_plan_ttl
    having sum(a.amt_rub)>= h.month_plan_ttl
)
--select  *from s_kv01_lc_src;
--select  * from s_kv01_lc_bok;
,
s_kv01_lc as (
     select trunc(lc_end_dt, 'MM')   as from_dt, 
             lc_end_dt                as till_dt,
             contract_id, contract_eid, month_plan_ttl,
             sum(amt_rub)             as amt_rub, 
             sum(amt_w_nds_rub)       as amt_w_nds_rub,
             sum(reward)              as reward
       from s_kv01_lc_src
       group by lc_end_dt, contract_id, contract_eid, month_plan_ttl
   )
