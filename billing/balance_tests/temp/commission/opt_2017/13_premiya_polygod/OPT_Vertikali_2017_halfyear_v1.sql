 ...
 s_skv as (
    ...
    d.*,
           case
            ...
            then bo.pk_comm.calc_verticals_skv(d.amt_rub, d.from_dt)
            else 0
           end                                  as reward
      from (
        select d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.nds,
               ...
               sum(amt_w_nds_rub)               as amt_w_nds_rub,
               sum(amt_rub)                     as amt_rub
          from S_OPT_2017_VERTICALS_SKV d
         where failed = 0
           and excluded = 0
...
s_opt_2017_verticals as (select contract_id,
       contract_eid,
       from_dt,
       till_dt,
       nds,
...
--select * from s_opt_2017_verticals;
