...
--select  * from xxxx_ui_contract_apex;
,
s_brands as (
        select /*+ materialize */
...
--               c.finish_dt,
--               trunc((select dt from counting_date), 'MM') - 1)
...
s_acts_lc as (
    select 
	  ...
	  join (
        select d.*, hy.till_dt,
            sum(d.lc_turnover) over(partition by contract_id, hy.till_dt) as turnover_ttl             
            from XXXX_LOYAL_CLIENTS_CONTR_ATTR d 
            join s_half_years                     hy on d.collateral_end_dt between hy.from_dt and hy.till_dt
        where COLLATERAL_FIRST_DT>= (select fin_year_dt from s_dates)
...
--               ,d.turnover_ttl*d.lc_months_count*1.2 as qwe
          from s_acts_lc                 d
...
--
--select * from s_opt_2015_prof_skv;
,
s_skv_result as (
    ...
    select d.*,
           case
            ...
            then bo.pk_comm.calc_prof_skv(d.amt_rub, d.from_dt, d.discount_type)
            else 0
           end                                  as reward
      from (
...
select distinct s."CONTRACT_ID",s."CONTRACT_EID",s."FROM_DT",s."TILL_DT",s."NDS",s."CURRENCY",s."DISCOUNT_TYPE",s."REWARD_TYPE",s."TURNOVER_TO_CHARGE",s."REWARD_TO_CHARGE",s."TURNOVER_TO_PAY_W_NDS",s."TURNOVER_TO_PAY",s."REWARD_TO_PAY",s."REWARD_TO_PAY_SRC"
  from (
