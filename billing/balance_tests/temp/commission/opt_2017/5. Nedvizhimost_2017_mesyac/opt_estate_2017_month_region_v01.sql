...
--                 )
           ...
           case nvl(c.commission_type, c.discount_type)
            ...
            when 36 then 
                -- Видеореклама должа учитываться как мейдика, BALANCE-24734
                -- для старых счетов. для новых - показываем как видео
                case when c.invoice_dt < date'2017-03-01' 
                then 1      
                else 36
                end
...
s_acts_temp as (
    ...
    b.contract_eid,
     ...
     where b.commission_type in (1, 2, 3, 4, 5, 6, 8, 10, 11, 12, 13, 16, 36)
   and (
          -- base, prof
          (
...
s_plans as (
    select contract_id                                         as contract_id,
     ...
     and plan_dt < trunc(date'2017-12-31', 'MM')
