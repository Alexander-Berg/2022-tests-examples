...
with
-- отсюда:
-- https://hg.yandex-team.ru/balance/agency_reward/file/4a2ea7de8da3/db/views/v_opt_2015_estate.sql
-- опорные даты
s_dates as (
		-- начало фин. года
...
-- Подписанные планы. pos == 1 — ближайший слева подписанный план относительно
-- даты "ПО" рассчетного периода (31/30 число прошлого месяца)
s_plans as (
    select contract_id                                         as contract_id,
     ...
     and plan_dt < trunc((select dt from counting_date), 'MM')

),

-- Какой план брать для каждого периода
s_plans_dts as (
	select d.contract_id, d.from_dt, d.till_dt, max(p.from_dt) as plan_dt
	  from s_acts_all   d
	  left outer
	...
	select d.*,
	  ...
	  from (
		select d.contract_id, d.from_dt, d.till_dt,
		  ...
		  from s_acts_all   d
		  join s_plans_dts  t on t.contract_id = d.contract_id
							 and t.from_dt = d.from_dt
		  left outer
		  join s_plans      p on p.contract_id = d.contract_id
							 and p.from_dt = t.plan_dt
...
s_kv_pre as (
	select d.contract_id, contract_eid,
		   ...
		   sum(case
				-- BALANCE-23429: до 2016-07-01 считаем по "старому"
				-- (чтобы контроль оборотов не сломался)
				when d.from_dt < date'2016-07-01' then
					decode(f.failed, 0, d.amt_to_charge*0.4, 0)
				else
					decode(f.failed, 0, d.amt_to_charge*0.3, 0) +
