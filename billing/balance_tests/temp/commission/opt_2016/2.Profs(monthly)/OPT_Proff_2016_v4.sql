...
--                                                and oebs.comiss_date >= date'2015-03-01'   -- просто для использования вьюшки, можно закомментить
   ...
   and (
        ...
        and (
                -- BALANCE-22085
                b.invoice_dt >= date'2016-03-01' and b.discount_type in (1, 2, 3, 7, 12, 25)
...
s_acts as (
   ...
   and (
        ...
        and (
                -- BALANCE-22085
                xxx.act_dt  >= date'2016-03-01' and b.discount_type in (1, 2, 3, 7, 12, 25)
...
s_kv_control as (
    select d.*,
           ...
           when (
            ...
            and (
                    -- Договоры 2015 года
                    (contract_from_dt < date'2016-03-01' and is_there_boc = 0)
                    or
                    -- Договоры 2016 года
                    (
                        contract_from_dt >= date'2016-03-01'
...
s_kv_pre as (
    select d.contract_id, contract_eid,
           ...
           sum(case
                -- договора, заключенные в 2016 году
                when d.contract_from_dt >= date'2016-03-01' then
                    d.amt_to_charge*decode(d.discount_type,
                        ...
                        decode(f.failed,
                            0, 0.08,
                            0))
                -- договора, заключенные в 2015 году и продленные
                    ...
                    when d.contract_from_dt >= date'2016-03-01' then
                        decode(d.discount_type,
                            ...
                            25, case
                                    ...
                                    when d.amt_to_charge >= 2000000 then 0.14
                                    when d.amt_to_charge >= 1000000 then 0.12
                                    when d.amt_to_pay >=   50000 then 0.10
                                    else 0
                                end,
                            -- Для остальных — 8%
                            0.08)
                    -- договора, заключенные в 2015 году и продленные
                    else 0.1 end)           as reward_to_pay
      from s_kv_src         d
      -- BALANCE-19851
      left outer
