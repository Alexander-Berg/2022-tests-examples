...
, s_agency_stats_curr as (
    select a.agency_id                                              as agency_id,
           ...
           sum(a.amount - a.amount_nds - a.amount_nsp)              as amt
...
s_agency_stats_prev as (
    SELECT a.agency_id                                            as agency_id,
           ...
           sum(decode(q.from_dt,
                trunc(a.act_dt, 'MM'), a.amount - a.amount_nds - a.amount_nsp,
                0))                                               as amt_fm,
           sum(a.amount - a.amount_nds - a.amount_nsp)            as amt
      from xxxx_new_comm_contract_basic         a
      join s_quarters       q on a.act_dt between q.from_dt and q.till_dt
...
s_q as (
    select d.*,
           xxx_calc_prof_q(
                amt_rub,
                sum(amt_ag) over (partition by agency_id, from_dt),
                sum(amt_ag_prev) over(partition by agency_id, from_dt),
                sum(amt_ag_prev_fm) over(partition by agency_id, from_dt)
           )                                as reward
      from (
        select d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.agency_id,
               ...
               sum(amt_ag_prev)             as amt_ag_prev,
               sum(amt_ag_prev_fm)          as amt_ag_prev_fm
          from s_opt_2015_prof_q d
         where failed = 0
           and excluded = 0
