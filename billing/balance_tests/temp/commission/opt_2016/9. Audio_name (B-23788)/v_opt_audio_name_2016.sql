with
s_contract as (
    select 240035 as id, '49185/16' eid, 3001035 as person_id
      from dual
),
...
from   xxxx_new_comm_contract_basic  xxxx
	  join s_contract                       cc on cc.id = xxxx.contract_id
...
s_acts_non_name as (  
   select  
       ...
       join xxxx_currency_rate               cr on cr.cc = xxxx.currency
	    ...
	    cross join s_contract cc 
      where
        xxxx.hidden <4 
        and xxxx.person_id != cc.person_id
        and nvl(xxxx.contract_id , -1) != cc.id
        and nvl(xxxx.commission_type, xxxx.discount_type) in (29)
    )         
...
s_payments_name as (
    select xxxx.contract_eid                                        as contract_eid,
	 ...
	 join s_contract                       cc on cc.id = xxxx.contract_id     
     where nvl(xxxx.commission_type, xxxx.discount_type) in (29)
),
s_payments as (
    select contract_id, contract_eid, reward_type,
           from_dt, till_dt,
