...
s_base_src as (  
   select  distinct
     ...
     from (
      ...
      where xxxx.discount_type in (1, 2, 3, 7, 11, 12, 14, 19)
)
...
select contract_id, contract_eid, from_dt, till_dt, payment_type,
         ...
         sum(amt_to_charge*case
                when discount_type in (7, 1, 2, 3)  then 0.13
         ...
         sum(amt_to_charge*case
                when discount_type in (7, 11, 1, 2, 3)
                then 0.02
                else 0
  ...
  from (
    ...
    where (
            -- BALANCE-22085
            control_dt >= date'2016-03-01' and discount_type in (7, 1, 2, 3)
         or control_dt  < date'2016-03-01' and discount_type in (7, 11, 1, 2, 3)
         )
...
--           )                                    as reward_to_pay,
--           --BALANCE-21119
--           (least(delkredere_to_charge_sum, delkredere_to_pay_sum) -
--                least(delkredere_to_charge_sum_prev, delkredere_to_pay_sum_prev)
--           )                                    as delkredere_to_pay
            reward_to_pay,
            delkredere_to_pay
      from (
--            select d.*,   
...
where discount_type <>11 or discount_type is null
--where discount_type =11
