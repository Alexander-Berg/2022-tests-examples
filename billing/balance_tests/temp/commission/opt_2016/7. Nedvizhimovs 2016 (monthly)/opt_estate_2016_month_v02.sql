...
s_plans as (
    select contract_id                                         as contract_id,
           trunc(plan_dt, 'Q')                                    as from_dt,
           add_months(trunc(plan_dt, 'Q'), 3)-1/84600             as till_dt,
           max(plan_dt)                                           as plan_dt,
           max(plan_sum) keep (dense_rank last order by plan_dt) as plan
      from xxxx_new_comm_contract_basic
     where plan_dt is not null
     group by contract_id, trunc(plan_dt, 'Q'),
              add_months(trunc(plan_dt, 'Q'), 3)-1/84600
)
--select * from s_plans;
,
s_kv_control as (
    select d.*,
      ...
      from (
        select d.contract_id, d.from_dt, d.till_dt,
          ...
          from s_acts_all   d
          left outer join s_plans      p on p.contract_id = d.contract_id
                              -- т.к. считаем каждый месяц, а план задается
                              -- поквартально, то ищем вхождение интервалов
                             and d.from_dt between p.from_dt and p.till_dt
...
s_kv_pre as (
    select d.contract_id, contract_eid,
           ...
           sum(decode(f.failed, 0, d.amt_to_charge*0.3, 0)) as reward_to_charge,
           sum(decode(f.failed2, 0, d.amt_to_charge*0.1, 0))
                                   as reward_to_charge2,
            f.failed2,
           sum(d.amt_to_pay)            as turnover_to_pay,
           sum(d.amt_to_pay_w_nds)      as turnover_to_pay_w_nds,
           -- BALANCE-19851
           -- Штрафовать по оплатам не должны
           sum(decode(f.failed, 0, d.amt_to_pay*0.3, 0) +
                decode(f.failed2, 0, d.amt_to_pay*0.1, 0)
               )        as reward_to_pay
      from s_kv_src         d
      left outer
      join s_kv_control     f on f.contract_id = d.contract_id
