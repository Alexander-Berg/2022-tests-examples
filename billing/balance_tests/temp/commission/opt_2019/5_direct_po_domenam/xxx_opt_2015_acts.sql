...
--                 )
          ...
          nvl(
           (select distinct a.value_num
            from s_attrs_src a
            where c.invoice_dt between a.from_dt and a.till_dt
             and a.code = 'AR_PAYMENT_CONTROL_TYPE'
             and a.contract_id = c.contract_id), 0
       )                                        as payment_control_type,
...
s_brands as (
        select /*+ materialize */
               ...
               c.start_dt           as dt,
               trunc(nvl(c.finish_dt, date'2019-04-03'))
...
--                                               and nvl(c.finish_dt, (select dt from counting_date))
         ...
         group by atr.key_num, atr.start_dt, c.start_dt, nvl(c.finish_dt, date'2019-04-03')
)
