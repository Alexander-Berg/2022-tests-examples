...
--         оплату счета не начисляется и не перечисляется
     ...
     d.is_early_payment = 1
       ...
       and d.invoice_dt >= date'2017-03-01'
      and d.invoice_dt <  date'2019-03-01'
           )  d
     group by d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.payment_type,
...
s_kv10_src as (
    select d.contract_id, contract_eid,
           ...
           -- BALANCE-30925: не учитываем премии от актов, которые случились после
           --                начала работы нового КО (по периодам), чтобы не увеличивать
           --                сумму нарастающим итогом
           case
            -- BALANCE-31096: для досрочных оплат пропускаем
            when d.from_dt >= date'2019-03-01'
             and d.discount_type = 7
             and d.turnover_to_charge = 0
            then d.reward_to_charge
              -- для остальных нельзя ничего пропускать
            when d.from_dt >= date'2019-03-01'
            then 0
