CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."XXX_OPT_2015_ACTS" ("CONTRACT_EID", "CONTRACT_ID", "INVOICE_EID", "INVOICE_ID", "INVOICE_DT", "INVOICE_TYPE", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "CURRENCY", "NDS", "PAYMENT_TYPE", "COMMISSION_TYPE", "PAYMENT_CONTROL_TYPE", "DISCOUNT_TYPE", "DISCOUNT_TYPE_SRC", "IS_AGENCY_CREDIT_LINE", "IS_LOYAL", "CLIENT_ID", "ORDER_ID", "SERVICE_ID", "SERVICE_ORDER_ID", "AGENCY_ID", "ACT_ID", "ACT_EID", "ACT_DT", "FROM_DT", "TILL_DT", "AMT_W_NDS", "AMT", "AMT_W_NDS_RUB", "AMT_RUB", "BRAND_ID", "NDS_COUNT", "CURRENCY_COUNT") AS 
  with
counting_date as (select date'2019-05-03' as dt from dual),
...
--           x.act_dt                                   as act_dt,  
      ...
      where xxxx.discount_type in (1, 2, 3, 7, 11, 12, 13, 14, 19, 27,  36, 37, 42)
)
--select * from s_base_2013_src;
,
  ...
  where (
                                                ...
                                                (
                                                    ...
                                                    nvl(c.commission_type,
                                                        c.discount_type) in
                                                    (1, 2, 3, 7, 11, 12, 14, 17, 19, 22, 25, 28, 29, 36, 37, 40)
                                                )
                                                   or
                                                (
                                                    -- Беларусия
...
s_brands as (
        select /*+ materialize */
               ...
               c.start_dt           as dt,
               trunc(nvl(c.finish_dt, date'2019-06-03'))
...
--                                               and nvl(c.finish_dt, (select dt from counting_date))
         ...
         group by atr.key_num, atr.start_dt, c.start_dt, nvl(c.finish_dt, date'2019-06-03')
)
...
s_ar_acts_src as (
   ...
   and (
        ...
        and (
                ...
                xxx.act_dt >= date'2019-03-01' and b.contract_till_dt > date'2019-03-01' and b.discount_type in (1, 2, 3, 7, 12, 36, 37, 38, 40)
                -- BALANCE-24516: новые условия применимы только актов нового
                -- фин.года при условиия, что договор продлен
                 or xxx.act_dt >= date'2017-03-01' and b.contract_till_dt > date'2017-03-01' and b.discount_type in (1, 2, 3, 7, 12, 36, 37, 38)
                -- BALANCE-22085
...
-- BALANCE-24734: добавляем 36 тип, чтобы старые счета учесть
  ...
  and (
   ...
   select b.contract_eid,
           ...
           at.amount*cr.rate                                 as amt_w_nds_rub,
           (at.amount-at.amount_nds-at.amount_nsp)*cr.rate as amt_rub
       from s_base_2013_src                       b
       join xxxx_new_comm_contract_basic     xxx on b.invoice_id = xxx.invoice_id
                                              and xxx.hidden <4
                                              and xxx.act_dt >= date'2019-04-01'
      join xxxx_act_trans                   at on at.act_id = xxx.act_id
      join xxxx_order                       o  on o.order_id = at.parent_order_id                    
      join xxxx_currency_rate              cr on cr.cc = b.currency
                                              and cr.rate_dt = trunc(xxx.act_dt)
...
from s_acts_src;
 ...
 with
counting_date as (select date'2019-06-03' as dt from dual),
...
--select * from s_base_2013_src order by  invoice_id ;
,
