...
-- под объединение не попадут оферы по естественным причинам
...
s_q_growth_not_consdted as (
    select d.contract_id, d.contract_eid,
      ...
      from (
        ...
        d.contract_id,
               d.contract_eid,
               q.contract_id                            as main_contract_id,
               qq.from_dt                              as from_dt,
               decode(d.discount_type,
                    -- Справочник считаем как Директ, т.к. шкала по общему обороту
                    12, 7,
                    -- Вся медийка под номером 1 будет
          ...
          from s_acts      d
           join s_quarters_2             qq on d.act_dt between qq.from_dt and qq.till_dt
                                        -- смотрим данные только за последние 3 месяца
                                        and trunc((select dt from counting_date), 'MM') - 1 between qq.from_dt and qq.till_dt
          join s_ar_consolidations_q     q on q.cons_type = 1
                                            -- консолидированные договоры
                                             and  q.linked_contract_id = d.contract_id
          ...
          from s_acts       d
           join s_quarters_2             qq on d.act_dt between qq.from_dt and qq.till_dt
                                        ...
                                        and trunc((select dt from counting_date), 'MM') - 1 between qq.from_dt and qq.till_dt
          where   
            d.contract_id in ( select q.contract_id 
                                  from s_ar_consolidations_q   q  
                                  where cons_type = 1 )
...
--s_q_audio as (
-- select 
-- '10001/13' as CONTRACT_EID,
--10001 as CONTRACT_ID,
--1 as COMMISSION_TYPE,
--date'2018-03-01'   as FROM_DT,
--date'2018-06-01'-1/24/60/60 as TILL_DT,
--29 as DISCOUNT_TYPE,
--'RUR' as CURRENCY,
--1 as NDS,
--1 as AMT_RUB,
