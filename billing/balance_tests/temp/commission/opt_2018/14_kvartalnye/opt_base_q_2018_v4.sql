-- CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."V_OPT_2015_BASE_Q" ("CONTRACT_EID", "CONTRACT_ID", "AGENCY_ID", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "DISCOUNT_TYPE", "FROM_DT", "TILL_DT", "TILL_DT_FC", "AMT_W_NDS_RUB", "AMT", "AMT_FOR_FORECAST", "AMT_AG", "AMT_AG_Q", "AMT_AG_PREV", "AMT_AG_PREV_FM", "FAILED", "EXCLUDED") AS 
  
 --!!!!!!!В БРЕНДАХ НАДО ЕЖЕМЕСЯЧНО МЕНЯТЬ ДАТУ(160+ СТРОКИ)!!!!!!!!
  with
...
--select * from s_excluded_clients;
...
--               nvl(s.is_signed, s.is_faxed)             as sign_dt
          ...
          join xxxx_contract_signed_attr       fd on 
           ...
           join s_quarters                           q on q.till_dt between cp.start_dt and fd.value_str
...
select cp.contract_id,     cp.contract_id||'/18'    as contract_eid,
       cp.start_dt,
       cp.finish_dt,
--       cp.sign_dt,
        ...
        select d.*,
          ...
          from (
            select d.contract_id, 
              ...
              from s_acts_src             d
              join s_quarters                     q  on d.act_dt between q.from_dt
                                                                     and q.till_dt
             where d.contract_id in (select contract_id from s_curr_contracts)
             group by q.from_dt,
                      d.contract_id,
                      d.brand_id
               ) d
 ...
 s_agency_stats_src as (
    select a.agency_id,
     ...
     where a.commission_type in (1, 8, 21)
       ...
       and a.is_loyal = 0
     group by a.agency_id, q.from_dt, a.from_dt,
           decode(a.discount_type,
                -- Справочник считаем как Директ, т.к. шкала по общему обороту
                12, 7,
                -- Вся медийка под номером 1 будет
...
s_opt_2015_base_q as (  
   ...
   -- если косолидация есть для договора, то надо весь оборот
       ...
       -- сохраняем оригинальные номера договоров
       d.contract_eid                       as orig_contract_eid,
       d.contract_id                        as orig_contract_id,
       c.cons_type,
       d.agency_id,
...
-- За квартал
       ...
       case
        when round(b.max_ratio, 2) >= 0.7 then 1
        else 0
       end                      as failed_bok,
       0                        as excluded
  ...
  left outer
  join s_bok                    b on b.contract_id = d.contract_id
                                 and b.from_dt = d.from_dt
...
s_q_by_month as (
    ...
    select contract_eid, contract_id, from_dt, till_dt, agency_id,
     ...
     where failed = 0
       and excluded = 0
     group by contract_eid, contract_id, from_dt, till_dt, agency_id, failed_bok,
...
s_q_growth as (
    select d.*,
            case
            when failed_bok = 1 and discount_type = 7 then 0
            else
...
s_q_growth_not_consdted as (
    select orig_contract_eid            as contract_eid,
           orig_contract_id             as contract_id,
           contract_id                  as main_contract_id,
     ...
     where failed = 0
       and excluded = 0
       and (
                -- только те договоры, которые необходимо
                -- пропорционально консолидировать
                cons_type = 1
                -- и их гланые договоры
            or contract_id in (select contract_id
                                 from s_ar_consolidations_q
                                where cons_type = 1)
           )
     group by orig_contract_eid, orig_contract_id, contract_id,
              from_dt, till_dt, discount_type
)
