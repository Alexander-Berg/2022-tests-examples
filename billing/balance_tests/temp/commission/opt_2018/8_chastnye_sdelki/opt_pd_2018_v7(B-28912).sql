  ...
  where (
                                               (
                                                    ...
                                                    nvl(c.commission_type,
                                                        c.discount_type) in
                                                    (1, 2, 3, 7, 11, 12, 14, 17, 19, 22, 25, 28, 29, 36, 37, 38)
                                                )
                                                 or
                                                (
                                                    -- Маркет
...
--           nvl(brand.main_client_id, xxx.client_id)          as brand_id,
      ...
      join xxxx_act_trans                   at on at.act_id = xxx.act_id
...
--      join s_brands                               brand on brand.client_id = xxx.client_id
   ...
   and (
        ...
        and (
                ...
                xxx.act_dt >= date'2017-03-01' and b.contract_till_dt > date'2017-03-01' and b.discount_type in (1, 2, 3, 7, 12, 36, 37, 38)
                -- BALANCE-22085
...
,
s_src_notification as (
    select * from xxxx_deal_notification
)

, 
s_last as (
    select external_id, max(doc_date) as doc_date
      from s_src_notification
      -- BALANCE-28534: смотрим только нотификации до конца прошлого месяца
       where doc_date <= trunc((select dt from counting_date), 'MM') -1/24/60/60
     group by external_id
)
,

s_ar_deal_notification as 
(
select d."ID",d."EXTERNAL_ID",d."DOC_DATE",d."DOC_NUMBER"
  from s_src_notification                d
  join s_last               l on l.external_id = d.external_id
                             and l.doc_date = d.doc_date

)
--select * from s_ar_deal_notification;
,
...
--select * from s_last_stat_insert_dt;
,
...
s_ar_deal_stats as (
	...
	select 
      ...
      d."SHOWS",d."CLICKS",
      d."COST",
      d."AMT_RUR_WO_NDS",d."COSTCUR",d."INSERT_DT",
	     dl.name                      as deal_name,    --VARCHAR2(512 BYTE) Указанное площадкой название сделки. Ограничение в директе - 255 символов
		   dl.agency_rev_ratio  as agency_reward_ratio,--NUMBER  Вознаграждение агентства, выбранное площадкой. В долях единицы (0,12 означает 12%)
		   dn.doc_date,   -- DATE Дата уведомления о подтверждении условий сделки
		   dn.doc_number  -- VARCHAR2(64 BYTE)?  Номер уведомления о подтверждении условий сделки
    ...
    from s_stat_src                d
     ...
     left outer
    join s_ar_deal_notification dn on dn.external_id = d.deal_external_id
)
...
s_dt as (
    select trunc((select dt from counting_date), 'MM') - 1/24/60/60 as dt from dual
)
--select  *from s_dt;
,
    ...
    select contract_id as contract2_id, 
          code,
           ...
           where code = 'PERSONAL_DEAL_BASE_PCT'
           		and start_dt <= (select dt from s_dt)
           group by contract_id,code
...
s_acts as (
    select contract_id, contract_eid, from_dt, till_dt, invoice_id, invoice_eid,
      ...
      join s_dt                 d on d.dt between a.from_dt and a.till_dt
      -- BALANCE-27852
       and a.commission_type in (1, 2, 8, 21)
       -- BALANCE-28535
       and a.discount_type = 38
...
s_stats_src  as (
    ...
    service_id, service_order_id,
           ...
           -- BALANCE-27943: сумма сделки
           -- соотношение сделки к заказу смотрим за всю историю
           sum(cost) over (
                partition by service_id, service_order_id,
                             deal_external_id)                  as cost_by_deal,
           sum(cost) over( 
                partition by service_id, service_order_id)      as cost_by_order
      from s_ar_deal_stats   s
        -- BALANCE-27943: сделки в будущем не надо учитывать
      join s_dt                 d on s.till_dt <= d.dt
)

--select  *from s_stats_src;
,
...
s_stats as (
    select s.*,
           -- BALANCE-27943:
           -- соотношение сделки к заказу смотрим за всю историю
           cost_by_deal/cost_by_order                           as ratio
      from s_stats_src          s
      join s_dt                 d on d.dt between s.from_dt and s.till_dt
...
s_rewards as (
    select 
           a.contract_id, a.contract_eid, a.from_dt, a.till_dt,
           a.invoice_id, a.invoice_eid, a.order_id, a.act_id, a.act_eid,
           s.deal_name, s.deal_external_id, s.pct,
           s.doc_date, s.doc_number,
           -- сумма сделки в акте
           sum(a.amt_rub*s.ratio)                                as deal_amt,
           -- база сделки для расчета премии
           sum(a.amt_rub*s.ratio/(1 + a.ya_n_fox_pct))           as base_amt,
           -- премия сделки
           sum((a.amt_rub*s.ratio/(1 + a.ya_n_fox_pct))*s.pct)   as reward
--*
      from s_acts       a
      join s_stats      s on s.service_id = a.service_id
                         and s.service_order_id = a.service_order_id
    group by a.contract_id, a.contract_eid, a.from_dt, a.till_dt,
           a.invoice_id, a.invoice_eid, a.order_id, a.act_id, a.act_eid,
           s.deal_name, s.deal_external_id, s.pct,
           s.doc_date, s.doc_number
)

--select  *from s_rewards order by contract_id;
,

s_control as (
    select deal_external_id, sum(deal_amt) deal_amt
      from s_rewards
     group by deal_external_id
    having sum(deal_amt) >= 50000
)

--select  * from s_control;
...
select r.contract_id,
       ...
       r.act_eid,
       r.order_id,
       r.deal_name,                                           -- название сделки
       r.doc_date,                                            -- дата уведомления
       r.doc_number,                                          -- номер уведомления
       r.deal_amt                          as turnover_ttl,    -- оборот сделки из акта
       r.base_amt                         as turnover_reward, -- база для расчета премии
       r.pct,
       decode(c.deal_external_id,
            null, 0,
            r.reward)                     as reward
  from s_rewards r
  left outer
  join s_control        c on c.deal_external_id = r.deal_external_id
--  where deal_external_id in (select deal_external_id from s_control)
