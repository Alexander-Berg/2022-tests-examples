...
s_q_src as (
    select a.contract_id, a.contract_eid,
           a.contract_from_dt, a.contract_till_dt,
           q.from_dt, q.till_dt,
           -- только для прогноза
           a.till_dt                as till_dt_fc,
           a.nds, a.currency,
           p.plan,
           a.amt_rub,
           a.amt_w_nds_rub
...
s_acts_last_q as (
    ...
    from s_opt_2017_estate_q   a
        -- только прошлый квартал
    where trunc((select dt from counting_date), 'MM') -1/86400 between a.from_dt and a.till_dt
...
, s_q as (
    select a.*,
          bo.pk_comm.calc_estate_q(a.amt_rub, a.plan)          as reward
...
--                   смотрим только первые 2 месяца кваратала (+1 на всякий)
                   and r.from_dt between a.from_dt  and add_months(a.from_dt, 1) + 1
          ...
          1= (
            ...
            and r.reward_to_charge > 0
            and r.from_dt = add_months(a.from_dt, 2)
           )
        )
)
,
