...
--select  * from s_acts_boc;
,

s_opt_2018_belarus_q as (
select d.contract_eid, d.contract_id,
       d.contract_from_dt, d.contract_till_dt,
...
--select * from s_opt_2018_belarus_q;
,

s_quart as (
   select d.*,
          case
           when month_count = 3 then
               bo.pk_comm.calc_belarus_q(d.amt)
           else 0
          end                              as reward
     from (
       select d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.nds,
              d.currency,
              count(distinct till_dt_fc)   as month_count,
              sum(amt_w_nds)               as amt_w_nds,
              sum(amt)                     as amt
         from s_opt_2018_belarus_q      d
        where failed = 0
          and excluded = 0
...
s_opt_2018_belarus as (
  select contract_id,
       ...
       2                    as reward_type,
       amt                  as turnover_to_charge,                          -- оборот к начислению
       reward               as reward_to_charge,                            -- к начислению
       amt_w_nds            as turnover_to_pay_w_nds,
       amt                  as turnover_to_pay,                             -- оборот к перечислению
       reward               as reward_to_pay_src,                           -- к перечислению
       reward               as reward_to_pay                                -- к перечислению
  from s_quart
...
select s."CONTRACT_ID",s."CONTRACT_EID",s."FROM_DT",s."TILL_DT",s."NDS",s."CURRENCY",s."DISCOUNT_TYPE",s."REWARD_TYPE",s."TURNOVER_TO_CHARGE",s."REWARD_TO_CHARGE",s."TURNOVER_TO_PAY_W_NDS",s."TURNOVER_TO_PAY",s."REWARD_TO_PAY",s."REWARD_TO_PAY_SRC"
  from (
