CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."X_V_OPT_2015_INVOICES" ("CONTRACT_EID", "CONTRACT_ID", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "INVOICE_EID", "INVOICE_ID", "INVOICE_DT", "INVOICE_TYPE", "PAYMENT_CONTROL_TYPE", "TOTAL_SUM", "CURRENCY", "NDS", "NDS_PCT", "LOYAL_CLIENTS", "DISCOUNT_TYPE", "DISCOUNT_TYPE_SRC", "PAYMENT_TYPE", "IS_AGENCY_CREDIT_LINE", "COMMISSION_TYPE") AS 
     ...
     with
--менять даты в этом формате date'2020-02-03' по всему файлу на желаемую дату запуска расчета
--т.е. если надо посчитать в феврале за январь, то указываем  date'2020-02-03'
counting_date as (select date'2020-02-03' as dt from dual),
...
s_brands as (
        select 
               ...
               atr.collateral_dt,
               c.dt,
               trunc(nvl(c.finish_dt, date'2020-02-03'))
         ...
         where atr.code = 'BRAND_CLIENTS'
           ...
           and atr.value_num = 1
         group by atr.key_num, atr.collateral_dt, c.dt, nvl(c.finish_dt, date'2020-02-03')
),
...
--       or (b.commission_type in (10, 16) and 1=1)
       or (b.commission_type in (10) and 1=1)
          -- BALANCE-33446: в Python/SQS более не считаем. Будет считаться в бункере
       or (b.commission_type in (16) and a.dt < date'2020-01-01')
...
--       or (b.commission_type in (10, 16) and 1=1)
       or (b.commission_type in (10) and 1=1)
          -- BALANCE-33446: в Python/SQS более не считаем. Будет считаться в бункере
       or (b.commission_type in (16) and b.invoice_dt < date'2020-01-01')
...
CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."X_V_AR_AGENCIES_STATS" ("AGENCY_ID", "SERVICE_ID", "SERVICE_ORDER_ID", "CLIENT_ID", "MONTH", "AMT") AS 
  ...
  with
-- закомментила для полугодовых расчетов
--  counting_date as (select date'2020-02-03' as dt from dual),
--
--s_dates as (
...
,  s_acts_src as (
    ...
    join s_hy h
       ...
       and a.from_dt < trunc((select  dt from counting_date), 'MM')-1
    where a.commission_type in (1, 2, 8, 21, 23)
      and a.discount_type in (1, 2, 3, 7, 36, 37)
)
-- БОК по договору (до консолидации)
, s_bok as (
...
-- Акты (консолидации учитывается только для типа "на главный")
, s_acts as (
           ...
           a.amt_w_nds,
            case
              when a.discount_type = 7
               and round(b.max_ratio, 2) >= 0.7
              then 1
              else 0
           end                                                      as failed_bok
      from s_acts_src                   a
      left outer
      join bo.x_v_ar_consolidations_hy c on c.linked_contract_id = a.contract_id
      left outer
      join s_bok                        b on b.contract_id = a.contract_id
...
, s_acts_res  as (
    ...
    from s_acts a
      -- исключаем записи, где есть консолидация и сфейленный БОК
        -- чтобы не было по одному договору несколько записей
     where not (a.cons_type = 2 and a.failed_bok = 1)
...
-- Считаем консолидированный оборот.
--  - для консолидации "на главный" не учитываем БОК
--  - для консолидации "попроция" просто
, s_acts_cons as (
    select a.contract_id,
           a.discount_type,
           -- общий оборот для консолидированных пропорционально
           sum(a.amt) over (partition by
                nvl(c.contract_id, a.contract_id),
                a.discount_type)                as amt
      from (
            select a.contract_id, a.discount_type,
                   sum(a.amt)                   as amt
              from s_acts                       a
             group by a.contract_id, a.discount_type
           )                                a
      left outer
      join x_v_ar_consolidations_hy    c on c.linked_contract_id = a.contract_id
                                             and c.cons_type = 1
...
select a.contract_id,
  ...
  from s_acts_res                       a
left outer  
join s_acts_cons                      c on c.contract_id = a.contract_id
                                         and c.discount_type = a.discount_type;


----------------BALANCE-33255-------------BALANCE-33172---без ТК и шкал----------------------
CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."X_V_AR_INVOICES" ("CONTRACT_EID", "CONTRACT_ID", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "INVOICE_EID", "INVOICE_ID", "INVOICE_DT", "INVOICE_TYPE", "PAYMENT_CONTROL_TYPE", "TOTAL_SUM", "CURRENCY", "NDS", "NDS_PCT", "LOYAL_CLIENTS", "DISCOUNT_TYPE", "DISCOUNT_TYPE_SRC", "PAYMENT_TYPE", "IS_AGENCY_CREDIT_LINE", "COMMISSION_TYPE") AS 
...
select 
  ...
  join s_attrs                          ca_ct  on ca_ct.contract_id = c.id
                                              and ca_ct.code = 'WHOLESALE_AGENT_PREMIUM_AWARDS_SCALE_TYPE'
                                              and ca_ct.value_num is not null
  -- тип оплаты:
  --  2                    предоплата
  --  3                    постоплата
  join s_attrs                          ca_pt  on ca_pt.contract_id = c.id
  ...
  CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."X_V_AR_INVOICES_COMM" ("CONTRACT_EID", "CONTRACT_ID", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "INVOICE_EID", "INVOICE_ID", "INVOICE_DT", "CURRENCY", "NDS", "NDS_PCT", "INVOICE_TYPE", "TOTAL_SUM", "LOYAL_CLIENTS", "ENDBUYER_ID", "ENDBUYER_INN", "PERSON_INN", "IS_OPT", "IS_THERE_OLD_ACTS", "DISCOUNT_TYPE", "DISCOUNT_TYPE_SRC", "PAYMENT_TYPE", "IS_AGENCY_CREDIT_LINE", "PAYMENT_CONTROL_TYPE", "COMMISSION_PAYBACK_TYPE", "COMMISSION_PAYBACK_PCT", "COMMISSION_TYPE") AS 
  select c.external_id                            as contract_eid,
       c.id                                     as contract_id,
       fr.dt                                    as contract_from_dt,
       tl.value_dt                              as contract_till_dt,
...
CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."X_V_AR_ACTS_ALL" ("CONTRACT_EID", "CONTRACT_ID", "INVOICE_EID", "INVOICE_ID", "INVOICE_DT", "INVOICE_TYPE", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "CURRENCY", "NDS", "PAYMENT_TYPE", "COMMISSION_TYPE", "PAYMENT_CONTROL_TYPE", "DISCOUNT_TYPE", "DISCOUNT_TYPE_SRC", "IS_AGENCY_CREDIT_LINE", "IS_LOYAL", "CLIENT_ID", "ORDER_ID", "SERVICE_ID", "SERVICE_ORDER_ID", "AGENCY_ID", "ACT_ID", "ACT_EID", "ACT_DT", "FROM_DT", "TILL_DT", "AMT_W_NDS", "AMT", "AMT_W_NDS_RUB", "AMT_RUB", "BRAND_ID") AS 
  with
counting_date as (select date'2020-02-03' as dt from dual),
...
select 
  ...
  join bo.x_mv_currency_rate            cr on cr.cc = b.currency
   ...
   select  b.contract_eid,
          ...
          from bo.x_v_ar_invoices_comm       b
          join bo.x_t_act                         a  on a.invoice_id = b.invoice_id
                                                  and a.hidden < 4
                                                  and a.dt >= date'2018-01-01'
          join bo.x_t_act_trans                   at on at.act_id = a.id
...
CREATE OR REPLACE FORCE EDITIONABLE VIEW "BO"."X_V_AR_PAYMENTS" ("CONTRACT_EID", "CONTRACT_ID", "INVOICE_EID", "INVOICE_ID", "INVOICE_DT", "INVOICE_TYPE", "CONTRACT_FROM_DT", "CONTRACT_TILL_DT", "CURRENCY", "NDS", "PAYMENT_TYPE", "COMMISSION_TYPE", "DISCOUNT_TYPE", "DISCOUNT_TYPE_SRC", "IS_AGENCY_CREDIT_LINE", "INVOICE_TOTAL_SUM", "INVOICE_TOTAL_SUM_W_NDS", "PAYMENT_CONTROL_TYPE", "DOC_DATE", "OEBS_DT", "PAYMENT_NUMBER", "FROM_DT", "TILL_DT", "AMT", "AMT_W_NDS", "AMT_BY_INVOICE", "PAYMENTS_CURR_BY_INVOICE_W_NDS", "IS_FULLY_PAID_PRE", "IS_EARLY_PAYMENT_PRE", "IS_EARLY_PAYMENT_TRUE_PRE", "IS_2018", "FULLY_PAID_POS", "EARLY_PAYMENT_POS", "EARLY_PAYMENT_TRUE_POS", "AMT_TTL_W_NDS", "AMT_TTL", "PAYMENT_COUNT_BY_CONTRACT", "PAYMENT_COUNT_BY_INVOICE", "IS_FULLY_PAID", "IS_EARLY_PAYMENT", "IS_EARLY_PAYMENT_TRUE") AS 
  with
