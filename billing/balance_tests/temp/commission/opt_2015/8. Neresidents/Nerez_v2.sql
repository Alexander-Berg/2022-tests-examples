...
s_payments as (
    select b.contract_eid,
           ...
           oebs.oebs_payment*100/(100 + b.nds*b.nds_pct) * cr.rate       as amt_rub,
           oebs.oebs_payment * cr.rate        as amt_w_nds_rub
      from s_base_src                       b
      -- платежи
	  ...
	  join xxxx_currency_rate               cr on cr.cc = b.currency
                                              and cr.rate_dt = trunc(oebs.comiss_date)
     where b.commission_type = 57
...
select contract_id, contract_eid, from_dt, till_dt, payment_type,
  ...
  from (
     ...
     -- BALANCE-21122
    where discount_type in (7, 11, 1, 2, 3)
