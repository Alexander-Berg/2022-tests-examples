...
s_base_src as (  
   select  distinct
     ...
     from (
      ...
      where xxxx.discount_type in (1, 2, 3, 7, 11, 12, 14, 19, 36)
)
...
select contract_id, contract_eid, from_dt, till_dt, payment_type,
         ...
         sum(amt_to_charge*case
                when discount_type in (7, 1, 2, 3, 36)  then 0.13
         ...
         sum(amt_to_charge*case
                when discount_type in (7, 11, 1, 2, 3, 36)
                then 0.02
                else 0
  ...
  from (
     ...
     -- BALANCE-21122
    where discount_type in (7, 11, 1, 2, 3, 36)
