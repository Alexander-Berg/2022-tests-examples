...
s_years as (
    select date'2015-03-01' from_dt, date'2016-01-01'-1/24/60/60 as till_dt from dual
     union all
    select date'2016-01-01' from_dt, date'2017-01-01'-1/24/60/60 as till_dt from dual
),
...
select distinct
  ...
  where contract_commission_type in (1, 2, 3, 4, 5, 8)
    and (
                                                -- BALANCE-17175
                                                (
...
--                                                    BALANCE-20041
                                                ...
                                                (
                                                    ...
                                                    contract_commission_type in (10) and 1=1
                                                )
                                                or
                                                (
                                                    -- иначе, как ранее
...
s_payments as (
    select b.contract_eid,
     ...
     where b.commission_type = 3
        -- Для сервисов: Спецпроекты
       and b.discount_type in (17)
),
-- ----------------------------------------------------------------------------
-- Базовое КВ (месячное)
-- ----------------------------------------------------------------------------
...
,
s_OPT_2015_SPEC_SKV as (select d.contract_eid, d.contract_id,
       d.contract_from_dt,
       d.contract_till_dt,
