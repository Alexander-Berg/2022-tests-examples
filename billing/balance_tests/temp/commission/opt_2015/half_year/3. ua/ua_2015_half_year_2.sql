...
s_skv_1 as (
    select d.contract_eid, d.contract_id,
      ...
      join s_half_years                 h  on d.act_dt between h.from_dt
                                                         and h.till_dt
    -- Директ, Медийка (вся), Справочник, Маркет
...
s_opt_2015_ua_skv as (select d."CONTRACT_EID",d."CONTRACT_ID",d."CONTRACT_FROM_DT",d."CONTRACT_TILL_DT",d."CURRENCY",d."DISCOUNT_TYPE",d."COMMISSION_TYPE",d."CLIENT_ID",d."FROM_DT",d."TILL_DT",d."TILL_DT_FC",d."AMT_W_NDS",d."AMT_FOR_FORECAST",d."AMT",
...
s_skv as (
    select d.*,
      ...
      from (
        select d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.currency,
               sum(amt_w_nds)               as amt_w_nds,
               sum(amt)                     as amt
          from s_opt_2015_ua_skv d
         where failed = 0
         group by d.contract_eid, d.contract_id, d.from_dt, d.till_dt, d.currency
           ) d
)
,
