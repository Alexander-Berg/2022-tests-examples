...
--       xxx.external_id                                    as act_eid,
  ...
  join xxxx_currency_rate            cr on cr.cc = b.currency
                                          and cr.rate_dt = trunc(xxx.act_dt)
 where b.commission_type in (1, 2, 3, 4, 5, 8)
   and (
          -- base
          (b.commission_type = 1 and b.discount_type in (1, 2, 3, 7, 11, 12))
          -- prof
       or (b.commission_type = 2 and b.discount_type in (1, 2, 3, 7, 11, 12))
          -- spec
       or (b.commission_type = 3 and b.discount_type in (1, 2, 3))
          -- auto
       or (b.commission_type = 4 and b.discount_type in (1, 2, 3))
          -- sprav
       or (b.commission_type = 5 and b.discount_type = 12)
          -- base region
       or (b.commission_type = 8 and b.discount_type in (1, 2, 3, 7, 11, 12))
       )
)
...
--  select * from s_acts_lc;
 ...
 s_skv_lc as (
     select d.*
      ...
      where d.amt_rub >= d.lc_turnover*6*1.1
 )
...
--,
 ...
 s_skv as (select d.*,
       xx_calc_base_skv(d.amt_forecast, d.from_dt) as reward
    from (
      select d.contract_eid, d.contract_id,
             d.from_dt, d.till_dt,
             d.contract_from_dt,
             d.contract_till_dt,
             sum(amt_rub_not_lc)*100    as amt_forecast,
             sum(amt_rub_not_lc)              as amt_fact
        from s_opt_2015_base_skv d
...
s_opt_2015_base as (select contract_id,
  ...
  from (
        ...
        select contract_eid, contract_id,
               ...
               'RUR'                            as currency,
               1                                as nds,
               amt_fact                          as turnover_to_charge,
               amt_fact                           as reward_to_charge,
               amt_forecast                          as turnover_to_pay,
               amt_fact                   as turnover_to_pay_w_nds,
               amt_fact                           as reward_to_pay,
               2                                as type
          from s_skv
