...
--                             and contract_id = (select qwe from s_dates)                 
...
--       and b.contract_id = (select qwe from s_dates)
)
    ...
    select d.*,
      ...
      from (
        select d.contract_eid, d.contract_id, d.from_dt, d.till_dt,
               -- BALANCE-15641
               sum(amt_rub_not_lc)               as amt_w_nds_rub,
               sum(amt_rub_lc)                     as amt_rub
...
-- s_skv AS ( 
------- select D.*,
-------               XX_CALC_PROF_SKV(D.AMT_FORECAST, D.FROM_DT) as REWARD
--------         from (
--            select d.contract_eid, d.contract_id,
--                   d.from_dt, d.till_dt,
--                   d.contract_from_dt,
--                   d.contract_till_dt,
--                   sum(amt_rub_not_lc)    as amt_forecast,
