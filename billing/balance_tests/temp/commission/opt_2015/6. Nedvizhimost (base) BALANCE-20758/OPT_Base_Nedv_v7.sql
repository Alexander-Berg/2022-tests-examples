...
--       xxx.external_id                                    as act_eid,
 ...
 where xxx_test_commission_type(b.commission_type, b.discount_type) = 1
)
...
s_payments as (
    select b.contract_eid,
     ...
     where b.commission_type = 1
       and xxx_test_commission_type(1, b.discount_type) = 1
)
...
s_kv_src as (
    select contract_eid, contract_id,
           currency, discount_type,
 		   decode(discount_type, 29, 0.4, 0.1) as reward_pct,
           nds, from_dt, till_dt, payment_type,
...
s_kv_control_src as (
    select d.*,
      ...
      from (
        select contract_id, from_dt, till_dt, client_id,  discount_type,
               nds_count, currency_count,
			    amt_rub_estate, client_count_estate,
			-- общий оборот по клиенту
               sum(amt_rub)                                  as amt_rub,
			-- оборот по директу по клиенту
               sum(decode(discount_type, 7, amt_rub, null))  as amt_rub_direct
          from (
                select d.*,
							-- оборот по недвижимости по договору
                          sum(decode(discount_type, 29, amt_rub, null))
                           over (partition by contract_id) as amt_rub_estate,
                         -- кол-во клиентов по недвижимости
                          count(distinct decode(discount_type,
                                  29, client_id, null))
                           over (partition by contract_id) as client_count_estate,
         ...
         group by contract_id, from_dt, till_dt, client_id,discount_type,
					amt_rub_estate, client_count_estate,
                  nds_count, currency_count
           ) d
)
--select * from s_kv_control_src order by contract_id;
,
-- Штрафы
...
s_kv_control as (
    select d.*,
           case
          when discount_type = 29 then
                   case
                  when client_count_estate >= 2
                    and amt_rub_estate >= 30000
                   then 0
                   else 1
                    end
              else
      ...
      from (
        select d.*,
          ...
          from (
            select contract_id, from_dt, till_dt, discount_type,
                   nds_count, currency_count,
                   amt_rub_estate, client_count_estate,
                   sum(amt_rub)             as amt_rub,
             ...
             group by contract_id, from_dt, till_dt,  discount_type,
					amt_rub_estate, client_count_estate,
                      nds_count, currency_count
               ) d
           ) d
...
--select * from s_kv_control order by contract_id;
,
...
--           d.reward_pct,
           ...
           sum(d.amt_to_charge*decode(
                 f.failed, 0, d.reward_pct,
           ...
           sum(d.amt_to_pay*reward_pct) as reward_to_pay
      from s_kv_src         d
      -- BALANCE-19851
      left outer
...
--              , f.failed,     d.reward_pct
)
--select * from s_kv_pre order by contract_id;
,
