...
def test_1():
    ...
    person_id = steps.PersonSteps.create(client_id, 'usp')
    ...
    # steps.ActsSteps.generate(client_id, force=1, date=BASE_DT)
    campaigns_qty_2 = 30
    steps.CampaignsSteps.do_campaigns(service_id, service_order_id_2, {'Bucks': campaigns_qty_2, 'Money': 0}, 0,
                                      BASE_DT)
    steps.ActsSteps.generate(client_id, force=1, date=BASE_DT)
    ...
    steps.OrderSteps.transfer([{'order_id': dpt_order_id, 'qty_old': QTY, 'qty_new': QTY / 2, 'all_qty': 0}],
