...
from btestlib.constants import Services, PersonTypes, Paysyses, PaymentMethods
...
# steps.InvoiceSteps.create_fast_payment(invoice_id)
...
res = steps.TrustApiSteps.pay_request(passport_id=1120000000073173, request_id=request_id,
                                      payment_method_id=PaymentMethods.CARD.id,
                                      currency_code='RUB', person_id=person_id)

# Печать чека
query = "select id from bo.t_payment where invoice_id = :invoice_id"
payment_id = db.balance().execute(query, {'invoice_id': invoice_id})[0]
...
with reporter.step(u'Получаем краткие данные чека из таблицы BO.T_FISCAL_RECEIPT'):
    ...
    reporter.log('Ссылка на чек: https://check-renderer.testing.yandex.net/?n=%s&fn=%s&fpd=%s'
                 % (check_data['receipt_fd'], check_data['receipt_fn'], check_data['receipt_fpd']))
