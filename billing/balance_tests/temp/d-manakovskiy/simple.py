...
from balance.tests.overdraft.autooverdraft import prepare_direct_contract
from btestlib import utils
from btestlib.constants import Regions, Services, ContractCommissionType, Firms, Currencies, ContractPaymentType
# from balance.balance_templates import Contexts
...
context = Contexts.SHOP_HK_CONTEXT
QTY = D('250')
COMPLETIONS = D('99.99')
...
person_params = {
    # 'purchase_order': u'123456abcdef'
}
person_id = None or steps.PersonSteps.create(invoice_owner, context.person_type.code, person_params)

# Создаём договор:
contract_id, _ = steps.ContractSteps.create_contract_new(
    ContractCommissionType.NO_AGENCY,
    {
        'CLIENT_ID': client_id,
        'PERSON_ID': person_id,
        'SERVICES': [
        ...
        'FIRM': Firms.SPB_SOFTWARE_1483.id,
        'CURRENCY': Currencies.EUR.num_code,
        'PAYMENT_TYPE': ContractPaymentType.PREPAY,
        'COUNTRY': Regions.GR.id,
        'DT': NOW - relativedelta(months=15),
        'FINISH_DT': NOW + relativedelta(months=12),
        'IS_SIGNED': utils.Date.to_iso(utils.Date.nullify_time_of_date(NOW - relativedelta(months=15)))
    }
)
# contract_id = None
...
for _ in xrange(1):
    ...
    steps.OrderSteps.create(
        ...
        params={
            'AgencyID': agency_id,
            'ManagerUID': context.manager.uid,
        },
    )
    orders_list.append(
        {
...
# Выставляем счёт
invoice_id, _, _ = steps.InvoiceSteps.create(
    request_id,
    person_id,
    context.paysys.id,
    credit=0,
...
steps.CampaignsSteps.do_campaigns(
    ...
    {
        context.product.type.code: COMPLETIONS,
    },
    0,
    COMPLETIONS_DT,
)
...
# Выставляем акт
acts_list = steps.ActsSteps.generate(invoice_owner, force=1, date=ACT_DT)

print "Созданные акты: %s" % acts_list

steps.ExportSteps.export_oebs(product_id=context.product.id)
