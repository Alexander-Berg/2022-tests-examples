...
TOLOKA_INC_USD_CONTEXT = Contexts.TOLOKA_FISH_USD_CONTEXT.new(firm=Firms.TOLOKA_INC,
...
CREDIT_LIMIT_USD = Decimal('78')
QTY = Decimal('50')


def toloka_contracts(is_postpay):
    context = TOLOKA_INC_USD_CONTEXT
    ...
    if not is_postpay:
        contract_params.update({'PAYMENT_TYPE': 2})
    else:
    ...
    # orders_list = [
    #     {'ServiceID': context.service.id, 'ServiceOrderID': service_order_id_1, 'Qty': 1,
    #      'BeginDT': ALMOST_OVERDUE_DT_NOT_RUB},
    #     {'ServiceID': context.service.id, 'ServiceOrderID': service_order_id_2, 'Qty': 1,
    #      'BeginDT': OVERDUE_DT}
    # ]
    ...
    # steps.CampaignsSteps.do_campaigns(context.service.id, service_order_id_1, {'Bucks': 1}, 0, ALMOST_OVERDUE_DT_NOT_RUB)
    # steps.ActsSteps.create(invoice_id_1, ALMOST_OVERDUE_DT_NOT_RUB)
    # steps.CampaignsSteps.do_campaigns(context.service.id, service_order_id_2, {'Bucks': 1}, 0, OVERDUE_DT)
