...
from btestlib.constants import TransactionType, Collateral, Products, Services, CorpTaxiOrderType
...
PERSONAL_ACC_SUM_1 = D('15300')
PAYMENT_SUM_1 = D('2000')
REFUND_SUM_1 = D('500')
PERSONAL_ACC_SUM_2 = D('16000')
...
def test_act_generation_prepay(personal_acc_sum_1, payment_sum_1, refund_sum_1, personal_acc_sum_2,
    ...
    api.test_balance().MigrateContractToDecoupling(contract_id)
...
def test_promocode(adjust_quantity, apply_on_create, contract_services):
    ...
    context = CONTEXT.new(special_contract_params=deepcopy(CONTEXT.special_contract_params),
                          contract_services=contract_services)
    context_create_contract = CONTEXT.new(special_contract_params=deepcopy(CONTEXT.special_contract_params),
    ...
    service_id = max([Services.TAXI_CORP_CLIENTS.id])
    service_order_id = steps.OrderSteps.get_order_id_by_contract(contract_id, 650)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.mark.skip(reason='https://st.yandex-team.ru/BALANCE-30676')((D('0'), REFUND_SUM_1, PAYMENT_SUM_1)),
