...
BASE_DATE = utils.Date.nullify_time_of_date(dt.datetime.now()-dt.timedelta(days=55))
...
def test_default_processing_chain():
    ...
    client_id_1, person_id_1, contract_id_1, _ = steps.ContractSteps.create_partner_contract(TAXI_RU_CONTEXT,
                                                                                 is_offer=1,
                                                                                 additional_params=
                                                                                 {'start_dt': BASE_DATE - dt.timedelta(days=30)})
    client_id_2, person_id_2, contract_id_2, _ = steps.ContractSteps.create_partner_contract(TAXI_RU_CONTEXT,
                                                                                 is_offer=1,
                                                                                 additional_params=
                                                                                 {'start_dt': BASE_DATE - dt.timedelta(days=30)})
    client_id_3, person_id_3, contract_id_3, _ = steps.ContractSteps.create_partner_contract(TAXI_RU_CONTEXT,
                                                                                 is_offer=1,
                                                                                 additional_params=
                                                                                 {'start_dt': BASE_DATE - dt.timedelta(days=30)})
    ...
    compls = [
        prepare_row(888, client_id=client_id_1),
        prepare_row(999, client_id=client_id_2, currency=Currencies.USD.iso_code, service_id=Services.TAXI_128.id),
        prepare_row(1111, client_id=client_id_1, amount=D('-999.123'), currency=Currencies.USD.iso_code, product=TaxiOrderType.promocode_tlog),
        prepare_row(2222, client_id=client_id_3, service_id=Services.TAXI_CORP_CLIENTS.id,
