...
file_path = '/Users/yuelyasheva/Downloads/Чек-лист 1003.xls'
sheet_AI = u'АИ'
sheet_CI = u'КИ'
sheet_new_CI = u'новый КИ'
...
def get_ci_stat():
    ...
    # 7 (H) - признак авто (hermione) (пустое, если нет)

    columns = {'header': 0, 'smoke': 4, 'manual': 5, 'assessors': 6, 'automated': 7}
    ...
    for rownum in range(1, ci_sheet.nrows):
        ...
        if not row[columns['header']]:
            ...
            # если в колонке "авто (hermione) не пусто
            if row[columns['automated']]:
...
def get_new_ci_stat():
    xls_file = xlrd.open_workbook(file_path)
    new_ci_sheet = xls_file.sheet_by_name(sheet_new_CI)

    # 0 (А) - признак заголовка (пустое, если нет)
    # 4 (E) - признак смоук (пустое, если нет)
    # 5 (F) - признак ручной регресс (пустое, если нет)
    # 6 (G) - признак авто (hermione) (пустое, если нет)

    columns = {'header': 0, 'smoke': 4, 'manual': 5, 'automated': 6}
    ...
    for rownum in range(1, new_ci_sheet.nrows):
        row = new_ci_sheet.row_values(rownum)
...
def fill_test_cases_ci_stat():
    ci_stat = get_ci_stat()
    dst = yt.TablePath('//home/balance-test/metrics/test_cases_ci_stat_with_auto', append=True)
...
def fill_test_cases_new_ci_stat():
    new_ci_stat = get_new_ci_stat()
    dst = yt.TablePath('//home/balance-test/metrics/test_cases_new_ci_stat', append=True)
    new_ci_stat.update({'dt': datetime.datetime.today().strftime('%Y-%m-%d')})
    # new_ci_stat.update({'dt': datetime.datetime(2021,11,1).strftime('%Y-%m-%d')})
    write_test_table(dst, [new_ci_stat])

print(get_ci_stat())
# fill_test_cases_ci_stat()


# print(get_new_ci_stat())
# fill_test_cases_new_ci_stat()
