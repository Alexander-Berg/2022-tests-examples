...
INPUT_DATA = {
    ...
    StagerProject.CONNECT: (lambda c: {
        'aggregations': [
            make_connect_input(Products.CONNECT, qty=177, str_dt=c['str_dt']),
            make_connect_input(Products.CONNECT_MONEY, qty=21, str_dt=c['str_dt']),
        ]
    }),
    ...
    StagerProject.TAXI: (lambda c: {
        'tl_revenues_in': [
            make_taxi_input(transaction_id=1, client_id=111580562, clid='1', currency=Currencies.RUB,
                            transaction_type=TransactionType.PAYMENT, amount=42.7569, str_dt=c['str_dt']),
            make_taxi_input(transaction_id=2, client_id=111580562, clid='1', currency=Currencies.USD,
                            transaction_type=TransactionType.REFUND, amount=1.3423, str_dt=c['str_dt'],
                            aggregation_sign=-1),
            make_taxi_input(transaction_id=3, client_id=111580562, clid='2', currency=Currencies.RUB,
                            transaction_type=TransactionType.PAYMENT, amount=17.1819, str_dt=c['str_dt']),
            make_taxi_input(transaction_id=4, client_id=111580562, clid='4', currency=Currencies.RUB,
                            transaction_type=TransactionType.PAYMENT, product=TaxiOrderType.subsidy_tlog,
                            aggregation_sign=-1,
                            amount=1.00, str_dt=c['str_dt']),

            make_taxi_input(transaction_id=5, client_id=111580546, clid='1', currency=Currencies.RUB,
                            transaction_type=TransactionType.PAYMENT, amount=31.0234, str_dt=c['str_dt']),
            make_taxi_input(transaction_id=6, client_id=111580546, clid=None, currency=Currencies.RUB,
                            transaction_type=TransactionType.REFUND, amount=15.8763, str_dt=c['str_dt'],
                            aggregation_sign=-1),

            make_taxi_input(transaction_id=7, client_id=111580538, clid='5', currency=Currencies.RON,
        ...
        # пустые, так как здесь процесится старая версия, которая нигде не используется
        'order_billings': [],
        'marketplace_billings': []
    })
}
...
EXPECTED_DATA = {
    ...
    StagerProject.CONNECT: (lambda c: {
        'calculations': [
            make_connect_output(Products.CONNECT, date=c['str_dt'], qty=177, qti=utils.dround(177.0 / 30, 6)),
            make_connect_output(Products.CONNECT_MONEY, date=c['str_dt'], qty=21, qti=1.0),
        ]
    }),
    ...
    StagerProject.TAXI: (lambda c: {
        ...
        'tl_revenues_out': [
            make_taxi_output_revenue(client_id=111580562, currency=Currencies.RUB, last_transaction_id=3,
                                     amount=59.9388, str_dt=c['str_dt']),
            make_taxi_output_revenue(client_id=111580562, currency=Currencies.USD, last_transaction_id=2,
                                     amount=-1.3423, str_dt=c['str_dt']),
            make_taxi_output_revenue(client_id=111580546, currency=Currencies.RUB, last_transaction_id=6,
                                     amount=15.1471, str_dt=c['str_dt']),
            make_taxi_output_revenue(client_id=111580562, currency=Currencies.RUB, last_transaction_id=4,
                                     amount=-1.0, str_dt=c['str_dt'], product=TaxiOrderType.subsidy_tlog),
            make_taxi_output_revenue(client_id=111580538, currency=Currencies.RON, last_transaction_id=7,
