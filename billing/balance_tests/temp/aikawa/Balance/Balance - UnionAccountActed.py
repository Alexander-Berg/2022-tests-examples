...
steps.CampaignsSteps.do_campaigns(SERVICE_ID, service_order_id, {MSR: 200}, 0, dt)
...
steps.CampaignsSteps.do_campaigns(SERVICE_ID, service_order_id, {MSR: 100}, 0, dt)
...
orders_list = [
    {'ServiceID': SERVICE_ID, 'ServiceOrderID': service_order_id, 'Qty': 300, 'BeginDT': dt}
]
request_id = steps.RequestSteps.create(client_id=client_id, orders_list=orders_list, additional_params=dict(InvoiceDesireDT=dt))
...
# steps.CampaignsSteps.do_campaigns(SERVICE_ID, service_order_id, {MSR: 50}, 0, dt)

steps.OrderSteps.make_optimized(parent_order_id)

steps.OrderSteps.merge(parent_order_id, order_id_list)
