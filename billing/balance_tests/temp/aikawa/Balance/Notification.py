...
def test2():
    ...
    service_order_ids_list.append(service_order_id)
    order_ids_list.append(order_id)
    request_ids_list.append(request_id)
    #
    service_order_id = steps.OrderSteps.next_id(service_id=SERVICE_ID)
    order_id = steps.OrderSteps.create(client_id=client_id, service_order_id=service_order_id, product_id=PRODUCT_ID, service_id=SERVICE_ID)
    orders_list = [
    ...
    invoice_id,_,_= steps.InvoiceSteps.create(request_id=request_ids_list[0], person_id=person_id, paysys_id=PAYSYS_ID,
                                                                  credit=0, contract_id=None, overdraft=0,endbuyer_id=None)
    steps.InvoiceSteps.pay(invoice_id, payment_dt=dt)
