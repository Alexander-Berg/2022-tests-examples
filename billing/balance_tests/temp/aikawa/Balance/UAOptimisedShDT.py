...
def creating_order(order_params, client_id, person_id, service_id, product_id, msr):
    ...
    if 'with_shipment' in order_params:
        print steps.CampaignsSteps.do_campaigns(service_id, service_order_id, {msr: order_params['with_shipment']}, 0, dt-datetime.timedelta(days=1))
    return order_id
...
def creating_fish_order_group(order_params_list):
    ...
    for position, order_params in enumerate(order_params_list):
        if 'Parent' in order_params:
            ...
            steps.OrderSteps.make_optimized(parent_order_params['order_id'])
            break
    ...
    print db.balance().execute(
        "select state from t_Export where classname = 'Client' and object_id = :object_id and type = 'UA_TRANSFER'",
        {'object_id': client_id})
    ...
    print db.balance().execute(
        ...
        {'object_id': client_id})
    print db.balance().execute(
        "update t_export set update_dt = :dt, state = 1 where classname = 'Client' and object_id = :object_id and type = 'UA_TRANSFER'",
                                 {'dt': datetime.datetime.now() - datetime.timedelta(hours=11), 'object_id': client_id})
    print db.balance().execute(
        "select state from t_Export where classname = 'Client' and object_id = :object_id and type = 'UA_TRANSFER'",
        {'object_id': client_id})
    print db.balance().execute(
        "select update_dt from t_Export where classname = 'Client' and object_id = :object_id and type = 'UA_TRANSFER'",
        {'object_id': client_id})
    # steps.OrderSteps.ua_enqueue([client_id])
    # print db.balance().execute("select state from t_Export where classname = 'Client' and object_id = :object_id and type = 'UA_TRANSFER'", {'object_id': client_id})



    # steps.OrderSteps.increase_priority(client_id)
    # steps.CommonSteps.wait_for_export('UA_TRANSFER', client_id)
    # print steps.CommonSteps.get_pickled_value("select input from t_export where classname = 'Client' and type = 'UA_TRANSFER' and object_id = {0}".format(client_id))
    # # steps.CommonSteps.export('UA_TRANSFER', 'Client', client_id)
    # return order_params_list

# order_params_listorder_params = [
...
#     , {'with_shipment': D('111.33'), 'with_payment': D('150'), 'expected_difference_between_funds_and_ship': D('0')}
# ]
#
# result = creating_fish_order_group(order_params)
# for order in result:
#     print counting_difference_between_funds_and_ship(order['order_id'])
#     print order['expected_difference_between_funds_and_ship']
#     assert counting_difference_between_funds_and_ship(order['order_id']) == order['expected_difference_between_funds_and_ship']
