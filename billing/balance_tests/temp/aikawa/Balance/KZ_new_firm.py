...
def test_KZ_firm_direct_client(params, service_id):
    ...
    for person_id in PERSONS_ID:
        client_id = db.balance().execute('''select client_id from t_person where id = :person_id''', {'person_id': person_id})[0]['client_id']
        # person_id = steps.PersonSteps.create(client_id, params['PERSON_TYPE'])
        person_id = person_id

        # if not db.balance().execute('''select * from t_export where classname = 'Person' and object_id = :person_id''', {'person_id': person_id})[0]:
        # db.balance().execute('''INSERT INTO t_export(CLASSNAME, OBJECT_ID, TYPE) VALUES('Person', :person_id, 'OEBS')''', {'person_id': person_id})
