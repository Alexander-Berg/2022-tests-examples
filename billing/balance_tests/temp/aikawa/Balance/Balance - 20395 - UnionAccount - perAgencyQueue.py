...
def agency_only():
    ...
    mtl.rpc.Balance.UpdateCampaigns([{"ServiceID": service_id, "ServiceOrderID": service_order_id,
                                      "dt": datetime.datetime.now(), "stop": 0, "Bucks": 50, "Money": 0}])
    time.sleep(15)
    ...
    def wait(client_id, agency_id, parent_order_id, child_order_id):
        timeout = 700
        timer = 0
        interval = 5
        while timer < timeout:
            time.sleep(interval)
            timer += interval
            client_state = test_rpc.ExecuteSQL('balance',
                "select object_id, state from t_export where type = 'UA_TRANSFER' and object_id in ({0})".format(
                    client_id))[0]
            agency_state = test_rpc.ExecuteSQL('balance',
                "select object_id, state from t_export where type = 'UA_TRANSFER' and object_id in ({0})".format(
                    agency_id))[0]
            parent_order_state = test_rpc.ExecuteSQL('balance',
                "select object_id, state from t_export where type = 'UA_TRANSFER' and object_id in ({0})".format(
                    parent_order_id))[0]
            child_order_state = test_rpc.ExecuteSQL('balance',
                "select object_id, state from t_export where type = 'UA_TRANSFER' and object_id in ({0})".format(
                    child_order_id))[0]
            # print ('Client: {0}'.format(client_state), 'parent order:{0}'.format(parent_order_state), 'child order:{0}'.format(child_order_state))
            print ('Client: {0}'.format(client_state), 'agency_id:{0}'.format(agency_state),
                   'parent order:{0}'.format(parent_order_state), 'child order:{0}'.format(child_order_state))
            if agency_state['state'] == 1: return 'Waiting for {0} sec'.format(timer)

    wait(client_id, agency_id, parent_order_id, child_order_id)


# agency_only()

def agency_and_client_both():
    agency_id = None
    client_id = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'LastMan'})
    order_owner = client_id
    invoice_owner = client_id
    ...
    rpc.Balance2.CreateOrUpdateOrdersBatch(16571028, [
        {'ServiceID': service_id, 'ServiceOrderID': ServiceOrderIdList[0], 'ProductID': product_id,
         'ClientID': client_id, 'AgencyID': None, 'GroupServiceOrderID': ServiceOrderIdList[1],
         'GroupWithoutTransfer': 1}
