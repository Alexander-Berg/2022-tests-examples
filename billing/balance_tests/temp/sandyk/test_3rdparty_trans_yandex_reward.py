...
def create_refund(purchase_token, trust_payment_id, order_id, sum):
    Refund = api.simple().server.BalanceSimple.CreateRefund(service_token, {'purchase_token': purchase_token, 'user_ip': '127.0.0.1',
                                                               'reason_desc': 'test1',
...
def do_refund(purchase_token, trust_refund_id):
    status = api.simple().server.BalanceSimple.DoRefund(service_token, {'purchase_token': purchase_token, 'user_ip': '127.0.0.1',
...
def method_tickets():
    Order = api.simple().server.BalanceSimple.CreateOrderOrSubscription(service_token, {'uid': '167324860', 'user_ip': '127.0.0.1',
                                                                           'region_id': 225,
                                                                           'service_product_id': service_product_id})
    Order2 = api.simple().server.BalanceSimple.CreateOrderOrSubscription(service_token, {'uid': '167324860', 'user_ip': '127.0.0.1',
                                                                           'region_id': 225,
    ...
    Basket = api.simple().server.BalanceSimple.CreateBasket(service_token, {'user_ip': '127.0.0.1', 'currency': 'RUB', 'orders': [
    ...
    PaymentForm = api.simple().server.BalanceSimple.PayBasket(service_token, {'purchase_token': purchase_token,
...
@pytest.mark.tickets('BALANCE-21544')
def test_3rdparty_trans_yandex_reward():
    ...
    assert reward[0]['yandex_reward'] is None, 'Check yandex_reward for payment %s'%payment_id

if __name__ == "__main__":
    # test_3rdparty_trans_yandex_reward()
    pytest.main("test_3rdparty_trans_yandex_reward.py -v")