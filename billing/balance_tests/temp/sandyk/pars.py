...
for contract_id in contracts:
        ...
        for client_id in contracts[contract_id]:
                ...
                for x in contracts[contract_id][client_id]:
                    data_for_act={}
                    service_id = test_rpc.ExecuteSQL('balance', "select service_id from T_SERVICE_DISCOUNT_TYPES where DISCOUNT_TYPE_ID = :disc", { 'disc': int(x['serv'])}) [0]['service_id']
...
##данные для актов
                    data_for_act['client_id'] = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'Petrov3'})
                    order_owner   = client_id
                    invoice_owner = agency_id
                    if order_owner == invoice_owner: agency_id = None
                    data_for_act['service_id'] = test_rpc.ExecuteSQL('balance', "select service_id from T_SERVICE_DISCOUNT_TYPES where DISCOUNT_TYPE_ID = :disc", { 'disc': int(x['serv'])}) [0]['service_id']
...
##                     str(x['comiss'])[5:15] +' 00:00:00'
                    ...
                    if data_for_act['currency'] == 'RUR':
                            ...
                            rate = test_rpc.ExecuteSQL('balance', "select rate from T_CURRENCY_RATE where cc = :currency and  rate_dt = to_date(:act_dt,\'DD.MM.YYYY HH24:MI:SS\')", {'currency': str(data_for_act['currency']),'act_dt':data_for_act['act_dt'].strftime(sql_date_format)}) [0]['rate']
                ...
                if data_for_act['currency'] == 'RUR':
                    ...
                    person_id = mtl.create_person(agency_id, 'usp', {'phone':'234'})
        ...
        if comm <= 30:
            if payment==2:
                contract_id=mtl.create_contract2('opt_prem',data_for_contract)
            if payment==3:
                contract_id=mtl.create_contract2('opt_prem_post',data_for_contract )
...
for con in c_d:
    for data in all_d:
         if all_d[data]['contract_id'] == str(c_d[con]['id']):
            ...
            if order_owner == invoice_owner: agency_id = None
            service_order_id = mtl.get_next_service_order_id(all_d[data]['service_id'])
            order_id = mtl.create_or_update_order (all_d[data]['client_id'], all_d[data]['product_id'], all_d[data]['service_id'], service_order_id ,  {'TEXT':'Py_Test order'})
            order_id = mtl.create_or_update_order (order_owner,  all_d[data]['product_id'], all_d[data]['service_id'], service_order_id,
            {'TEXT':'Py_Test order'}, agency_id = invoice_owner, manager_uid = None)
...
##        , {'ServiceID': service_id, 'ServiceOrderID': service_order_id3, 'Qty': qty, 'BeginDT': begin_dt}
            ...
            print invoice_owner
            print orders_list
            print all_d[data]['inv_dt']
            request_id = mtl.create_request (invoice_owner, orders_list,  all_d[data]['inv_dt'])
            if c_d[con]['payment'] == 2:
                is_credit = 0
            else:
                is_credit = 1
            invoice_id = mtl.create_invoice (request_id, c_d[con]['person_id'], all_d[data]['paysys_id'], is_credit, c_d[con]['contract_id'], overdraft = 0)

            print all_d[data]['payment_sum']
            mtl.OEBS_payment(invoice_id, all_d[data]['payment_sum'], all_d[data]['payment_dt'])
            mtl.do_campaigns(all_d[data]['service_id'], service_order_id, {'Bucks': all_d[data]['qty'], 'Money': 0}, 0, all_d[data]['act_dt'])
            mtl.act_accounter(invoice_owner, 1, all_d[data]['act_dt'])
            print '-------------------------------------------------------------------'
