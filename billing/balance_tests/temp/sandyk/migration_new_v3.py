    ...
    for passport_id in passport_ids:
        for service in services:
            if service in FIX_SERVICES:  # and len(services) > 1:
                ...
                client_in_link = cur.execute(
                    '''select client_id from t_service_client where service_id=23 and passport_id=:passport_id''',
                    {'passport_id': passport_id}
                ).fetchall()
                if len(client_in_link) > 0:
                    # take already existing client from link and rebind all objects to him
                    new_pk = client_in_link[0][0]
                    log('using existing client %s, passport %s' % (client_id, passport_id))
                else:
                    # clone existing client and rebind to him
