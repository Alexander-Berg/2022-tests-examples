...
def test_method():
    ...
    act_id = \
        db.balance().execute('select  id  from t_act where client_id  =:client_id', {'client_id': invoice_owner})[0][
            'id']
    return '[MT]: https://balance-admin.greed-tm1f.yandex.ru/invoice-publish.xml?ft=html&rt=act&object_id=%s&mt-login=yb-adm&mt-password=get_secret(*UsersPwd.YANDEX_TEAM_REG_CQR5_PWD)' % act_id
