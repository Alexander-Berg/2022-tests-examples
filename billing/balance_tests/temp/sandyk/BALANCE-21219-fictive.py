...
def sprav():
    ...
    contract_id = steps.ContractSteps.create_contract_new('opt_agency', {'CLIENT_ID': invoice_owner,
                                                                         'PERSON_ID': person_id,
                                                                         'IS_FAXED': START_DT, 'DT': START_DT,
                                                                         'FIRM': 10, 'SERVICES': [SERVICE_ID],
                                                                         'PAYMENT_TYPE': 3})[0]
    ...
    steps.CampaignsSteps.do_campaigns(SERVICE_ID, service_order_id, {'Units': 40}, 0, campaigns_dt=DT)
    steps.CampaignsSteps.do_campaigns(SERVICE_ID, service_order_id1, {'Units': 60}, 0, campaigns_dt=DT)
    ...
    steps.CampaignsSteps.do_campaigns(SERVICE_ID, service_order_id, {'Units': 70}, 0, campaigns_dt=DT)
