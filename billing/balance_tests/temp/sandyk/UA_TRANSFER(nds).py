...
def test_UA_priority():
    ...
    db.balance().execute(
        'update (select * from T_INVOICE_ORDER where invoice_id = :invoice_id ) set TAX_POLICY_PCT_ID=null',
    ...
    for i in xrange(ORDERS_COUNT):
        ...
        steps.CampaignsSteps.do_campaigns(service_id, service_order_ids[i], {'Bucks': QTY/10*i, 'Money': 0}, 0, datetime.datetime(2015,11,22))
