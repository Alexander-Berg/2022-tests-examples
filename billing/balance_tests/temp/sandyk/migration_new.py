    ...
    filtered_clients = cur.execute(
        '''
            ...
            where passport_id in (
                ...
                where service_id=23 and client_id in %s
             )
        ''' % sqlarray(ids)
    ).fetchall()
    log("%d clients " % len(filtered_clients))
    filtered_client_ids = set(i[0] for i in filtered_clients)
    if ids is not None and len(filtered_client_ids) != len(ids):
        raise Exception('bad client ids in the list, %d passed, %d found' % (len(ids), len(filtered_client_ids)))
    log("%d clients after filtering, %s" % (len(filtered_clients), filtered_clients))

    # print len(clients), clients

    for client, _ in filtered_clients:
        try:
        ...
        for service in services:
            if service in FIX_SERVICES:  # and len(services) > 1:
                ...
                for orders in paginate(client_service_orders, IN_OPERATOR_PAGE):
                    ...
                    update_invoice_order.extend(
                        {'new_client_id': new_pk, 'invoice_order_id': i[1]} for i in client_service_invoices)
                    client_service_invoices = [i[0] for i in client_service_invoices]
                    update_invoices.extend(
                        {'new_client_id': new_pk, 'invoice_id': invoice} for invoice in client_service_invoices)
                    ...
                    update_request_order.extend(
                        {'new_client_id': new_pk, 'request_order_id': i[1]} for i in client_service_requests)
                    client_service_requests = [i[0] for i in client_service_requests]
                    update_requests.extend(
                        {'new_client_id': new_pk, 'request_id': request} for request in client_service_requests)
                # ------------ EXTPROPS
...
def make_broken(conn):
    ...
    cur.executemany(
        '''delete from t_extprops where value_num=23 and object_id=:client_id and classname='Client' and attrname='service_id' and passport_id=:passport_id''',
        clients
    )
    ...
    for k, v in parent_transactions_map.items():
        for kk, vv in parent_transactions_map.items():
            if k != kk:
                if v & vv:
                    logging.debug(
                        'client %s is not safe, passports %s and %s have common parent transactions %s and %s' % (
                        client_id, k, kk, v, vv))
                    return False
...
if __name__ == '__main__':
    ...
    secrets.get_secret(*secrets.DbPwd.BALANCE_BO_PWD)
