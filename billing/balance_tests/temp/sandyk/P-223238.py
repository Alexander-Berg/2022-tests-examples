...
after = datetime.datetime(2015,9,30)
    # datetime.datetime.now()
##datetime.datetime(2015,2,26,11,0,0)
current = datetime.datetime(2015,9,30)
    # datetime.datetime.now()
...
##
   ...
   #     {'ServiceID': service_id, 'ServiceOrderID': service_order_id, 'Qty': qty, 'BeginDT': begin_dt}
       ...
       # ]
   # request_id = mtl.create_request (invoice_owner, orders_list, datetime.datetime.now())
...
##    mtl.OEBS_payment(invoice_id, None, None)
   service_order_id2 = 22471226
   mtl.do_campaigns(11, service_order_id2, {'Bucks': 90, 'Money': 0}, 0, datetime.datetime.now())
