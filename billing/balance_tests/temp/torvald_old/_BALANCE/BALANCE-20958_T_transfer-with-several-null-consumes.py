...
act_dt = after
migrate_dt = after
manager_uid2 = '241593318'
manager_uid = None


##------------------------------------------------------------------------------
def data_generator(mode=1):
    ...
    order_id3 = mtl.create_or_update_order(order_owner2, product_id, service_id, service_order_id3,
                                           {'TEXT': 'Py_Test order'}, agency_id=agency_id, manager_uid=manager_uid)

    service_order_id4 = mtl.get_next_service_order_id(service_id)
    order_id4 = mtl.create_or_update_order(order_owner3, product_id, service_id, service_order_id4,
                                           {'TEXT': 'Py_Test order'}, agency_id=agency_id, manager_uid=manager_uid)

    service_order_id5 = mtl.get_next_service_order_id(service_id)
    order_id5 = mtl.create_or_update_order(order_owner4, product_id, service_id, service_order_id5,
                                           {'TEXT': 'Py_Test order'}, agency_id=agency_id, manager_uid=manager_uid)
    ...
    current_qty = qty
    new_qty = current_qty - 0.000009
    rpc.Balance.CreateTransferMultiple(16571028, [
        {"QtyOld": current_qty, "ServiceOrderID": service_order_id, "ServiceID": 7, "QtyNew": new_qty, 'AllQty': 0}], \
                                       [{"QtyDelta": "1", "ServiceOrderID": service_order_id2, "ServiceID": 7}])
    current_qty = new_qty
    new_qty = current_qty - 0.000010
    rpc.Balance.CreateTransferMultiple(16571028, [
        {"QtyOld": current_qty, "ServiceOrderID": service_order_id, "ServiceID": 7, "QtyNew": new_qty, 'AllQty': 0}], \
                                       [{"QtyDelta": "1", "ServiceOrderID": service_order_id3, "ServiceID": 7}])
    current_qty = new_qty
    new_qty = current_qty - 0.000011
    rpc.Balance.CreateTransferMultiple(16571028, [
        {"QtyOld": current_qty, "ServiceOrderID": service_order_id, "ServiceID": 7, "QtyNew": new_qty, 'AllQty': 0}], \
                                       [{"QtyDelta": "1", "ServiceOrderID": service_order_id4, "ServiceID": 7}])

    mtl.get_direct_discount(client_id3, datetime.datetime(2015, 8, 1), pct=3)
    mtl.do_campaigns(7, service_order_id4, {'Bucks': 0.000001, 'Money': 0}, 0, campaigns_dt)
    mtl.get_direct_discount(client_id4, datetime.datetime(2015, 8, 1), pct=5)
    mtl.do_campaigns(7, service_order_id5, {'Bucks': 0.000001, 'Money': 0}, 0, campaigns_dt)

    if mode == 1:
        transfer_result = rpc.Balance.CreateTransferMultiple(16571028, \
                                                             [{"QtyOld": "0.000011",
                                                               "ServiceOrderID": service_order_id5, "ServiceID": 7,
                                                               "QtyNew": "0.000011", 'AllQty': 1}],
                                                             [{"QtyDelta": "1", "ServiceOrderID": service_order_id,
                                                               "ServiceID": 7}], 1)

    if mode == 2:
        transfer_result = rpc.Balance.CreateTransferMultiple(16571028, \
                                                             [{"QtyOld": "0.000010",
                                                               "ServiceOrderID": service_order_id4, "ServiceID": 7,
                                                               "QtyNew": "0.000010", 'AllQty': 1}],
                                                             [{"QtyDelta": "1", "ServiceOrderID": service_order_id,
                                                               "ServiceID": 7}], 1)
    ...
    if mode == 3:
        transfer_result = rpc.Balance.CreateTransferMultiple(16571028, \
                                                             [{"QtyOld": "0.000011",
                                                               ...
                                                               "QtyNew": "0.000011", 'AllQty': 1},
                                                              {"QtyOld": "0.000010",
                                                               "ServiceOrderID": service_order_id2, "ServiceID": 7,
                                                               "QtyNew": "0.000010", 'AllQty': 1}, \
                                                              {"QtyOld": "0.000009",
                                                               "ServiceOrderID": service_order_id3, "ServiceID": 7,
                                                               "QtyNew": "0.000009", 'AllQty': 1}], \
                                                             [{"QtyDelta": "1", "ServiceOrderID": service_order_id,
                                                               "ServiceID": 7}], 1)

    if mode == 10:
        transfer_result = rpc.Balance.CreateTransferMultiple(16571028, [
            {"QtyOld": "0.000021", "ServiceOrderID": service_order_id4, "ServiceID": 7, "QtyNew": "99.999970",
             'AllQty': 1},
            {"QtyOld": "0.000010", "ServiceOrderID": service_order_id2, "ServiceID": 7, "QtyNew": "99.999970",
             'AllQty': 1}, \
            {"QtyOld": "0.000021", "ServiceOrderID": service_order_id3, "ServiceID": 7, "QtyNew": "99.999970",
             'AllQty': 1}], \
                                                             [{"QtyDelta": "1", "ServiceOrderID": service_order_id,
                                                               "ServiceID": 7}])
...
# Test_1 [Transfer_acted]
def test_transfer_with_only_one_null_consume_without_discount():
    transfer_result = data_generator(mode=1)
    print transfer_result


def test_transfer_with_only_one_null_consume_with_3_discount():
    transfer_result = data_generator(mode=2)
    print transfer_result


def test_transfer_with_only_3_null_consumes_with_diff_discount():
    transfer_result = data_generator(mode=3)
    print transfer_result


if __name__ == '__main__':
    test_transfer_with_only_one_null_consume_with_3_discount()
    pass
