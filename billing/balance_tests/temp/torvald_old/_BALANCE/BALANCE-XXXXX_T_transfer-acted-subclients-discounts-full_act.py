...
def test_client():
    ...
    order_id3 = mtl.create_or_update_order(order_owner4, product_id, service_id, service_order_id4,
                                           {'TEXT': 'Py_Test order'}, agency_id=agency_id, manager_uid=manager_uid)
    ...
    ## Here we should make discount for client_id2 (3%)
    ...
    time.sleep(3)
    ## Here we should make discount for client_id2 (3%)
    mtl.do_campaigns(7, service_order_id3, {'Bucks': 11, 'Money': 0}, 0, campaigns_dt)
    ## Here we should make discount for client_id2 (5%)
    mtl.do_campaigns(7, service_order_id4, {'Bucks': 11, 'Money': 0}, 0, campaigns_dt)
    mtl.act_accounter(invoice_owner, 1, campaigns_dt)
