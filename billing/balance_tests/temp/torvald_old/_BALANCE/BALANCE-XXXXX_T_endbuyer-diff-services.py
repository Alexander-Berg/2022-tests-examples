...
def test_client():
    ## Агенство
    agency_id = mtl.create_client({'IS_AGENCY': 1, 'NAME': u'UL Агенство для КП2'})
    ## Привязка к UID
    mtl.link_client_uid(agency_id, uid)
    ## Плательщки
    person_id = mtl.create_person(agency_id, 'ur', {'phone': '234'})
    ## Договор
    ...
    ## Клиент
    client_id = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'UL КП Директ без скидки2'})
    ...
    mtl.do_campaigns(orders_list[0]['ServiceID'], orders_list[0]['ServiceOrderID'], {'Bucks': 10, 'Money': 0}, 0, None)
    ## Открутки
    ##    mtl.do_campaigns(service_id, service_order_id, {'Bucks': 10, 'Money': 0}, 0, campaigns_dt)
    ## Оплата счета
    ##    mtl.OEBS_payment(invoice_id, None, None)
    ####Генерация актов
    ##    act_dt = datetime.datetime(2014,12,31)
    ##
    ##    p = mtl.create_act (invoice_id, act_dt = None)

    #### Бюджет
    ##    test.ExecuteSQL('''Insert into t_client_direct_budget (ID,CLIENT_ID,END_DT,CLASSNAME,BUDGET,CURRENCY,UPDATE_DT)
    ##        values (S_CLIENT_DIRECT_BUDGET_ID.nextval,:agency_id,TRUNC(SYSDATE, 'MONTH'),'DirectDiscountCalculator','13374',null,sysdate) ''',
    ##        {'agency_id':agency_id})

    ## -----------------------------------------------------------------

    ## Клиент
    client_id = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'UL КП Директ со скидкой'})
    ## Бюджет
    test.ExecuteSQL('''Insert into t_client_direct_budget (ID,CLIENT_ID,END_DT,CLASSNAME,BUDGET,CURRENCY,UPDATE_DT)
    ...
    invoice_id2, orders_list2 = mtl.create_force_invoice(client_id, person_id, campaigns_list2, paysys_id,
                                                         invoice_dt=invoice_dt, agency_id=agency_id
                                                         , credit=1
                                                         , contract_id=contract['ID']
                                                         )
    ##Открутки для случая с несколькими заказами в счёте
    mtl.do_campaigns(orders_list2[0]['ServiceID'], orders_list2[0]['ServiceOrderID'], {'Bucks': 60, 'Money': 0}, 0,
                     None)

    ##----------------------------------------------------------------

    ## Клиент
    client_id = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'UL КП Маркет'})
    service_id = 11
    product_id = 2136
    ...
    invoice_id3, orders_list3 = mtl.create_force_invoice(client_id, person_id, campaigns_list3, paysys_id,
                                                         invoice_dt=invoice_dt, agency_id=agency_id
                                                         , credit=1
                                                         , contract_id=contract['ID']
                                                         )
    ##Открутки для случая с несколькими заказами в счёте
    mtl.do_campaigns(orders_list3[0]['ServiceID'], orders_list3[0]['ServiceOrderID'], {'Bucks': 20, 'Money': 0}, 0,
                     None)

    ##----------------------------------------------------------------

    ## Клиент
    client_id = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'UL КП Медиа без откруток'})
    service_id = 70
    product_id = 503103
    ...
    invoice_id4, orders_list4 = mtl.create_force_invoice(client_id, person_id, campaigns_list4, paysys_id,
                                                         invoice_dt=invoice_dt, agency_id=agency_id
                                                         , credit=1
                                                         , contract_id=contract['ID']
                                                         )
    ...
    ## Клиент
    client_id = mtl.create_client({'IS_AGENCY': 0, 'NAME': u'UL КП Директ со скидкой в пердоплату'})
    service_id = 7
    product_id = 1475
    ...
    invoice_id5, orders_list5 = mtl.create_force_invoice(client_id, person_id, campaigns_list5, paysys_id,
                                                         invoice_dt=invoice_dt, agency_id=agency_id
                                                         )
    ## Оплата счета
    mtl.OEBS_payment(invoice_id5, None, None)
    ##Открутки для случая с несколькими заказами в счёте
    mtl.do_campaigns(orders_list5[0]['ServiceID'], orders_list5[0]['ServiceOrderID'], {'Bucks': 10, 'Money': 0}, 0,
                     None)

    ##Генерация актов
    act_dt = datetime.datetime(2015, 1, 31)
    force = 1
    p = mtl.act_accounter(agency_id, force, act_dt)

    mtl.do_campaigns(orders_list[0]['ServiceID'], orders_list[0]['ServiceOrderID'], {'Bucks': 17, 'Money': 0}, 0, None)
    mtl.do_campaigns(orders_list[1]['ServiceID'], orders_list[1]['ServiceOrderID'], {'Bucks': 20, 'Money': 0}, 0, None)
    mtl.do_campaigns(orders_list2[0]['ServiceID'], orders_list2[0]['ServiceOrderID'], {'Bucks': 66, 'Money': 0}, 0,
                     None)
    mtl.do_campaigns(orders_list3[0]['ServiceID'], orders_list3[0]['ServiceOrderID'], {'Bucks': 33, 'Money': 0}, 0,
                     None)
    mtl.do_campaigns(orders_list4[0]['ServiceID'], orders_list4[0]['ServiceOrderID'], {'Bucks': 33, 'Money': 0}, 0,
                     None)
    mtl.do_campaigns(orders_list5[0]['ServiceID'], orders_list5[0]['ServiceOrderID'], {'Bucks': 66, 'Money': 0}, 0,
                     None)
