...
def test_client():
    ...
    service_order_id2 = mtl.get_next_service_order_id(11)
    order_id2 = mtl.create_or_update_order(order_owner, 2136, 11, service_order_id2,
                                           {'TEXT': 'Py_Test order'}, agency_id=agency_id, manager_uid=manager_uid)
    ...
    orders_list2 = [
        {'ServiceID': 11, 'ServiceOrderID': service_order_id2, 'Qty': qty, 'BeginDT': begin_dt}
        ##        , {'ServiceID': 11, 'ServiceOrderID': service_order_id2, 'Qty': qty, 'BeginDT': begin_dt}
        ##        , {'ServiceID': service_id, 'ServiceOrderID': service_order_id3, 'Qty': qty, 'BeginDT': begin_dt}
    ]
    request_id2 = mtl.create_request(invoice_owner, orders_list2, request_dt)
    invoice_id2 = mtl.create_invoice(request_id2, person_id, paysys_id, overdraft=0)
    ...
    mtl.do_campaigns(11, service_order_id2, {'Bucks': 30, 'Money': 0}, 0, campaigns_dt)
    ##    mtl.act_accounter(invoice_owner, 1, campaigns_dt)
    ...
    rpc.Balance.CreateTransferMultiple(16571028, [
        {"QtyOld": "100.000000", "ServiceOrderID": service_order_id, "ServiceID": 7, "QtyNew": "20.000000",
         'AllQty': 1}], [{"QtyDelta": "92.158000", "ServiceOrderID": service_order_id2, "ServiceID": 11}])
    rpc.Balance.CreateTransferMultiple(16571028, [
    ...
    print '... (3)';
    time.sleep(3)
    mtl.do_campaigns(7, service_order_id3, {'Bucks': 6, 'Money': 0}, 0, campaigns_dt)
    mtl.do_campaigns(7, service_order_id4, {'Bucks': 16, 'Money': 0}, 0, campaigns_dt)
    mtl.act_accounter(invoice_owner, 1, campaigns_dt)
