...
after = datetime.datetime(2015, 4, 24, 11, 0, 0)
disc_dt = datetime.datetime(2015, 4, 24, 11, 0, 0)
...
def test_client():
    ...
    ##    contract_id = mtl.create_contract2('ukr_opt_ag_prem',{'client_id': agency_id, 'person_id': person_id, 'dt': '2015-03-15T00:00:00'})
    contract_id = None
    ...
    ##    mtl.do_campaigns(7, service_order_id, {'Bucks': 99, 'Money': 0}, 0, campaigns_dt)
    ##    mtl.act_accounter(client_id, 1, after)
    ##
    test_rpc.ExecuteSQL("update T_PERSON set hidden = 1 where id = :person_id", {'person_id': person_id})
    ...
    request_id = mtl.create_request(invoice_owner, orders_list, disc_dt, {'QtyIsAmount': 0})
    invoice_id = mtl.create_invoice(request_id, person_id2, paysys_id2, credit=0, contract_id=contract_id, overdraft=0,
                                    endbuyer_id=None)
    mtl.OEBS_payment(invoice_id, None, None)
    mtl.do_campaigns(7, service_order_id, {'Bucks': 99, 'Money': 0}, 0, campaigns_dt)
    mtl.act_accounter(client_id, 1, after)

    print(rpc.Balance.GetFirmCountryCurrency({'client_id': client_id}))
