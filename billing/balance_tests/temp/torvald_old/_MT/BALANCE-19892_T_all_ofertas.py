...
def data_generator(paysys_id, person_type, dt=datetime.datetime.now(), orders=['7-1475'], multi=None,
    ...
    if multi:
        if multi == 'RUB':
            mtl.create_client({'CLIENT_ID': client_id, 'REGION_ID': '225', 'CURRENCY': 'RUB',
                               ...
                               'SERVICE_ID': 7, 'CURRENCY_CONVERT_TYPE': 'MODIFY'})
        elif multi == 'UAH':
            mtl.create_client({'CLIENT_ID': client_id, 'REGION_ID': '187', 'CURRENCY': 'UAH',
                               'MIGRATE_TO_CURRENCY': datetime.datetime.now() + datetime.timedelta(seconds=5),
                               'SERVICE_ID': 7, 'CURRENCY_CONVERT_TYPE': 'MODIFY'})
        test_rpc.ExecuteSQL(
    ...
    if mode == 'paypreview':
        print (
            "{0}https://balance.{1}.yandex.ru/paypreview.xml?person_id={2}&request_id={3}&paysys_id={4}&contract_id={5}&coupon=&mode=ci".format(
                auto_prefix, mtl.host, person_id, request_id, paysys_id, contract_id if contract_id else ''))
        ...
        print ("{0}https://balance-admin.{1}.yandex.ru/invoice-publish.xml?ft=html&object_id={2}".format(
            auto_prefix, mtl.host, invoice_id))
    elif mode == 'invoice':
        invoice_id = mtl.create_invoice(request_id, person_id, paysys_id, credit=0, contract_id=contract_id,
                                        overdraft=0, endbuyer_id=None)
...
if __name__ == '__main__':
    ...
    # <debug
    argv = sys.argv
    argv.append('-paysys_id');
    argv.append(1003)
    argv.append('-person_type');
    argv.append('sw_yt')
    argv.append('-invoice_dt');
    argv.append(datetime.datetime(2015, 6, 3, 11, 0, 0))
    ##    argv.append('-orders'); argv.append(['98-505057', '98-505057'])
    argv.append('-orders');
    argv.append(['7-1475'])
    ##    argv.append('-multi'); argv.append('RUB')
    argv.append('-mode');
    argv.append('paypreview')
    # debug>
    # ------------------------------------------------------------------------------
