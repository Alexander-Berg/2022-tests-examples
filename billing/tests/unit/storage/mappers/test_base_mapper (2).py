...
from billing.yandex_pay_plus.yandex_pay_plus.storage.entities.base import Entity, autogenerate_uuid
from billing.yandex_pay_plus.yandex_pay_plus.storage.mappers.base import BaseMapperCRUD
from billing.yandex_pay_plus.yandex_pay_plus.storage.types import UUID

metadata = sa.MetaData(schema='yandex_pay_plus')
...
async def create_table(dbconn):
    ...
    create table yandex_pay_plus.entity_with_uuid (
        uuid_id uuid,
        seq_id bigserial,
...
async def test_save__updates_updated(mocker, mapper, persistent_entity):
    ...
    mocker.patch('billing.yandex_pay_plus.yandex_pay_plus.storage.mappers.base.func.now', mocker.Mock(return_value=now))
...
@pytest.mark.asyncio
async def test_forge_created_datetime(mapper, persistent_entity):
    entity = await mapper.get(persistent_entity.uuid_id, persistent_entity.seq_id)
    created = entity.created

    forged = created - timedelta(days=1)
    await mapper.forge_created_datetime(persistent_entity.uuid_id, persistent_entity.seq_id, created=forged)

    entity.created = forged
    assert_that(
        await mapper.get(persistent_entity.uuid_id, persistent_entity.seq_id),
        equal_to(entity),
    )
