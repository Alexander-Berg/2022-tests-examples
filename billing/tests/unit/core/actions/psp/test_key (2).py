...
from billing.yandex_pay_plus.yandex_pay_plus.core.actions.psp.key import CreatePSPAuthKeyAction
from billing.yandex_pay_plus.yandex_pay_plus.core.exceptions import (
    CoreAuthKeyAlgNotSupportedError,
...
async def test_create_key_returns_key(storage, key):
    psp = await create_psp_entity(
        storage,
        PSP(psp_id=uuid.uuid4(), psp_external_id='ext-id'),
    )
...
@pytest.mark.asyncio
async def test_create_key_when_psp_not_exists__raises_error(key):
...
@pytest.mark.asyncio
async def test_create_key_when_unknown_alg__raises_error(key):
    non_existent_psp_id = uuid.uuid4()
