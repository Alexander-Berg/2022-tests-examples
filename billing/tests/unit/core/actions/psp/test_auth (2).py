...
from billing.yandex_pay_plus.yandex_pay_plus.core.actions.psp.auth import AuthPSPRequestAction
from billing.yandex_pay_plus.yandex_pay_plus.core.actions.psp.create_entity import create_psp_entity
from billing.yandex_pay_plus.yandex_pay_plus.core.exceptions import CoreAccessDenyError
...
async def psp(storage):
    return await create_psp_entity(
        storage,
        PSP(psp_id=uuid.uuid4(), psp_external_id='gw')
    )
...
@pytest.mark.asyncio
async def test_token_expired(params, mocker, yandex_pay_plus_settings, get_detached_token):
    yandex_pay_plus_settings.API_PSP_JWS_LIFETIME_SECONDS = 3600
    mocker.patch(
        'billing.yandex_pay_plus.yandex_pay_plus.core.actions.psp.auth.utcnow',
