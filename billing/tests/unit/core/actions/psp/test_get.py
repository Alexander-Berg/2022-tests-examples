...
from billing.yandex_pay.yandex_pay.core.exceptions import CorePSPNotFoundError
...
@pytest.mark.asyncio
async def test_get_psp(public_key, rands, mocker):
    mock_duckgo_response = {
        'code': 200,
        'status': 'success',
        'data': {},
    }
    mock_verify_recipient_key = mocker.patch.object(
        DuckGoClient, 'verify_recipient_key', return_value=mock_duckgo_response
    )
    ...
    mock_verify_recipient_key.assert_called_once_with(public_key, 'xxxyyy')
