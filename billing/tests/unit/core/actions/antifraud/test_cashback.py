...
from billing.yandex_pay.yandex_pay.interactions import AntifraudClient
...
def user(uid, login_id):
    return User(uid=uid, tvm_ticket='ticket', login_id=login_id)
...
async def test_should_return_true_and_should_not_call_antifraud_client_if_disabled(
    user, mock_antifraud_client, yandex_pay_settings
):
    yandex_pay_settings.CASHBACK_ANTIFRAUD_ENABLED = False
    result = await CheckCashbackAllowedAction(
        user=user,
        external_id='some_id',
        amount=10,
...
async def test_should_return_false_on_bad_antifraud_statuses(
    user, mock_antifraud_client, yandex_pay_settings, antifraud_status
):
