...
@pytest.fixture
def url(integration):
    return f'/api/web/v1/integrations/{integration.integration_id}'
...
class TestUpdateIntegration:
    ...
    def mock_update_integration_action(self, mock_action, integration):
        return mock_action(UpdateIntegrationAction, integration)

    @pytest.mark.asyncio
    async def test_returned(self, payload, app, url, integration, now):
        r = await app.patch(url, json=payload)
    ...
    @pytest.mark.asyncio
    async def test_action_called(self, payload, app, url, mock_update_integration_action, integration, user):
        await app.patch(url, json=payload)

        mock_update_integration_action.assert_run_once_with(
            user=user,
...
class TestDeleteIntegration:
    @pytest.fixture(autouse=True)
    def mock_delete_integration_action(self, mock_action, integration):
        return mock_action(DeleteIntegrationAction)

    @pytest.mark.asyncio
    async def test_delete(self, app, url, integration, now):
        r = await app.delete(url)

        assert_that(r.status, equal_to(200))

    @pytest.mark.asyncio
    async def test_action_called(self, app, url, mock_delete_integration_action, integration, user):
        await app.delete(url)

        mock_delete_integration_action.assert_run_once_with(
            user=user,
            integration_id=integration.integration_id,
        )
