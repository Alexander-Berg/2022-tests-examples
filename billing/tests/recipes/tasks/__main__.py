...
def start(argv):
    ...
    binary_path = yatest.common.binary_path("billing/configshop/cmd/runtasks/runtasks")
    logging.info('Binary path: ' + binary_path)
    execution = yatest.common.execute(
        [binary_path, "-c", source_path() + "/billing/configshop/configs/core/dev.yaml",
            "--processor-config", source_path() + "/billing/configshop/configs/processor/dev.yaml"],
        stdout=open(os.path.join(yatest.common.output_path(), 'configshop-tasks.out'), 'w'),
        stderr=open(os.path.join(yatest.common.output_path(), 'configshop-tasks.err'), 'w'),
        wait=False,
    ...
    if not is_ready:
        raise Exception('tasks not ready')
