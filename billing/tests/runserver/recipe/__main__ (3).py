...
def start(argv):
    ...
    set_env("{{cookiecutter.PROJECT_NAME}}_RUNSERVER_PORT", str(server_port))
    os.environ["RECIPE_PORT"] = str(server_port)

    binary_path = yatest.common.binary_path("{{cookiecutter.project_path}}/cmd/runserver/runserver")
    logging.info('Binary path: ' + binary_path)
    execution = yatest.common.execute(
        [binary_path, "-c", source_path() + "/{{cookiecutter.project_path}}/configs/core/dev.yaml"],
        stdout=open(yatest.common.output_path() + '/runserver.out', 'w'),
        stderr=open(yatest.common.output_path() + '/runserver.err', 'w'),
        wait=False,
    )

    with open("{{cookiecutter.project_name}}_runserver.pid", "w") as f:
        f.write(str(execution.process.pid))


def stop(argv):
    with open("{{cookiecutter.project_name}}_runserver.pid") as f:
        pid = f.read()
