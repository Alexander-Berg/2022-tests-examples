...
@pytest.fixture(autouse=True)
def setup_tvm(yandex_pay_settings, rands, aioresponses_mocker):
    # Since we allow passthrough for localhost, setting up custom host for tvm
    from billing.yandex_pay.yandex_pay.interactions.base import TVM_CONFIG
    yandex_pay_settings.TVM_HOST = TVM_CONFIG['host'] = 'tvm'
    ...
    def tvm_callback(url, **kwargs):
        ...
        return CallbackResult(
            status=200,
            payload={
                yandex_pay_settings.TVM_CLIENT: {
                    'tvm_id': dst,
    ...
    aioresponses_mocker.get(
        re.compile(f'^http://{yandex_pay_settings.TVM_HOST}:{yandex_pay_settings.TVM_PORT}/tvm/tickets.*$'),
