...
from billing.yandex_pay_plus.yandex_pay_plus.api.handlers.utility import PingDBHandler, PingHandler

SWAGGER_EXCLUDED_HANDLERS = (PingHandler, PingDBHandler)
...
def swagger_included_handlers(api_handlers) -> Set[Type[BaseHandler]]:
    return {handler for handler in api_handlers if not issubclass(handler, SWAGGER_EXCLUDED_HANDLERS)}
...
@pytest.fixture
async def swagger_expected_paths(public_app, swagger_included_handlers) -> Set[str]:
    urls = chain.from_iterable(public_app.app.urls)
    expected_paths = {_strip_path_regex(url.path) for url in urls if url.handler in swagger_included_handlers}
    return expected_paths


@enable_swagger_setting
@pytest.mark.asyncio
async def test_swagger_json_accessible(public_app, swagger_expected_paths):
    response = await public_app.get('api/doc/swagger.json')
    ...
    expected_definitions_part = has_entries(
        {
            'OrderResponseSchema': has_entries(
                {
...
@enable_swagger_setting
@pytest.mark.asyncio
async def test_swagger_ui_accessible(public_app):
    response = await public_app.get('api/doc')
    assert_that(response.status, equal_to(200))


@pytest.mark.asyncio
@pytest.mark.parametrize('url', ['api/doc/swagger.json', 'api/doc'])
async def test_can_turn_off_swagger(public_app, url):
    response = await public_app.get(url)
