...
@pytest.mark.asyncio
async def test_deletes_origin_and_related(app, partner, merchant, origin, layout, moderation, disable_tvm_checking):
    partner_id = partner.partner_id
    merchant_id = merchant.merchant_id
    origin_id = origin['origin_id']

    r = await app.delete(f'/api/web/v1/partners/{partner_id}/origins/{origin_id}')
...
@pytest.mark.asyncio
async def test_calls_drop_file(app, partner, origin, layout, moderation, disable_tvm_checking, mock_file_storage):
    partner_id = partner.partner_id
    origin_id = origin['origin_id']
    ...
    mock_file_storage.ctx_result.drop_file.assert_awaited_once_with(f'/origin-layouts/{layout_document_id}')
