...
func TestClient_findMaster(t *testing.T) {
	...
	// master
	httpmock.RegisterResponder("GET", "http://host1:4440/user/login",
		httpmock.NewStringResponder(200, ""))
	// not active master
	httpmock.RegisterResponder("GET", "http://host2:4440/user/login",
		httpmock.ConnectionFailure)
	// some host, that alive, but respond non 200
	httpmock.RegisterResponder("GET", "http://host3:4440/user/login",
...
func TestClient_Switch(t *testing.T) {
	...
	httpmock.Activate()
	t.Cleanup(httpmock.DeactivateAndReset)
	httpmock.RegisterResponder("GET", "http://host-01-sas.prod.vertis.yandex.net:4440/user/login",
		httpmock.NewStringResponder(200, ""))
