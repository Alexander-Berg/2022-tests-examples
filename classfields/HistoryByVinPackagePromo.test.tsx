...
import HistoryByVinPackagePromo from './HistoryByVinPackagePromo';
...
it('правильно отправляет метрику и эвент-лог на маунт, если это промо', () => {
    shallow(
        <HistoryByVinPackagePromo reportsBundles={ reportsBundlesMock } sendEventsToMarketing={ mockFunction }/>,
...
it('не отправляет метрику и эвент-лог на маунт, если это остаток пакета', () => {
    shallow(
        <HistoryByVinPackagePromo reportsBundles={ reportsBundlesWithQuotaMock } sendEventsToMarketing={ mockFunction }/>,
        { context: { ...contextMock, store } },
    );
...
it('прокидывает кнопке правильный from', () => {
    ...
    expect(wrapper.find('Connect(VinHistoryPaywallButton)').at(0).props()).toHaveProperty('from', 'vincheck_bundle');
});
