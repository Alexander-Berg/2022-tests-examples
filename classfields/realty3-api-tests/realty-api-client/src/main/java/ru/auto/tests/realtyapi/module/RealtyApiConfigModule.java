...
@Log4j
public class RealtyApiConfigModule extends AbstractModule {

    @Provides
    @Singleton
    public RealtyApiConfig provideRealtyApiConfig() {
        RealtyApiConfig config = ConfigFactory.create(RealtyApiConfig.class,
                (Properties) System.getProperties().clone(), System.getenv());
        Properties properties = new Properties();
        config.fill(properties);
        Set<String> propertiesName = Arrays.stream(RealtyApiConfig.class.getDeclaredMethods())
                .map(m -> m.getDeclaredAnnotation(Key.class).value()).collect(Collectors.toSet());
        properties.entrySet().removeIf(e -> !propertiesName.contains(e.getKey()));
