...
public class RealtyApiV1Provider implements Provider<ApiClient> {
    ...
    public ApiClient get() {
        ApiClient.Config realtyApiConfig = ApiClient.Config
                .apiConfig()
                .reqSpecSupplier(() -> baseReqSpecBuilder(config.getRealtyApiTestingURI(), config.getRealtyApiV1Path())
                        .setConfig(baseRestAssuredConfig().logConfig(getLogConfig(config.isRestAssuredLoggerEnabled())))
                        .addFilter(new SwaggerCoverageRestAssured(new FileSystemOutputWriter(Paths.get("swagger-coverage-output-v1")))));
