...
public class StatShowsOffersAggregatedLevelsTest {

    private static final String IGNORING_PATH = "response.values[*].value[*].date";
    ...
    @Test
    public void shouldStatShowOffersAggregatedLevelsHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.stat().offersAggregatedShows()
        ...
        MatcherAssert.assertThat(request.apply(api), jsonEquals(request.apply(prodApi))
                .whenIgnoringPaths(IGNORING_PATH));
    }
}
