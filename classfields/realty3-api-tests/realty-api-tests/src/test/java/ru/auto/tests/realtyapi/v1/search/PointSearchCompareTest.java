...
@Title("GET /pointSearch.json")
@RunWith(Parameterized.class)
@GuiceModules(RealtyApiModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class PointSearchCompareTest {

    private static final String RESOURCE_FILE = "search/offer_search_test_data.txt";
...
//        JsonObject response = api.pointSearchjson().reqSpec(req -> req.addQueryParams(queryParams))
//                .execute(validatedWith(shouldBe200Ok())).as(JsonObject.class, GSON);
//
//        JsonObject responseProd = prodApi.pointSearchjson().reqSpec(req -> req.addQueryParams(queryParams))
//                .execute(validatedWith(shouldBe200Ok())).as(JsonObject.class, GSON);
//
