...
public class CreateSubscriptionSuccessWithoutOAuthTest {
    @Rule
    @Inject
    ...
    @Test
    public void shouldSuccessCreateSubscriptionWithoutOAuth() {
        SubscriptionRequest reqBody = getValidSubscriptionRequest();
        reqBody.getQuery().setCategory(offerCategory);

        SubscriptionResponse resp = api.subscriptions().createSubscriptionRoute().reqSpec(authSpec()).body(reqBody)
