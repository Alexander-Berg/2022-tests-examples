...
@Title("GET /phone/totalCalls/user/{uid}")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyApiModule.class)
public class TotalCallsUserTest {
    ...
    @Test
    @Owner(SCROOGE)
    public void shouldTotalCallUserHasNoDiffWithProduction() {
        Account account = am.create();
        String token = oAuth.getToken(account);
        String offerId = adaptor.createOffer(token).getResponse().getId();

        Function<ApiClient, JsonObject> request = apiClient -> apiClient.phone().totalCallsRoute()
                .reqSpec(authSpec())
                .uidPath(account.getId()).offerIdQuery(offerId)
