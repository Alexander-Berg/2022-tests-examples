...
public class EditUserWithExplicitUidDifferentBoolTypeTest {
    @Rule
    @Inject
    ...
    @Test
    public void shouldEditUserWithDifferentBoolType() {
        ...
        UpdateExplicitUserResp resp = api.user().updateUserRoute().reqSpec(authSpec()).authorizationHeader(token)
                ...
                .body(reqBody.callCenter(type).licenseAgreement(type).redirectPhones(type))
                .execute(validatedWith(shouldBe200OkJSON())).as(UpdateExplicitUserResp.class, GSON);

        Assertions.assertThat(resp).hasCallCenter(reqBody.getCallCenter())
                .hasLicenseAgreement(reqBody.getLicenseAgreement())
                .hasRedirectPhones(reqBody.getRedirectPhones());
    }
