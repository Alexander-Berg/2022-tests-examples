...
public class OfferSiteSearchPrimarySaleTest {
    ...
    public static Collection<Object[]> getParameters() throws IOException {
        return parseParams("primarySale", "search/offer_with_site_search_test_cases.txt");
    }
    ...
    @Test
    @Owner(ARTEAMO)
    public void shouldPrimarySaleV2SameWithRequested() {
        boolean primarySale = query.get("primarySale").equals("YES");

        MatcherAssert.assertThat(offers,
                everyItem(jsonPartMatches("primarySaleV2", equalTo(primarySale))));

        Assertions.assertThat(offers).describedAs("Все офферы должны иметь primarySale = " + primarySale)
                .allSatisfy(offer -> MatcherAssert.assertThat(offer, jsonPartMatches("primarySaleV2",
                        equalTo(primarySale))));
    }
    ...
    public void shouldPriceInUpperBound() {
        ...
        MatcherAssert.assertThat(offers,
                everyItem(jsonPartMatches("price.valueForWhole", lessThanOrEqualTo(priceMax))));

        Assertions.assertThat(offers).describedAs("Все офферы должны иметь цену <= " + priceMax)
                .allSatisfy(offer -> MatcherAssert.assertThat(offer, jsonPartMatches("price.valueForWhole",
