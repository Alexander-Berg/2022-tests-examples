...
import static ru.auto.tests.realtyapi.v1.api.StatApi.UserAggregatedShowsOper.LEVEL_QUERY;
...
public class GetStatShowsAggregatedUserCompareTest {
    ...
    public static Collection<Consumer<RequestSpecBuilder>> getParameters() {
        ...
        defaultOfferType().forEach(offerType ->
                params.add(req -> req.addParam(LEVEL_QUERY, ALL).addParam(TYPE_QUERY, offerType)
        ...
        defaultOfferCategory().forEach(offerCategory ->
                params.add(req -> req.addParam(LEVEL_QUERY, ALL).addParam(TYPE_QUERY, RealtyResponseOfferResponse.OfferTypeEnum.SELL)
                        .addParam(CATEGORY_QUERY, offerCategory)
                        .addParam(SPAN_QUERY, span)));

        defaultStatAggregation().forEach(statAggregation -> params.add(req -> req.addParam(LEVEL_QUERY, statAggregation).addParam(TYPE_QUERY, RealtyResponseOfferResponse.OfferTypeEnum.SELL)
                .addParam(CATEGORY_QUERY, RealtyResponseOfferResponse.OfferCategoryEnum.APARTMENT)
                .addParam(SPAN_QUERY, span)));
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.stat().userAggregatedShows()
                .reqSpec(authSpec())
