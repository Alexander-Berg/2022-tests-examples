...
@Title("DELETE /phone/redirect/{uid}")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyApiModule.class)
public class DeletePhoneRedirectForUserTest {
    ...
    public void shouldSee403WithNoAuth() {
        api.phone().turnOffRedirectRoute()
    ...
    public void shouldSee404WithInvalidUid() {
        api.phone().turnOffRedirectRoute().reqSpec(authSpec())
                .uidPath(getRandomString())
    ...
    @Test
    @Owner(ARTEAMO)
    public void shouldSee200ForUserWithoutRedirectPhones() {
        Account account = am.create();
    ...
    @Test
    @Owner(ARTEAMO)
    public void shouldTurnOnRedirectForUser() {
        Account account = am.create();
        String token = oAuth.getToken(account);
        adaptor.vosUser(token);
        ...
        api.phone().turnOnRedirectRoute().reqSpec(authSpec())
                .uidPath(accountId)
                .execute(validatedWith(shouldBe200Ok()));
        ...
        Assertions.assertThat(response.get("redirectPhones").getAsBoolean())
                .describedAs("У пользователя должен выключиться подменный номер")
                .isFalse();
    }
}
