...
public class UpdateUserDraftTest {
    ...
    public void shouldSee403WithoutHeaders() {
        api.draft().updateDraftForUserRoute().offerIdPath(getRandomOfferId())
    ...
    public void shouldSee401WithoutOAuth() {
        api.draft().updateDraftForUserRoute().reqSpec(authSpec()).offerIdPath(getRandomOfferId())
                .userPath(ME)
    ...
    public void shouldSee404ForNotExistOffer() {
        ...
        api.draft().updateDraftForUserRoute().offerIdPath(offerId)
                .reqSpec(authSpec()).authorizationHeader(token)
                .reqSpec(req -> req.setBody(ftlProcessor.processOffer(APARTMENT_SELL.value())))
                .userPath(ME)
                .execute(validatedWith(shouldBeCode(SC_NOT_FOUND)));
    ...
    public void shouldSee400WithEmptyBody() {
        ...
        api.draft().updateDraftForUserRoute().offerIdPath(offerId).reqSpec(authSpec())
