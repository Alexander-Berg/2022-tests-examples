...
@Title("GET /stat/shows/aggregated/{offerId}")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyApiModule.class)
public class GetStatShowsAggregatedByIdTest {
    ...
    public void shouldSee403WithoutHeaders() {
        ...
        api.stat().thisUserOfferAggregatedShows()
    ...
    @Test
    public void shouldStatShowsWithStartAndEndTimeHasNoDiffWithProduction() {
        Account account = am.create();
        String token = oAuth.getToken(account);
        ...
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.stat().thisUserOfferAggregatedShows()
                .reqSpec(authSpec()).xAuthorizationHeader(token)
                .offerIdPath(offerId)
                .levelQuery(ALL)
