...
public class GetAllOfferSnippetsForUserCompareTest {
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> request = apiClient -> api.usersOffers().getSnippetsRoute()
                .reqSpec(authSpec())
        ...
        MatcherAssert.assertThat(request.apply(api), jsonEquals(request.apply(prodApi))
                .whenIgnoringPaths("response.offers[0].content.publishingInfo.secondsUntilExpiry").when(IGNORING_ARRAY_ORDER));
    }
}
