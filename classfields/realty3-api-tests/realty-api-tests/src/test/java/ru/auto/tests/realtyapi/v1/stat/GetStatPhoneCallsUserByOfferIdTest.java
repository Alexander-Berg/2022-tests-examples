...
@Title("GET /stat/phoneCalls/user/{uid}/{offerId}")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyApiModule.class)
public class GetStatPhoneCallsUserByOfferIdTest {
    ...
    public void shouldSee403WithoutHeaders() {
        ...
        api.stat().someUserOfferPhoneCalls()
    ...
    @Test
    public void shouldSee200ForNotInvalidUser() {
        Account account = am.create();
        String token = oAuth.getToken(account);
        String uid = getRandomString();
        int span = getRandomShortInt();

        api.stat().someUserOfferPhoneCalls().reqSpec(authSpec()).xAuthorizationHeader(token)
                .uidPath(uid)
                .offerIdPath(getRandomOfferId())
                .levelQuery(ALL)
                .spanQuery(span)
    ...
    @Test
    public void shouldPhoneCallsWithStartAndEndTimeHasNoDiffWithProduction() {
        Account account = am.create();
        String token = oAuth.getToken(account);
        ...
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.stat().someUserOfferPhoneCalls()
