...
public class GetAgencyClientsCompareTest {
    @Rule
    @Inject
    ...
    public void shouldHasNoDiffWithProduction() {
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.cabinet().agencyClients()
                .reqSpec(authSpec())
    ...
    @Test
    @Owner(ARTEAMO)
    public void shouldHasValidClients() {
        RealtyApiCabinetAgencyClientsResponse response = api.cabinet().agencyClients().reqSpec(authSpec())
                .billingDomainPath(domain)
                .xUidHeader(uid)
                .userPath(ME)
        ...
        response.getResponse().getClientsInfo()
                .forEach(c -> ids.add(c.getClient().getId().toString()));

        Assertions.assertThat(ids).containsOnlyOnce(clientId);
    }
}
