...
public class SearchSendEventTest {
    ...
    public void shouldSeeSearchEventText() {
        ...
        eventSteps.withEventType(SEARCH).singleEventWithParams(event)
    ...
    public void shouldSeeSearchEventTextMetro() {
        urlSteps.testing().path(MOSKVA).queryParam(METRO_ID_PARAM, METRO_1)
                .queryParam(METRO_ID_PARAM, METRO_2).queryParam(TEXT_PARAM, TEXT).open();
    ...
    public void shouldSeeSearchEventManyParameters() {
        urlSteps.testing().path(MOSKVA).path(MOSKVA).path(MOBILNIE_TELEFONI).path(STATE_USED)
