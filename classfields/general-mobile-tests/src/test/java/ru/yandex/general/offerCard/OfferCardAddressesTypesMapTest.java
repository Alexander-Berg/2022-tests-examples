...
public class OfferCardAddressesTypesMapTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Отображение адреса в попапе с картой")
    public void shouldSeeAddressInMapPopup() {
        basePageSteps.onOfferCardPage().spanLink(SHOW_MAP).click();

        basePageSteps.onOfferCardPage().popup().addressList().get(0).should(hasText(address));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Отображение пина на карте для каждого типа адреса")
    public void shouldSeePinInMapPopup() {
        basePageSteps.onOfferCardPage().spanLink(SHOW_MAP).click();

        basePageSteps.onOfferCardPage().popup().map().pinList()
