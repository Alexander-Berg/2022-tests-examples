...
public class SnippetStatisticsGraphBarsTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Отображение столбцов статистики на сниппете с разным кол-вом дней")
    public void shouldSeeMyOfferSnippetStatisticsGraph() {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Попап статистики оффера с разным количеством дней")
    public void shouldSeeMyOfferSnippetStatisticsPopupGraph() {
        basePageSteps.onMyOffersPage().snippetFirst().statistics().bars().click();

        basePageSteps.onMyOffersPage().statisticsPopup().dataBarsExceptToday().should(hasSize(dataBarsCount));
        basePageSteps.onMyOffersPage().statisticsPopup().dataBarsToday().should(hasSize(todayDataBarsCount));
        basePageSteps.onMyOffersPage().statisticsPopup().noDataBars().should(hasSize(noDataBarsCount));
    }
