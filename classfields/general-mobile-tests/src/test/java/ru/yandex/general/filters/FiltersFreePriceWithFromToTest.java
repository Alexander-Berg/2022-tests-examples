...
import static ru.yandex.general.mobile.page.ListingPage.FREE;
...
public class FiltersFreePriceWithFromToTest {
    ...
    public void shouldSeeResetFromPrice() {
        ...
        basePageSteps.onListingPage().filters().filterBlock(FREE).switcher().click();

        basePageSteps.onListingPage().filters().filterBlock(PRICE).inputWithFloatedPlaceholder(PRICE_FROM).should(hasValue(""));
        basePageSteps.wait500MS();
        basePageSteps.onListingPage().filters().showOffers().click();
        basePageSteps.onListingPage().skeleton().waitUntil(not(isDisplayed()));

        urlSteps.path(Pages.FREE).queryParam(SORTING_PARAM, SORT_BY_RELEVANCE_VALUE).shouldNotDiffWithWebDriverUrl();
    }
    ...
    public void shouldSeeResetToPrice() {
        ...
        basePageSteps.onListingPage().filters().filterBlock(PRICE).inputWithFloatedPlaceholder(PRICE_TO).should(hasValue(""));
        basePageSteps.wait500MS();
        basePageSteps.onListingPage().filters().showOffers().click();
        basePageSteps.onListingPage().skeleton().waitUntil(not(isDisplayed()));
    ...
    public void shouldSeeResetFreeByFromPrice() {
        urlSteps.open();
        basePageSteps.onListingPage().searchBar().filters().click();
        basePageSteps.onListingPage().filters().filterBlock(FREE).switcher().waitUntil(isDisplayed()).click();
        basePageSteps.wait500MS();
        basePageSteps.onListingPage().filters().filterBlock(PRICE).inputWithFloatedPlaceholder(PRICE_FROM).sendKeys(MIN_PRICE);
        basePageSteps.wait500MS();
        basePageSteps.onListingPage().filters().showOffers().click();
        ...
        urlSteps.queryParam(PRICE_MIN_URL_PARAM, MIN_PRICE).queryParam(SORTING_PARAM, SORT_BY_RELEVANCE_VALUE)
    ...
    public void shouldSeeResetFreeByToPrice() {
        ...
        urlSteps.queryParam(PRICE_MAX_URL_PARAM, MAX_PRICE).queryParam(SORTING_PARAM, SORT_BY_RELEVANCE_VALUE)
                .shouldNotDiffWithWebDriverUrl();
    }
    ...
    public void shouldSeeResetFromPriceDirectLink() {
        ...
        urlSteps.testing().path(MOSKVA).path(NOUTBUKI).path(Pages.FREE)
