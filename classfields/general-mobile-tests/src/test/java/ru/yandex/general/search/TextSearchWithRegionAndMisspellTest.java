...
public class TextSearchWithRegionAndMisspellTest {
    ...
    public void before() {
        ...
        basePageSteps.onListingPage().wrapper().input().sendKeys(TEXT_WITH_MISSPELL);
    ...
    public void shouldSeeMisspellH1() {
        basePageSteps.onListingPage().h1().should(
                hasText(format("Объявления по запросу «%s» в Новосибирске", TEXT_WITHOUT_MISSPELL)));
    }
    ...
    public void shouldSeeSearchMisspell() {
        ...
        basePageSteps.onListingPage().h1().waitUntil(
                hasText(format("Объявления по запросу «%s» в Новосибирске", TEXT_WITH_MISSPELL)));
