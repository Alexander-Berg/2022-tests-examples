...
@Feature("Экран «Доставка»")
@DisplayName("Тесты на экране «Доставка»")
@RunWith(GuiceTestRunner.class)
@GuiceModules(GeneralProxyMobileWebModule.class)
public class FormStepDeliveryTest {
    ...
    public void before() {
        ...
        offerAddSteps.withCategory(category).fillToDeliveryStep();
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход на экран «Адрес» по кнопке «Назад»")
    public void shouldSeeBackToAddressScreen() {
        offerAddSteps.onFormPage().screen(DELIVERY).back().click();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Закрываем форму, не меняя доставку, проверяем что оказались на главной")
    public void shouldSeeCloseForm() {
        offerAddSteps.onFormPage().screen(DELIVERY).close().click();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Открывается экран «Мы готовы! Публикуем?» после экрана доставки")
    public void shouldSeeFinalScreenAfterDelivery() {
        offerAddSteps.onFormPage().screen(DELIVERY).button(NEXT).click();

        offerAddSteps.onFormPage().screen(WE_READY_PUBLISH).should(isDisplayed());
    }
