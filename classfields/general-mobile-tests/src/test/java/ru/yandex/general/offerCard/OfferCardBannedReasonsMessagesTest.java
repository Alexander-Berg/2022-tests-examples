...
public class OfferCardBannedReasonsMessagesTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Отображается одна причина бана на карточке")
    public void shouldSeeOneBanReason() {
        mockRule.graphqlStub(mockResponse().setCard(mockCard(BASIC_CARD).setStatus(BANNED)
                        .setBanReasons(WRONG_OFFER_CATEGORY).setIsOwner(true).build())
                .setCategoriesTemplate().setRegionsTemplate().build()).withDefaults().create();
        urlSteps.testing().path(CARD).path(ID).open();

        basePageSteps.onOfferCardPage().message(MESSAGE_ERROR).title().should(hasText(WRONG_OFFER_CATEGORY.getTitle()));
        basePageSteps.onOfferCardPage().message(MESSAGE_ERROR).text().should(hasText(WRONG_OFFER_CATEGORY.getReasonNoLinks()));
    }
