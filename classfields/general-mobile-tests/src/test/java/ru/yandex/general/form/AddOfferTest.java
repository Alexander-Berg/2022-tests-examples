...
@Feature("Тесты на размещение оффера от начала и до публикации")
...
public class AddOfferTest {
    ...
    private static final String SECOND_ADDRESS_TITLE = "Второй адрес";
    private static final String SALLARY = "45000";
    private static final String WORK_TIME = "Время работы от 1 до 60 час";
    private static final String WORK_TIME_VALUE = "37";
    ...
    public void shouldAddOfferWithFreePrice() {
        offerAddSteps.withFreePrice().addOffer();

        offerAddSteps.onOfferCardPage().successPublishMessage().waitUntil(isDisplayed());
        Card offerCard = graphqlSteps.getOfferCard(urlSteps.getOfferId());

        assertThat("Цена - «Даром»", offerCard.getPrice().getCurrentPrice().getTypename(), equalTo("Free"));
    }
    ...
    public void shouldAddOffer2Address() {
        ...
        SoftAssertions.assertSoftly(s -> {
            ...
            s.assertThat(offerCard.getContacts().getAddresses().get(1).getAddress().getAddress())
                    .as(SECOND_ADDRESS_TITLE).isEqualTo(SECOND_ADDRESS);
        });
    }
    ...
    public void shouldAddOfferWithAttributes() {
        ...
        List<Attribute> attributes = Arrays.asList(
                workTime().withInputValue(WORK_TIME_VALUE),
    ...
    public void shouldAddVakansiiOffer() {
        ...
        SoftAssertions.assertSoftly(s -> {
            s.assertThat(offerCard.getPrice().getCurrentPrice().getSalaryRur()).as("Зарплата").isEqualTo(SALLARY);
            s.assertThat(offerCard.getTitle()).as("Название").isEqualTo(VAKANCIYA_MENEGER.getTitle());
            s.assertThat(offerCard.getContacts().getAddresses().get(FIRST).getAddress().getAddress()).as("Адрес")
                    .isEqualTo(ADDRESS);
            s.assertThat(offerCard.getCategory().getShortName()).as("Категория")
                    .isEqualTo("Менеджеры");
            s.assertThat(offerCard.getCategory().getParents().get(FIRST).getShortName()).as("Родительская категория")
                    .isEqualTo(VAKANCIYA_MENEGER.getParentCategory());
        });
    }
    ...
    public void shouldAddRezumeOfferWithoutSallary() {
        offerAddSteps.withCategory(REZUME_IN_SELLING).addOffer();
