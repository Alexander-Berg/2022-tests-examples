...
@Epic(EVENTS_FEATURE)
@DisplayName("Отправка «snippetClick», «snippetShow» с листинга похожих офферов")
@RunWith(GuiceTestRunner.class)
@GuiceModules(GeneralProxyMobileWebModule.class)
public class SimilarCardSendEventTest {
    ...
    @Before
    public void before() {
        urlSteps.testing().path(CARD).path(CARD_ID).path(SLASH);

        mockRule.graphqlStub(mockResponse()
                .setCard(mockCard(BASIC_CARD).setId(CARD_ID).similarOffers(asList(snippet)).build())
                .setCurrentUserExample()
                .setCategoriesTemplate()
                .setRegionsTemplate().build()).withDefaults().create();

        event = event().setContext(context().setBlock(BLOCK_SIMILAR).setPage(PAGE_CARD).setReferer(urlSteps.toString()))
                .setPortalRegionId(REGION_ID)
                .setTrafficSource(trafficSource());
        eventOffer = offer().setPrice(price(PRICE))
                .setOfferId(ID)
                .setCategoryId(CATEGORY_ID)
                .setOfferVersion(OFFER_VERSION)
    ...
    @DisplayName("Отправка «snippetClick» с листинга похожих офферов")
    public void shouldSeeSnippetClickSendEventFromSimilarCardListing() {
        basePageSteps.scrollingToElement(basePageSteps.onListingPage().snippetFirst().waitUntil(isDisplayed()));
    ...
    @DisplayName("Отправка «snippetShow» с листинга похожих офферов")
    public void shouldSeeSnippetShowSendEventFromSimilarCardListing() {
