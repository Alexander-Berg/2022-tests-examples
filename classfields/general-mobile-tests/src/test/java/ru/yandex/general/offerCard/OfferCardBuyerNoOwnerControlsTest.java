...
public class OfferCardBuyerNoOwnerControlsTest {
    ...
    public void shouldNotSeeEditButtonBuyerCard() {
        ...
        basePageSteps.onOfferCardPage().spanLink(EDIT).should(not(isDisplayed()));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Не отображается «Снять с публикации» на карточке покупателя")
    public void shouldNotSeeMoreButtonBuyerCard() {
        urlSteps.open();

        basePageSteps.onOfferCardPage().spanLink(DEACTIVATE).should(not(isDisplayed()));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Не отображается блок статистики на карточке покупателя")
    public void shouldNotSeeDaysUntilExpireBuyerCard() {
        urlSteps.open();

        basePageSteps.onOfferCardPage().statistics().should(not(isDisplayed()));
    }
