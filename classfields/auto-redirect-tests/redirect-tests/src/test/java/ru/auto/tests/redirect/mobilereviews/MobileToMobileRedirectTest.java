package ru.auto.tests.redirect.mobilereviews;
...
@DisplayName("Редиректы mobile -> mobile")
@Feature(REDIRECT)
@RunWith(Parameterized.class)
@GuiceModules(RedirectClientModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class MobileToMobileRedirectTest {

    private static final String RESOURCE = "mobilereviews/mobile_to_mobile.csv";
    ...
    public static List getParameters() {
        InputStream dataStream = MobileToMobileRedirectTest.class.getClassLoader().getResourceAsStream(RESOURCE);
    ...
    @Test
    @Owner(TIMONDL)
    @Category({MobileToMobile.class})
    @DisplayName("Проверка редиректа")
    public void shouldSeeRedirect() throws IOException {
        Response response = redirectSteps.makeMobileRequestWithFollow(urlBeforeRedirect);
