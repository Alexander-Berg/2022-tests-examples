...
/**
  * Basically copy-paste from scalatest EitherValues, with these changes:
  * 1. value method reports about actual value caused test failure
  * 2. Trait is renamed to avoid confusion
  * 3. Error messages are hardcoded, because they are generated as resources while building scalatest,
    ...
    def value: L = {
      try {
        ...
        case cause: NoSuchElementException =>
          throw new TestFailedException((_: StackDepthException) => Some(s"The either on which left.value was invoked was not defined as a Left: $leftProj"), Some(cause), pos)
      }
    }
  }
  ...
  class RightValuable[L, R](rightProj: Either.RightProjection[L, R], pos: source.Position) {
    ...
    def value: R = {
      try {
        ...
        case cause: NoSuchElementException =>
          throw new TestFailedException((_: StackDepthException) => Some(s"The Either on which right.value was invoked was not defined as a Right: $rightProj"), Some(cause), pos)
      }
    }
  }
}
...
/**
  ...
  * </pre>
  */
object BetterEitherValues extends BetterEitherValues
