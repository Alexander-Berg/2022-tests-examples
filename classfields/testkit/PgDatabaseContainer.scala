...
import ru.yandex.auto.vin.decoder.storage.orders.OrdersTable
...
trait PgDatabaseContainer extends Logging {

  implicit val t: Traced = Traced.empty

  implicit private val m: MetricsSupport = TestOperationalSupport

  protected def databaseSchemaList =
    Seq(
      ReportBuildQueueDao.reportBuildQueueTable,
      OrdersTable.orders
    ).map(_.schema).reduce(_ ++ _)

  private val dollar = "$$"

  protected def createDb: DBIOAction[Unit, NoStream, Effect.Schema] =
    DBIO.seq(
