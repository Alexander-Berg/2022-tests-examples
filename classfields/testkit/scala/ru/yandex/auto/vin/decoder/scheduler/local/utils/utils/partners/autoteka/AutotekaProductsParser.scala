...
import ru.yandex.auto.vin.decoder.raw.autoteka.products.model.AutotekaProductsRawModel
import ru.yandex.auto.vin.decoder.raw.validation.ValidationFieldErrors.InvalidVinError

import java.io.{File, FileInputStream}

object AutotekaProductsParser extends App {

  val manager = AutotekaProductsRawModelManager(FileFormats.Json, ",", EventType.NOVOCAR_PRODUCTS)
  ...
  val recordsWithProductsCount: Int = success.count(_.model.products.nonEmpty)

  println(
    s"records with products list - $recordsWithProductsCount (${(recordsWithProductsCount * 1.0 / success.size * 100).toInt} %)"
  )
  val recordsWithProductsMoreONeCount: Int = success.count(_.model.products.length > 1)

  println(
    s"records with products list size > 1 - $recordsWithProductsMoreONeCount (${(recordsWithProductsMoreONeCount * 1.0 / success.size * 100).toInt} %)"
  )
  ...
  private val isAllTheSame = (l: List[AutotekaProductsRawModel]) => {
