...
object AutoruCreditApplicationsParser extends App {

  val manager =
    new AutoruFormatCreditApplicationRawModelManager(EventType.RGS_BANK_CREDIT_APPLICATIONS, FileFormats.Csv)

  val dir = new File("/Users/shcherbakov-b/Downloads/carfax")
  ...
  val recordsWithAmountCount: Int = successInfo.count(_.amount.nonEmpty)

  println(
    s"records with amount list - $recordsWithAmountCount (${(recordsWithAmountCount * 1.0 / successInfo.size * 100).toInt} %)"
  )
