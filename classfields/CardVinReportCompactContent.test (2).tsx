...
const shallowRenderComponent = ({ showFreeBlock = true, items = ITEMS }: Partial<Props>) => {
    return shallow(
...
describe('CardVinReportCompactContent обычный', () => {
    it('правильно обрабатывает нажатие на "бесплатный отчёт', () => {
        const wrap = shallowRenderComponent({});
        wrap.find('.CardVinReportCompactContent__linkShowFree').simulate('click');
    ...
    it('правильно рендерит развернутое оглавление', () => {
        const wrap = shallowRenderComponent({});
        const itemsElements = wrap.find('.CardVinReportCompactContent__item');
        expect(itemsElements).toHaveLength(6);
        wrap.setState({ showAllItems: true });
        wrap.update();
        const itemsElementsAll = wrap.find('.CardVinReportCompactContent__item');
