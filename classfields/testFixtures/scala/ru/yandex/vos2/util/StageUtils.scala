...
trait StageUtils {
  ...
  def asyncProcess(
    ...
    checkShouldProcess: Boolean = false
  ): ProcessingState = {
  ...
  def asyncProcessWithStatusHistoryUpdate(
    stage: ProcessingStage,
    state: ProcessingState,
    getStatus: OfferOrBuilder => CompositeStatus
  ): ProcessingState = {
    val state1 = stage.process(state)
    val asyncUpdate = new AsyncState(state1)
  ...
  def process(offer: Offer): Matcher[ProcessingStage] = {
    Matcher { stage ⇒
      MatchResult(stage.shouldProcess(offer), "stage should process offer", "stage should not process offer")
    }
  }

  def notProcess(offer: Offer): Matcher[ProcessingStage] = {
    Matcher { stage ⇒
      MatchResult(!stage.shouldProcess(offer), "stage should not process offer", "stage should process offer")
    }
  }
