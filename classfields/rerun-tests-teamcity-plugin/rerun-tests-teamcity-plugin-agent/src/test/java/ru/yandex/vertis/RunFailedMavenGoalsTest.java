...
public class RunFailedMavenGoalsTest {
    ...
    public void buildWithRerunFeature() throws IOException {
        ...
        Files.write(tmpFolder.newFile(OLD_FAILED).toPath(), "test1".getBytes());

        parameters.put("runnerArgs", "default args");
    }


    @Test
    public void shouldParseGoals() {
        parameters.put(GOALS, goals);
        when(runner.getRunnerParameters()).thenReturn(parameters);

        runFailedAdapter.beforeRunnerStart(runner);
        verify(runner, times(count)).addRunnerParameter(any(), any());
    }
