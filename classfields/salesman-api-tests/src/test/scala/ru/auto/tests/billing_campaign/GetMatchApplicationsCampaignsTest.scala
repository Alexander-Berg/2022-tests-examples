...
@DisplayName(
  "GET /service/{service}/billing/campaign/match-applications/client/{clientId}"
)
@GuiceModules(Array(classOf[SalesmanApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetMatchApplicationsCampaignsTest {
  ...
  @Test
  @Owner(TIMONDL)
  def shouldGetMatchApplicationsCampaignsHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) =>
      apiClient.billingCampaign.getMatchApplicationsCampaignsRoute
        .reqSpec(defaultSpec)
        .servicePath(SERVICE)
        .clientIdPath(CLIENT_ID)
        .xSalesmanUserHeader(SALESMAN_USER)
