...
@DisplayName("GET /service/{service}/goods/category/{category}")
@GuiceModules(Array(classOf[SalesmanApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetGoodsCompareTest {

  private val OFFER_ID = 1097256974
  ...
  @Test
  @Owner(TIMONDL)
  def shouldGetGoodsRouteHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) =>
      apiClient.goods.getGoodsRoute
        .reqSpec(defaultSpec)
        .servicePath(SERVICE)
        .categoryPath(CARS.getValue.toLowerCase)
        .offerQuery(OFFER_ID)
