...
@DisplayName("GET /service/{service}/quota")
@GuiceModules(Array(classOf[SalesmanApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetQuotaRequestCompareTest {
  ...
  @Test
  @Owner(TIMONDL)
  def shouldGetQuotaRequestRouteHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) =>
      apiClient.quota.getQuotaRequestRoute
        .reqSpec(defaultSpec)
        .servicePath(SERVICE)
