...
@DisplayName(
  "GET /service/{service}/product/vin-history/client/{clientId}/reports"
)
@GuiceModules(Array(classOf[SalesmanApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetVinHistoryBoughtReportsTest {
  ...
  def shouldSee404WithInvalidService(): Unit =
    api.product.getVinHistoryBoughtReports
      .reqSpec(defaultSpec)
  ...
  @Test
  @Owner(TIMONDL)
  def shouldGetVinHistoryBoughtReportsHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) =>
      apiClient.product.getVinHistoryBoughtReports
        .reqSpec(defaultSpec)
        .servicePath(SERVICE)
        .clientIdPath(CLIENT_ID)
        .xSalesmanUserHeader(SALESMAN_USER)
