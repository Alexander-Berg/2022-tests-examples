...
@DisplayName("GET /service/{service}/goods/recent")
@GuiceModules(Array(classOf[SalesmanApiModule]))
@RunWith(classOf[GuiceTestRunner])
class FindRecentGoodsCompareTest {
  ...
  @Test
  @Owner(TIMONDL)
  def shouldFindRecentGoodsRouteHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) =>
      apiClient.goods.findRecentGoodsRoute
        .reqSpec(defaultSpec)
        .servicePath(SERVICE)
        .sinceQuery(OffsetDateTime.now.minusDays(1).toString)
