...
@DisplayName("GET /service/{service}/configurations")
@GuiceModules(Array(classOf[SalesmanApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetConfigsTest {
  ...
  @Test
  @Owner(TIMONDL)
  def shouldGetConfigsHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) =>
      apiClient.configurations.getConfigs
        .reqSpec(defaultSpec)
        .servicePath(SERVICE)
