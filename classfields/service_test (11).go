...
func testGetEndPoints(t *testing.T, data *test.Data, result map[string]utils.StringSet) {
	...
	s := NewService(log, infoS, static.NewService(log, static.NewConf()))
	resourceNames := make([]string, 0)
	for name := range result {
		resourceNames = append(resourceNames, name)
	}
	endpoints, err := s.getEndpoints(resourceNames, "", "")
	require.NoError(t, err)
	actual := make(map[string]utils.StringSet)
	for _, e := range endpoints {
		ads, ok := actual[e.ClusterName]
		if !ok {
			ads = utils.NewStringSet()
			actual[e.ClusterName] = ads
		}
		for _, es := range e.Endpoints {
			for _, e := range es.LbEndpoints {
				ads.Add(e.Endpoint.Address.SocketAddress.Address)
			}
		}
	}
