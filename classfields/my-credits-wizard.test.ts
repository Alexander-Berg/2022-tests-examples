...
import controller from './my-credits-wizard';

describe('контроллер my-credits-wizard', () => {
    ...
    it('если нет анкеты в драфте или активной, редиректит в my-credits', function() {
        publicApi
            .get('/1.0/session/')
            ...
            .then(
                ...
                (error) => {
                    expect(error).toMatchObject({
                        error: {
                            code: RedirectError.CODES.CREDITS_LK_WIZARD_TO_ROOT,
                        },
                    });
                },
            );
