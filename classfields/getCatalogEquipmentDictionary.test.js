...
const block = require('./getCatalogEquipmentDictionary');
    ...
    beforeEach(() => {
        publicApi
            ...
            .reply(200, {
                dictionary_v1: {
                    values: [
                        { code: 'xenon', name: 'Ксеноновые/Биксеноновые фары', group: 'Обзор' },
                    ],
                },
                status: 'SUCCESS',
            });
    ...
    it('должен закешировать запрос с правильным ключом', () => {
        return de.run(blockWithCache, { context, params: { category: 'cars' } })
            .then(() => {
                expect(
                    cache.get({
                        key: 'descript3-publicApiCatalog:///1.0/reference/catalog/{category}/dictionaries/v1/equipment?category=cars',
                    }),
                ).toBeDefined();
            });
    });

    it('должен преобразовать массив в объект', () => {
        return de.run(blockWithCache, { context, params: { category: 'cars' } })
            .then((result) => {
                expect(result).toEqual({
                    xenon: { code: 'xenon', name: 'Ксеноновые/Биксеноновые фары', group: 'Обзор' },
                });
            });
    });
});
