...
public class GetFeatureSnippetsTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.reviews().featureSnippet().subjectPath(AUTO).categoryPath(CARS)
    ...
    public void shouldSee400WithIncorrectSubject() {
        api.reviews().featureSnippet().subjectPath(Utils.getRandomString()).categoryPath(Utils.getRandomString())
    ...
    public void shouldSee400WithIncorrectCategory() {
        ...
        api.reviews().featureSnippet().subjectPath(AUTO).categoryPath(incorrectCategory)
                .reqSpec(defaultSpec()).execute(validatedWith(shouldBe400UnknownCategoryError(incorrectCategory)));
    }

    @Test
    @Owner(DSKUZNETSOV)
    public void shouldSee400WithoutModel() {
        api.reviews().featureSnippet().subjectPath(AUTO).categoryPath(CARS)
                .markQuery(Utils.getRandomString())
                .featureQuery(Utils.getRandomString())
                .reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)));
    }
    ...
    @Test
    @Owner(DSKUZNETSOV)
    public void shouldSee400WithoutFeature() {
        api.reviews().featureSnippet().subjectPath(AUTO).categoryPath(CARS)
                .markQuery(Utils.getRandomString())
                .modelQuery(Utils.getRandomString())
                .reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)));
