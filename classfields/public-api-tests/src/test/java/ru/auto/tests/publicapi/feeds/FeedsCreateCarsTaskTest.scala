...
import ru.auto.tests.publicapi.feeds.FeedsCreateCarsTaskTest.TestAccount
...
import ru.auto.tests.publicapi.operations.feeds.tasks.taskSettings
...
class FeedsCreateCarsTaskTest(section: SectionEnum, feedSettingsCheckboxValue: Boolean) extends CreateCarsFeedsTaskOps with Authorization {
  ...
  def shouldSeeSuccess(): Unit = {
    ...
    val feedUrl = s"feed_${section.name}.xml"
    ...
    val result = createCarsTask(testTaskSettings, section)
    ...
    MatcherAssert.assertThat(lastTask.getCategory.getCategory, Matchers.equalTo(CategoryEnum.CARS))
    MatcherAssert.assertThat(lastTask.getCategory.getSection, Matchers.equalTo(section))
  }

}

object FeedsCreateCarsTaskTest {

  private val TestAccount = Account.builder()
    .login("FeedsCreateCarsTaskTest@regress.apiauto.ru")
    .password("autoru")
    .id("70519768")
    .build()
