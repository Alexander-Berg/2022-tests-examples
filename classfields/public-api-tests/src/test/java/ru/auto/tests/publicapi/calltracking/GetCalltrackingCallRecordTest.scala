...
import ru.auto.tests.publicapi.model.{AutoApiCalltrackingCallListingRequest, AutoApiCalltrackingCallRecordLinkRequest, AutoApiErrorResponse}
...
@DisplayName("POST /calltracking/call/record")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetCalltrackingCallRecordTest {

  private val CALL_ID = 1199159L
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.calltracking.getRecordLinkCalltracking.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.calltracking.getRecordLinkCalltracking.reqSpec(defaultSpec)
      .body(new AutoApiCalltrackingCallRecordLinkRequest)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val request = (apiClient: ApiClient) => apiClient.calltracking.getRecordLinkCalltracking
      .reqSpec(defaultSpec)
      .body(new AutoApiCalltrackingCallRecordLinkRequest().callId(CALL_ID))
