...
@DisplayName("GET /dealer/users/groups")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class DealerUsersGroupsListTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().getUsersGroupsList().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee401WithoutSession() {
        api.dealer().getUsersGroupsList().reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)));
    }
    ...
    public void shouldSee403WithUserSession() {
        ...
        AutoApiErrorResponse response = api.dealer().getUsersGroupsList().reqSpec(defaultSpec())
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> req = apiClient -> apiClient.dealer().getUsersGroupsList().reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId)
