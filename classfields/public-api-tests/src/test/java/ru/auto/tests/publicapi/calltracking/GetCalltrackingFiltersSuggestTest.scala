...
import ru.auto.tests.publicapi.model.{AutoApiCalltrackingCallListingRequest, AutoApiCalltrackingSuggestFilterRequest, AutoApiErrorResponse, AutoApiRequestPagination}
...
@DisplayName("POST /calltracking/call/filter/suggest")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetCalltrackingFiltersSuggestTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.calltracking.suggestFilter.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.calltracking.suggestFilter.reqSpec(defaultSpec)
      .body(new AutoApiCalltrackingSuggestFilterRequest)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val requestBody = new AutoApiCalltrackingSuggestFilterRequest()

    val request = (apiClient: ApiClient) => apiClient.calltracking.suggestFilter
      .reqSpec(defaultSpec)
      .body(requestBody)
