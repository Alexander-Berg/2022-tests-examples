...
@DisplayName("GET /feeds/history")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class FeedsHistoryListTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.feeds().getHistoryFeed().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee401WithoutSession() {
        api.feeds().getHistoryFeed().reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)));
    }
    ...
    public void shouldSee401WithUserSession() {
        ...
        AutoApiErrorResponse response = api.feeds().getHistoryFeed().reqSpec(defaultSpec())
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> req = apiClient -> apiClient.feeds().getHistoryFeed().reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId)
