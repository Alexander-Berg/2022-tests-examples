...
class PostDealerAliasesTest {

  private val origin: String = "msk0001"
  private val externalId: String = "0001"
  ...
  def shouldSeeSuccess(): Unit = {
    val response = api.dealer.createAlias().reqSpec(xUserCMExpertAliases)
      .body(requestBody)
      .executeAs(validatedWith(shouldBe200OkJSON))

    assertThat(response).hasMessage("Alias added, 1 entries affected")
  }

  @Test
  @Owner(AVGRIBANOV)
  def shouldSeeErrorWithoutTokenCMExpert(): Unit = {
    val response = api.dealer.createAlias()
      .body(requestBody)
