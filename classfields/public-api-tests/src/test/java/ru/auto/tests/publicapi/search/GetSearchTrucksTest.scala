...
@DisplayName("GET /search/trucks")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetSearchTrucksTest {
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    val request = (apiClient: ApiClient) => apiClient.search.searchTrucks
      .reqSpec(defaultSpec)
      .contextQuery("listing")
      .trucksCategoryQuery("TRUCK")
