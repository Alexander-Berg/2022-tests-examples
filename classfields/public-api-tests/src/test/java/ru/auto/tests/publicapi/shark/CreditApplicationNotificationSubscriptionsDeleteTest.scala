...
@DisplayName("DELETE /shark/credit-application/notification-subscriptions/{credit_application_id}")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class CreditApplicationNotificationSubscriptionsDeleteTest {

  import CreditApplicationNotificationSubscriptionsDeleteTest._
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.shark.notificationUnsubscribeCreditApp()
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val notificationUnsubscribe = api.shark.notificationUnsubscribeCreditApp()
      .reqSpec(defaultSpec)
      .creditApplicationIdPath(getRandomString)
...
object CreditApplicationNotificationSubscriptionsDeleteTest {
