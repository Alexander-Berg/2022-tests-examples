...
@DisplayName("GET /dealer/campaign/products")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetCampaignProductsListTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.getDealerProductCampaignsList.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.getDealerProductCampaignsList.reqSpec(defaultSpec)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.dealer.getDealerProductCampaignsList
      .reqSpec(defaultSpec)
