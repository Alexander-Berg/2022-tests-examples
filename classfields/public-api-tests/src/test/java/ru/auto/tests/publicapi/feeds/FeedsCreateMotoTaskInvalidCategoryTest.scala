...
class FeedsCreateMotoTaskInvalidCategoryTest(section: SectionEnum) extends CreateMotoFeedsTaskOps with Authorization {
  ...
  def shouldSee400WhenInvalidMotoCategory(): Unit = {
    ...
    assertApiError(BAD_REQUEST, Some(expectedError)) {
      createMotoTask(defaultTaskSettings, category = "INVALID_CATEGORY", section.name)
...
object FeedsCreateMotoTaskInvalidCategoryTest {
