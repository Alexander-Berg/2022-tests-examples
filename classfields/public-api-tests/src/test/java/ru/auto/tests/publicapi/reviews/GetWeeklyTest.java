...
@DisplayName("GET /reviews/{subject}/presets/{category}/weekly")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class GetWeeklyTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.reviews().weekly().subjectPath(AUTO).categoryPath(CARS).execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    public void shouldSee400WithInvalidSubject() {
        String invalidSubject = Utils.getRandomString();
        api.reviews().weekly().subjectPath(invalidSubject).categoryPath(CARS).reqSpec(defaultSpec())
    ...
    public void shouldSee400WithInvalidCategory() {
        ...
        api.reviews().weekly().subjectPath(AUTO).categoryPath(invalidCategory).reqSpec(defaultSpec())
                .execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)));
    }
}
