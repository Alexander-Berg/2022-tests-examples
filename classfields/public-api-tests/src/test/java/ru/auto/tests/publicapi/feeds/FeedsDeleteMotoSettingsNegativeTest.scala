...
class FeedsDeleteMotoSettingsNegativeTest(category: MotoCategoryEnum, section: SectionEnum) extends DeleteMotoFeedsSettingsOps with Authorization {
  ...
  def shouldSee401WhenNoAuth(): Unit = {
    deleteMotoSettingsWithoutAuth(category, section)
      .execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)))
  }
...
object FeedsDeleteMotoSettingsNegativeTest {
