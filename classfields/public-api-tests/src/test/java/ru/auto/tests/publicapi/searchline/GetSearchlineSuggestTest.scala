...
@DisplayName("GET /searchline/suggest")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetSearchlineSuggestTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.searchline.suggestSearchline()
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    val request = (apiClient: ApiClient) => apiClient.searchline.suggestSearchline()
      .reqSpec(defaultSpec)
      .queryQuery("диз")
