...
import ru.auto.tests.publicapi.model.{AutoApiErrorResponse, RuAutoApiAuctionCallAuctionRequestContext, RuAutoApiAuctionPlaceBidRequest}
...
@DisplayName("POST /dealer/auction/place-bid")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
@Owner(MONEY_DUTY)
class PlaceBidAuctionTest extends AuctionBaseTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.placeBid.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  def shouldSee401WithoutSession(): Unit = {
    val response = api.dealer.placeBid
      .reqSpec(defaultSpec)
      .body(new RuAutoApiAuctionPlaceBidRequest())
  ...
  def shouldSee400WithoutContextMarkCode(): Unit = {
    val body = new RuAutoApiAuctionPlaceBidRequest()
  ...
  @Test
  def shouldSee400WithoutBid(): Unit = {
    val body = new RuAutoApiAuctionPlaceBidRequest()
      .context(
  ...
  def shouldSee200PlaceBid(): Unit = {
    ...
    Assertions.assertThat(stateAfter.get.getCurrentBid).isEqualTo(newBid)
  }
}
