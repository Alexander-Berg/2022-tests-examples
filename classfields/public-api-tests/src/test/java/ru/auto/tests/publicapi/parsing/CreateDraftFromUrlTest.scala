...
@DisplayName("POST /parsing/create-draft")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class CreateDraftFromUrlTest {

  private val URL = "https://www.avito.ru/gayduk/avtomobili/hyundai_tucson_2020_2033975112"
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.parsing.createDraftFromParsedOfferByUrl()
  ...
  @Owner(TIMONDL)
  @Ignore
  def shouldSee404WithInvalidParsingUrl(): Unit = {
  ...
  @Test
  @Owner(TIMONDL)
  def shouldCreateDraftFromParsingUrl(): Unit = {
    val account = accountManager.create()
    val sessionId = adaptor.login(account).getSession.getId

    val response = api.parsing.createDraftFromParsedOfferByUrl()
      .reqSpec(defaultSpec)
      .urlQuery(URL)
      .xSessionIdHeader(sessionId)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.parsing.createDraftFromParsedOfferByUrl()
      .reqSpec(defaultSpec)
      .urlQuery(URL)
      .xSessionIdHeader(sessionId)
