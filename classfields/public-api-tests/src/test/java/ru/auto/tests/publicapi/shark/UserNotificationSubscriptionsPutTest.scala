...
@DisplayName("PUT /shark/user/notification-subscriptions")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class UserNotificationSubscriptionsPutTest {

  import UserNotificationSubscriptionsPutTest._
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.shark.notificationSubscribeUser()
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val notificationSubscribe = api.shark.notificationSubscribeUser()
      .reqSpec(defaultSpec)
...
object UserNotificationSubscriptionsPutTest {
