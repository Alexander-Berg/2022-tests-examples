...
/**
 * Generated API tests for all status codes ComplectList
 */
@DisplayName("GET /reference/catalog/{category}/complectations")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class ComplectListTest {
    @Rule
    @Inject
    ...
    @Ignore
    @Owner("generated")
    @Description("Check status code 400 and response for GET /reference/catalog/{category}/complectations (Ошибка в запросе)")
    public void shouldSee400ComplectList() {
        String category = Utils.getRandomString();
        api.catalog().complectList().reqSpec(defaultSpec())
