...
class FeedsCreateTrucksTaskInvalidCategoryTest(section: SectionEnum) extends CreateTrucksFeedsTaskOps with Authorization {
  ...
  def shouldSee400WhenInvalidTrucksCategory(): Unit = {
    ...
    assertApiError(BAD_REQUEST, Some(expectedError)) {
      createTrucksTask(defaultTaskSettings, category = "INVALID_CATEGORY", section.name)
...
object FeedsCreateTrucksTaskInvalidCategoryTest {
