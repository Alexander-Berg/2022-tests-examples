...
public class CreateUsersGroupNegativeTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().createUsersGroup().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee401WithoutSession() {
        api.dealer().createUsersGroup().reqSpec(defaultSpec())
    ...
    public void shouldSee403WithUserSession() {
        ...
        AutoApiErrorResponse response = api.dealer().createUsersGroup().reqSpec(defaultSpec())
                .body(validUsersGroup)
