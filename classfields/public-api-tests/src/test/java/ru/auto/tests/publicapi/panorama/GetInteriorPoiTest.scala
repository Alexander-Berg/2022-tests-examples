...
@DisplayName("GET /panorama/interior/{panorama_id}/poi")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetInteriorPoiTest {
  ...
  def getNonExistingPoi(): Unit = {
    val panoramaId = "1758404935-1646253559364-PIwNx"
    adaptor.deleteAllInteriorPoi(panoramaId)

    val res = api.interiorPanorama()
      .getPoiInt
      .reqSpec(defaultSpec)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    adaptor.addInteriorPoi(panoramaId, poi1)
    ...
    adaptor.addInteriorPoi(panoramaId, poi2)
