...
import static ru.auto.tests.publicapi.consts.Owners.DEALER_PRODUCTS;
...
import static ru.auto.tests.publicapi.testdata.DealerAccounts.getFeedsDisabledTestAccount;

@DisplayName("GET /feeds/settings")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class FeedsSettingsListTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.feeds().getSettingsFeed().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee401WithoutSession() {
        api.feeds().getSettingsFeed().reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)));
    }
    ...
    public void shouldSee401WithUserSession() {
        ...
        AutoApiErrorResponse response = api.feeds().getSettingsFeed().reqSpec(defaultSpec())
    ...
    public void shouldSee403IfNoAclResource() {
        Account dealer = getFeedsDisabledTestAccount();
        String sessionId = adaptor.login(dealer).getSession().getId();

        AutoApiErrorResponse response = api.feeds().getSettingsFeed()
                .reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId)
                .execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
                .as(AutoApiErrorResponse.class, GSON);

        assertThat(response).hasStatus(ERROR).hasError(CUSTOMER_ACCESS_FORBIDDEN)
                .hasDetailedError(format("Permission denied to FEEDS:Read for user:%s", dealer.getId()));
    }
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> req = apiClient -> apiClient.feeds().getSettingsFeed().reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId)
