...
import static ru.auto.tests.publicapi.consts.Owners.KNKMX;
import static ru.auto.tests.publicapi.model.AutoApiReview.SubjectEnum.AUTO;
import static ru.auto.tests.publicapi.ra.RequestSpecBuilders.defaultSpec;
import static ru.auto.tests.publicapi.ra.ResponseSpecBuilders.shouldBeSuccess;

/**
 * Created by Karpenko Maksim (knkmx@yandex-team.ru) on 07/06/2018.
 */


@DisplayName("GET /reviews/{subject}/anon-draft")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class GetAnonDraftTest {
    ...
    @Test
    @Owner(KNKMX)
    public void shouldSee404WhenNoAnonDraft() {
        Account account = am.create();
        AutoApiLoginResponse loginResult = adaptor.login(account);
        api.reviews().getAnonDraft().subjectPath(AUTO).reqSpec(defaultSpec())
    ...
    public void shouldSee400WithIncorrectSubject() {
        api.reviews().getAnonDraft().subjectPath(Utils.getRandomString()).reqSpec(defaultSpec())
    ...
    public void shouldMigrateReviewDraftFromAnonymToAuthUser() {
        ...
        AutoApiReviewResponse anonDraft = api.reviews().getAnonDraft().subjectPath(AUTO).reqSpec(defaultSpec())
        ...
        api.reviews().getAnonDraft().subjectPath(AUTO).reqSpec(defaultSpec())
                .xSessionIdHeader(authSession).xDeviceUidHeader(deviceUid).executeAs(validatedWith(shouldBeCode(SC_NOT_FOUND)));

        prodApi.reviews().getAnonDraft().subjectPath(AUTO).reqSpec(defaultSpec())
                .xSessionIdHeader(authSession).xDeviceUidHeader(deviceUid).executeAs(validatedWith(shouldBeCode(SC_NOT_FOUND)));
        ...
        MatcherAssert.assertThat(listingResponse
                ...
                hasItem(anonDraft.getReview().getId())
        );
