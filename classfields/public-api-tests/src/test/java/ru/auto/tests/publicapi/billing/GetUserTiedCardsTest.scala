...
@DisplayName("GET /billing/{salesmanDomain}/tied-cards")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetUserTiedCardsTest {
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.billing.getTiedCards.reqSpec(defaultSpec)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.billing.getTiedCards
      .reqSpec(defaultSpec)
      .salesmanDomainPath(DOMAIN)
      .paymentSystemIdQuery(PAYMENT_SYSTEM_ID)
      .xSessionIdHeader(sessionId)
