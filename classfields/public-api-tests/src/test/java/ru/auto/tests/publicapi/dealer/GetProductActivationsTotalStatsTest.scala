...
@DisplayName("GET /dealer/wallet/product/activations/total-stats")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetProductActivationsTotalStatsTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.getProductActivationsTotalStats.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.getProductActivationsTotalStats.reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
      .toQuery(DATE_TO)
  ...
  def shouldSee400WithoutFromDate(): Unit = {
    api.dealer.getProductActivationsTotalStats.reqSpec(defaultSpec)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.dealer.getProductActivationsTotalStats
      .reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
      .toQuery(DATE_TO)
