...
class FeedsSaveMotoSettingsNegativeTest(category: MotoCategoryEnum, section: SectionEnum) extends SaveMotoFeedsSettingsOps with Authorization {
  ...
  def shouldSee401WithoutSession(): Unit = {
    saveMotoSettings(defaultFeedSettings, category, section)
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    saveMotoSettingsWithoutAuth(defaultFeedSettings, category, section)
  ...
  def shouldSee400WhenEmptySource(): Unit = {
    ...
    assertApiError(BAD_PARAMS_DETAILS, Some(expectedError)) {
      saveMotoSettings(invalidSettings, category, section)
...
object FeedsSaveMotoSettingsNegativeTest {
