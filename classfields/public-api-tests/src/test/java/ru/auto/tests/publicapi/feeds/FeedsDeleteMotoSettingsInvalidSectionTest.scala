...
class FeedsDeleteMotoSettingsInvalidSectionTest(category: MotoCategoryEnum) extends DeleteMotoFeedsSettingsOps with Authorization {
  ...
  def shouldSee400WhenInvalidMotoSection(): Unit = {
    ...
    assertApiError(BAD_REQUEST, Some(expectedError)) {
      deleteMotoSettings(category.name, section = "INVALID_SECTION")
  ...
  @Test
  @Owner(DEALER_PRODUCTS)
  def shouldSee400WhenUnknownMotoSection(): Unit = {
    ...
    assertApiError(BAD_PARAMS_DETAILS, Some(expectedError)) {
      deleteMotoSettings(category.name, section = "SECTION_UNKNOWN")
        .xSessionIdHeader(getSessionId(Accounts.TestAccount))
        .execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)))
    }
  }
  ...
  @Test
  @Owner(DEALER_PRODUCTS)
  def shouldSee404WhenMissingMotoSection(): Unit = {
    assertApiError(BAD_REQUEST) {
      deleteMotoSettings(category.name, section = "")
        .xSessionIdHeader(getSessionId(Accounts.TestAccount))
...
object FeedsDeleteMotoSettingsInvalidSectionTest {
