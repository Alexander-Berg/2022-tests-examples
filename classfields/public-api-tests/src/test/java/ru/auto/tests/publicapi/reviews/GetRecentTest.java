...
public class GetRecentTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.reviews().recent().subjectPath(AUTO).categoryPath(CARS).execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    public void shouldSee400WithInvalidSubject() {
        String invalidSubject = Utils.getRandomString();
        api.reviews().recent().subjectPath(invalidSubject).categoryPath(CARS).reqSpec(defaultSpec())
    ...
    public void shouldSee400WithInvalidCategory() {
        ...
        api.reviews().recent().subjectPath(AUTO).categoryPath(invalidCategory).reqSpec(defaultSpec())
                .execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)));
    }
}
