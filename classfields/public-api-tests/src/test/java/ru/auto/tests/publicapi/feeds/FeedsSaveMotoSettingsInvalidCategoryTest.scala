...
class FeedsSaveMotoSettingsInvalidCategoryTest(section: SectionEnum) extends SaveMotoFeedsSettingsOps with Authorization {
  ...
  def shouldSee400WhenInvalidMotoCategory(): Unit = {
    ...
    assertApiError(BAD_REQUEST, Some(expectedError)) {
      saveMotoSettings(defaultFeedSettings, category = "INVALID_CATEGORY", section.name)
...
object FeedsSaveMotoSettingsInvalidCategoryTest {
