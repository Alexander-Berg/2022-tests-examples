...
import static ru.auto.tests.publicapi.model.AutoApiErrorResponse.ErrorEnum.STATUS_CONFLICT;
...
public class ActivateMotoTruckTest {
    ...
    public void shouldSee404WithWrongCategory() {
        ...
        AutoApiErrorResponse errorResponse = api.userOffers().activate().categoryPath(category.name()).offerIDPath(offerId)
                .reqSpec(defaultSpec()).xSessionIdHeader(sessionId).execute(validatedWith(shouldBeCode(SC_NOT_FOUND))).as(AutoApiErrorResponse.class);
        assertThat(errorResponse).hasError(OFFER_NOT_FOUND).hasStatus(ERROR).hasDetailedError(OFFER_NOT_FOUND.name());
    }


    @Test
    public void shouldSee409IfActivateActiveOffer() {
        Account account = am.create();
        String sessionId = adaptor.login(account).getSession().getId();
        String offerId = adaptor.createOffer(account.getLogin(), sessionId, category).getOfferId();

        AutoApiErrorResponse response = api.userOffers().activate().categoryPath(category.name()).offerIDPath(offerId).reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId).execute(validatedWith(shouldBeCode(SC_CONFLICT))).as(AutoApiErrorResponse.class);
        assertThat(response).hasError(STATUS_CONFLICT).hasStatus(ERROR).hasDetailedError("Offer already active");
    }
}
