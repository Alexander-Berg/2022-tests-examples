...
@DisplayName("GET /search/{category}/context/recommend-new-in-stock")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetSearchContextRecommendNewInStockTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.search.recommendNewInStockGET
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    val request = (apiClient: ApiClient) => apiClient.search.recommendNewInStockGET
      .reqSpec(defaultSpec)
      .categoryPath("cars")
