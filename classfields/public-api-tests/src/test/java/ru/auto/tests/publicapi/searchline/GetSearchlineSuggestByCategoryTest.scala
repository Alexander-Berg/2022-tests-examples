...
@DisplayName("GET /searchline/suggest/{category}")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetSearchlineSuggestByCategoryTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.searchline.suggestForCategory
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    val request = (apiClient: ApiClient) => apiClient.searchline.suggestForCategory
      .reqSpec(defaultSpec)
      .categoryPath("cars")
      .queryQuery("диз")
