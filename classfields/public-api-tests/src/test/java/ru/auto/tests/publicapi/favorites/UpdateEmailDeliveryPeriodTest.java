...
public class UpdateEmailDeliveryPeriodTest {
    ...
    public static List<Object[]> getParameters() {
        return Arrays.asList(new Object[]{"60s", "3600s", "14400s", "86400s", "604800s"});
    }

    @Test
    public void shouldUpdatePeriodEmailDelivery() {
        String sessionId = adaptor.login(account).getSession().getId();
        String id = adaptor.addSearch(CARS, sessionId).getId();

        api.userFavorites().upsertEmailDelivery().idPath(id).body(getEnabledEmailDelivery().period(period))
        ...
        AutoApiEmailDeliveryAssert.assertThat(adaptor.getSavedSearch(sessionId, id).getSearch()
                .getDeliveries().getEmailDelivery()).hasPeriod(period);
    }
