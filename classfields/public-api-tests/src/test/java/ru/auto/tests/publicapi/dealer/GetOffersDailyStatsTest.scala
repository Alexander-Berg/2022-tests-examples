...
@DisplayName("GET /dealer/offers-daily-stats")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetOffersDailyStatsTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.getOffersDailyStats.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.getOffersDailyStats.reqSpec(defaultSpec)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.dealer.getOffersDailyStats
      .reqSpec(defaultSpec)
      .fromDateQuery(DATE_FROM)
