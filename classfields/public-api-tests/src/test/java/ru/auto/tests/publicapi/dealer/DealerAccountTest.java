...
@DisplayName("GET /dealer/account")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class DealerAccountTest {
    @Rule
    @Inject
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().getDealerAccount().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(DSKUZNETSOV)
    public void shouldSee401WithoutSession() {
        api.dealer().getDealerAccount().reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)));
    }

    @Test
    @Owner(DSKUZNETSOV)
    public void shouldSeeGetAccountHasNoDiffWithProduction() {
        String sessionId = adaptor.login(getDemoAccount()).getSession().getId();

        Function<ApiClient, JsonObject> req = apiClient -> apiClient.dealer().getDealerAccount()
