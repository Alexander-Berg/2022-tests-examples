...
@DisplayName("PUT /dealer/campaign/product/{product}/activate")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class ActivateCampaignProductTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.activateDealerProductCampaign()
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.activateDealerProductCampaign().reqSpec(defaultSpec)
      .productPath(PRODUCT)
