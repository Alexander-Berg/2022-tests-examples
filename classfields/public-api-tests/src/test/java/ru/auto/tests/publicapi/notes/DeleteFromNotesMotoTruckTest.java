...
public class DeleteFromNotesMotoTruckTest {
    ...
    @Test
    @Owner(SCROOGE)
    public void shouldSeeSuccessWithWrongCategory() {
        ...
        api.userNotes().deleteNote().categoryPath(category.name()).offerIdPath(offerId)
                .reqSpec(defaultSpec()).xSessionIdHeader(sessionId).execute(validatedWith(shouldBeSuccess()));
    }

    @Test
    @Owner(SCROOGE)
    public void shouldSeeSuccessWithWrongCategoryForAnonym() {
        Account account = am.create();
        String sessionId = adaptor.login(account).getSession().getId();
