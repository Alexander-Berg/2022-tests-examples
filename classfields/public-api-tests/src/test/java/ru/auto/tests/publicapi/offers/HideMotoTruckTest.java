...
public class HideMotoTruckTest {
    ...
    public void shouldSee404WithWrongCategory() {
        ...
        AutoApiErrorResponse errorResponse = api.userOffers().hide().categoryPath(category.name()).offerIDPath(offerId)
                .reqSpec(defaultSpec()).xSessionIdHeader(sessionId).body(getOfferHideRequest()).execute(validatedWith(shouldBeCode(SC_NOT_FOUND))).as(AutoApiErrorResponse.class);
