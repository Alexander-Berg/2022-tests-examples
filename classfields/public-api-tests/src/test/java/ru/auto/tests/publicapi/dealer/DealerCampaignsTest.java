...
@DisplayName("GET /dealer/campaigns")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class DealerCampaignsTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().getDealerCampaigns().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee401WithoutSession() {
        api.dealer().getDealerCampaigns().reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)));
    }
    ...
    public void shouldSee401WithUserSession() {
        ...
        AutoApiErrorResponse response = api.dealer().getDealerCampaigns().reqSpec(defaultSpec())
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> req = apiClient -> apiClient.dealer().getDealerCampaigns().reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId)
