...
import ru.auto.tests.publicapi.model.AutoApiErrorResponse.ErrorEnum.{BAD_PARAMS_DETAILS, SESSION_NOT_FOUND}
...
class DeleteCommentNegativeTest {
  ...
  def shouldSee401WhenNotAuth(): Unit = {
    val response = api.comments.deleteComment().reqSpec(defaultSpec)
      .topicGroupPath(TOPIC_GROUP)
      .topicIdPath(TOPIC_ID)
      .commentIdPath(COMMENT_ID)
