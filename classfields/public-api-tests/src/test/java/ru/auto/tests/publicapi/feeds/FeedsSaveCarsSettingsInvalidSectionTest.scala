...
class FeedsSaveCarsSettingsInvalidSectionTest extends SaveCarsFeedsSettingsOps with Authorization {
  ...
  def shouldSee400WhenInvalidCarSection(): Unit = {
    ...
    assertApiError(BAD_REQUEST, Some(expectedError)) {
      saveCarsSettings(defaultFeedSettings, section = "INVALID_SECTION")
  ...
  def shouldSee400WhenUnknownCarSection(): Unit = {
    ...
    assertApiError(BAD_PARAMS_DETAILS, Some(expectedError)) {
      saveCarsSettings(defaultFeedSettings, section = "SECTION_UNKNOWN")
        .xSessionIdHeader(getSessionId(Accounts.TestAccount))
        .execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)))
    }
  }
