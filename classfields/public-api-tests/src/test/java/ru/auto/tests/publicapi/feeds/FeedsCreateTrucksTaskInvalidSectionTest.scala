...
class FeedsCreateTrucksTaskInvalidSectionTest(category: TruckCategoryEnum) extends CreateTrucksFeedsTaskOps with Authorization {
  ...
  def shouldSee400WhenInvalidTrucksSection(): Unit = {
    ...
    assertApiError(BAD_REQUEST, Some(expectedError)) {
      createTrucksTask(defaultTaskSettings, category.name, section = "INVALID_SECTION")
...
object FeedsCreateTrucksTaskInvalidSectionTest {
