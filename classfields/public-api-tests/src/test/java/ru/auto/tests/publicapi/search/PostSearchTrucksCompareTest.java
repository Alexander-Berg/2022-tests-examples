...
@DisplayName("POST /search/trucks")
@GuiceModules(PublicApiModule.class)
@RunWith(Parameterized.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class PostSearchTrucksCompareTest {
    ...
    public static Collection<AutoApiSearchSearchRequestParameters> getParameters() {
        ...
        return newArrayList(
                ...
                new AutoApiSearchSearchRequestParameters().markModelNameplate(newArrayList("CITROEN")).trucksParams(new AutoApiSearchTrucksSearchRequestParameters().trucksCategory(LCV)),
                new AutoApiSearchSearchRequestParameters().markModelNameplate(newArrayList("CITROEN#NEMO")).trucksParams(new AutoApiSearchTrucksSearchRequestParameters().trucksCategory(LCV))
        );
    }

    @Test
    public void shouldSearchTrucksHasNoDifferenceWithProduction() {
        Function<ApiClient, JsonObject> req = apiClient -> apiClient.search().postSearchTrucks()
                .body(body).pageQuery(1).pageSizeQuery(2)
        ...
        MatcherAssert.assertThat(req.apply(api), jsonEquals(req.apply(prodApi))
                .whenIgnoringPaths("pagination.total_offers_count", "count")
        );
    }
}
