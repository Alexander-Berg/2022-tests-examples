...
@DisplayName("GET /dealer/wallet/daily-balance-stats")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetDailyBalanceStatsTest {

  private val DATE_FROM = "2020-10-26"
  private val DATE_TO = "2020-11-25"
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.getDailyBalanceStats.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.getDailyBalanceStats.reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
      .toQuery(DATE_TO)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.dealer.getDailyBalanceStats
      .reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
      .toQuery(DATE_TO)
