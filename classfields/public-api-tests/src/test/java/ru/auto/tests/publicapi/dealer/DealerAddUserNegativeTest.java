...
import static ru.auto.tests.publicapi.model.AutoApiErrorResponse.ErrorEnum.USER_NOT_FOUND;
...
public class DealerAddUserNegativeTest {

    private static final Long GROUP_ID = 8L;
    private static final String GROUP_ID_FIELD_ERROR = "Request is missing required query parameter 'group'";
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().linkUser().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }
    ...
    public void shouldSee403WithUserSession() {
        ...
        AutoApiErrorResponse response = api.dealer().linkUser().reqSpec(defaultSpec())
    ...
    @Test
    @Owner(TIMONDL)
    public void shouldSee404UserNotFound() {
        String userEmail = getRandomEmail();
        String dealerSessionId = adaptor.login(getDemoAccount()).getSession().getId();
        ...
        assertThat(response).hasStatus(ERROR)
                .hasError(USER_NOT_FOUND)
                .hasDetailedError(USER_NOT_FOUND.getValue());
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee400WithoutGroupId() {
        String dealerSessionId = adaptor.login(getDemoAccount()).getSession().getId();
        ...
        Assertions.assertThat(response.getDetailedError()).contains(GROUP_ID_FIELD_ERROR);
    }
