...
public class FavoritesListMotoTruckTest {
    ...
    @Test
    public void shouldSeeEmptyFavoritesWithWrongCategory() {
    ...
    @Test
    @Owner(SCROOGE)
    public void shouldSeeEmptyFavoritesWithWrongCategoryForAnonym() {
        Account account = am.create();
        String accountSessionId = adaptor.login(account).getSession().getId();

        String anonymSessionId = adaptor.session().getSession().getId();
        String offerId = adaptor.createOffer(account.getLogin(), accountSessionId, CARS).getOfferId();
