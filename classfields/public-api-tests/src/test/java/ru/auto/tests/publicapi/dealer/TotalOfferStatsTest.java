...
public class TotalOfferStatsTest {
    @Rule
    @Inject
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().getOfferProductActivationsTotalStats().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(DSKUZNETSOV)
    public void shouldSee400WithInvalidSortQuery() {
        api.dealer().getOfferProductActivationsTotalStats().sortQuery(Utils.getRandomString())
