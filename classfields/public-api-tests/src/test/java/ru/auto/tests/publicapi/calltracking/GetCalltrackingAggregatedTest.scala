...
import ru.auto.tests.publicapi.model.{AutoApiCalltrackingCallListingRequest, AutoApiErrorResponse, AutoApiRequestPagination}
...
@DisplayName("POST /calltracking/aggregated")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetCalltrackingAggregatedTest {

  private val REQUEST_BODY = new AutoApiCalltrackingCallListingRequest
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.calltracking.getAggregatedCalls.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.calltracking.getAggregatedCalls.reqSpec(defaultSpec)
      .body(REQUEST_BODY)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val request = (apiClient: ApiClient) => apiClient.calltracking.getAggregatedCalls
      .reqSpec(defaultSpec)
      .body(REQUEST_BODY)
