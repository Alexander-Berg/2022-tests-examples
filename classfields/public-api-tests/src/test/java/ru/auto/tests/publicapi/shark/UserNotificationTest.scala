...
@DisplayName("PUT /shark/user/notification/{user_id}")
...
class UserNotificationTest {
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.shark.sendNotification()
      .reqSpec(defaultSpec)
      .userIdPath(accountManager.create().getId)
