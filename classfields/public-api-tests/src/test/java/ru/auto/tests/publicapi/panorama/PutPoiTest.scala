...
@DisplayName("PUT /panorama/{panorama_id}/poi")
...
class PutPoiTest {
  ...
  @Test
  def updatePoi(): Unit = {
    val panoramaId = "1762861353-1649349550224-Ymp8N"
    adaptor.deleteAllExteriorPoi(panoramaId)

    val x1 = Random.between(1, 1000)
    val y1 = Random.between(1, 1000)
    val poi = adaptor.makeExteriorPoi(x1, y1, getRandomString, Optional.empty)
    ...
    val poiToUpdate = adaptor.makeExteriorPoi(x2, y2, text2, Optional.empty)
    ...
    waitUntil(
      () => adaptor.getAllExteriorPoi(panoramaId).getPoi.get(0).getPoint.getCoordinates.getX,
  ...
  def failToUpdateNonExistingPoi(): Unit = {
    val panoramaId = "1745035603-1649349300742-nqQxV"
    adaptor.deleteAllExteriorPoi(panoramaId)

    val x = Random.between(1, 1000)
    val y = Random.between(1, 1000)
    val poiToUpdate = adaptor.makeExteriorPoi(x, y, getRandomString, Optional.empty)
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.exteriorPanorama.updatePoiExt()
