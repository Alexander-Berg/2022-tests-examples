...
import static ru.auto.tests.publicapi.model.AutoApiOffer.StatusEnum.REMOVED;
...
public class DeleteOfferCategoriesTest {
    ...
    public void shouldDeleteOffer() {
        ...
        api.userOffers().hideOffer().categoryPath(category.name()).offerIDPath(offerId)
                .reqSpec(defaultSpec()).xSessionIdHeader(sessionId).execute(validatedWith(shouldBeSuccess()));

        AutoApiOfferAssert.assertThat(adaptor.getOffer(category, offerId).getOffer()).hasStatus(REMOVED);
    }
}
