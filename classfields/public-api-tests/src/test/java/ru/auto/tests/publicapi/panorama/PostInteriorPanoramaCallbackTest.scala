...
@DisplayName("POST /panorama/interior/callback")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class PostInteriorPanoramaCallbackTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.interiorPanorama.callbackInt()
      .execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  def shouldSee400WithoutBody(): Unit = {
    val response = api.interiorPanorama.callbackInt().reqSpec(defaultSpec)
