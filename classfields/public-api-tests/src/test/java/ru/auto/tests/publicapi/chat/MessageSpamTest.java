...
@DisplayName("GET /chat/message/spam")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class MessageSpamTest {
    @Rule
    @Inject
    ...
    @Test
    @Owner(ABORUNOV)
    public void shouldSeeNoSpamMessages() {
        ...
        adaptor.createMessage(sessionId1, roomId);
        ...
        Assertions.assertThat(messageListingResponse1.getMessages()).isNull();
        ...
        AutoApiMessageListingResponse messageListingResponse2 = api.chat().getSpamMessages().roomIdQuery(roomId)
                ...
                .executeAs(validatedWith(shouldBe200OkJSON()));
        Assertions.assertThat(messageListingResponse2.getMessages()).isNull();
    }
    ...
    public void shouldSeeSpamMessages() {
        ...
        adaptor.markChatMessageAsSpam(message.getId(), account2);
        AutoApiMessageListingResponse messageListingResponse1 = api.chat().getSpamMessages().roomIdQuery(roomId)
                .reqSpec(defaultSpec()).xSessionIdHeader(sessionId1)
                .executeAs(validatedWith(shouldBe200OkJSON()));
