...
@DisplayName("GET /search/{category}/context/premium-new-cars")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetSearchContextPremiumNewCarsTest {
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    val request = (apiClient: ApiClient) => apiClient.search.premiumNewCarsGET
      .reqSpec(defaultSpec)
      .categoryPath("cars")
