...
@DisplayName("GET /dealer/users")
@RunWith(GuiceTestRunner.class)
@GuiceModules(PublicApiModule.class)
public class DealerUsersListTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.dealer().getUsersList().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    @Owner(TIMONDL)
    public void shouldSee401WithoutSession() {
        api.dealer().getUsersList().reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_UNAUTHORIZED)));
    }
    ...
    public void shouldSee403WithUserSession() {
        ...
        AutoApiErrorResponse response = api.dealer().getUsersList().reqSpec(defaultSpec())
    ...
    public void shouldHasNoDiffWithProduction() {
        ...
        Function<ApiClient, JsonObject> req = apiClient -> apiClient.dealer().getUsersList().reqSpec(defaultSpec())
                .xSessionIdHeader(sessionId)
