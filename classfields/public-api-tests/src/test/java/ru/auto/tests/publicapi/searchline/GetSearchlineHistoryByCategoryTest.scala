...
@DisplayName("GET /searchline/history/{category}")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetSearchlineHistoryByCategoryTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.searchline.getHistoryByCategory
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val request = (apiClient: ApiClient) => apiClient.searchline.getHistoryByCategory
      .reqSpec(defaultSpec)
      .categoryPath(CARS)
