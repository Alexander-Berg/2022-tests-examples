...
import ru.auto.tests.publicapi.model.{AutoApiErrorResponse, VertisBankerPaymentMethodPatch}
...
@DisplayName("PUT /billing/{salesmanDomain}/tied-cards")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class ChangeUserTiedCardInfoTest {
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.billing.updateCardBilling().reqSpec(defaultSpec)
      .salesmanDomainPath(DOMAIN)
      .paymentSystemIdQuery(PAYMENT_SYSTEM_ID)
      .cardIdQuery(CARD_ID)
      .body(new VertisBankerPaymentMethodPatch)
  ...
  def shouldChangePreferredSettingToTrue(): Unit = {
    ...
    api.billing.updateCardBilling().reqSpec(defaultSpec)
      .salesmanDomainPath(DOMAIN)
      .paymentSystemIdQuery(PAYMENT_SYSTEM_ID)
      .cardIdQuery(CARD_ID)
      .body(new VertisBankerPaymentMethodPatch().preferred(true))
      .xSessionIdHeader(sessionId)
      .execute(validatedWith(shouldBe200OkJSON))

    val currentUserInfo = adaptor.getUserInfo(sessionId)
    assertThat(currentUserInfo.getTiedCards.get(0)).hasPreferred(true)
  }
}
