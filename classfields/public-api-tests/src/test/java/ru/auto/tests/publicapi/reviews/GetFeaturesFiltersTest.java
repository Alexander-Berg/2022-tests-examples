...
public class GetFeaturesFiltersTest {
    ...
    public static List<Object[]> getParameters() {
        return Arrays.asList(featureSearchFilters());
    }

    private static Object[][] featureSearchFilters() {
        return new Object[][]{
                {CARS, "VAZ", "2107", "2307270"},
                {MOTO, "YAMAHA", "YZF_R1", ""},
                {TRUCKS, "FIAT_PROFESSIONAL", "DUCATO", ""}
        };
    }
    ...
    public void shouldGetFeaturesHasNoDiffWithProduction() {
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.reviews().features().subjectPath(AUTO)
                .categoryPath(category).markQuery(mark).modelQuery(model).superGenQuery(generation)
