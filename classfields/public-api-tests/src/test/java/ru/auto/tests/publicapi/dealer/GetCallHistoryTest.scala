...
@DisplayName("GET /dealer/call-history")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetCallHistoryTest {

  private val DATE_FROM = "2018-01-01T00:00:00.000+03:00"
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.getDealerCallHistory.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.getDealerCallHistory.reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.dealer.getDealerCallHistory
      .reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
