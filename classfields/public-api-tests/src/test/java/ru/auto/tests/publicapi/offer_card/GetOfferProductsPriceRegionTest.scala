...
@DisplayName("GET /offer/{category}/{offerID}/products-price/region")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class GetOfferProductsPriceRegionTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.offerCard.productsPricesInRegion
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val request = (apiClient: ApiClient) => apiClient.offerCard.productsPricesInRegion
      .reqSpec(defaultSpec)
      .categoryPath("cars")
      .offerIDPath(offerId)
      .latitudeQuery("55.942398")
      .longitudeQuery("37.31966")
