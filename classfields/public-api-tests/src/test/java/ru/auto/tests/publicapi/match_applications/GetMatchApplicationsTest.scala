...
class GetMatchApplicationsTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.matchApplications.getMatchApplications.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.matchApplications.getMatchApplications.reqSpec(defaultSpec)
      .fromQuery(FROM_DATE)
      .toQuery(TO_DATE)
      .pageNumQuery(PAGE_NUMBER)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val request = (apiClient: ApiClient) => apiClient.matchApplications.getMatchApplications
      .reqSpec(defaultSpec)
      .fromQuery(FROM_DATE)
      .toQuery(TO_DATE)
      .pageNumQuery(PAGE_NUMBER)
