...
@DisplayName("GET /dealer/walk-in/stats")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class GetWalkInStatsTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.getWalkInStats.execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.getWalkInStats.reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
      .toQuery(DATE_TO)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    ...
    val req = (apiClient: ApiClient) => apiClient.dealer.getWalkInStats
      .reqSpec(defaultSpec)
      .fromQuery(DATE_FROM)
      .toQuery(DATE_TO)
