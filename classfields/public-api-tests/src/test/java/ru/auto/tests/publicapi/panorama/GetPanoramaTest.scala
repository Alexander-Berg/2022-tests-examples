...
class GetPanoramaTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.exteriorPanorama.getPanoramaExt
  ...
  def shouldSee404WithInvalidPanoramaId(): Unit = {
    ...
    val response = api.exteriorPanorama.getPanoramaExt.reqSpec(defaultSpec)
  ...
  def shouldHasNoDiffWithProduction(): Unit = {
    val req = (apiClient: ApiClient) => apiClient.exteriorPanorama.getPanoramaExt
      .reqSpec(defaultSpec)
