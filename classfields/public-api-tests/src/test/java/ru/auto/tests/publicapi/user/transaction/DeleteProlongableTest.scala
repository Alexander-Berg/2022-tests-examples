...
@DisplayName("DELETE /user/transaction/{transactionId}/prolongable")
@RunWith(classOf[GuiceTestRunner])
@GuiceModules(Array(classOf[PublicApiModule]))
class DeleteProlongableTest {
  ...
  @Test
  @Owner(TIMONDL)
  def shouldSee200DeleteProlongable(): Unit = {
    ...
    api.userTransaction.deleteProlongableTransaction().reqSpec(defaultSpec)
      .transactionIdPath(transactionId)
      .domainQuery(DOMAIN)
      .xSessionIdHeader(sessionId)
      .execute(validatedWith(shouldBe200OkJSON))
