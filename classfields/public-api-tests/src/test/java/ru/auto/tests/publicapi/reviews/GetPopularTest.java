...
@DisplayName("GET /reviews/{subject}/presets/{category}/popular")
...
public class GetPopularTest {
    ...
    public void shouldSee403WhenNoAuth() {
        api.reviews().popular().subjectPath(AUTO).categoryPath(CARS).execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    public void shouldSee400WithInvalidSubject() {
        String invalidSubject = Utils.getRandomString();
        api.reviews().popular().subjectPath(invalidSubject).categoryPath(CARS).reqSpec(defaultSpec())
    ...
    public void shouldSee400WithInvalidCategory() {
        ...
        api.reviews().popular().subjectPath(AUTO).categoryPath(invalidCategory).reqSpec(defaultSpec())
                .execute(validatedWith(shouldBeCode(SC_BAD_REQUEST)));
    }
}
