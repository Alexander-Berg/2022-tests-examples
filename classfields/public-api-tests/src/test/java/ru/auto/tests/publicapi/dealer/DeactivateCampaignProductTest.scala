...
import ru.auto.tests.publicapi.model.AutoApiBillingCampaignHeader.InactiveReasonEnum.MANUALLY_DISABLED
...
@DisplayName("DELETE /dealer/campaign/product/{product}")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class DeactivateCampaignProductTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.dealer.deactivateDealerProductCampaign()
  ...
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.dealer.deactivateDealerProductCampaign().reqSpec(defaultSpec)
      .productPath(PRODUCT)
  ...
  def shouldSeeSuccess(): Unit = {
    ...
    assertThat(campaignProduct.getCampaign).hasIsEnabled(false).hasIsActive(false)
      .hasInactiveReason(MANUALLY_DISABLED)
  }
}
