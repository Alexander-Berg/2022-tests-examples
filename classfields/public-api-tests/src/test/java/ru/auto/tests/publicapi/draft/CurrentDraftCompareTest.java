...
public class CurrentDraftCompareTest {
    ...
    public void shouldCreateDraft() {
        ...
        JsonObject response = api.draft().currentDraft().categoryPath(category.name()).xSessionIdHeader(sessionId)
                .reqSpec(defaultSpec()).reqSpec(r -> {
                    r.setContentType(JSON);
                    r.setBody(body);
                })
                .execute(validatedWith(shouldBeSuccess())).as(JsonObject.class, GSON);

        JsonObject responseProd = prodApi.draft().currentDraft().categoryPath(category.name()).xSessionIdHeader(sessionId)
                .reqSpec(defaultSpec()).reqSpec(r -> {
                    r.setContentType(JSON);
                    r.setBody(body);
                })
        ...
        MatcherAssert.assertThat(response, jsonEquals(responseProd).whenIgnoringPaths(
                ...
                "offer.additional_info.update_date", "offer.created", "offer.id", "offer.state.upload_url",
                "offer.state.sts_upload_url", "offer.state.document_photo_upload_urls"));
    }
}
