...
@DisplayName("PUT /calltracking/settings/apply_for_all_calls")
@GuiceModules(Array(classOf[PublicApiModule]))
@RunWith(classOf[GuiceTestRunner])
class UpdateCalltrackingSettingForAllCallsTest {
  ...
  def shouldSee403WhenNoAuth(): Unit = {
    api.calltracking.applySettingsForAllCalls().execute(validatedWith(shouldBeCode(SC_FORBIDDEN)))
  }

  @Test
  @Owner(TIMONDL)
  def shouldSee401WithoutSessionId(): Unit = {
    val response = api.calltracking.applySettingsForAllCalls().reqSpec(defaultSpec)
  ...
  def shouldSuccessUpdateSettings(): Unit = {
    ...
    api.calltracking.applySettingsForAllCalls().reqSpec(defaultSpec)
