...
import type { OwnProps } from './ExternalLinkWarningWidget';
import ExternalLinkWarningWidget from './ExternalLinkWarningWidget';
...
describe('ExternalLinkWarningWidget', () => {
    ...
    it('при клике отправит метрику close, запишет новые данные в ls и не будет отправлять никаких экшенов', () => {
        const { store, wrapper } = shallowRenderComponent({ props, initial_state });
    ...
    it('при клике отправит экшен, если это последнее соообщение в чате', () => {
        const { store, wrapper } = shallowRenderComponent({ props: { ...props, is_last_message: true }, initial_state });

        wrapper.find('span').simulate('click');

        expect(store.getActions()).toEqual([ { payload: '333_111', type: 44 } ]);
    });
});

function shallowRenderComponent({ initial_state, props }: { props: OwnProps & AbstractOwnProps; initial_state: Partial<ModelState> }) {
