...
import GarageCardTaxDesktop from './GarageCardTaxDesktop';
...
it('должен открыть регионовыбиралку', async() => {
    ...
    // 1. открываем попап
    const link = queryByRole('button', { name: /регион регистрации/i });
    link && userEvent.click(link);

    // 2. смотрим на попап
    expect(document.body.querySelector('.GarageCardTax__popup')).toBeInTheDocument();
});

it('если статус равен CAN_BE_CLARIFIED, то рендерит компонент', async() => {
    await renderComponent();

    const element = document.querySelector('[class^=GarageCardTaxDesktop]');
