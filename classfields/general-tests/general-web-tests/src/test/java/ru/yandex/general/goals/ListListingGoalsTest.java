...
public class ListListingGoalsTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Цель «LISTING_OFFERS_SHOW» при открытии листинга категории, листинг списком")
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("1 цель «LISTING_OFFERS_FILTER» при выборе двух фильтров цены в попапе «Все фильтры», листинг списком")
    public void shouldSeeOneGoalPriceFromAllFiltersListListing() {
    ...
    public void shouldSeeSortGoalListListing() {
        ...
        basePageSteps.onListingPage().popup().radioButtonWithLabel("Сначала дешевле")
    ...
    public void shouldSeeBreadcrumbsCategoryClickGoalFromListListing() {
        ...
        basePageSteps.onListingPage().breadcrumbsItem("Компьютеры").click();
