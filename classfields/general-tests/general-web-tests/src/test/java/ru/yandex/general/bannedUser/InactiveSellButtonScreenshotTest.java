...
@DisplayName("Скриншот задизейбленой кнопки и попапа «Разместить»")
@RunWith(Parameterized.class)
@GuiceModules(GeneralWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class InactiveSellButtonScreenshotTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Скриншот задизейбленой кнопки «Разместить», светлая/тёмная темы")
    public void shouldSeeInactiveSellButtonScreenshot() {
        Screenshot testing = compareSteps.takeScreenshot(basePageSteps.onListingPage().createOffer());

        urlSteps.setProductionHost().open();
        Screenshot production = compareSteps.takeScreenshot(basePageSteps.onListingPage().createOffer());
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Скриншот тултипа по задизейбленой кнопки «Разместить», светлая/тёмная темы")
    public void shouldSeeInactiveSellButtonPopupHoverScreenshot() {
        basePageSteps.onListingPage().createOffer().hover();
