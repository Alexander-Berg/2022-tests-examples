...
public class NavigationFromOfferCardToLkUserInfoPopupTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с карточки через ссылки из попапа «Юзер инфо» на странички ЛК")
    public void shouldSeeCardToLkPagesFromUserInfoPopup() {
        basePageSteps.onOfferCardPage().header().avatar().click();
        basePageSteps.onOfferCardPage().userInfoPopup().link(linkName).click();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с карточки через ссылки из попапа «Юзер инфо» из прилипшего хэдера на странички ЛК")
    public void shouldSeeCardToLkPagesFromUserInfoPopupFloated() {
