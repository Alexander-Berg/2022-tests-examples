...
@DisplayName("Переход с «Мои объявления» на другие страницы ЛК через попапа «Юзер инфо»")
@RunWith(Parameterized.class)
@GuiceModules(GeneralWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class NavigationMyOffersUserInfoPopupTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с «Мои объявления» через ссылки из попапа «Юзер инфо» на странички ЛК")
    public void shouldSeeMyOffersToLkPagesFromUserInfoPopup() {
        basePageSteps.onMyOffersPage().header().avatar().click();
        basePageSteps.onMyOffersPage().userInfoPopup().link(linkName).click();
