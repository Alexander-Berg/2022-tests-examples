...
public class ContactsAddressesAddTest {
    ...
    public void shouldSeeAddAddress() {
        basePageSteps.onContactsPage().addressesList().get(0).sendKeys(ADDRESS);
        basePageSteps.onContactsPage().suggestItem(ADDRESS).waitUntil(isDisplayed()).click();
    ...
    public void shouldSeeAddDistrict() {
        basePageSteps.onContactsPage().addressesList().get(0).sendKeys(DISTRICT);
        basePageSteps.onContactsPage().suggestItem(DISTRICT).waitUntil(isDisplayed()).click();
        basePageSteps.onContactsPage().popupNotification(CHANGES_SAVED).waitUntil(isDisplayed());
        basePageSteps.refresh();
        basePageSteps.wait500MS();
    ...
    public void shouldSeeAddSubway() {
        basePageSteps.onContactsPage().addressesList().get(0).sendKeys(METRO);
        basePageSteps.onContactsPage().suggestItem(METRO).waitUntil(isDisplayed()).click();
        basePageSteps.onContactsPage().popupNotification(CHANGES_SAVED).waitUntil(isDisplayed());
        basePageSteps.refresh();
        basePageSteps.wait500MS();
    ...
    public void shouldSeeAddAddressSubway() {
        ...
        basePageSteps.onContactsPage().spanLink(ADD_ADDRESS).click();
        basePageSteps.onContactsPage().addressesList().get(1).sendKeys(DISTRICT);
        basePageSteps.onContactsPage().suggestItem(DISTRICT).waitUntil(isDisplayed()).click();
        basePageSteps.onContactsPage().popupNotification(CHANGES_SAVED).waitUntil(isDisplayed());

        basePageSteps.onContactsPage().spanLink(ADD_ADDRESS).click();
        basePageSteps.onContactsPage().addressesList().get(2).sendKeys(METRO);
        basePageSteps.onContactsPage().suggestItem(METRO).waitUntil(isDisplayed()).click();
        basePageSteps.onContactsPage().popupNotification(CHANGES_SAVED).waitUntil(isDisplayed());
    ...
    public void shouldSeeChangeAddress() {
        ...
        basePageSteps.onContactsPage().addressesList().get(0).sendKeys(ADDRESS_2);
        basePageSteps.onContactsPage().suggestItem(ADDRESS_2).waitUntil(isDisplayed()).click();
        basePageSteps.onContactsPage().popupNotification(CHANGES_SAVED).waitUntil(isDisplayed());
        basePageSteps.refresh();
        basePageSteps.wait500MS();
