...
public class SellerProfileUserScoreTest {
        ...
        mockRule.graphqlStub(mockResponse()
                .setPublicProfile(
                        profileResponse().snippets(asList(
                                ...
                                .setSellerName("Тестовый Продавец").build())
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Разное кол-во полосок активности в юзер-скоре на странице продавца")
    public void shouldSeeProfileSidebarBadgeScore() {
        basePageSteps.onProfilePage().sidebar().activeScoreIndicatorList().should(hasSize(activeIndicatorsCount));
    }
