...
public class SeoAggregateOfferShemaOrgListingMarkingTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Schema.org разметка AggregateOffer на листинге без офферов")
    public void shouldSeeShemaOrgAggregateOfferOnListingWithoutOffers() {
        mockRule.graphqlStub(mockResponse.setSearch(
        ...
        SoftAssertions.assertSoftly(s -> {
            s.assertThat(jSoupSteps.getItempropContent("name")).as("Название категории").isEqualTo("Электроника");
            s.assertThat(jSoupSteps.select("div[itemprop='offers']").attr("itemtype")).as("itemtype").isEqualTo("http://schema.org/AggregateOffer");
            s.assertThat(jSoupSteps.getItempropContent("lowPrice")).as("Минимальная цена").isEqualTo(String.valueOf(0));
            s.assertThat(jSoupSteps.getItempropContent("highPrice")).as("Максимальная цена").isEqualTo(String.valueOf(0));
            s.assertThat(jSoupSteps.getItempropContent("offerCount")).as("Кол-во офферов").isEqualTo(String.valueOf(0));
            s.assertThat(jSoupSteps.getItempropContent("priceCurrency")).as("Тип валюты").isEqualTo("RUB");
        });
    }
