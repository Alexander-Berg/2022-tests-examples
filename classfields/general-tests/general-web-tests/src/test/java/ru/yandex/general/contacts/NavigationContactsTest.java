...
@DisplayName("Навигация с раздела «Настройки профиля» в ЛК")
@RunWith(GuiceTestRunner.class)
@GuiceModules(GeneralWebModule.class)
public class NavigationContactsTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с «Настройки профиля» в «Избранное» по клику в хедере")
    public void shouldSeeContactsToFavoritesFromHeader() {
        basePageSteps.onContactsPage().header().linkWithTitle(FAVORITES_TITLE).click();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с «Настройки профиля» в «Избранное» по клику в прилипшем хедере")
    public void shouldSeeContactsToFavoritesFromFloatedHeader() {
        basePageSteps.scrollDown(PXLS_TO_FLOAT_HEADER);
        basePageSteps.onContactsPage().floatedHeader().linkWithTitle(FAVORITES_TITLE).click();
