...
@DisplayName("Переход с «Избранное» на другие страницы ЛК через попапа «Юзер инфо»")
@RunWith(Parameterized.class)
@GuiceModules(GeneralWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class NavigationFavoritesUserInfoPopupTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с «Избранное» через ссылки из попапа «Юзер инфо» на странички ЛК")
    public void shouldSeeFavoritesToLkPagesFromUserInfoPopup() {
        basePageSteps.onFavoritesPage().header().avatar().click();
        basePageSteps.onFavoritesPage().userInfoPopup().link(linkName).click();
