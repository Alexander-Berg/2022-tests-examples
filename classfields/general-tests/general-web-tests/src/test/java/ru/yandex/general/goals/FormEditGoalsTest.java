...
@Epic(GOALS_FEATURE)
@DisplayName("Цели на форму редактирования")
...
public class FormEditGoalsTest {
    ...
    public void before() {
        ...
        offerAddSteps.onMyOffersPage().snippetFirst().spanLink("Редактировать").click();
    }
    ...
    @Owner(ALEKS_IVANOV)
    @Feature(FORM_OFFER_CATEGORY_MANUAL_SELECT_SHOW)
    @DisplayName("Цель «FORM_OFFER_CATEGORY_MANUAL_SELECT_SHOW» по ХК на форме редактирования")
    public void shouldSeeFormOfferCategoryManualSelectShowGoalFromBreadcrumbs() {
        offerAddSteps.onFormPage().breadcrumbs().link("Все категории").click();
