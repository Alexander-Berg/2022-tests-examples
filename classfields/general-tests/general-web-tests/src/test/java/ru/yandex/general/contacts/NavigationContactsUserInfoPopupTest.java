...
@DisplayName("Переход с «Настройки профиля» на другие страницы ЛК через попапа «Юзер инфо»")
@RunWith(Parameterized.class)
@GuiceModules(GeneralWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class NavigationContactsUserInfoPopupTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с «Настройки профиля» через ссылки из попапа «Юзер инфо» на странички ЛК")
    public void shouldSeeContactsToLkPagesFromUserInfoPopup() {
        basePageSteps.onContactsPage().header().avatar().click();
        basePageSteps.onContactsPage().userInfoPopup().link(linkName).click();
