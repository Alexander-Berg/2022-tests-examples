...
public class OfferCardTotalStatisticsTest {
    ...
    public static Collection<Object[]> getData() {
        return asList(new Object[][]{
                {"Общая + дневная статистика", 20, 12, 44, 51, 66, 88},
                {"Общая + пустая дневная статистика", 126, 99, 43, 0, 0, 0},
    ...
    public void shouldSeeOfferCardTotalStatistics() {
        basePageSteps.onOfferCardPage().sidebar().statistics().viewsCount().should(
                hasText(formatStatistic(totalViews, todayViews)));
        basePageSteps.onOfferCardPage().sidebar().statistics().contactsCount().should(
                hasText(formatStatistic(totalContacts, todayContacts)));
        basePageSteps.onOfferCardPage().sidebar().statistics().favoritesAddedCount().should(
