...
@DisplayName("Переход с «Статистика» на другие страницы ЛК через сайдбар")
@RunWith(Parameterized.class)
@GuiceModules(GeneralWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class NavigationStatisticsSidebarTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Переход с «Статистика» на другие страницы ЛК через сайдбар")
    public void shouldSeeStatisticsToOtherLkPagesFromSidebar() {
        basePageSteps.onFavoritesPage().lkSidebar().link(linkName).click();
