...
import static ru.yandex.general.page.OfferCardPage.SHOW_FULL;
...
public class OfferCardInactiveBuyerTest {
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Отображение сообщения неактивного оффера, карточка минимизирована, с разными причинами неактивности")
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Карточка минимизирована, нет описания и атрибутов, с разными причинами неактивности")
    public void shouldSeeInactiveOfferMinimized() {
        urlSteps.testing().path(CARD).path(ID).open();

        basePageSteps.onOfferCardPage().minimizedCard().should(isDisplayed());
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Разворачивание неактивного оффера для покупателя")
    public void shouldSeeInactiveOfferMaximize() {
        urlSteps.testing().path(CARD).path(ID).open();
        basePageSteps.onOfferCardPage().button(SHOW_FULL).click();

        basePageSteps.onOfferCardPage().minimizedCard().should(not(isDisplayed()));
        basePageSteps.onOfferCardPage().button(SHOW_FULL).should(not(isDisplayed()));
