...
from cm_avito_converter.models.event import (
    HttpEvent,
    ObjectEvent,
    ObjectEventDetails,
    ObjectEventMessage,
...
def build_ymq_event_message(body: str) -> YMQEventMessage:
    return YMQEventMessage(
        event_metadata=YMQEventMetadata(
            event_id=random_lower_string(), event_type=random_lower_string(), created_at=datetime.now()
        ),
        details=YMQEventDetails(queue_id=random_lower_string(), message=YMQMessage(body=body)),
    )


def build_ymq_event(messages: List[YMQEventMessage]) -> YMQEvent:
    return YMQEvent(messages=messages)
