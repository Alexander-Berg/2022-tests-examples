import json
from datetime import datetime
from app.schemas.enums import Transmission, EngineType, GearType, Color, Source, SteeringWheel
from app.converters.import_converters.auto.converter import ImportConverter
from app.parsers.parsed_value import Expected, NonParsed, NoValue, Unexpected
from app.parsers.s3.auto.haraba.cars.avito.parser import HarabaAvitoCarsParser

from tests.parsers.base import TestParser


URL = "https://www.avito.ru/1065517972"


class TestHarabaCarsAvito(TestParser):
    def setUp(self):
        test_json = json.load(open("tests/resources/test_haraba_avito.json"))
        self.parser2 = HarabaAvitoCarsParser(URL, test_json)
        self.ici = ImportConverter(HarabaAvitoCarsParser(URL, test_json), Source.HARABA_FRESH, None)

    async def test_all_parsing(self):
        await self.ici.convert()

    async def test_parse_raw_body_type(self):
        parser = HarabaAvitoCarsParser(URL, {"car_body": "Хетчбэк"})
        self.assert_parsed_value(Expected("Хетчбэк"), parser.parse_raw_body_type())

    async def test_parse_horse_power(self):
        parser = HarabaAvitoCarsParser(URL, {"car_engine_power": "81"})
        self.assert_parsed_value(Expected(81), parser.parse_horse_power())

    async def test_parse_transmission(self):
        parser = HarabaAvitoCarsParser(URL, {"car_transmission": "механика"})
        self.assert_parsed_value(Expected(Transmission.MECHANICAL), parser.parse_transmission())

    async def test_parse_displacement(self):
        parser = HarabaAvitoCarsParser(URL, {"car_engine_volume": "1,6"})
        self.assert_parsed_value(Expected(1600), parser.parse_displacement())

    async def test_parse_engine_type(self):
        parser = HarabaAvitoCarsParser(URL, {"car_engine_gas": "Бензин"})
        self.assert_parsed_value(Expected(EngineType.GASOLINE), parser.parse_engine_type())

    async def test_parse_gear_type(self):
        parser = HarabaAvitoCarsParser(URL, {"car_drive": "передний"})
        self.assert_parsed_value(Expected(GearType.FRONT), parser.parse_gear_type())

    async def test_is_4_wd(self):
        parser = HarabaAvitoCarsParser(URL, {"car_drive": "передний"})
        self.assert_parsed_value(Expected(False), parser.is_4_wd())

    async def test_parse_fn(self):
        parser = HarabaAvitoCarsParser(URL, {"car_name": "ВАЗ (LADA) 2114 Samara"})
        self.assert_parsed_value(Expected("ВАЗ (LADA) 2114 Samara"), parser.parse_fn())

    async def test_parse_mark(self):
        parser = HarabaAvitoCarsParser(URL, {"car_name": "ВАЗ (LADA) 2114 Samara"})
        self.assert_parsed_value(NonParsed(), parser.parse_mark())

    async def test_parse_model(self):
        parser = HarabaAvitoCarsParser(URL, {"car_name": "ВАЗ (LADA) 2114 Samara"})
        self.assert_parsed_value(NonParsed(), parser.parse_model())

    async def test_parse_color(self):
        parser = HarabaAvitoCarsParser(URL, {"car_color": "Красный"})
        self.assert_parsed_value(Expected(Color.RED), parser.parse_color())

    async def test_parse_steering_wheel(self):
        parser = HarabaAvitoCarsParser(URL, {"car_leftrighthand": "Левый"})
        self.assert_parsed_value(Expected(SteeringWheel.LEFT), parser.parse_steering_wheel())

    async def test_parse_address(self):
        parser = HarabaAvitoCarsParser(URL, {"listing_city": "Лениногорск"})
        addresses = parser.parse_address()
        self.assertEqual(1, len(addresses))
        self.assert_parsed_value(Expected("Лениногорск"), addresses[0])

    async def test_parse_price(self):
        parser = HarabaAvitoCarsParser(URL, {"car_price": 105000})
        self.assert_parsed_value(Expected(105000), parser.parse_price())

    async def test_parse_owner_name(self):
        parser = HarabaAvitoCarsParser(URL, {"dealership_name": "test_name"})
        self.assert_parsed_value(Expected("test_name"), parser.parse_owner_name())

    async def test_parse_owners_count(self):
        parser = HarabaAvitoCarsParser(URL, {"car_owners_number": 4})
        self.assert_parsed_value(Expected(4), parser.parse_owners_count())
        parser = HarabaAvitoCarsParser(URL, {"car_owners_number": "4"})
        self.assert_parsed_value(Unexpected("4"), parser.parse_owners_count())

    async def test_parse_phones(self):
        parser = HarabaAvitoCarsParser(URL, {"listing_phone": "+7(986)889-3033, +7(986)889-3022"})
        self.assert_parsed_phones(["79868893033", "79868893022"], parser.parse_phones())

    async def test_parse_description(self):
        parser = HarabaAvitoCarsParser(URL, {"car_extrainfo": "В отличном состоянии!"})
        self.assert_parsed_value(Expected("В отличном состоянии!"), parser.parse_description())

    async def test_parse_vin(self):
        parser = HarabaAvitoCarsParser(URL, {"car_vin": None})
        self.assert_parsed_value(NoValue(), parser.parse_vin())

    async def test_parse_year(self):
        parser = HarabaAvitoCarsParser(URL, {"car_year": 2008})
        self.assert_parsed_value(Expected(2008), parser.parse_year())

    async def test_parse_mileage(self):
        parser = HarabaAvitoCarsParser(URL, {"car_mileage": 220000})
        self.assert_parsed_value(Expected(220000), parser.parse_mileage())
        parser = HarabaAvitoCarsParser(URL, {"car_mileage": "220000"})
        self.assert_parsed_value(Unexpected("220000"), parser.parse_mileage())

    async def test_parse_photos(self):
        parser = HarabaAvitoCarsParser(
            URL,
            {
                "images": [
                    "https://67.img.avito.st/640x480/10148199967.jpg",
                    "https://66.img.avito.st/640x480/10148199966.jpg",
                ]
            },
        )
        self.assert_parsed_value(
            Expected(
                [
                    "https://67.img.avito.st/640x480/10148199967.jpg",
                    "https://66.img.avito.st/640x480/10148199966.jpg",
                ]
            ),
            parser.parse_photos(),
        )

    async def test_parse_is_dealer(self):
        parser = HarabaAvitoCarsParser(URL, {"seller_type": "Частное лицо"})
        self.assert_parsed_value(Expected(False), parser.parse_is_dealer())
        parser = HarabaAvitoCarsParser(URL, {"seller_type": "Автодилер"})
        self.assert_parsed_value(Expected(True), parser.parse_is_dealer())

    async def test_parse_displayed_publish_date(self):
        parser = HarabaAvitoCarsParser(URL, {"listing_date": "2020-12-21T23:59:21"})
        self.assert_parsed_value(Expected(datetime(2020, 12, 21, 23, 59, 21)), parser.parse_displayed_publish_date())

    async def test_parse_views(self):
        parser = HarabaAvitoCarsParser(URL, {})
        views = parser.parse_views()
        self.assert_parsed_value(NonParsed(), views["total"])
        self.assert_parsed_value(NonParsed(), views["daily"])

    async def test_parse_dealer_name(self):
        parser = HarabaAvitoCarsParser(URL, {"dealership_name": "test_name"})
        self.assert_parsed_value(Expected("test_name"), parser.parse_dealer_name())
        parser = HarabaAvitoCarsParser(URL, {})
        self.assert_parsed_value(NoValue(), parser.parse_dealer_name())

    async def test_parser_seller_name(self):
        parser = HarabaAvitoCarsParser(URL, {"seller_name": "test_name"})
        self.assert_parsed_value(NonParsed(), parser.parse_seller_name())

    async def test_parser_seller_url(self):
        parser = HarabaAvitoCarsParser(URL, {"seller_url": URL})
        self.assert_parsed_value(NonParsed(), parser.parse_seller_url())

    async def test_parse_is_phone_protected(self):
        parser = HarabaAvitoCarsParser(URL, {"listing_phone_protected": True})
        self.assert_parsed_value(Expected(True), parser.parse_is_phone_protected())
        parser = HarabaAvitoCarsParser(URL, {"listing_phone_protected": False})
        self.assert_parsed_value(Expected(False), parser.parse_is_phone_protected())
        parser = HarabaAvitoCarsParser(URL, {"listing_phone_protected": "true"})
        self.assert_parsed_value(Unexpected("true"), parser.parse_is_phone_protected())
        parser = HarabaAvitoCarsParser(URL, {"listing_phone_protected": "false"})
        self.assert_parsed_value(Unexpected("false"), parser.parse_is_phone_protected())
