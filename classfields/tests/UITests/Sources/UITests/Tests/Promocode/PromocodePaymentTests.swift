...
final class PromocodePaymentTests: BaseTest {
    ...
    func test_promocodeTransportPayments() {
    ...
    func test_promocodeReportsPayments() {
        launch(options: .init(overrideAppSettings: ["showVASPaymentListExpandedByDefault": "true"]))
        let steps: PaymentOptionsSteps<CarReportsListSteps> = mainSteps
            .openCarReportsList()
            .tapOnBuyReports()
        ...
        let promocodeActivationExpectation = expectationForRequest { req -> Bool in
            if(req.method == "POST" && req.uri.lowercased() == "/promocode/promocode") {
                if let data = req.messageBody,
                   let request = try? Auto_Api_PromocodeActivationRequest(jsonUTF8Data: data) {
                    return request.validation.products == ["offers-history-reports-10"]
                }
            }
            return false
        }
