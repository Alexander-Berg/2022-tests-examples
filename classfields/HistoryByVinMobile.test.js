...
const HistoryByVinMobile = require('./HistoryByVinMobile');
...
it('не должен выполнять поиск по vin, когда показываем отчёт по этому же vin', () => {
    ...
    const wrapper = shallow(
        <HistoryByVinMobile params={{ vin_or_license_plate: 'A111AA11' }}/>, { context: { ...contextMock, store } },
    ).dive();

    wrapper.find('Connect(VinCheckSnippetMobile)').simulate('inputSubmit', 'A111AA11');
...
describe('метрики', () => {
    it('отправляем метрику ошибки', () => {
        const store = mockStore(state);
        const wrapper = shallow(
            <HistoryByVinMobile/>, { context: { ...context, store } },
        ).dive();
    ...
    it('отправляет метрику поиска', () => {
        ...
        wrapper.setProps(props);
        wrapper.find('Connect(VinCheckSnippetMobile)').simulate('inputSubmit', 'B111BB11');
