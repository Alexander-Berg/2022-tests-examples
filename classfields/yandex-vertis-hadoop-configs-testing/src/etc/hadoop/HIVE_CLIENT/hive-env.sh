      ...
      if [ "$SERVICE" = "cli" ]; then
      if [ -z "$DEBUG" ]; then
      export HADOOP_OPTS="$HADOOP_OPTS -XX:NewRatio=12 -XX:MaxHeapFreeRatio=40 -XX:MinHeapFreeRatio=15 -XX:+UseParallelGC -XX:-UseGCOverheadLimit"
      else
      ...
      export HADOOP_HEAPSIZE=12082 # Setting for HiveMetastore
      else
