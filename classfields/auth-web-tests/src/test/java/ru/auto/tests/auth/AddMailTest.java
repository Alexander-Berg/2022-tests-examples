...
public class AddMailTest {
    ...
    @Before
    public void before() {
        mockRule.newMock().with("auth/Session",
                "auth/User",
                "auth/UserConfirm",
                "auth/UserEmailChange",
                "auth/UserEmailRequestChangeCode",
                "desktop/SearchCarsBreadcrumbsEmpty").post();

        urlSteps.subdomain(SUBDOMAIN_AUTH).path(EMAIL).path(CHANGE)
    ...
    public void shouldAddMail() {
        basePageSteps.onAuthPage().input("Новый адрес почты", "79271056173@mail.ru");
        basePageSteps.onAuthPage().button(CONTINUE_BUTTON).click();
        basePageSteps.onAuthPage().input(SMS_CODE, "6737");
        basePageSteps.onAuthPage().input(MAIL_CODE, "614993");

        urlSteps.testing().path(SLASH).shouldNotSeeDiff();
        basePageSteps.onMainPage().header().avatar().hover();
        basePageSteps.onMainPage().header().authDropdown().userId().waitUntil(hasText("id 46950924"));
    }
}
