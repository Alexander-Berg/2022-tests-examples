...
@Log4j
public class MoishaApiConfigModule extends AbstractModule {

    @Provides
    @Singleton
    public MoishaApiConfig provideCabinetApiConfig() {
        MoishaApiConfig config = ConfigFactory.create(MoishaApiConfig.class,
                (Properties) System.getProperties().clone(), System.getenv());

        Properties properties = new Properties();
        config.fill(properties);
        Set<String> propertiesNames = Arrays.stream(MoishaApiConfig.class.getDeclaredMethods())
