...
import ru.auto.tests.moisha.models.Transports.Cars
import ru.auto.tests.moisha.models._
...
@DisplayName("POST /service/{service}/price/ for reports and others")
...
class MoishaNonOfferProductsTest(
    product: Products.Value,
    clientRegionId: Int,
    transport: Transports.Value,
    category: Categories.Value) {
  import MoishaNonOfferProductsTest._
  ...
  @Test
  def shouldHaveIdenticalPricesForNonOfferProducts(): Unit = {
...
object MoishaNonOfferProductsTest {
  val mark = "BMW"
  val price = 100000000
  val creationTs = "2020-12-13T23:59:59.999+03:00"

  val products = List(VinHistory, GibddHistoryReport, CmExpertHistoryReport, ApplicationCreditSingle)
  ...
  def getParameters: util.Collection[Array[Object]] = {
    val data: Array[Array[Object]] = (for {
      ...
      region <- regions
      category <- categories
    } yield Array(product, region.asInstanceOf[Object], Cars, category)).toArray
