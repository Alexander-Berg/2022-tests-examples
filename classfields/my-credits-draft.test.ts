...
import controller from './my-credits-draft';

describe('контроллер my-credits-draft', () => {
    ...
    it('если нет анкеты в драфте, редиректит в my-credits', function() {
        publicApi
            .get('/1.0/session/')
            ...
            .then(
                ...
                (error) => {
                    expect(error).toMatchObject({
                        error: {
                            code: RedirectError.CODES.CREDITS_LK_DRAFT_TO_ROOT,
                        },
                    });
                },
            );
