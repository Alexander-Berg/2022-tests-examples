...
@DisplayName("GET /client/{client_id}")
@GuiceModules(CabinetApiModule.class)
@RunWith(GuiceTestRunner.class)
public class GetLastClientIdTest {
    ...
    @Test
    public void shouldGetInfoClientIdHasNoDiffWithProduction() {
        Function<ApiClient, JsonObject> request = apiClient -> apiClient.client().getClient()
                .clientIdPath(DEALER_ID).xAutoruOperatorUidHeader("11296277").reqSpec(defaultSpec())
    ...
    public void shouldGetStatusOkForManager() {
        api.client().getClient().clientIdPath(DEALER_ID).xAutoruOperatorUidHeader(MANAGER_ID)
                .reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_OK)));
    }

    @Test
    public void shouldGetStatusForbidden() {
        api.client().getClient().clientIdPath(DEALER_ID).xAutoruOperatorUidHeader("1129627")
                .reqSpec(defaultSpec()).execute(validatedWith(shouldBeCode(SC_FORBIDDEN)));
    }

    @Test
    public void shouldGetStatusNotFound() {
        api.client().getClient().clientIdPath("201").xAutoruOperatorUidHeader(MANAGER_ID)
