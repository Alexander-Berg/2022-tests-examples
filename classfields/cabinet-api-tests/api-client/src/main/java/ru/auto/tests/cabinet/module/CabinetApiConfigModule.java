...
@Log4j
public class CabinetApiConfigModule extends AbstractModule {

    @Provides
    @Singleton
    public CabinetApiConfig provideCabinetApiConfig() {
        CabinetApiConfig config = ConfigFactory.create(CabinetApiConfig.class,
                (Properties) System.getProperties().clone(), System.getenv());

        Properties properties = new Properties();
        config.fill(properties);
        Set<String> propertiesNames = Arrays.stream(CabinetApiConfig.class.getDeclaredMethods())
