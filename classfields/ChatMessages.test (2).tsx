...
import ChatMessages, { OwnProps } from './ChatMessages';
import { ModelChatType, ModelClientType, ModelPaymentModal, ModelState } from '../models';
    ...
    describe('не покажется', () => {
        ...
        it('в чате недвиги', () => {
            props.chat = chat_mock
                ...
                .withSubject({ seller_type: 'PRIVATE' })
                .withOrigin(ModelClientType.REALTY)
                .value();
            initialState.config = config_mock.withClient(ModelClientType.REALTY).value();
    ...
    describe('при успешной оплате ', () => {
        ...
        it('сделает редирект (десктоп)', () => {
            ...
            return get_vin_page_url_promise
                .then(() => {})
                .then(() => {
                    expect(ajax_request).toHaveBeenCalledTimes(2);
                    expect(ajax_request.mock.calls[1]).toMatchSnapshot();

                    expect(global.location.assign).toHaveBeenCalledTimes(1);
                    expect(global.location.assign).toHaveBeenCalledWith('url-to-vin-report-page');
                });
        });

        it('сделает редирект (мобилка)', () => {
            initialState.config = config_mock.withIsMobile(true).value();
            const { page } = shallowRenderComponent({ props, initialState });
            simulatePaymentStatusChange(page, 'OPENED');
...
function shallowRenderComponent({ props, initialState }: { props: OwnProps; initialState: Partial<ModelState> }) {
