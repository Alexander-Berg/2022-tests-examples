...
describe('в разметке присутствуют ссылки', () => {
    ...
    it('кнопка в шапке - на пост', () => {
        expect(wrapper.find(Actionable).at(1).prop('href')).toBe('/journal/post/kviz-ugadayte-skolko-stoit-arenda-kvartiry-na-foto/');
    });

    it('кнопка в футере - на пост', () => {
        expect(wrapper.find(Actionable).at(2).prop('href')).toBe('/journal/post/kviz-ugadayte-skolko-stoit-arenda-kvartiry-na-foto/');
    });
});
