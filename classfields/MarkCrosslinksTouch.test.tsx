...
import MarkCrosslinks from './MarkCrosslinksTouch';
...
it('должен показать 7 ссылок', () => {
    const tree = shallowComponent({});
    // 8 ссылка - сама кнопка показать всё
    expect(tree.find('.MarkCrosslinksTouch__link')).toHaveLength(8);
    expect(tree.find('.MarkCrosslinksTouch__link_hidden')).toHaveLength(79);
});
...
it('если нет тачек в продаже не рендерим провязку', () => {
    ...
    const tree = shallowComponent({ state: newState });
    expect(tree).toEqual({});
});
...
it('должен правильно отработать клик по показать все и скрыть', () => {
    ...
    expect(tree.find('.MarkCrosslinksTouch__link')).toHaveLength(87);
    expect(tree.find('.MarkCrosslinksTouch__link_hidden')).toHaveLength(0);
    tree.find('Link').last().simulate('click');
    expect(tree.find('.MarkCrosslinksTouch__link')).toHaveLength(8);
    expect(tree.find('.MarkCrosslinksTouch__link_hidden')).toHaveLength(79);
});
