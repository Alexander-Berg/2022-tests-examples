...
import ru.auto.ara.core.testdata.GEAR_TYPE_PARAMS
...
class GearTypeTest {
    ...
    private val GEAR_FIELD_NAME = "Привод"
    private val GEAR_PARAM = "cars_params.gear_type"
    private val GEAR_PARENT_PARAM = "cars_params"
    private val FIRST_OPTION = GEAR_TYPE_PARAMS[0]
    private val SECOND_OPTION = GEAR_TYPE_PARAMS[1]
    ...
    fun setUp() {
        ...
        performFilter { clickFieldWithOverScroll(FIELD_NAME, GEAR_FIELD_NAME) }
    }
    ...
    fun shouldSeeEngineTypePickerControls() {
        checkFilter {
            ...
            isMultiselectNotCheckedOptionsDisplayed(GEAR_TYPE_PARAMS.map { it.first() })
            isBottomSheetTitleDisplayed(GEAR_FIELD_NAME)
        }
    }
    ...
    fun shouldClearValuesByClearButton() {
        performFilter {
            ...
            clickFieldWithOverScroll(FIELD_NAME, GEAR_FIELD_NAME)
            timeRule.setTime(time = "00:01")
            clickClearButton()
            clickAcceptButton()
        }.checkResult {
            isContainer(GEAR_FIELD_NAME, "")
            countWatcher.checkNotRequestBodyParameter(GEAR_PARENT_PARAM)
        }
    }
    ...
    fun shouldSelectMultiValues() {
        performFilter {
            ...
            isContainer(GEAR_FIELD_NAME, "${FIRST_OPTION[0]}, ${SECOND_OPTION[0]}")
            countWatcher.checkRequestBodyArrayParameter(GEAR_PARAM, setOf(FIRST_OPTION[1], SECOND_OPTION[1]))
        }
    }
    ...
    fun shouldApplyParamsWhenClosedBySwipe() {
        performFilter {
            ...
            isContainer(GEAR_FIELD_NAME, FIRST_OPTION[0])
            countWatcher.checkRequestBodyArrayParameter(GEAR_PARAM, setOf(FIRST_OPTION[1]))
        }
    }
