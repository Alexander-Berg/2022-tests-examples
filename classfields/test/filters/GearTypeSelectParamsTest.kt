...
class GearTypeSelectParamsTest(private val testParams: TestParameter) {
    ...
    fun setUp() {
        ...
        performFilter {
            clickFieldWithOverScroll(FIELD_NAME, GEAR_TYPE_FIELD_NAME)
    ...
    companion object {
        ...
        const val FIELD_NAME = "Пробег, км"
        const val GEAR_TYPE_FIELD_NAME = "Привод"
        const val GEAR_TYPE_PARAM = "cars_params.gear_type"

        @JvmStatic
        @Parameterized.Parameters(name = "index={index} {0}")
        fun data(): Collection<Array<out Any?>> = GEAR_TYPE.map { arrayOf(it) }

        private val GEAR_TYPE = GEAR_TYPE_PARAMS.map { (name, param) ->
            TestParameter(
                name = name,
                param = param,
                dispatchers = listOf(CountDispatcher("cars", countWatcher)),
                checkAtFilterScreenAndWatcher = {
                    isContainer(GEAR_TYPE_FIELD_NAME, name)
                    countWatcher.checkRequestBodyArrayParameter(GEAR_TYPE_PARAM, setOf(param))
                },
                checkPicker = {
        ...
        data class TestParameter(
            ...
            override fun toString() = "gear_type=$param"
        }
    }
}
