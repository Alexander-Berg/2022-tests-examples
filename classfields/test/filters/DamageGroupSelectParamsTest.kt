...
class DamageGroupSelectParamsTest(private val testParams: TestParameter) {
    ...
    fun setUp() {
        ...
        performFilter {
            clickFieldWithHintWithOverScroll(FIELD_NAME, DAMAGE_GROUP_FIELD_NAME)
    ...
    companion object {
        ...
        const val DAMAGE_GROUP_FIELD_NAME = "Состояние"
        const val DAMAGE_GROUP_PARAM = "damage_group"

        @JvmStatic
        @Parameterized.Parameters(name = "index={index} {0}")
        fun data(): Collection<Array<out Any?>> = DAMAGE_GROUP.map { arrayOf(it) }

        private val DAMAGE_GROUP = DAMAGE_GROUP_PARAMS.map { (name, param) ->
            TestParameter(
                name = name,
                param = param,
                dispatchers = listOf(CountDispatcher("cars", countWatcher)),
                checkAtFilterScreenAndWatcher = {
                    isInputContainer(DAMAGE_GROUP_FIELD_NAME, name)
                    countWatcher.checkRequestBodyParameter(DAMAGE_GROUP_PARAM, param)
                },
                checkPicker = {
                    DAMAGE_GROUP_PARAMS.map { (optionName) ->
                        if (optionName == name) {
        ...
        data class TestParameter(
            ...
            override fun toString() = "damage_group=$param"
        }
    }
}
