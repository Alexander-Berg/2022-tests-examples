...
class ExpiringWarningErrorPercentileReservoirSpec
    extends WordSpec
    with Matchers {
  ...
  "ExpiringWarningErrorPercentileReservoir" should {
    "check last error" in {
      ...
      val reservoir =
        new ExpiringWarningErrorPercentileReservoir(10, 50, 3.seconds, 100)
      ...
      // 30/100 errors => warning

      (0 until 70).foreach(_ => reservoir.ok())

      r.runHealthChecks().get(name) should beWarning

      // 5/100 errors => OK

      (0 until 25).foreach(_ => reservoir.ok())
