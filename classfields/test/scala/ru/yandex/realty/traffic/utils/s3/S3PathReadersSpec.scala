package ru.yandex.realty.traffic.utils.s3
...
class S3PathReadersSpec extends JUnitRunnableSpec {

  private lazy val SpecConfig = DefaultConfigProvider.provide(Seq("s3-path-readers-spec.conf"))
  ...
  private def readFile(configName: String) =
    Task.fromTry(S3PathConfigReaders.filePathFromConfig(SpecConfig.getConfig(configName)))

  private def readDir(configName: String) =
    Task.fromTry(S3PathConfigReaders.directoryPathFromConfig(SpecConfig.getConfig(configName)))

  private def readPath(configName: String) =
    Task.fromTry(S3PathConfigReaders.pathFromConfig(SpecConfig.getConfig(configName)))

  private def failOnConfigRead() =
    suite("should correctly fail on configs")(
      testM("when file reading and dir presented") {
        assertM(readDir("valid-file").run)(fails(anything))
      },
      testM("when dir reading and file presented") {
        assertM(readFile("valid-dir").run)(fails(anything))
      },
      testM("when missing dir property") {
        assertM(readDir("only-bucket-property").run)(fails(anything))
      },
      testM("when missing file property") {
        assertM(readFile("only-bucket-property").run)(fails(anything))
      },
      testM("when invalid file") {
        assertM(readFile("invalid-file").run)(fails(anything))
      },
      testM("when invalid dir") {
        assertM(readDir("invalid-dir").run)(fails(anything))
      }
    )
  ...
  private def correctlyReadFromConfig() = {
    ...
    def testRead[T](name: String, expected: T, reader: String => Task[T], labelsSuffix: String = "") =
      testM(s"correctly read `$name`$labelsSuffix") {
        reader(name).map(actual => assertTrue(actual == expected))
      }

    suite("correctly read from config")(
      testRead[S3FilePath]("valid-file", file, readFile),
      testRead[S3FilePath]("one-line-file", file, readFile),
      testRead[S3DirPath]("valid-dir", dir, readDir),
      testRead[S3DirPath]("one-line-dir", dir, readDir),
      testRead[S3Path]("valid-file", file, readPath, labelsSuffix = " when S3Path model requested"),
      testRead[S3Path]("one-line-file", file, readPath, labelsSuffix = " when S3Path model requested"),
      testRead[S3Path]("valid-dir", dir, readPath, labelsSuffix = " when S3Path model requested"),
      testRead[S3Path]("one-line-dir", dir, readPath, labelsSuffix = " when S3Path model requested")
    )
  }
}
