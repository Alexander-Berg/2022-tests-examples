...
import ru.yandex.realty.proto.offer.{DealStatus => ProtoDealStatus, TaxationForm => ProtoTaxationForm}
...
class RentOfferProtoConverterSpec extends SpecBase with Matchers {

  /*
  val converter = new RentOfferProtoConverter(StorageBuilder.buildCurrencyStorage())
  val convertSettings = ConvertSettings(Currency.RUR, AreaUnit.SQUARE_METER)
  val now = new Instant
  val TWO_DAYS: Duration = Duration.standardDays(2)

  "RentOfferProtoConverter" should {
    "Convert full fields with history " in {
      ...
      result.getDealStatus should be(ProtoDealStatus.DEAL_STATUS_DIRECT_RENT)
      result.getUtilitiesIncluded should be(false)
      result.getTaxationForm should be(ProtoTaxationForm.TAXATION_FORM_NDS)
      result.hasRentConditions should be(true)
    }
  ...
  def buildOffer(): Offer = {
    ...
    transaction.setDealStatus(DealStatus.DIRECT_RENT)
    transaction.setTransactionCondition(TransactionCondition.CLEANING_INCLUDED, true)
    transaction.setUtilitiesIncluded(false)
    transaction.setTaxationForm(TaxationForm.NDS)
    transaction.setRendCondition(RentCondition.PETS, false)
