...
trait StageSupport[A, S, B <: Stage[A, S]] extends Matchers with MockitoSupport {
  ...
  def checkIgnored(processingState: ProcessingState[S]): Assertion = {
    createProcessingStage().shouldProcess(processingState) shouldBe false
  }

  def checkProcessed(id: A, processingState: ProcessingState[S]): NotMockedException = {
  ...
  def checkShouldProcess(processingState: ProcessingState[S]): Assertion = {
    val stage = createProcessingStage()
    stage.shouldProcess(processingState) shouldBe true
  }
