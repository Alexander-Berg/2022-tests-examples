...
@Ignore
class TradesoftClientTest extends AnyFunSuite {
  ...
  val service = new RemoteHttpService(
    ...
    endpoint = new HttpEndpoint("api.parts-catalogs.com", 443, "https"),
  ...
  val client = new TradesoftClient(service, token, partnerEventClient, rateLimiter)
