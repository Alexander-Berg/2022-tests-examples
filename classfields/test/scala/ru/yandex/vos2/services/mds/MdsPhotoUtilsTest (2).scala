...
class MdsPhotoUtilsTest extends FunSuite {
  test("empty compressed name will not show up in name method") {
    val photoBuilder = Photo.newBuilder()
    photoBuilder.setName("aaa")
    photoBuilder.getCompressedBuilder.setCompressedName("")

    assert(MdsPhotoUtils.name(photoBuilder, Compressed) == "aaa")
  }

  val realtyMdsUtils = MdsPhotoUtils("writeUrl", "readUrl", RealtyMdsSettings)

  test("missing group name is not accepted py parse") {
    val urlParsed = parseMDSPhoto("https://avatars.yandex.net/get-realty/add.3858273751401290741141866327448314666/orig")
  ...
  test("missing size name is not accepted py parse") {
    val urlParsed = parseMDSPhoto("https://avatars.yandex.net/get-realty/12345/add.3858273751401290741141866327448314666")
    assert(urlParsed.isEmpty)
  ...
  test("valid url is parsed") {
    val urlParsed = parseMDSPhoto("https://avatars.yandex.net/get-realty/12345/add.3858273751401290741141866327448314666/orig")
