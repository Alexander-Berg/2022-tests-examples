...
class DefaultSmsSenderClientTest extends AnyFunSuite with MockHttpClientHelper {
  ...
  test("sms send to phone not from white list but it is on staff") {
    ...
    val client = this.getClient(200, xmlSuccess, staffCode = staffResponseCode, phone = phone, isProduction = false)
    ...
    assert(
      argCaptorRequest.getValue.asInstanceOf[HttpPost].getURI.toString
        == expectedUrl
    )
