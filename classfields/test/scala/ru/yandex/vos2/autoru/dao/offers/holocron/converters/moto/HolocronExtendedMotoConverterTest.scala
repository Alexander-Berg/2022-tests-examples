...
import ru.yandex.vos2.AutoruModel.AutoruOffer.MotoInfo.{MotoCategory => VosMotoCategory}
...
class HolocronExtendedMotoConverterTest extends AnyFunSuite with MockitoSupport with OptionValues {
  ...
  private val holocronExtendedConverter =
    new HolocronExtendedMotoConverter(currencyRates, offerFormConverter, regionTree)
  ...
  test("moto category conversion") {
    val offer = createOffer(category = Category.MOTO)
    ...
    offer.getOfferAutoruBuilder.getMotoInfoBuilder.setCategory(VosMotoCategory.MOTO_CAT_SCOOTERS)

    assert(
      holocronExtendedConverter.convert(offer.build(), DateTime.now()).getVosMoto.getMotoCategory ==
        "scooters"
    )
  }
}
