package ru.yandex.vos2.autoru.dao.offers.holocron.converters.moto
...
class HolocronMotoConverterTest extends AnyWordSpec with MockitoSupport with OptionValues {
  ...
  private class Fixture {
    ...
    val holocronConverter = new HolocronMotoConverter(
      mdsPhotoUtils,
      currencyRates,
      offerFormConverter,
      regionTree
    ...
    when(motoCatalog.getCardByMarkModel(?, ?)).thenReturn(Some(MotoCard(MotoCatalogCardMessage.getDefaultInstance)))
    when(motoCatalog.markNameByCode(?)).thenReturn(Some("model"))
    when(currencyRates.convert(?, ?, ?)).thenReturn(Some(BigDecimal(10)))
  }

  "HolocronMotoConverter" should {
    "not send deleted photos" in new Fixture {
      ...
      assert(holoOffer.getMoto.getPhotosCount == 1)
      assert(holoOffer.getMoto.getPhotos(0) == "//avatars.mds.yandex.net/get-autoru-all/100500/image2/1200x900")
    }
    ...
    "set url for used car" in new Fixture {
      ...
      offer.getOfferAutoruBuilder.getSellerBuilder.setUserRef("a_12345")
      assert(
        holocronConverter.convert(offer.build(), DateTime.now()).getMoto.getUrl ==
          "https://auto.ru/mototsikly/used/sale/100500-hash"
      )
    }
    ...
    "set url for new car" in new Fixture {
      ...
      assert(
        holocronConverter.convert(offer.build(), DateTime.now()).getMoto.getUrl ==
          "https://auto.ru/mototsikly/new/sale/100500-hash"
      )
    }
    ...
    "set price from price_rub" in new Fixture {
      ...
      offer.getOfferAutoruBuilder.getPriceBuilder
        ...
        .setCreated(123456L)
      assert(holocronConverter.convert(offer.build(), DateTime.now()).getMoto.getPrice == 100)
    }
    ...
    "set price without currency rates if price_rub is 0 but currency is BasicsModel.Curreny.RUB" in new Fixture {
      ...
      assert(holocronConverter.convert(offer.build(), DateTime.now()).getMoto.getPrice == 1000)
    }
    ...
    "set price using currency rates if price_rub is 0" in new Fixture {
      ...
      assert(holocronConverter.convert(offer.build(), DateTime.now()).getMoto.getPrice == 10)
    }

    "set displacement from moto_info.engine_volume" in new Fixture {
      val offer: Offer.Builder = createOffer(category = Category.MOTO)
      offer.setOfferID("100500-hash")
      offer.getOfferAutoruBuilder.getMotoInfoBuilder.setEngineVolume(1998)
      offer.getOfferAutoruBuilder.getSellerBuilder.setUserRef("a_12345")
      assert(
        holocronConverter.convert(offer.build(), DateTime.now()).getRawAuto.getMotoInfo.getDisplacement == 1998
      )
    }
  }
}
