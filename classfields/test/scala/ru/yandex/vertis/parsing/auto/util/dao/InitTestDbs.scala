package ru.yandex.vertis.parsing.auto.util.dao

import ru.yandex.vertis.parsing.auto.components.TestDockerParsingComponents
...
trait InitTestDbs extends Logging {
  ...
  def initDb(): Unit = {
    ...
    components.parsingShard.master.jdbc.update("SET FOREIGN_KEY_CHECKS=0;")
    autoruSchemaFile.foreach(sql => {
      //log.info(sql)
      components.parsingShard.master.jdbc.update(sql)
    })
    components.parsingShard.master.jdbc.update("SET FOREIGN_KEY_CHECKS=1;")
  }
