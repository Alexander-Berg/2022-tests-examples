...
class QueryAsyncRetriesTest extends AnyFunSuite with Matchers with MockitoSupport {
  ...
  test("generic error") {
    when(tableClient.getOrCreateSession(?)).thenReturn(completedFuture(Result.success[Session](session)))
    ...
    intercept[UnexpectedResultException] {
      Await.result(ydb.queryAsync("")("select * from series"), Duration.Inf)
    }
  }
