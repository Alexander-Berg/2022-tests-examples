...
import ru.yandex.vertis.shark.sender.impl.Raiffeisen1CreditApplicationBankSender.Raiffeisen1BankSenderEnvironment
...
object Raiffeisen1CreditApplicationBankSenderSpec extends TestCreditApplicationBankSender {

  override def creditProduct: CreditProductId = Raiffeisen1CreditApplicationBankSender.CreditProductId
  ...
  override def bankSenderLayer: RLayer[TestEnvironment, CreditApplicationBankSender] = {
    ...
    val senderLayer: URLayer[Raiffeisen1BankSenderEnvironment, CreditApplicationBankSender] = (for {
      bankClient <- ZIO.service[RaiffeisenBankClient.Service]
      converter <- ZIO.service[RaiffeisenBankConverter.Service]
    } yield new Raiffeisen1CreditApplicationBankSender(converter, bankClient)).toLayer
