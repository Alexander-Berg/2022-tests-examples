...
/**
  * Basically copy-paste from scalatest TryValues, with these changes:
  * 1. success method reports about failure cause in case of failure, not just gives "some failure happened"-like message
  * 2. failure method reports about success value in case of success
  * 3. Trait is renamed to avoid confusion
  * 4. Error messages are hardcoded, because they are generated as resources while building scalatest,
  * and we don't have access to these resources
  *
  * PR to upstream is opened here: https://github.com/scalatest/scalatest/pull/1338
  * but it isn't merged yet :(
  *
  * Copy-pasted from vertis-testkit. Can't depend on it until scalatest will be updated from 3.0.5 to 3.0.8.
  */
/*
...
trait BetterTryValues {
  ...
  /**
    ...
    */
  implicit def convertTryToSuccessOrFailure[T](theTry: Try[T])(implicit pos: source.Position): SuccessOrFailure[T] =
    new SuccessOrFailure(theTry, pos)
  ...
  class SuccessOrFailure[T](theTry: Try[T], pos: source.Position) {
      ...
      theTry match {
        ...
        case success =>
          throw new TestFailedException(
            (_: StackDepthException) => Some(s"The Try on which failure was invoked was not a Failure: $success"),
            None,
            pos
          )
      }
    ...
    def success: Success[T] = {
      theTry match {
        ...
        case Failure(cause) =>
          throw new TestFailedException(
            (_: StackDepthException) => Some("The Try on which success was invoked was not a Success."),
            Some(cause),
            pos
          )
      }
...
/**
  ...
  * </pre>
  */
object BetterTryValues extends BetterTryValues
