...
import ru.auto.salesman.model.telepony.ApiModel.CallCostResponse
...
/**
  * @author brs-lphv
  */
class AutoruCallPriceEstimateServiceSpec extends AnyWordSpec with Matchers with AsyncSpecBase with MockitoSupport {
  ...
  "RealtyCommercialCallPriceService" should {
    "retry on http failure" in {
      val cost = 9999
      val client = mockHttpClient(cost, failCount = 2)
      val service = new AutoruCallPriceEstimateService("some/url", client)
      service.getPrice(callFact, campaignHeader) shouldBe Success(cost)
    }
  }

  private def mockHttpClient(cost: Long, failCount: Int) = {
    val client = mock[SyncHttpClient]
    val counter = new AtomicLong()
    val response = CallCostResponse.newBuilder().setCost(cost).build()
