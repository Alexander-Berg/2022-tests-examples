...
/**
  * @author ruslansd
  */
class JdbcRobokassaPaymentSystemDaoSpec extends PaymentSystemDaoSpec with JdbcSpecTemplate {

  override val accounts =
    new JdbcAccountDao(database)

  override val payments =
    new JdbcPaymentSystemDao(database, PaymentSystemIds.Robokassa)
}
