...
class SimpleCompositeCollectorSpec
  extends WordSpec
    with Matchers {

  "SimpleCompositeCollector" should {
    ...
    "register duplicate counter" in {
      ...
      collector.register(anotherCounter) should be theSameInstanceAs anotherCounter

      val anotherCounterWithLables = Counter.build("name", "help")
        .labelNames("foo", "bar")
        .create()
      collector.register(anotherCounterWithLables) should be theSameInstanceAs anotherCounterWithLables
    }
    ...
    "not register duplicate wrapped counter" in {
      ...
      collector.register(wrappedCollector1) should be theSameInstanceAs wrappedCollector1
    ...
    "register collectors with same name and different scala types" in {
      val registry = new SimpleCompositeCollector
