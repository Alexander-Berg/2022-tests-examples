...
trait AutoRuPremiumSpec extends SingleProductDailyPolicySpec {
  ...
  "Premium policy" should {
    ...
    "return correct price for Krasnodar" in {
      checkPolicy(correctInterval, premium(50.rubles), priceIn(0L, Long.MaxValue), inRegion(RegKrasnodar))
    }

    "return correct price for Voronezh" in {
      checkPolicy(correctInterval, premium(50.rubles), priceIn(0L, Long.MaxValue), inRegion(RegVoronezh))
    }

    "return correct price for NNov" in {
      checkPolicy(correctInterval, premium(50.rubles), priceIn(0L, Long.MaxValue), inRegion(RegNNovgorod))
    }

    "return correct price for Ryazan" in {
      checkPolicy(correctInterval, premium(50.rubles), priceIn(0L, Long.MaxValue), inRegion(RegRyazan))
    }
