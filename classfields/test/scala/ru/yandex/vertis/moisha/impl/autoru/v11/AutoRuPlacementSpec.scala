...
trait AutoRuPlacementSpec extends SingleProductDailyPolicySpec {
  ...
  it should passForCarsUsedInChelyabinsk
  it should failOnOtherCategoriesInChelyabinsk
  ...
  def passForCarsUsedInChelyabinsk(): Unit = {
    "return correct price in first day for cars used in Chelyabinsk RG" in {
      for (reg <- RegChelyabinskGroup) {
        checkPolicy(
          correctInterval,
          placement(100.rubles),
          priceIn(0L, 300.thousands),
    ...
    "return correct prices in other days for cars used in Chelyabinsk RG" in {
      for (reg <- RegChelyabinskGroup) {
        checkPolicy(
          correctInterval,
          placement(2.rubles),
  ...
  def failOnOtherCategoriesInChelyabinsk(): Unit = {
    "fail if cars new in Chelyabinsk RG" in {
      for (reg <- RegChelyabinskGroup) {
        checkPolicyFailure(
          correctInterval,
          Products.Placement,
    ...
    "fail if other transport in Chelyabinsk RG" in {
      for {
        reg <- RegChelyabinskGroup
        transport <- transportsWithoutCars
        category <- Categories.values
      } {
        checkPolicyFailure(
