...
trait AutoRuPlacementSpec extends SingleProductDailyPolicySpec {
  ...
  it should passForCarsUsedInCentral
  it should failOnOtherCategoriesInCentral
  ...
  def passForCarsUsedInCentral(): Unit = {
    ...
    "return correct price in first day for cars used in Central" in {
      for {
        reg <- RegCentralGroup
        city <- citiesGen
      } {
    ...
    "return correct prices in other days for cars used in Central" in {
      for {
        reg <- RegCentralGroup
        city <- citiesGen
      } {
  ...
  def failOnOtherCategoriesInCentral(): Unit = {
    val CityLipetsk: RegionId = 9
    val CitySmolensk: RegionId = 12
    ...
    "fail if cars new in Central" in {
      for {
        reg <- RegCentralGroup
        city <- citiesGen
      } {
    ...
    "fail if other transport in Central" in {
      for {
        reg <- RegCentralGroup
        city <- citiesGen
        transport <- Transports.values.filterNot(Set(Cars))
