...
trait AutoRuBoostSpec extends SingleProductDailyPolicySpec {
  ...
  "Boost policy" should {
    ...
    "return correct price for Krasnodar" in {
      checkPolicy(correctInterval, boost(20.rubles), priceIn(0L, Long.MaxValue), inRegion(RegKrasnodar))
    }

    "return correct price for Voronezh" in {
      checkPolicy(correctInterval, boost(20.rubles), priceIn(0L, Long.MaxValue), inRegion(RegVoronezh))
    }

    "return correct price for NNovgorod" in {
      checkPolicy(correctInterval, boost(20.rubles), priceIn(0L, Long.MaxValue), inRegion(RegNNovgorod))
    }

    "return correct price for Ryazan" in {
      checkPolicy(correctInterval, boost(20.rubles), priceIn(0L, Long.MaxValue), inRegion(RegRyazan))
    }
