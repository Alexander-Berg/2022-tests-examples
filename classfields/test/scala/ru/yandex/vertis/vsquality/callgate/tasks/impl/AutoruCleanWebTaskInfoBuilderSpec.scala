...
class AutoruCleanWebTaskInfoBuilderSpec extends SpecBase with TaskInfoBuilderUtil {

  override def loadRes(name: String): String =
    readResourceFileAsString(s"/tasks/task-info-builder/autoru-offers/$name.json")

  private val builder = new AutoruCleanWebTaskInfoBuilder[F]

  "AutoruCleanWebTaskInfoBuilder" should {
