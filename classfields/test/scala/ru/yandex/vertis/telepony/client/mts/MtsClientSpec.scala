...
//    connectionParams
  ...
  "MtsClient" should {
    "not provide unknown number info" in {
      client.getUniversalNumber(PhoneGen.next).failed.futureValue
    }
    "provide universal numbers" in {
      val phones = client.getUniversalNumbers().futureValue
    ...
    "update universal number description" in {
      ...
      forAll(Gen.const(un), updateUniversalNumberGen(un.phone)) { (_, request) =>
        client.updateUniversalNumber(request).futureValue
        val actual = client.getUniversalNumber(un.phone).futureValue
...
//        mts - slowpoke
    "provide call detail records" ignore {
      ...
      val records = client.getCallDetailRecords(request).futureValue
...
//        mts stores records only for 30 days, so you should provide actual url
    "load record" ignore {
      ...
      val recordOpt = client.getCallRecord(url).futureValue
      inside(recordOpt) {
        case Some(record) =>
    ...
    "get empty sounds" in {
      ...
      client.getSounds(phone).futureValue shouldEqual Seq.empty
    }
    ...
    "put sound" in {
      ...
      clear(phone).futureValue
      client.putSound(Sound.UpdateRequest(phone, snd)).futureValue
      val sounds = client.getSounds(phone).futureValue
    ...
    "delete sound" in {
      ...
      clear(phone).futureValue
      client.putSound(Sound.UpdateRequest(phone, snd)).futureValue
      client.deleteSound(sound).futureValue
      Thread.sleep(1000)
      client.getSounds(phone).futureValue shouldEqual Seq.empty
    }
    ...
    "menu" ignore {
      forAll(MenuValueGen) { menuValue =>
        ...
        client.updateMenu(menu).futureValue
        val actual = client.getMenu(phone).futureValue
    ...
    "set and delete master" in {
      ...
      client.setMaster(request).futureValue
      client.deleteMaster(request.phone).futureValue
    }
  }

  private def clear(phone: Phone): Future[Unit] =
    for {
      sounds <- client.getSounds(phone)
      _ <- Future.sequence(sounds.map(client.deleteSound))
      _ <- client.updateMenu(Menu(phone, menuWithName("empty")))
    } yield ()
  ...
  private def fetchNumber() = {
    val numberF = for {
      numbers <- client.getUniversalNumbers()
      candidate = Random.shuffle(numbers.diff(badNumbers).toList).head
      number <- client.getUniversalNumber(candidate)
    } yield number
    numberF.futureValue
  }
