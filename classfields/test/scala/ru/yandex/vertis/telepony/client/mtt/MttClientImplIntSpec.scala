...
import ru.yandex.vertis.telepony.service.impl.mtt.{MttCallRecordClientImpl, MttClientImpl, MttGlobalApiClientImpl, MttPhoneClientImpl}
...
object MttClientImplIntSpec extends TestPrometheusComponent {
  ...
  def createMttPhoneClient(implicit am: ActorMaterializer): MttPhoneClient = {
    val sendReceive = PipelineBuilder.buildSendReceiveWithCredentials(
      credentials = WebApiCommonCredentials,
      proxy = None,
      maxConnections = 2
    )
  ...
  def createMttCallRecordClient(
      implicit
      am: ActorMaterializer): MttCallRecordClient = {
    val sendReceive = PipelineBuilder.buildSendReceiveWithCredentials(
      credentials = RecordsCredentials,
      proxy = None,
      maxConnections = 2
    )
    val httpClient = HttpClientBuilder.fromSendReceive("mtt-client-spec", sendReceive)
  ...
  def createMttGlobalBlackListClient(
      implicit
      am: ActorMaterializer): MttGlobalApiClient = {
    val sendReceive =
      PipelineBuilder.buildSendReceiveWithCredentials(credentials = GApiCredentials, proxy = None, maxConnections = 2)
    val httpClient = HttpClientBuilder.fromSendReceive("mtt-client-spec", sendReceive)
  ...
  def createMttClient(implicit am: ActorMaterializer): MttClient =
