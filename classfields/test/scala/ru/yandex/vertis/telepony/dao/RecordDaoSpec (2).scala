...
trait RecordDaoSpec extends SpecBase with BeforeAndAfterEach with DatabaseSpec {
  ...
  "RecordDao" should {
    ...
    "list for load only not after specified time" in {
      val r1 = sampleRecord.copy(id = "1", time = DateTime.now(TimeZone).minusHours(4))
      val r2 = sampleRecord.copy(id = "2", time = DateTime.now(TimeZone).minusHours(2))

      dao.createIfNotExists(r1).databaseValue.futureValue
      dao.createIfNotExists(r2).databaseValue.futureValue

      val records =
        dao.listForLoad(sampleRecord.operator, DateTime.now(TimeZone).minusHours(3), 24.hours, 1.hour).databaseValue.futureValue
    ...
    "list for load only not downloaded record" in {
      ...
      val records =
        dao.listForLoad(sampleRecord.operator, DateTime.now(TimeZone).minusDays(1), 24.hours, 1.hour).databaseValue.futureValue
      records should ===(List(r2))
    }
    ...
    "list for load only of specified operator" in {
      ...
      val records =
        dao.listForLoad(Operators.Mtt, DateTime.now(TimeZone).minusDays(1), 24.hours, 1.hour).databaseValue.futureValue
      records should ===(List(r2))
    }
