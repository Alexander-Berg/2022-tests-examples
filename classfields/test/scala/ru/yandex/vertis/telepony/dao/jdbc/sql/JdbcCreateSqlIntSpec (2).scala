...
abstract class JdbcCreateSqlIntSpec extends SpecBase with BeforeAndAfterAll {
  ...
  lazy val dbSteps: Database = new JdbcBuilder {
    override def databaseName: String = s"telepony_unit_test_dbSteps"

    private val sqlFolder = new File(this.getClass.getResource(sqlBaseDir).getPath)
    private val sqlFiles: List[File] = List.empty
        // TODO sqlFolder.listFiles().toList.filter(_.getName.matches("\\d{3}\\.sql")).sortBy(_.getName)
