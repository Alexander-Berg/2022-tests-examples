...
trait CommonCallDaoSpec extends SpecBase with BeforeAndAfterEach with BeforeAndAfterAll with DatabaseSpec {
  ...
  trait DefaultEnvironment {
    ...
    val expectedCalls = Seq(expectedCall).map(call => TeleponyCall.from(call, domain, call.hasRecord, Set.empty))
    ...
    val calls =
      (
        Seq(emptyTagAppCall, fullTagAppCall, partTagAppCall).map(TeleponyCall.from(_, domain)) ++
          ...
          Seq(emptyTagCall, fullTagCall, partTagCall).map(call =>
            TeleponyCall.from(call, domain, call.hasRecord, Set.empty)
          ) ++
    ...
    val calls =
      (
        Seq(expectedAppCall, unexpectedAppCall).map(TeleponyCall.from(_, domain)) ++
          Seq(expectedCall, unexpectedCall).map(call => TeleponyCall.from(call, domain, call.hasRecord, Set.empty)) ++
