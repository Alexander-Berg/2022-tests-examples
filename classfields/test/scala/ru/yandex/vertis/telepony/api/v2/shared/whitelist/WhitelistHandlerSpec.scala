...
class WhitelistHandlerSpec extends RouteTest with MockitoSupport {
  ...
  private def createHandler(whitelist: WhitelistService, whitelistHistoryService: WhitelistHistoryService): Route =
    seal(
      new WhitelistHandler {
        override protected def whitelistService: WhitelistService = whitelist

        override protected def whitelistHistoryService: WhitelistHistoryService = whitelistHistoryService
      }.route
    )
