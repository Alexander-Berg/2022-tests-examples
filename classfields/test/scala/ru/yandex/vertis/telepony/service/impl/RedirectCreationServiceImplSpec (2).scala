...
class RedirectCreationServiceImplSpec extends SpecBase with MockitoSupport {
  ...
  trait Test {
    ...
    val request = {
      ...
      r.copy(
        options = r.options.map(_.copy(callbackInfo = None)),
        antiFraudOptions = antiFraud
      )
    }
    ...
    val redirect = ActualRedirectGen.next.copy(antiFraud = antiFraud)
  ...
  "RedirectCreationServiceImpl" should {
    ...
    "return suitable if exists" in new Test {
      ...
      when(mockRS.touchRedirectAsync(request, redirect)).thenAnswer(_ => Future.unit)
    ...
    "return suitable if exists and update antifraud" in new Test {
      val changedRedirect = redirect.copy(antiFraud = Set.empty)
      val getResult = Suitable(changedRedirect)
      when(mockRS.get(request.key)(rc)).thenReturn(Future.successful(allRedirects))
      when(mockRIS.getExisting(allRedirects, request)).thenReturn(Future(getResult))
      when(mockRS.touchRedirectAsync(request, changedRedirect)).thenAnswer(_ => Future.unit)
      when(mockRS.updateAntifraudUnsafe(changedRedirect.id, request.antiFraudOptions)(rc)).thenAnswer(_ => Future.unit)
      ...
      Mockito.verify(mockRS).get(request.key)(rc)
      Mockito.verify(mockRIS).getExisting(allRedirects, request)
      Mockito.verify(mockRS).touchRedirectAsync(request, changedRedirect)
    }
    ...
    "fail to getOrCreate if invalid options" in new Test {
      val service = new RedirectCreationServiceImpl(mockRIS, mockRS, mockLocalRCS, mockSP)
      val requestWithUnexpectedOptions: RedirectServiceV2.CreateRequest = request.copy(options =
        Some(
