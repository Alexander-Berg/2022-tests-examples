...
class ResolutionServiceImplSpec extends SpecBase with MockitoSupport {
  ...
  def mockBlacklistService(blockInfo: Seq[BlockInfo]): BlacklistService = {
    ...
    when(blackListService.getListForTime(?, ?)).thenReturn(Future.successful(blockInfo))
    blackListService
  }
  ...
  def mockDomainAonBlacklistProvider(blockInfo: Option[AonBlockInfo]): DomainAonBlacklistProvider = {
    ...
    when(domainAonBlackListService.getForTime(?, ?)).thenReturn(Future.successful(blockInfo))
  ...
  "ResolutionService" should {
    ...
    "return empty block info for the time when no block info" in {
      val time = DateTimeGen.next

      val resolutionService =
        new ResolutionServiceImpl(passBlacklistService, passDomainAonBlacklistProvider, passPrefixBlacklist)
      val resolutions = resolutionService.getBlockResolutions(source, time).futureValue
      resolutions shouldBe empty
    }

    "return relevant for the time blacklist block info" in {
      val time = DateTimeGen.next
    ...
    "return relevant for the time aon block info" in {
      val time = DateTimeGen.next
