...
class BlacklistServiceIntSpec extends SpecBase with IntegrationSpecTemplate with BeforeAndAfterEach {
  ...
  "BlacklistService" should {
    ...
    "get some when time is passed and deadline is not over" in {
      ...
      val updateTime = time.minusHours(2)
      val info = BlockInfoGen.next.copy(deadline = Some(deadlineIsNotOver), updateTime = updateTime)
      blacklist.add(info).futureValue
      blacklist.getListForTime(info.source, time).futureValue should contain only info
    }

    "get not outdated when time is passed" in {
      val time = DateTime.now
      val updateTime = time.minusHours(2)
      val source = BlockInfoGen.next.source

      val info1 = BlockInfoGen.next.copy(source = source, deadline = Some(deadlineIsOver), updateTime = updateTime)
      val info2 = BlockInfoGen.next.copy(source = source, deadline = Some(deadlineIsNotOver), updateTime = updateTime)

      blacklist.add(info1).futureValue
      blacklist.add(info2).futureValue

      blacklist.getListForTime(source, time).futureValue should contain only info2
    }

    "get none when time is passed and deadline is over" in {
      val info = BlockInfoGen.next.copy(deadline = Some(deadlineIsOver))
      blacklist.add(info).futureValue
      blacklist.getListForTime(info.source, DateTime.now).futureValue shouldBe empty
    }

    "get none when time is passed and block info is update" in {
      val time = DateTime.now
      val updateTime = time.plusHours(1)
      val info = BlockInfoGen.next.copy(deadline = Some(deadlineIsNotOver), updateTime = updateTime)
      blacklist.add(info).futureValue
      blacklist.getListForTime(info.source, time).futureValue shouldBe empty
    }
