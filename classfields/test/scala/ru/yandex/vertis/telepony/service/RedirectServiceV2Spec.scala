...
import ru.yandex.vertis.telepony.service.RedirectServiceV2.{Clear, ComplainRequest, CreateFromExistingResponse, CreateRequest, Filter, ListRedirectRequest, RedirectKeyFilter, Update, UpdateOptionsRequest}
...
trait RedirectServiceV2Spec
  ...
  private def prepareOperatorNumber(
    ...
    account.operator match {
      case Operators.Mts =>
        mtsClients(account).asInstanceOf[InMemoryMtsClient].registerNewUniversalNumber(phone)
      case Operators.Mtt =>
  ...
  "Redirect service" should {
    ...
    "create redirects for different tags" in {
      ...
      forAll(tagPairGen, MoscowPhoneGen, MoscowPhoneGen) {
        case ((tag1, tag2), phone1, phone2) =>
          prepareOperatorNumber(phone1)
          prepareOperatorNumber(phone2)
    ...
    "delete both parts of double redirect" in {
      ...
      redirectServiceV2.updateOptions(
