...
import ru.yandex.vertis.telepony.model.{CallbackOrder, Phone, RefinedSource, Tag, TypedDomains}
...
class CallbackOrderServiceImplIntSpec extends SpecBase with IntegrationSpecTemplate with BeforeAndAfterEach {
  ...
  private lazy val service = new CallbackOrderServiceImpl(dao, callbackOrderSink)
