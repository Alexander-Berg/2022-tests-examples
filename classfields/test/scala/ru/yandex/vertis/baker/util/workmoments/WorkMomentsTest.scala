package ru.yandex.vertis.baker.util.workmoments
...
class WorkMomentsTest extends AnyFunSuite with OptionValues with MockitoSupport {
  ...
  test("durationToNextWorkMoment") {
    ...
    setTime(now.withHourOfDay(12).withMinuteOfHour(0).withSecondOfMinute(0).withMillisOfSecond(0))
    ...
    setTime(now.withHourOfDay(15).withMinuteOfHour(30).withSecondOfMinute(0).withMillisOfSecond(0))
