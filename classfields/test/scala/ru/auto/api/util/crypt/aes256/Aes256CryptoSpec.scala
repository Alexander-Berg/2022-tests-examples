package ru.auto.api.util.crypt.aes256

import org.apache.commons.codec.binary.Base64
import ru.auto.api.ApiOfferModel.Offer
import ru.auto.api.BaseSpec
import ru.auto.api.model.gen.BasicGenerators
import ru.auto.api.util.crypt.CryptoUtils
import ru.auto.api.util.crypt.CryptoUtils.RichSecretKey

class Aes256CryptoSpec extends BaseSpec {
  "Aes256KeyGen" should {
    "generate same key from same seed" in {
      val seed = BasicGenerators.readableString.next
      val key1 = Aes256KeyGen.generate(seed).key.toBase64Str
      val key2 = Aes256KeyGen.generate(seed).key.toBase64Str
      key1 shouldBe key2
    }

    "generate different keys without seed" in {
      val key1 = Aes256KeyGen.generateWithRandomSeed.key.toBase64Str
      val key2 = Aes256KeyGen.generateWithRandomSeed.key.toBase64Str
  ...
  "Aes256Crypto" should {
    "deserialize key from base64 and bytes" in {
      import ru.auto.api.util.crypt.CryptoUtils.RichSecretKey
      val key = Aes256KeyGen.generateWithRandomSeed.key
      val bytesKey = key.getEncoded
      val base64StrKey = key.toBase64Str
      val base64Key = key.toBase64

      val fromBase64Key = Aes256KeyGen.fromBase64(base64Key).key
      val fromBase64StrKey = Aes256KeyGen.fromBase64Str(base64StrKey).key
      val fromBytesKey = Aes256KeyGen.fromBytes(bytesKey).key
    ...
    "encrypt and decrypt msg with same Crypto" in {
      val text =
        ...
        """.stripMargin
      val key = Aes256KeyGen.generateWithRandomSeed
      val crypto = new Aes256Crypto(key)
      val encrypted = crypto.encrypt(text)
      val decrypted = crypto.decryptToStr(encrypted)
      decrypted shouldBe text
    }

    "encrypt twice with different encrypted texts" in {
      val text =
        """
          |A block cipher works on units of
          |a fixed size (known as a block size),
      ...
      val key = Aes256KeyGen.generateWithRandomSeed
      val crypto = new Aes256Crypto(key)
      val encrypted = crypto.encrypt(text.getBytes("UTF-8"))
      val encrypted2 = crypto.encrypt(text.getBytes("UTF-8"))
      encrypted shouldNot be(encrypted2)
    }
    ...
    "encrypt and decrypt msg with different Crypto" in {
      ...
      val text =
        """
          ...
          |Several padding schemes exist.
        """.stripMargin
      val key1 = Aes256KeyGen.generateWithRandomSeed
      val crypto1 = new Aes256Crypto(key1)
      val key2 = Aes256KeyGen.fromBase64Str(key1.key.toBase64Str)
      val crypto2 = new Aes256Crypto(key2)
