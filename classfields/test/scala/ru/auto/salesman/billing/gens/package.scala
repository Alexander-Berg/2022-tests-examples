...
package object gens extends MockitoSupport {
  ...
  def get[T <: Message.Builder](builder: T): Gen[Message] = {
    ...
    def _getRepeated(field: FieldDescriptor): Gen[Any] =
      if (field.isRepeated)
        Gen
          .choose(0, 15)
          .flatMap(i => Gen.sequence(Traversable.fill(i)(_getTyped(field))))
    ...
    def _getTyped(field: FieldDescriptor): Gen[Any] =
      field.getType.getJavaType match {
        ...
        case BYTE_STRING =>
          Gen.alphaStr.map(s => ByteString.copyFrom(s.getBytes))
        case DOUBLE => Gen.posNum[Double]
        case FLOAT => Gen.posNum[Float]
        case ENUM => Gen.oneOf(field.getEnumType.getValues.asScala)
  ...
  def campaignHeaderProducer: Iterator[CampaignHeader] =
    Iterator
      .continually(
        get(Model.CampaignHeader.newBuilder()).sample
      )
      .flatten
      .map { case x: CampaignHeader =>
        x
      }

  val CampaignsClientMock: CampaignsClient = {
    val m = mock[CampaignsClient]
    when(m.getCampaignHeaders)
      .thenReturn(Success(campaignHeaderProducer.take(10).toList))
    when(m.getCampaignHeaders(?))
      .thenReturn(Success(campaignHeaderProducer.take(10).toList))

    when(m.getValuableCampaigns())
      .thenReturn(Success(campaignHeaderProducer.take(10).toList))
    when(m.getValuableCampaigns(?))
      .thenReturn(Success(campaignHeaderProducer.take(10).toList))
