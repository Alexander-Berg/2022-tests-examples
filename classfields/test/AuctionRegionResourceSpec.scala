...
import auto.dealers.trade_in_notifier.storage.testkit.AuctionRegionPalmaMock
...
import zio.test.{assert, environment, DefaultRunnableSpec, TestFailure, ZSpec}

import scala.concurrent.duration._

object AuctionRegionResourceSpec extends DefaultRunnableSpec {

  override def spec: ZSpec[_root_.zio.test.environment.TestEnvironment, Any] = {

    suite("AuctionRegionResourceSpec")(
      testM("get true - if Palma has AuctionInRegion == 1") {
        for {
          res <- TradeInRegionRepository.exists(regionId => ZIO.succeed(regionId.id == 1))
        } yield assert(res)(equalTo(true))
      },
      testM("get false - if Palm doesn't have AuctionInRegion == -1") {
        for {
          res <- TradeInRegionRepository.exists(regionId => ZIO.succeed(regionId.id == -1))
  ...
  private def createEnvironment: ZLayer[Any, TestFailure[RuntimeException], TradeInRegionRepository] = {
    ...
    val layer =
      environment.testEnvironment ++ resourceLoader ++ config ++ palma ++ Logging.live >>>
        TradeInRegionResource.live(Open_for.AUCTION)
