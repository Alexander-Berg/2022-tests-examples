...
class PanoramaBadgeTest {
    ...
    @Test
    fun shouldSeePanoramaOnExtendedOffer() {
        webServerRule.routing { delegateDispatcher(extendedOfferWithPanoramaDispatcher) }
        ...
        performListingOffers { scrollToFirstSnippet() }
            .checkResult {
                isPanoramaBadgeDisplayed()
            }
    }

    @Test
    fun shouldSeePanoramaOnCommonOffer() {
        webServerRule.routing { delegateDispatcher(commonOfferWithPanoramaDispatcher) }
        activityTestRule.launchDeepLinkActivity(DEFAULT_LISTING_DEEPLINK)
        performSearchFeed { waitFirstPageLoaded(1) }
        performListingOffers { scrollToFirstSnippet() }
            .checkResult {
