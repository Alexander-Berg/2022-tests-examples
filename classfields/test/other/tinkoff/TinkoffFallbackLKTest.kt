...
class TinkoffFallbackLKTest(private val testParameter: TestParameter) {
    ...
    var ruleChain = baseRuleChain(
        ...
        SetupAuthRule(),
        SetupTimeZoneRule(),
        SetupTimeRule("01.01.2020"),
        arguments = TestMainModuleArguments(
            testExperiments = experimentsOf {
                Experiments::loanBrokerEnabled then false
                Experiments::fallbackBank then "sravni"
                Experiments::creditBankCard then toLowerString()
            }
        )
    )
