...
class PlanButtonsClickTest : BaseTest() {
    ...
    private fun checkAllStates(
        ...
        screen.apply {
            ...
            gallery.isViewStateMatches(
                getTestRelatedFilePath("floorPlanGalleryItemFrom$source")
            )

            gallery.swipeRight()
            apartmentPlanButton.waitUntil { isCompletelyDisplayed() }
            apartmentPlanButton.click()
            gallery.isViewStateMatches(
                getTestRelatedFilePath("apartmentPlanGalleryItemFrom$source")
            )
        }
    }
