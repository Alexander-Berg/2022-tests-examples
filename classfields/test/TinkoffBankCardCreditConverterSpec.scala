...
object TinkoffBankCardCreditConverterSpec extends DefaultRunnableSpec with CreditApplicationGen {

  import magnolia._
  import ru.yandex.vertis.shark.model.Block._

  private lazy val converter = new TinkoffBankCardCreditConverter

  private val timestamp = Instant.now()

  override def spec: ZSpec[TestEnvironment, Any] = suite("TinkoffBankCardCreditConverter")(
    ...
    testM("check work other needed") {
      ...
      val res = for {
        ...
        converterContext = AutoConverterContext.forTest(
          timestamp = timestamp,
          creditApplication = autoruCreditApplicationWithErrors
        )
