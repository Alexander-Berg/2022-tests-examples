...
import ru.yandex.vertis.shark.client.bank.data.alfa.Entities.{AlfaBankLeadId, CreditPurpose, MaritalStatus}
...
object AlfaBankConverterSpec
  ...
  private lazy val converterLayer =
    ZLayer.requires[Blocking] ++
      ...
      AlfaBankConverter.livePil ++ regionsDictionaryLayer
  ...
  private val expected = Entities.Lead(
    ...
    product = Entities.Product.PIL,
    productType = Entities.ProductType.PIL,
    platformId = "alfapartners_cpa_7054_PIL-PIL-70field-approve-36772-autoru-0-0-webmaster",
    ...
    workStatus = "true",
    partnerLeadId = "test-claim-alfa-1",
    ...
    workStartDate = "2015-12-17".some,
    maritalStatus = MaritalStatus.Divorced.some,
    paymentDay = "17".some,
    creditTerm = "12".some,
    lendingAmount = "490000".some,
    creditPurpose = CreditPurpose.Car.some,
    gettingWay = "office".some,
    gettingCity = "7700000000000".some
  )

  override def spec: ZSpec[TestEnvironment, Any] =
    suite("AlfaBankConverter")(
      testM("convert") {
        val autoruCreditApplication: AutoruCreditApplication = sampleCreditApplication
        val claim = autoruCreditApplication.getClaimByCreditProductId("alfabank-1".taggedWith).get
