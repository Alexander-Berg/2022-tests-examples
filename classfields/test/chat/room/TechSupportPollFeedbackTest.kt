...
class TechSupportPollFeedbackTest {
    ...
    private val webServerRule = WebServerRule {
        userSetup()
        getRoomMessagesFirstPage(RoomMessages.POLL_FEEDBACK)
    ...
    fun shouldSeePresetMessages() {
        ...
        closeSoftKeyboard()
        checkChatRoom {
            interactions.onChatOptionButton("Ок").waitUntilIsCompletelyDisplayed()
            interactions.onChatOptionButton("Затянули").waitUntilIsCompletelyDisplayed()
        }
        checkPoll { isMediumVoteOnlyDisplayed() }
    }

    @Test
    fun shouldSendFeedback() {
        webServerRule.routing {
            postChatMessage("all_good").watch {
                checkRequestBodyParameters(
                    "payload.value" to "Ок",
                    "properties.tech_support_feedback.hash" to "a373c87c8669965aff76ca17c69ef1b7",
                    "properties.tech_support_feedback.selected_preset" to "all_good"
                )
            }
        }
        activityTestRule.launchDeepLinkActivity(TECH_SUPPORT_CHAT_DEEPLINK)
        ...
        performChatRoom {
            ...
            interactions.onChatOptionButton("Ок").waitUntilIsCompletelyDisplayed().performClick()
        }.checkResult {
            longWaitAnimation()
            isChatMessageDisplayed("Ок")
    ...
    fun shouldHideFeedbackPresetsAfterSendCommonMessage() {
        webServerRule.routing {
            postChatMessage("common_message").watch {
                ...
                checkNotRequestBodyParameter("properties.tech_support_feedback")
            }
        }
        activityTestRule.launchDeepLinkActivity(TECH_SUPPORT_CHAT_DEEPLINK)
