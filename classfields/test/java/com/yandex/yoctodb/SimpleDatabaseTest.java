...
public class SimpleDatabaseTest {
  ...
  public void buildDatabase() throws IOException {
    ...
    final Query q3 = select().where(eq("int", from(2))).and(in("int", from(2), from(1)));
  ...
  public void detectCorruptedDatabase() throws IOException {
    ...
    DatabaseFormat.getCurrent().getDatabaseReader().from(Buffer.from(data));
  }
  ...
  public void buildWithAllSupportedDataTypes() throws IOException {
    ...
    final Query q1 = select().where(eq("int_field_full", from(1)));
    ...
    final Query q2 = select().where(eq("string_field_full", from("doc2")));
  ...
  public void testWithNoContinuousIndexes() throws IOException {
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("text", "doc1234", RANGE_FILTERABLE)
            .withField("text", "doc1", RANGE_FILTERABLE)
            .withField("doc2_not_contain_this_field", "doc2", RANGE_FILTERABLE)
            .withField("doc2_not_contain_this_field2", 1, RANGE_FILTERABLE)
            .withPayload("payload1".getBytes()));
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("text", "doc2", RANGE_FILTERABLE)
            .withField("int", 2, RANGE_FILTERABLE)
            .withPayload("payload2".getBytes()));
    ...
    final Query q2 = select().where(eq("int", from(2))).and(in("int", from(2), from(1)));
  ...
  public void countTest() throws IOException {
    ...
    // less or equals
    for (int i = 0; i < DOCS; i++) {
      final Query q1 = select().where(lte("id", from(i)));
    ...
    // greater or equals
    for (int i = 0; i < DOCS; i++) {
      final Query q1 = select().where(gte("id", from(i)));
  ...
  public void filterLong() throws IOException {
    ...
    for (int i = 0; i < DOCS; i++) {
      dbBuilder.merge(
          DatabaseFormat.getCurrent()
              .newDocumentBuilder()
              .withField(
                  "f", String.format("%07d", i) + "veryLongFilterableFieldValue", RANGE_FILTERABLE)
              .withPayload(("payload" + i).getBytes()));
    }
    ...
    final Query q =
        select()
            .where(eq("f", from(String.format("%07d", DOCS / 2) + "veryLongFilterableFieldValue")));
  ...
  public void sort() throws IOException {
    ...
    final Query q1 =
        select().where(gte("id", from(0))).and(lte("id", from(DOCS))).orderBy(desc("id"));
  ...
  public void sortNaturally() throws IOException {
    ...
    final Query q1 = select().where(eq("f", from(0))).orderBy(desc("f"));
    final List<String> results = new ArrayList<>(DOCS);
    db.execute(q1, new StringProcessor(results));
  ...
  public void complexSort() throws IOException {
    ...
    final Query q = select().orderBy(desc("f1")).and(asc("f2"));
  ...
  public void rangeQuery() throws IOException {
    ...
    final Query q1 =
        select()
            .where(new SimpleRangeCondition("id", from(10), true, from(20), true))
    ...
    final Query q2 =
        select()
            .where(new SimpleRangeCondition("id", from(1000), true, from(2000), true))
            .orderBy(new SimpleDescendingOrder("id"));
  ...
  public void buildDatabaseWithBitSetIndex() throws IOException {
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("text", "doc1234", RANGE_FILTERABLE)
            .withField("text", "doc123456", RANGE_FILTERABLE)
            .withField("int", 1, RANGE_FILTERABLE)
            .withPayload("payload1".getBytes()));
  ...
  public void notQuery() throws IOException {
    ...
    final Query q1 = select().where(not(eq("int", from(1))));
    ...
    db.execute(
        ...
        new DocumentProcessor() {
          @Override
          public boolean process(int document, @NotNull Database database) {
            ids1.add(document);
            return true;
          }
        });
    ...
    final Query q2 = select().where(not(eq("int", from(1)))).and(eq("text", from("doc2")));
    ...
    final Query q3 = select().where(not(eq("int", from(1)))).and(not(eq("int", from(2))));
  ...
  public void orQuery() throws IOException {
    ...
    final Query q = select().where(or(eq("int", from(1)), eq("text", from("doc2"))));
  ...
  public void skipAndLimit() throws IOException {
    ...
    final DocumentProcessor processor =
        new DocumentProcessor() {
          @Override
          public boolean process(final int document, @NotNull final Database database) {
            assertEquals(ids.getAndIncrement(), document);
            return true;
          }
        };
    ...
    ids.set(DOCS / 4);
    assertEquals(DOCS, db.executeAndUnlimitedCount(qSkip, processor));
    assertEquals(DOCS, ids.get());
    ...
    ids.set(0);
    assertEquals(DOCS, db.executeAndUnlimitedCount(qLimit, processor));
    assertEquals(DOCS / 3, ids.get());
    ...
    ids.set(DOCS / 3);
    assertEquals(DOCS, db.executeAndUnlimitedCount(qSkipLimit, processor));
    assertEquals(5 * DOCS / 6, ids.get());
