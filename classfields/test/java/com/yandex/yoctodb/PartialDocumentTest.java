...
public class PartialDocumentTest {
  ...
  public void findDocumentsWithFieldValue() throws IOException {
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("a", 1, DocumentBuilder.IndexOption.RANGE_FILTERABLE)
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("b", 2, DocumentBuilder.IndexOption.RANGE_FILTERABLE)
    ...
    // Both documents
    final Query q4 = select().where(or(eq("a", from(1)), eq("b", from(2))));
  ...
  public void documentWithoutFields() throws IOException {
    ...
    // Document
    dbBuilder.merge(DatabaseFormat.getCurrent().newDocumentBuilder().withPayload("doc".getBytes()));
  ...
  public void emptyDocument() throws IOException {
    ...
    // Document
    dbBuilder.merge(DatabaseFormat.getCurrent().newDocumentBuilder());
  ...
  public void emptyPayload() throws IOException {
    ...
    // Document
    dbBuilder.merge(DatabaseFormat.getCurrent().newDocumentBuilder().withPayload(new byte[0]));
