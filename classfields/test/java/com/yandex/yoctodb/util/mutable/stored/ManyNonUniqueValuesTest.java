...
public class ManyNonUniqueValuesTest {
  ...
  private void testDB(final int size) throws IOException {
    ...
    for (int i = 0; i < 10; i++) {
      assertEquals(
          Integer.toString((0)),
          UnsignedByteArrays.toString(UnsignedByteArrays.from(db.getFieldValue(i, fieldName))));
    }
  ...
  private DocumentBuilder buildTestDocument(String value) {
    return DatabaseFormat.getCurrent().newDocumentBuilder().withField(fieldName, value, STORED);
  }
}
