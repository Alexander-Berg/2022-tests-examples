...
public class FoldedByteArrayIndexedListTest {
  ...
  public void checkOutputStream() throws IOException {
    ...
    final FoldedByteArrayIndexedList set = new FoldedByteArrayIndexedList(data, strings.size());
    ...
    final Buffer offsets = buf.slice().slice(shift, (offsetsCount) << 3);
    ...
    final Buffer elements = buf.slice().slice(shift, (buf.remaining() - shift));
    ...
    assertEquals(
        getValueFromBuffer(getValue(indexes, offsets, elements, 0, sizeOfIndexOffsetValue)), "NEW");
    assertEquals(
        getValueFromBuffer(getValue(indexes, offsets, elements, 1, sizeOfIndexOffsetValue)),
        "USED");
    assertEquals(
        getValueFromBuffer(getValue(indexes, offsets, elements, 2, sizeOfIndexOffsetValue)), "NEW");
    assertEquals(
        getValueFromBuffer(getValue(indexes, offsets, elements, 3, sizeOfIndexOffsetValue)), "NEW");
  }
  ...
  private Buffer getValue(
      Buffer indexes, Buffer offsets, Buffer elements, int docId, int sizeOfIndexOffsetValue) {
  ...
  private long getValueIndex(
      Buffer indexes, Buffer offsets, int docId, int sizeOfIndexOffsetValue) {
    int offsetIndex = getOffsetIndex(indexes, docId, sizeOfIndexOffsetValue);
  ...
  private int twoBytesToInt(Buffer indexes, int docId) {
    ...
    byte[] bytes = new byte[] {indexes.get(byteIndex), indexes.get(byteIndex + 1)};
