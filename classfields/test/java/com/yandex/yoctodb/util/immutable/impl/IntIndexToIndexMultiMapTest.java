...
public class IntIndexToIndexMultiMapTest {
  ...
  private IndexToIndexMultiMap build() throws IOException {
    ...
    assertEquals(V1DatabaseFormat.MultiMapType.LIST_BASED.getCode(), buf.getInt());

    final IndexToIndexMultiMap result = IntIndexToIndexMultiMap.from(buf);
  ...
  public void ascendingIterator() throws IOException {
    final Iterator<IntToIntArray> iter = build().ascending(LongArrayBitSet.one(VALUES));
  ...
  public void ascendingSparseIterator() throws IOException {
    ...
    final int step = 4;
    for (int i = 0; i < VALUES; i += step) filter.set(i);
  ...
  public void ascendingEmptyIterator() throws IOException {
    final Iterator<IntToIntArray> iter = build().ascending(LongArrayBitSet.zero(VALUES));
    assertFalse(iter.hasNext());
    iter.next();
  }
  ...
  public void descendingIterator() throws IOException {
    final Iterator<IntToIntArray> iter = build().descending(LongArrayBitSet.one(VALUES));
  ...
  public void descendingEmptyIterator() throws IOException {
    final Iterator<IntToIntArray> iter = build().descending(LongArrayBitSet.zero(VALUES));
    assertFalse(iter.hasNext());
    iter.next();
  }
