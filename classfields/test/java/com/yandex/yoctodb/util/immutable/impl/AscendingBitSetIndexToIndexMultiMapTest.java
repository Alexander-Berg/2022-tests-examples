...
public class AscendingBitSetIndexToIndexMultiMapTest {
  ...
  private static AscendingBitSetIndexToIndexMultiMap build() {
    ...
    final com.yandex.yoctodb.util.mutable.impl.AscendingBitSetIndexToIndexMultiMap mutable =
        new com.yandex.yoctodb.util.mutable.impl.AscendingBitSetIndexToIndexMultiMap(
    ...
    assertEquals(V1DatabaseFormat.MultiMapType.ASCENDING_BIT_SET_BASED.getCode(), buf.getInt());

    final AscendingBitSetIndexToIndexMultiMap result =
        AscendingBitSetIndexToIndexMultiMap.from(buf, DOCS, AllocatingArrayBitSetPool.INSTANCE);
