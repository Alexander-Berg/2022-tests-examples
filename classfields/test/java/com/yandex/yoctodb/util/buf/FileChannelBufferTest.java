...
public class FileChannelBufferTest extends BufferTest {
  ...
  protected Buffer bufferOf(byte[] data) {
    try {
      ...
      final FileChannel read =
          FileChannel.open(path, StandardOpenOption.READ, StandardOpenOption.DELETE_ON_CLOSE);
  ...
  private File createTempFile() throws IOException {
    final String suffix = String.valueOf(fileNumber.incrementAndGet());
    return File.createTempFile("file_channel_buffer_test_", suffix);
  }
  ...
  public void brokenSizeWithOffset() throws IOException {
    final FileChannel broken = mock(FileChannel.class);
    when(broken.size()).thenReturn(1024L).thenThrow(new IOException("Test"));
  ...
  public void getByte() throws IOException {
    ...
    when(broken.read(any(ByteBuffer.class), anyLong())).thenThrow(new IOException("Test"));
