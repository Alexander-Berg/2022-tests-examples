...
public abstract class BufferTest {
  ...
  private static final byte[] ALPHABET_BYTES = ALPHABET.getBytes(Charsets.US_ASCII);
  ...
  public void testGetToBuffer() {
    ...
    assertEquals(ByteBuffer.wrap(ALPHABET_BYTES), ByteBuffer.wrap(bytes, 0, ALPHABET_BYTES.length));
  }
  ...
  public void testGetToBufferWithOffset() {
    ...
    assertTrue(buf.hasRemaining());
    assertEquals(ByteBuffer.wrap(ALPHABET_BYTES, 0, 10), ByteBuffer.wrap(bytes, 128, 10));
  }
  ...
  public void testToString() {
    final Buffer buf = bufferOf(ALPHABET);
    assertTrue(buf.toString().contains(Integer.toString(ALPHABET_BYTES.length)));
  }
}
