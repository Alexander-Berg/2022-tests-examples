...
public class V1DatabaseReaderTest {
  ...
  private byte[] buildDatabase() throws IOException {
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("text", "doc1234", DocumentBuilder.IndexOption.FULL)
            .withField("int", 1, DocumentBuilder.IndexOption.FULL)
    ...
    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField("text", "doc2", DocumentBuilder.IndexOption.FULL)
            .withField("int", 2, DocumentBuilder.IndexOption.FULL)
  ...
  public void tooSmall() throws IOException {
    final byte[] bytes = Arrays.copyOf(buildDatabase(), MAGIC.length + getDigestSizeInBytes() - 1);
    INSTANCE.from(Buffer.from(bytes));
  }
}
