...
public class V1FullIndexBinaryTest {
  ...
  public void writingFixedLengthFullIndex() throws IOException {
    ...
    Assert.assertEquals(os.size(), outputStreamWritable.getSizeInBytes());

    Buffer byteBuffer = Buffer.from(os.toByteArray());

    final long fullSizeInBytes = byteBuffer.getLong();
    Assert.assertEquals(fullSizeInBytes, outputStreamWritable.getSizeInBytes() - 12);

    int segmentCode = byteBuffer.getInt();
    Assert.assertEquals(
        V1DatabaseFormat.SegmentType.FIXED_LENGTH_FULL_INDEX.getCode(), segmentCode);
    ...
    // elements - should equals to sorted collection of bytes (byteArraysDoc1 merged with
    // byteArraysDoc2)
    Assert.assertEquals(UnsignedByteArrays.from(new byte[] {3, 2, 1, 0}), elements.get(0));
    Assert.assertEquals(UnsignedByteArrays.from(new byte[] {7, 7, 7, 7}), elements.get(1));
  ...
  public void writingVariableLengthFullIndex() throws IOException {
    ...
    Assert.assertEquals(
        V1DatabaseFormat.SegmentType.VARIABLE_LENGTH_FULL_INDEX.getCode(), segmentCode);
    ...
    final List<UnsignedByteArray> elements = new ArrayList<>();
    for (int i = 0; i < elementsCount; i++) {
      final byte[] currentElementBytes =
          new byte[(int) elementOffsets[i + 1] - (int) elementOffsets[i]];
      byteBuffer.get(currentElementBytes);
      elements.add(UnsignedByteArrays.from(currentElementBytes));
    }
    // elements - should equals to sorted collection of bytes (byteArraysDoc1 merged with
    // byteArraysDoc2)
    Assert.assertEquals(elements.get(0), UnsignedByteArrays.from(new byte[] {3, 2, 1, 0}));
    Assert.assertEquals(elements.get(1), UnsignedByteArrays.from(new byte[] {7, 7, 7, 7, 7}));
