...
public class V1FilterableIndexBinaryTest {
  ...
  public void writingVariableLengthTest() throws IOException {
    ...
    assertEquals(os.size(), outputStreamWritable.getSizeInBytes());

    final Buffer byteBuffer = Buffer.from(os.toByteArray());
    final long fullSizeInBytes = byteBuffer.getLong();

    assertEquals(fullSizeInBytes, outputStreamWritable.getSizeInBytes() - 12);
    ...
    Assert.assertArrayEquals(
        new long[] {0, 1, 6, 8, 11, 15, 19, 22, 27, 29, 30, 31}, elementOffsets);
    ...
    //        }
    //        Assert.assertArrayEquals(new int[]{0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80},
    // indexOffsets);
    //
  ...
  public void writingFixedLengthTest() throws IOException {
    final V1FilterableIndex v1FilterableIndex =
        new V1FilterableIndex("fixed_length_field_name", true);
    ...
    //            indexOffsets[i] = currentOffset;
    //        }
    //        Assert.assertArrayEquals(new int[]{0, 8, 16, 24, 32, 44, 52, 60, 68, 76, 84},
    // indexOffsets);
    //
    //        final int[][] indexes = new int[11][];
    //        for (int i = 0; i < keysCount; i++) {
