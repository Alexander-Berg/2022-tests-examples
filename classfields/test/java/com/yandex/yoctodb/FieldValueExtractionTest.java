...
public class FieldValueExtractionTest {
  ...
  private DocumentProvider build(final IndexOption indexOption) throws IOException {
    final DatabaseBuilder dbBuilder = DatabaseFormat.getCurrent().newDatabaseBuilder();

    dbBuilder.merge(
        DatabaseFormat.getCurrent()
            .newDocumentBuilder()
            .withField(FIELD_NAME, 0, indexOption)
    ...
    return DatabaseFormat.getCurrent().getDatabaseReader().from(Buffer.from(os.toByteArray()));
  }
  ...
  public void notExtractFilterableFieldValue() throws IOException {
    final DocumentProvider db = build(IndexOption.RANGE_FILTERABLE);
