...
import com.yandex.yoctodb.immutable.ExecutionEngine;
...
public class CompositeFileDatabaseTest {
  ...
  private static ExecutionEngine db;
  ...
  private static File buildDatabase1(final String name) throws IOException {
    ...
    for (int i = 0; i < DOCS; i++) {
      builder.merge(
          FORMAT
              .newDocumentBuilder()
              .withField("field1", from("1"), RANGE_FILTERABLE, DocumentBuilder.IndexType.TRIE)
              .withField("field2", "2", RANGE_FILTERABLE)
  ...
  private static File buildDatabase2(final String name) throws IOException {
    ...
    for (int i = 0; i < DOCS; i++) {
      builder.merge(
          FORMAT
              .newDocumentBuilder()
              .withField("field1", "2", RANGE_FILTERABLE)
              .withField("field2", "1", RANGE_FILTERABLE)
              .withField("index", i, FULL)
              .withField("relevance", i, SORTABLE)
              .withField(LONG_STORED_FILED_NAME, Long.valueOf(i), STORED)
              .withField(INT_STORED_FILED_NAME, i, STORED)
  ...
  public static void beforeAll() throws IOException {
    db1 =
        READER.from(
            Buffer.from(new RandomAccessFile(buildDatabase1("1.dat"), "r").getChannel()),
            AllocatingArrayBitSetPool.INSTANCE,
            true);

    db2 =
        READER.from(
            Buffer.from(new RandomAccessFile(buildDatabase2("2.dat"), "r").getChannel()),
            AllocatingArrayBitSetPool.INSTANCE,
            true);
        ...
        db.executeAndUnlimitedCount(
            ...
            new DocumentProcessor() {
              ...
              public boolean process(final int document, @NotNull final Database database) {
                assertEquals(idSkip.getAndIncrement() % DOCS, document);
                return true;
              }
            }));
    ...
    assertEquals(
        ...
        db.executeAndUnlimitedCount(
            ...
            new DocumentProcessor() {
              @Override
              public boolean process(final int document, @NotNull final Database database) {
                assertEquals(idLimit.getAndIncrement(), document);
                return true;
              }
            }));
    ...
    assertEquals(
        ...
        db.executeAndUnlimitedCount(
            ...
            new DocumentProcessor() {
              @Override
              public boolean process(final int document, @NotNull final Database database) {
                assertEquals(idSkipLimit.getAndIncrement() % DOCS, document);
                return true;
              }
            }));
  ...
  public void filter() {
    for (int i = 0; i < DOCS; i++) {
      ...
      final Iterator<IndexedDatabase> dbs = asList(db1, db2).iterator();
      assertEquals(
          2,
          db.executeAndUnlimitedCount(
              q,
    ...
    final Query qDatabase1Docs =
        select().where(eq("field1", from("1"))).and(eq("field2", from("2")));
        ...
        db.executeAndUnlimitedCount(
            ...
            new DocumentProcessor() {
              ...
              public boolean process(final int document, @NotNull final Database database) {
                ...
                assertEquals(idDatabase1Docs.getAndIncrement(), document);
                return true;
              }
            }));
    ...
    final Query qDatabase2Docs =
        select().where(eq("field1", from("2"))).and(eq("field2", from("1")));
        ...
        db.executeAndUnlimitedCount(
            ...
            new DocumentProcessor() {
              ...
              public boolean process(final int document, @NotNull final Database database) {
                ...
                assertEquals(idDatabase2Docs.getAndIncrement(), document);
                return true;
              }
            }));
    assertEquals(DOCS, db.count(qDatabase2Docs));

    final Query qZero1 = select().where(eq("field1", from("1"))).and(eq("field2", from("1")));
    assertEquals(
        0,
        db.executeAndUnlimitedCount(
            qZero1,
    ...
    final Query qZero2 = select().where(eq("field1", from("2"))).and(eq("field2", from("2")));
    assertEquals(
        0,
        db.executeAndUnlimitedCount(
            qZero2,
  ...
  public void filterDatabase() {
    final Query query = select().where(eq("field1", from("2"))).skip(DOCS / 4).limit(DOCS / 4);
    ...
    final DocumentProcessor processor =
        new DocumentProcessor() {
          ...
          public boolean process(final int document, @NotNull final Database database) {
            assertEquals(db2, database);
            assertEquals(expected.getAndIncrement() - DOCS, document);
  ...
  @Test
  public void sort() {
    final Query query = select().orderBy(asc("relevance")).and(desc("index"));
...

    {
      ...
      for (int i = DOCS - 1; i >= 0; i--) assertEquals(i, docsIterator.next().intValue());
      for (int i = 0; i < DOCS; i++) assertEquals(i, docsIterator.next().intValue());
    }
  ...
  public void sortAndFilter() {
    for (int i = 0; i < DOCS; i++) {
      final Query query = select().where(eq("index", from(i))).orderBy(asc("relevance"));

      final List<Integer> docs = new ArrayList<>(2);
      final DocumentProcessor processor = new DoubleDatabaseProcessor(db1, db2, docs);
    ...
    for (int i = 0; i < DOCS; i++) {
      final Query query = select().where(eq("index", from(i))).orderBy(desc("relevance"));
      final List<Integer> docs = new ArrayList<>(2);
      final DocumentProcessor processor = new DoubleDatabaseProcessor(db2, db1, docs);
  ...
  public void emptyRangeRight() {
    final Query query =
        select()
            .where(in("relevance", from(DOCS), true, from(2 * DOCS), false))
  ...
  public void emptyRangeLeft() {
    final Query query =
        select()
            .where(in("relevance", from(-2 * DOCS), true, from(-DOCS), false))
            .orderBy(desc("relevance"));
  ...
  public void unindexedFieldSearch() {
    ...
    final Query query =
        select()
            .where(in("unindexed_field", from(-2 * DOCS), true, from(-DOCS), false))
            .orderBy(desc("relevance"));
  ...
  public void emptyCompositeDatabaseFieldSearch() {
    final ExecutionEngine db = READER.composite(new ArrayList<>());

    final Query query =
        select()
            .where(in("field1", from(-2 * DOCS), true, from(-DOCS), false))
            .orderBy(desc("relevance"));
