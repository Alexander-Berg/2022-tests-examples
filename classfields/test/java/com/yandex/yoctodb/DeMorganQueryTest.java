...
public class DeMorganQueryTest {
  private DocumentBuilder buildDocument(int x, int y, int z) {
    return DatabaseFormat.getCurrent()
        ...
        .withField("x", x, RANGE_FILTERABLE)
        .withField("y", y, RANGE_FILTERABLE)
        .withField("z", z, RANGE_FILTERABLE)
  ...
  private Database buildDatabase() throws IOException {
    ...
    return databaseFormat.getDatabaseReader().from(Buffer.from(os.toByteArray()));
  }

  private static class IdCollector implements DocumentProcessor {
    @NotNull private final List<Integer> ids;

    IdCollector(@NotNull final List<Integer> ids) {
      this.ids = ids;
    }
  ...
  public void each() throws IOException {
    ...
    for (int x = 0; x < 2; x++) {
      for (int y = 0; y < 2; y++) {
        for (int z = 0; z < 2; z++) {
          final Query query =
              select().where(and(eq("x", from(x)), eq("y", from(y)), eq("z", from(z))));
          ...
          ids.clear();
          assertEquals(1, db.executeAndUnlimitedCount(query, new IdCollector(ids)));
          assertEquals(singletonList(expectedId), ids);
        }
      }
    }
  ...
  public void notOne() throws IOException {
    ...
    for (int x = 0; x < 2; x++) {
      for (int y = 0; y < 2; y++) {
        for (int z = 0; z < 2; z++) {
          final Query query =
              select().where(not(and(eq("x", from(x)), eq("y", from(y)), eq("z", from(z)))));
          ...
          for (int i = 0; i < 8; i++) {
            if (i != unexpected) expected.add(i);
          }
          ...
          actual.clear();
          assertEquals(7, db.executeAndUnlimitedCount(query, new IdCollector(actual)));
          assertEquals(expected, actual);
        }
      }
    }
  ...
  public void allUsingOr() throws IOException {
    ...
    for (int x = 0; x < 2; x++) {
      for (int y = 0; y < 2; y++) {
        for (int z = 0; z < 2; z++) {
          ors.add(and(eq("x", from(x)), eq("y", from(y)), eq("z", from(z))));
        }
      }
    }
