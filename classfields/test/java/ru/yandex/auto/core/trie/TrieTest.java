...
public class TrieTest {
  private static final Logger log = Logger.getLogger(TrieTest.class);
  private static final long FIXED_SEED = 239017;
  private static final Random randomWithFixedSeed = new Random(FIXED_SEED);

  private class TrieTester {
    @Nullable private SubstringsFinder<String> indTrie = null;
    @Nullable private SubstringsFinder<String> indNaive = null;

    public TrieTester(@NotNull List<Pair<int[], String>> input) {
      indTrie = new NaiveSubstringsFinder<String>(input);
      indNaive = new Trie<String>(input);
    }

    public void processTexts(@NotNull List<int[]> texts) {
      for (int[] t : texts) {
        String resTrie = indTrie.find(t);
        String resNaive = indNaive.find(t);
        Assert.assertTrue(
            "<found> vs <not found>: " + Arrays.toString(t),
            ((resTrie == null) == (resNaive == null)));
        if (resTrie != null) {
          // Assert.assertTrue(resTrie + " vs " + resNaive + ": " + t, resTrie.compareTo(resNaive)
          // == 0);
        }
      }
  ...
  public void test3() {
    ...
    log.debug(
        "Test is passed. textsTotalLength = "
            + ind.getStat().getTextsTotalLength()
            + ", verticesCount = "
            + ind.getStat().getVerticesCount());
  }
  ...
  private class NaiveSubstringsFinder<T> implements SubstringsFinder<T> {
    @NotNull List<Pair<int[], T>> mem = newArrayList();

    public NaiveSubstringsFinder(@NotNull List<Pair<int[], T>> input) {
      mem = newArrayList(input);
    }

    @Nullable
    public T find(@NotNull int[] text) {
