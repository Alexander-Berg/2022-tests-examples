...
func TestClient_findMaster(t *testing.T) {
	...
	// master
	httpmock.RegisterResponder("GET", "http://host1:8080/login",
		httpmock.NewStringResponder(200, ""))
	// not active master
	httpmock.RegisterResponder("GET", "http://host2:8080/login",
		httpmock.ConnectionFailure)
	// some host, that alive, but respond non 200
	httpmock.RegisterResponder("GET", "http://host3:8080/login",
...
func TestClient_Switch(t *testing.T) {
	...
	httpmock.Activate()
	t.Cleanup(httpmock.DeactivateAndReset)
	httpmock.RegisterResponder("GET", "http://host-01-sas.prod.vertis.yandex.net:8080/login",
		httpmock.NewStringResponder(200, ""))
