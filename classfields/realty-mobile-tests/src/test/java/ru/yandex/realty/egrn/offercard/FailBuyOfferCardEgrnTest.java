...
public class FailBuyOfferCardEgrnTest {
    ...
    public void shouldSeeBuyButtonWithoutAuth() {
        basePageSteps.scrollToElement(basePageSteps.onOfferCardPage().purchasedReports().waitUntil(hasSize(1)).get(0));
        basePageSteps.onOfferCardPage().purchasedReports().waitUntil(hasSize(1)).get(0).click();
        basePageSteps.refreshUntil(() -> basePageSteps.onEgrnReportPage().errorPage(), exists(), EGRN_TIMEOUT);
    }
}
