...
@DisplayName("Показ телефона. Карточка новостройки. Телефон в микрокарточке")
...
public class CardShowPhoneMicrocardTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в микрокарточке. Бесплатный ЖК")
    public void shouldSeePhoneFreeJkNormalCase() {
        ...
        basePageSteps.onNewBuildingCardPage().stickyActions().link(CALL).click();
        basePageSteps.onNewBuildingCardPage().stickyActions().link(CALL)
                .should(hasHref(equalTo(format(TEL_HREF_PATTERN, TEST_PHONE))));
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в микрокарточке. Платный ЖК")
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ 500ки в микрокарточке.")
    ...
    @Description("ТУТ ВИДИМО БАГ")
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в микрокарточке. Спецпроект. Видим телефон из «phonesWithTag»")
