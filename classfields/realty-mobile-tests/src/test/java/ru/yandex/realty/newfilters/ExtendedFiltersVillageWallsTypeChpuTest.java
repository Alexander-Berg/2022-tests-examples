...
@DisplayName("Фильтр поиска по коттеджным поселкам. Материал стен")
@Feature(FILTERS)
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebMobileModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class ExtendedFiltersVillageWallsTypeChpuTest {

    private static final String WALLS_TYPE = "Материал стен";
    ...
    public void shouldSeeVillageWallsType() {
        urlSteps.testing().path(Pages.FILTERS).queryParam("rgid" , MO_RGID).villageFiltersMobile().open();
        basePageSteps.scrollToElement(basePageSteps.onMobileMainPage().searchFilters().byName(WALLS_TYPE));
        basePageSteps.onMobileMainPage().searchFilters().byName(WALLS_TYPE).button(name).click();
        basePageSteps.onMobileMainPage().searchFilters().applyFiltersButton().click();
        urlSteps.testing().path(MOSKVA_I_MO).path(KUPIT).path(KOTTEDZHNYE_POSELKI).path(expected)
