...
import static ru.yandex.realty.step.UrlSteps.RETAIL_URL_PARAM;
...
@DisplayName("Фильтры поиска по коммерческой недвижимости. Ремонт")
@Feature(FILTERS)
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebMobileModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class ExtendedFiltersCommercialRenovationChpuTest {
    ...
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                {"Косметический", "/s-kosmeticheskym-remontom/"},
                {"Дизайнерский", "/s-dizainerskym-remontom/"},
        });
    }
    ...
    public void openCommercialPage() {
        ...
        urlSteps.testing().path(Pages.FILTERS).queryParam(RGID, SPB_I_LO_RGID)
                .queryParam(COMMERCIAL_TYPE_URL_PARAM, RETAIL_URL_PARAM).queryParam(TYPE_URL_PARAM, SELL_URL_PARAM)
                .queryParam(CATEGORY_URL_PARAM, COMMERCIAL_URL_PARAM).open();
        basePageSteps.scrollToElement(basePageSteps.onMobileMainPage().searchFilters().byName("Ремонт"));
    }
    ...
    public void shouldSeeRenovationInUrl() {
        ...
        urlSteps.testing().path(SPB_I_LO).path(KUPIT).path(COMMERCIAL).path(TORGOVOE_POMESHCHENIE)
