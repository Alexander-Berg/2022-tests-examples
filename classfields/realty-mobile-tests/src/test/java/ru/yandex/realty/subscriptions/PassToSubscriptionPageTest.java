...
public class PassToSubscriptionPageTest {
    ...
    private static final String SUBSCRIBE_TO_ZHK = "подписаться";
    ...
    public void shouldSeeNewbuildingPass() {
        ...
        basePageSteps.scrollUntilExists(() -> basePageSteps.onNewBuildingCardPage().subscribeBlock().button(SUBSCRIBE_TO_ZHK));
        basePageSteps.onNewBuildingCardPage().subscribeBlock().button(SUBSCRIBE_TO_ZHK).waitUntil(not(isDisabled())).click();
        api.shouldSeeLastSubscriptionByUid(account.getId()).isNotNull();
        basePageSteps.onNewBuildingCardPage().link(SUBSCRIPTION).waitUntil(not(isDisabled())).click();
    ...
    public void shouldSeeSearchPass() {
        ...
        basePageSteps.onMobileSaleAdsPage().subscriptionForm().link(SUBSCRIPTION).click();
        api.shouldSeeLastSubscriptionByUid(account.getId()).isNotNull();
        basePageSteps.switchToNextTab();
    ...
    public void shouldSeePricePass() {
        ...
        basePageSteps.onOfferCardPage().priceHistoryModal().button(SUBSCRIBE)
                .should(hasClass(Matchers.not(containsString(BUTTON_DISABLED)))).click();
        basePageSteps.onOfferCardPage().priceHistoryModal().subscriptionMessage().link(SUBSCRIPTION).click();
        api.shouldSeeLastSubscriptionByUid(account.getId()).isNotNull();
        basePageSteps.switchToNextTab();
