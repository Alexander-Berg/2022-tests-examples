...
@Feature(NEWBUILDING)
@DisplayName("Длинное название новостройки")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class NewbuildingLongTitleTest {

    private static final String FULL_NAME = "Длинное название оффера целых пятьдесят два символа";
    ...
    public void before() {
        mockRuleConfigurable.favoritesStub(favoritesTemplate().addItem(format("site_%s", INT_ID)).build())
                .offerWithSiteSearchStub(offerWithSiteSearchTemplate().sites(asList(
                        mockSite(SITE_TEMPLATE).setId(INT_ID).setFullName(FULL_NAME))).buildSite())
                .createWithDefaults();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Новостройка с длинным названием в избранном")
    public void shouldSeeLongTitleNewbuilding() {
        basePageSteps.onFavoritesPage().offersList().get(FIRST).title().should(hasText(FULL_NAME));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Видим скриншот новостройки с длинным названием в избранном")
    public void shouldSeeLongTitleNewbuildingScreenshot() {
