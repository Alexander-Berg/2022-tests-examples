...
@Feature(VILLAGES)
@DisplayName("Длинное название КП")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class VillagesLongTitleTest {
    ...
    public void before() {
        mockRuleConfigurable.favoritesStub(favoritesTemplate().addItem(format("village_%s", STRING_ID)).build())
                .villageSearchStub(villageSearchTemplate().villages(asList(
                        mockVillage(VILLAGE_COTTAGE).setId(STRING_ID).setName(FULL_NAME))).build())
                .createWithDefaults();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("КП с длинным названием в избранном")
    public void shouldSeeLongTitleVillage() {
        basePageSteps.onFavoritesPage().offersList().get(FIRST).title().should(hasText(FULL_NAME));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Видим скриншот КП с длинным названием в избранном")
    public void shouldSeeLongTitleVillageScreenshot() {
