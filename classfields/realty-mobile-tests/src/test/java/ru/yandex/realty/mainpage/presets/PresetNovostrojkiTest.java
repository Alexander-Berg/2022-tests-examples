...
@Feature(MAIN)
@DisplayName("Ссылки пресета «Новостройки»")
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebMobileModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class PresetNovostrojkiTest {
    ...
    public static Collection<Object[]> links() {
        return asList(new Object[][]{
                {"С чистовой отделкой", "/moskva/kupit/novostrojka/chistovaya-otdelka/"},
                {"С отделкой под ключ", "/moskva/kupit/novostrojka/pod-kluch/"},
                {"Рядом с метро", "/moskva/kupit/novostrojka/ryadom-metro/"},
                {"Эконом-класс", "/moskva/kupit/novostrojka/zhk-econom/"},
                {"До 4 квартала 2022", "/moskva/kupit/novostrojka/sdacha-2022/"},
                {"Со скидками", "/moskva/kupit/novostrojka/?hasSpecialProposal=YES"}
        });
    }
    ...
    public void shouldSeePresetBuyFlatLinks() {
        basePageSteps.onMobileMainPage().preset(NOVOSTROJKI).link(title).should(hasHref(equalTo(
