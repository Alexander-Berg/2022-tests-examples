...
@DisplayName("Показ телефона. Карточка оффера")
@Feature(MAP)
@Link("https://st.yandex-team.ru/VERTISTEST-1599")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class OfferCardITouchShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Клик на «Позвонить»")
    public void shouldSeePhoneNormalCase() {
        ...
        basePageSteps.onOfferCardPage().makePhone().link(CALL_BUTTON).click();
        basePageSteps.onOfferCardPage().makePhone().link(CALL_BUTTON)
    ...
    public void shouldSeeTwoPhones() {
        ...
        urlSteps.testing().path(OFFER).path(offerId).open();
        basePageSteps.onOfferCardPage().makePhone().spanLink(CALL_BUTTON).click();
