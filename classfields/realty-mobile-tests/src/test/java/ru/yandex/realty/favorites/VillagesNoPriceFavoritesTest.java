...
@Feature(VILLAGES)
@DisplayName("Видим скриншот сниппета КП без цены в избранном")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class VillagesNoPriceFavoritesTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Видим скриншот сниппета КП без цены в избранном")
    public void shouldSeeNoPriceVillageScreenshot() {
        mockRuleConfigurable.favoritesStub(favoritesTemplate().addItem(format("village_%s", STRING_ID)).build())
                .villageSearchStub(villageSearchTemplate().villages(asList(
                        mockVillage(VILLAGE_COTTAGE).setId(STRING_ID).clearPrices())).build()).createWithDefaults();
