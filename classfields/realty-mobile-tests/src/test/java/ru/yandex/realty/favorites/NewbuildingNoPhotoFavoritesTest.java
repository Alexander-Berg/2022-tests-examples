...
import static ru.yandex.realty.mock.MockSite.SITE_WITHOUT_PHOTO;
...
@Feature(NEWBUILDING)
@DisplayName("Видим скриншот сниппета новостройки без фото в избранном")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class NewbuildingNoPhotoFavoritesTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Видим скриншот сниппета новостройки без фото в избранном")
    public void shouldSeeNoPhotoSiteScreenshot() {
        mockRuleConfigurable.favoritesStub(favoritesTemplate().addItem(format("site_%s", INT_ID)).build())
                .offerWithSiteSearchStub(offerWithSiteSearchTemplate().sites(asList(
                        mockSite(SITE_WITHOUT_PHOTO).setId(INT_ID))).buildSite()).createWithDefaults();
