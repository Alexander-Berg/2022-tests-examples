...
@DisplayName("Показ телефона. Листинг")
@Feature(LISTING)
@Link("https://st.yandex-team.ru/VERTISTEST-1599")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class ListingTouchShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Клик на «Позвонить» в листинге")
    public void shouldSeePhoneNormalCaseListing() {
        ...
        basePageSteps.onMobileSaleAdsPage().offer(FIRST).link(CALL_BUTTON).click();
        basePageSteps.onMobileSaleAdsPage().offer(FIRST).link(CALL_BUTTON)
    ...
    public void shouldSeePhone500Listing() {
        mockRuleConfigurable
                .cardStub(cardTemplate().offers(asList(offer)).build())
                .cardWithViewsStub(cardWithViewsTemplate().offer(offer).build())
                .offerWithSiteSearchStub(offerWithSiteSearchTemplate().offers(asList(offer)).build())
        ...
        basePageSteps.onMobileSaleAdsPage().offer(FIRST).link(CALL_BUTTON).should(isDisplayed());
    }
    ...
    public void shouldSeeTwoPhoneListing() {
        ...
        urlSteps.testing().path(MOSKVA).path(KUPIT).path(KVARTIRA).open();
        basePageSteps.onMobileSaleAdsPage().offer(FIRST).button(CALL_BUTTON).click();
        basePageSteps.onMobileSaleAdsPage().phones().should(hasSize(phoneList.size()));
        assertThat(basePageSteps.onMobileSaleAdsPage().phones().stream().map(p -> p.getAttribute("href"))
