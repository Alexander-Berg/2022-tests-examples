...
public class ShortcutsModalCloseTest {

    public static final String SHORTCUT = "Расположение";
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Закрытие модуля тепловых карт")
    ...
    public void shouldSeeOfferCardScreenShot() {
        ...
        basePageSteps.onOfferCardPage().heatMapShortcut(SHORTCUT).waitUntil(isSelected());
        basePageSteps.onOfferCardPage().popupVisible().closeCrossHeader().click();
        basePageSteps.onOfferCardPage().heatMapShortcut(SHORTCUT).should(not(isDisplayed()));
    }
}
