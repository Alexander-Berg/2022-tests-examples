...
@Feature(AMP_FEATURE)
@DisplayName("amp. Хлебные крошки")
...
public class BreadCrumbsTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Переход по хлебным крошкам")
    public void shouldSeeBreadCrumbsLinkAmp() {
        urlSteps.testing().path(AMP).path(MOSKVA).path(KUPIT).path(KVARTIRA).open();
        basePageSteps.onAmpSaleAdsPage().breadCrumb(label).click();
        urlSteps.fromUri(urlSteps.testing().toString() + expected).ignoreParam(AMP_PARAMETER)
