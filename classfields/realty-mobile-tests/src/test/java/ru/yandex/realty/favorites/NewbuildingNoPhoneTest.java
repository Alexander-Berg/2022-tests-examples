...
@Feature(NEWBUILDING)
@DisplayName("Нет телефона у новостройки в Избранном")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class NewbuildingNoPhoneTest {
    ...
    public void before() {
        mockRuleConfigurable.favoritesStub(favoritesTemplate().addItem(format("site_%s", INT_ID)).build())
                .offerWithSiteSearchStub(offerWithSiteSearchTemplate().sites(asList(
                        mockSite(SITE_TEMPLATE).setId(INT_ID).clearDeveloperPhones())).buildSite())
                .createWithDefaults();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Нет «Позвонить» у новостройки без телефона в избранном")
    public void shouldNotSeeCallButtonNewbuildingFavorites() {
        basePageSteps.onFavoritesPage().offersList().get(FIRST).spanLink(CALL).should(not(isDisplayed()));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Видим скриншот новостройки без телефона в избранном")
    public void shouldSeeNoPhoneNewbuildingScreenshot() {
