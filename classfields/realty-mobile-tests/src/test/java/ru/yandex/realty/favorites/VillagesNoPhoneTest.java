...
@Feature(VILLAGES)
@DisplayName("Нет телефона у КП в избранном")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class VillagesNoPhoneTest {
    ...
    public void before() {
        mockRuleConfigurable.favoritesStub(favoritesTemplate().addItem(format("village_%s", STRING_ID)).build())
                .villageSearchStub(villageSearchTemplate().villages(asList(
                        mockVillage(VILLAGE_COTTAGE).setId(STRING_ID).clearDeveloperPhones())).build())
                .createWithDefaults();
    ...
    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Нет «Позвонить» у КП без телефона в избранном")
    public void shouldNotSeeCallButtonVillageFavorites() {
        basePageSteps.onFavoritesPage().offersList().get(FIRST).spanLink(CALL).should(not(isDisplayed()));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @DisplayName("Видим скриншот КП без телефона в избранном")
    public void shouldSeeNoPhoneVillageScreenshot() {
