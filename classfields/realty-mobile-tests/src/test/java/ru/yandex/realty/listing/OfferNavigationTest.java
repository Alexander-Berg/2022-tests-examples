...
@Feature(LISTING)
@DisplayName("Переходы на офферы")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebMobileModule.class)
public class OfferNavigationTest {

    private static final int OFFER_INDEX = 4;
    ...
    public void shouldSeeOfferUrl() {
        urlSteps.testing().path(MOSKVA).path(KUPIT).path(KVARTIRA).open();
        basePageSteps.scrollUntilExists(() -> basePageSteps.onMobileSaleAdsPage().offer(OFFER_INDEX));
        String offerId = basePageSteps.onMobileSaleAdsPage().offer(OFFER_INDEX).offerId();
        basePageSteps.scrollElementToCenter(basePageSteps.onMobileSaleAdsPage().offer(OFFER_INDEX));
        basePageSteps.onMobileSaleAdsPage().offer(OFFER_INDEX).click();
