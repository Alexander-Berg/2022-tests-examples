...
public class TabsTest {
    ...
    public void shouldSeeTabUrl() {
        ...
        mockRuleConfigurable.developerStub(ENHANCED_DEV_ID,
                mockEnhancedDeveloper().setTabs(
                        tradeInTabTemplate().setWebSiteUrl(url)).build()).createWithDefaults();
        urlSteps.testing().path(ENHANCED_DEV_GEO_ID_PATH).path(ZASTROYSCHIK).path(ENHANCED_DEV_ID).open();

        basePageSteps.onDeveloperPage().tabs().get(FIRST).link().should(hasHref(equalTo(url)));
    }
    ...
    public void shouldSeeTabPhone() {
        ...
        basePageSteps.scrollToElement(basePageSteps.onDeveloperPage().tabs().get(FIRST).link());
        basePageSteps.onDeveloperPage().tabs().get(FIRST).link().click();
        basePageSteps.onDeveloperPage().tabs().get(FIRST).link().should(hasHref(equalTo(
                format("tel:%s", PHONE_IN_BUTTON))));
    }
