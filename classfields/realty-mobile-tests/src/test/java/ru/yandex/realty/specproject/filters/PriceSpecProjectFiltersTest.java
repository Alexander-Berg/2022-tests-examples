...
public class PriceSpecProjectFiltersTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Фильтр цены от")
    public void shouldSeePriceFrom() {
        String price = String.valueOf(getNormalPrice());
        basePageSteps.onSpecProjectPage().filters().priceFrom().sendKeys(price);
        basePageSteps.onSpecProjectPage().filters().submitButton().click();
        urlSteps.queryParam(UrlSteps.PRICE_MIN_URL_PARAM, price).shouldNotDiffWithWebDriverUrl();
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Фильтр цены до")
    public void shouldSeePriceTo() {
        String price = String.valueOf(getNormalPrice());
        basePageSteps.onSpecProjectPage().filters().priceTo().sendKeys(price);
        basePageSteps.onSpecProjectPage().filters().submitButton().click();
        urlSteps.queryParam(UrlSteps.PRICE_MAX_URL_PARAM, price).shouldNotDiffWithWebDriverUrl();
    }
}
