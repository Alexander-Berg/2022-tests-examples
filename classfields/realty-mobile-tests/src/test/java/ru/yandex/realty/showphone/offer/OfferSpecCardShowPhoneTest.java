...
public class OfferSpecCardShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Клик на «Позвонить». Спецпроект")
    public void shouldSeePhoneNormalCaseMicrocardSpec() {
        ...
        basePageSteps.onOfferCardPage().makePhone().link().click();
        basePageSteps.onOfferCardPage().makePhone().link()
                .should(hasHref(equalTo(getHrefForPhone(TEST_PHONE))));
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Клик на «Позвонить». Показ двух телефонов в галерее. Спецпроект")
