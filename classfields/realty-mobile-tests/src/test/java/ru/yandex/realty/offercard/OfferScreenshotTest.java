...
public class OfferScreenshotTest {
    ...
    public static Collection<Object[]> getTestParameters() {
        return asList(new Object[][]{
                {"Скриншот карточки оффера. Обычный", mockOffer(SELL_APARTMENT)},
                {"Скриншот карточки оффера. Устаревший день назад",
                        mockOffer(SELL_APARTMENT).setObsolete(now().minusDays(1))},
                {"Скриншот карточки оффера. C повысившейся ценой", mockOffer(SELL_APARTMENT).setIncreasedPrice()},
                {"Скриншот карточки оффера. C понизившейся ценой", mockOffer(SELL_APARTMENT).setDecreasedPrice()}
        });
    }
    ...
    public void before() {
        mockRuleConfigurable.cardStub(cardTemplate().offers(asList(offer)).build())
                ...
                .createWithDefaults();
        basePageSteps.resize(400, 10000);
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Скриншот карточки оффера")
    @Description("ПОЧЕМУ РЕКЛАМА ЯНДЕКС ТАКСИ РАЗНАЯ")
    @Category({Regression.class, Mobile.class})
    public void shouldSeeOfferCardScreenShot() {
        ...
        urlSteps.setMobileProductionHost();
