...
func newStore(db *sql.DB, logger mocks.SpyLogger) Store {
	readerOpts := &clickhousespanstore.TraceReaderOptions{
		MaxSpans:        0,
		OperationsTable: testOperationsTable,
		IndexTable:      testIndexTable,
		SpansTable:      testSpansTable,
	}
	archiveReaderOpts := &clickhousespanstore.TraceReaderOptions{
		SpansTable: testSpansArchiveTable,
	}
	return Store{
		db: db,
		writer: clickhousespanstore.NewSpanWriter(logger, db, nil, testIndexTable, testSpansTable, clickhousespanstore.EncodingJSON, 0, 0, 0),
		reader: clickhousespanstore.NewTraceReader(readerOpts, db),
		archiveWriter: clickhousespanstore.NewSpanWriter(logger, db, nil, testIndexTable, testSpansArchiveTable, clickhousespanstore.EncodingJSON, 0, 0, 0),
		archiveReader: clickhousespanstore.NewTraceReader(archiveReaderOpts, db),
	}
}
