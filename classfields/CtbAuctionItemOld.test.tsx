...
import CtbAuctionItemOld from './CtbAuctionItemOld';
...
describe('CtbAuctionItemOld - компонент для отрисовки заявки для c2b-аукциона в старом лк', () => {
    it('Рендерит экран оценки', () => {
        ...
        expect(wrappers.every((wrapper) => wrapper.find('.C2bAuctionItemOldActive__stepTitle').text() === 'Оценка')).toBe(true);
    });
    ...
    it('Рендерит экран готовой ценки', () => {
        ...
        const wrappers = statuses.map(({ alg, status }) => renderComponent({ status, alg }));
        expect(wrappers.every((wrapper) => wrapper.find('.C2bAuctionItemOldActive__stepTitle').text() === 'Оценка готова')).toBe(true);
    });
    ...
    it('Рендерит экран сбора предложений', () => {
        ...
        const wrappers = statuses.map(({ alg, status }) => renderComponent({ status, alg }));
        expect(wrappers.every((wrapper) => wrapper.find('.C2bAuctionItemOldActive__stepTitle').text() === 'Сбор предложений')).toBe(true);
    });
    ...
    it('Рендерит экран итоговой цены', () => {
        ...
        const wrappers = statuses.map(({ alg, status }) => renderComponent({ status, alg }));
        expect(wrappers.every((wrapper) => wrapper.find('.C2bAuctionItemOldActive__stepTitle').text() === 'Итоговая цена')).toBe(true);
    });
    ...
    it('Рендерит экран осмотра', () => {
        ...
        const wrappers = statuses.map(({ alg, status }) => renderComponent({ status, alg }));
        expect(wrappers.every((wrapper) => wrapper.find('.C2bAuctionItemOldActive__stepTitle').text() === 'Осмотр')).toBe(true);
    });
    ...
    it('Рендерит экран состоявшейся сделки', () => {
        ...
        const wrappers = statuses.map(({ alg, status }) => renderComponent({ status, alg }));
        expect(wrappers.every((wrapper) => wrapper.find('.C2bAuctionItemOldFinished__stepDescription').text() === 'Сделка\nсостоялась')).toBe(true);
    });
    ...
    it('Рендерит номер телефона', () => {
        ...
        const wrappers = statuses.map(({ alg, status }) => renderComponent({ status, alg }));
        expect(wrappers.every((wrapper) => wrapper.find('.C2BAuctionLinkContacts__link').first().prop('url') === SUPPORT_PHONE_NUMBER.telLink)).toBe(true);
    });
});
