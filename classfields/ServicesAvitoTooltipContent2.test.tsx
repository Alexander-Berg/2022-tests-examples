...
import ServicesAvitoTooltipContent from './ServicesAvitoTooltipContent2';
...
describe('покажет текст про цену, если она пришла', () => {
    it('для услуг увеличения просмотров', () => {
        ...
        const buttons = tree.find('.ServicesAvitoTooltipContent2__viewsUpButton');
        expect([ buttons.at(0).dive().text(), buttons.at(1).dive().text() ]).toEqual([ '1 день за 155 ₽', '7 дней за 1 000 ₽' ]);
    });

    it('для услуг изменения внешнего вида', () => {
        const tree = shallowRenderComponent(appearanceMock);
        expect(tree.find('.ServicesAvitoTooltipContent2__appearancePrice')).toExist();
    });
});
...
describe('не покажет текст про цену, если она не пришла', () => {
    ...
    it('для услуг изменения внешнего вида', () => {
        ...
        expect(tree.find('.ServicesAvitoTooltipContent2__appearancePrice')).not.toExist();
    });
});
...
function shallowRenderComponent(serviceConfig: ServiceConfig) {
    ...
    />, { context: { pageParams: {}, metrika: { sendParams: jest.fn() } } });
}
