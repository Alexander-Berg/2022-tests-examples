...
it('должен развернуть отчёт по клику на "показать отчёт" для авторизованного юзера', () => {
    ...
    const link = wrapper.find('ForwardRef(CardVinReportTemplate)').dive()
        .find('CardVinReportCompactContent').dive()
        .find('.CardVinReportCompactContent__linkShowFree');
    link.simulate('click');
    expect((wrapper.state() as { showReport: boolean }).showReport).toBe(true);
});
...
it('не должен показывать бесплатный отчет, если нет бесплатных пунктов (мобила)', () => {
...
it('не должен показывать бесплатный отчёт, если передан проп showFreeBlock = false', () => {
    ...
    expect(wrapper.find('ForwardRef(CardVinReportTemplate)').dive().find('VinReportFreeBlock').exists()).toBe(false);
});

it('должен редиректить по клику на "показать отчёт" для неавторизованного юзера', () => {
    ...
    wrapper.find('ForwardRef(CardVinReportTemplate)').dive()
        ...
        .find('.CardVinReportCompactContent__linkShowFree')
        .simulate('click');
    expect(window.location.assign).toHaveBeenCalledWith('https://autoru_frontend.auth_domain/login/?r=http%3A%2F%2Flocalhost%2F');
});
...
it('должен передавать авторизацию в кнопки покупки отчёта', () => {
    ...
    wrapper.find('ForwardRef(CardVinReportTemplate)').dive()
        .find('CardVinReportCompactContent').dive()
        .find('.CardVinReportCompactContent__linkShowFree').simulate('click');
...
it('не должен закрыть модал авторизации после успешной авторизации, если это не просмотр бесплатного отчёта', () => {
    ...
    expect((wrapper.state() as { showReport: boolean }).showReport).toBe(false);
});
...
it('должен закрыть модал авторизации и показать отчёт после успешной авторизации', () => {
    ...
    wrapper.setProps({ dispatch: () => {}, isAuth: true } as any);
...
it('не должен перерисовать компонент после покупки отчёта', () => {
    ...
    const wrapper = renderComponent({ ...REQUIRED_PROPS }, createStore({ state: { tryToBuyReport: true }, user: userWithoutAuthMock }));
...
describe('распродажа отчетов VinReportSalePromo получает vinReportPaymentParams', () => {
    ...
    it('в гараже', () => {
        ...
        const wrapper = renderComponent({ ...REQUIRED_PROPS, vinReportPaymentParams });
...
describe('метрики', () => {
    ...
    it('если в адресе есть action=showVinReport, отправит метрику о показе', () => {
        ...
        renderComponent(REQUIRED_PROPS, createStore(storeDetailed));
        ...
        describe('отправка цели при клике на "показать бесплатный отчет"', () => {
            it('отправляет цель при клике на кнопку "показать бесплатный отчет"', () => {
                const wrapper = renderComponent(REQUIRED_PROPS).find('ForwardRef(CardVinReportTemplate)')
                    .dive().find('CardVinReportCompactContent').dive()
                    .find('.CardVinReportCompactContent__linkShowFree');
