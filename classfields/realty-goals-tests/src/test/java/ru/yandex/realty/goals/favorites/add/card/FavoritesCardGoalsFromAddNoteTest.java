...
import static ru.yandex.realty.element.offercard.MorePopup.ADD_NOTE;
import static ru.yandex.realty.element.popups.NotePopup.SAVE_NOTE;
...
public class FavoritesCardGoalsFromAddNoteTest {
    ...
    @Parameterized.Parameters(name = "{index}. {0}. Добавить заметку -> добавляется в избранное")
    ...
    @Description("КАКОЕ ПРАВИЛЬНО БУДЕТ «offer_base_info» или «offer_floating_info»?")
    @Test
    @Owner(KANTEMIROV)
    public void shouldSeeCardFavoritesAddNoteGoal() {
        basePageSteps.onOfferCardPage().offerCardSummary().moreButton().click();
        basePageSteps.onOfferCardPage().topMorePopup().actionRow(ADD_NOTE).click();
        basePageSteps.onOfferCardPage().notePopup().inputField().sendKeys(randomAlphabetic(10));
        proxy.clearHarUntilThereAreNoHarEntries();
        basePageSteps.onOfferCardPage().notePopup().button(SAVE_NOTE).click();
