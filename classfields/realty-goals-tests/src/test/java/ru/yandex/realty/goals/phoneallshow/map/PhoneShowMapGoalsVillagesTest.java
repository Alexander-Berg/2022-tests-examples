...
import static ru.yandex.realty.consts.GoalsConsts.Parameters.MAPSERP_VILLAGES_ITEM;
...
public class PhoneShowMapGoalsVillagesTest {
    ...
    public void before() {
        ...
        phoneParams = params()
                .placement(MAPSERP_VILLAGES_ITEM)
                .pageType(MAP)
                .page(VILLAGES)
                .payed(TRUE);
    }
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Карта коттеджных поселков. Клик «Показать телефон»")
    public void shouldSeeMapPhoneShowGoal() {
        ...
        goalsSteps.urlMatcher(containsString(PHONE_ALL_SHOW)).withGoalParams(
                goal().setPhone(phoneParams)).withIgnoringPaths(PHONE_OFFER_ID).shouldExist();
    }
}