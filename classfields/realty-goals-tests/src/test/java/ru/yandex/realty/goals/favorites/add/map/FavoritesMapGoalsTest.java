package ru.yandex.realty.goals.favorites.add.map;
...
public class FavoritesMapGoalsTest {
    ...
    public MockOffer offerWithSiteSearchResponse;
    ...
    public void before() {
        ...
        mockRuleConfigurable
                .cardStub(cardTemplate().offers(asList(offerWithSiteSearchResponse)).build())
                .cardWithViewsStub(cardWithViewsTemplate().offer(offerWithSiteSearchResponse).build())
                .offerWithSiteSearchStub(offerWithSiteSearchTemplate().offers(asList(offerWithSiteSearchResponse))
                        .build()).createWithDefaults();
        basePageSteps.setWindowSize(1400, 1800);
        urlSteps.testing().path(MOSKVA).path(path).path(KARTA)
    ...
    public void shouldSeeMapFavoritesAddGoal() {
        basePageSteps.onMapPage().sidebar().snippetOffer(FIRST).buttonWithTitle(ADD_TO_FAV).click();
    ...
    @Test
    @Owner(KANTEMIROV)
    public void shouldSeeMapFavoritesAddNoteGoal() {
        basePageSteps.onMapPage().sidebar().snippetOffer(FIRST).buttonWithTitle(YOUR_NOTE).click();
        basePageSteps.onMapPage().sidebar().snippetOffer(FIRST).input().sendKeys(randomAlphabetic(10));
        basePageSteps.onMapPage().sidebar().snippetOffer(FIRST).noteSaveButton().click();
        goalsSteps.urlMatcher(containsString(FAVORITES_ADD_GOAL)).withGoalParams(
                goal().setFavorites(favoritesAddParams)).shouldExist();
    }
}