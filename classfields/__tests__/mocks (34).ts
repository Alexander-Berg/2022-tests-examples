...
import { IDevelopersListStore } from 'realty-core/view/react/modules/developers-list/redux/types';
...
export interface IGetDevelopersListOpts {
    currentPage?: number;
    pageCount?: number;
    noDevelopers?: boolean;
}

export const getDevelopersListMock = (opts?: IGetDevelopersListOpts): IDevelopersListStore => {
    const { currentPage = 1, pageCount = 1, noDevelopers = false } = opts ?? {};

    return {
        developers: noDevelopers ? undefined : getDevelopers(),
        paging: {
            page: {
                num: 1,
                size: 10,
