...
import flatStyles from '../../SearchFlatsByPresetFilters/styles.module.css';
...
const selectors = {
    ...
    select: `.${flatStyles.select} button`,
    selectors: {
        itemN: (n: number) => `.Menu__item:nth-of-type(${n})`,
    },
    findButton: `.${flatStyles.submit}`,
};
...
const FlatsComponent: React.FunctionComponent<{ store: DeepPartial<IUniversalStore> }> = ({ store }) => (
    <AppProvider rootReducer={rootReducer} initialState={store}>
        <ManagerSearch tab={ManagerSearchTab.PRESETS} />
    </AppProvider>
);

describe('ManagerSearchFlatsByPreset', () => {
    ...
    describe('смена фильтров', () => {
        renderOptions.forEach((option) => {
            it(`${option.viewport.width}px`, async () => {
                ...
                await page.click(selectors.select);

                expect(await takeScreenshot({ fullPage: true })).toMatchImageSnapshot();

                await page.click(selectors.selectors.itemN(3));
