...
import { NewbuildingCardVirtualTours } from '../';
...
describe('NewbuildingCardVirtualTours', () => {
    it(`Корректно рендерит большое количество туров`, async () => {
        for (const viewport of viewports) {
            await render(
                <AppProvider>
                    <NewbuildingCardVirtualTours
                        card={getSiteCardMock({ toursCount: 5, overviewsCount: 5 }) as ISiteCard}
                    />
                </AppProvider>,
                { viewport }
            );
    ...
    it(`Не рендерит один из блоков при отсутствии данных`, async () => {
        for (const viewport of viewports) {
            await render(
                <AppProvider>
                    <NewbuildingCardVirtualTours
                        card={getSiteCardMock({ toursCount: 0, overviewsCount: 5 }) as ISiteCard}
                    />
                </AppProvider>,
                { viewport }
            );
    ...
    it(`Корректно рендерит сетку для 1 тура`, async () => {
        for (const viewport of viewports) {
            await render(
                <AppProvider>
                    <NewbuildingCardVirtualTours
                        card={getSiteCardMock({ toursCount: 1, overviewsCount: 2 }) as ISiteCard}
                    />
                </AppProvider>,
                { viewport }
            );
