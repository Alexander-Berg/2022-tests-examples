...
import { IDevelopersListFiltersBaseProps, IDevelopersListState } from '../../types';
...
const getDevelopers = (): IDevelopersListData[] => {
    return [
        {
            id: '11111',
            name: 'Застройщик 1',
...
interface IDevelopersListFiltersMockOpts {
    hasDisabledBuildingClasses?: boolean;
}

export const getInitialStateMock = (opts: IDevelopersListFiltersMockOpts = {}) => ({
    geo,
    developersList: {
        ...
        currentPage: 1,
        dynamicFilters: {
            buildingClass: opts.hasDisabledBuildingClasses
                ? ['COMFORT_PLUS']
                : ['ECONOM', 'ELITE', 'COMFORT', 'BUSINESS', 'COMFORT_PLUS'],
            regions: [],
        },
        suggestions: [],
    },
...
const getFiltersMock = (hasParams = false): IDevelopersListState => ({
    selectedBuildingClass: hasParams ? 'COMFORT_PLUS' : '',
    developer: {},
    yearsRange: hasParams ? [5, 10] : [null, null],
    selectedFilters: hasParams ? [FiltersNames.selectedBuildingClass, FiltersNames.yearsRange] : [],
});
...
export const getComponentProps = (opts: { hasParams?: boolean } = {}): IDevelopersListFiltersBaseProps => ({
    filters: getFiltersMock(opts.hasParams),
    matchesCounter: 50,
    onFilterValueChange,
    onSubmit,
});
