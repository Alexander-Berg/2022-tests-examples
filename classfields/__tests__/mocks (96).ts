...
import { ISitePlansOffersSerpProps } from '../';
...
export const getInitialState = () => ({
    user: {
        favorites: ['6258444696742167171'],
        favoritesMap: {
            '6258444696742167171': true,
        },
    },
});
...
export const getProps = (apartmentType: ApartmentType, withPlan = false) => {
    const offers = getOffers();

    return ({
        card: getSiteCard(apartmentType),
        offers,
        plan: withPlan ? getPlan() : undefined,
        totalOffers: offers.length,
        sort: SitePlansOffersSortTypes.FLOOR,
    } as unknown) as ISitePlansOffersSerpProps;
};
