import { ISiteSnippetType } from 'realty-core/types/siteSnippet';

const imageURL = 'site.com/image.png';
const generalType = 'GENERAL';

function getSiteImages(count: number) {
    return {
        images: Array(count).fill(imageURL),
        appMiddleImages: Array(count).fill(imageURL),
        appLargeImages: Array(count).fill(imageURL),
        viewTypes: Array(count).fill(generalType),
    };
}

const commonSiteData = {
    price: {
        rooms: {},
    },
    flatStatus: 'ON_SALE' as const,
    siteSpecialProposals: [],
};

export const siteWithOneMetro = ({
    id: 1,
    name: 'Вишневый сад',
    fullName: 'ЖК «Вишневый сад»',
    location: {
        rgid: 193290,
        settlementRgid: 165705,
        populatedRgid: 741964,
        address: 'Москва, ул. Выборгская / ул. Адмирала Макарова',
        metro: {
            metroCityRgid: 741964,
            metroGeoId: 20370,
            rgbColor: '4f8242',
            metroTransport: 'ON_FOOT',
            name: 'Водный Стадион',
            timeToMetro: 7,
            lineColors: ['4f8242'],
        },
        subjectFederationRgid: 1,
    },
    developers: [
        {
            id: 32522,
            name: 'Концерн «КРОСТ»',
        },
    ],
    ...getSiteImages(1),
    ...commonSiteData,
} as unknown) as ISiteSnippetType;

export const siteWithSeveralMetro = ({
    id: 2,
    name: 'Невский',
    fullName: 'ЖК «Невский»',
    location: {
        rgid: 193290,
        settlementRgid: 165705,
        populatedRgid: 741964,
        address: 'Москва, ул. Заречная, вл. 2/1',
        metro: {
            metroCityRgid: 741964,
            metroGeoId: 20459,
            rgbColor: '099dd4',
            metroTransport: 'ON_FOOT',
            name: 'Фили',
            timeToMetro: 15,
            lineColors: ['099dd4', 'ed9f2d'],
        },
        subjectFederationRgid: 1,
    },
    developers: [
        {
            id: 324,
            name: 'Группа Компаний ПИК',
        },
    ],
    ...getSiteImages(3),
    ...commonSiteData,
} as unknown) as ISiteSnippetType;

export const siteWithSeveralDevelopers = ({
    id: 3,
    name: 'Счастье есть',
    fullName: 'ЖК «Счастье есть»',
    location: {
        rgid: 193290,
        settlementRgid: 165705,
        populatedRgid: 741964,
        address: 'Москва, пос. Московский, Родниковая ул.',
        metro: {
            metroCityRgid: 741964,
            metroGeoId: 144826,
            rgbColor: 'e4402d',
            metroTransport: 'ON_TRANSPORT',
            name: 'Саларьево',
            timeToMetro: 9,
            lineColors: ['e4402d'],
        },
        subjectFederationRgid: 1,
    },
    developers: [
        {
            id: 23,
            name: 'Элита',
        },
        {
            id: 56,
            name: 'Строим как можем',
        },
    ],
    ...getSiteImages(8),
    ...commonSiteData,
} as unknown) as ISiteSnippetType;
