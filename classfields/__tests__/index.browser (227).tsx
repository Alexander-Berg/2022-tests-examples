...
async function fillForm({
    ...
    await [lastName, firstName, middleName, email, phone].reduce((acc, text, index) => {
        return acc.then(() => page.type(`.${styles.applicationScreenInput}:nth-of-type(${index + 1}) input`, text));
    }, Promise.resolve());

    return page.click(`.${styles.applicationScreenCheckbox}`);
}

describe('MortgageApplicationForm (touch)', () => {
    ...
    it('рисует ошибку при вводе неправильного кода на экране подтверждения', async () => {
        ...
        await fillForm({});
        await page.evaluate(disableSetInterval);
        await page.click(`.${styles.applicationScreenSubmitButton}`);
...
/**
 * Отключение setInterval, чтобы таймер не влиял на прохождение теста
 */
function disableSetInterval() {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unused-vars
    window.setInterval = (_func: TimerHandler, _interval: number | undefined) => 0 as any;
}
