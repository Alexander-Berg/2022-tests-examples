...
import type { OwnProps } from './ModalVinReport';
import ModalVinReport from './ModalVinReport';
...
const REQUIRED_PROPS: OwnProps = {
    ...
    place: 'seller_popup',
    freeItemsOrder: [ 'pts_info' ],
    paidItemsOrder: [ 'mileages_graph' ],
};
...
beforeEach(() => {
    context = _.cloneDeep({
        ...contextMock,
        hasExperiment: (exp: string) => exp === 'AUTORUFRONT-21143_report_before_call',
    } as typeof contextMock);
    store = createStore();
});

const renderComponent = (props: OwnProps, customStore?: any) => {
    return shallow(
        <ModalVinReport { ...props }/>,
        { context: { ...context, store: customStore || store } },
    ).dive().find('Loadable').dive().find('CardVinReport').dive().find('Connect(CardVinReport)').dive().dive();
};
...
it('не должен показывать бесплатный отчет, если нет бесплатных пунктов', () => {
    ...
    const wrapper = renderComponent(REQUIRED_PROPS, createStore({ vinReport: { data: cardVinReport } }));
    ...
    expect(freeItemElements).not.toExist();
});
...
describe('не показываем ничего', () => {
    const renderComponent = (customStore = store, customProps?: OwnProps): ReturnType<typeof shallow> => {
        return shallow(
            <ModalVinReport { ...(customProps || REQUIRED_PROPS) }/>,
            { context: { ...context, store: customStore } },
        ).dive();
    };

    it('если отчёт оплачен', () => {
        const cardVinReportPaid = _.cloneDeep(cardVinReportFree);
        cardVinReportPaid.report!.report_type = ReportType.PAID_REPORT;
        delete cardVinReportPaid.billing;

        const customStore = createStore({ user: userWithoutAuthMock, vinReport: { data: cardVinReportPaid } });

        expect(renderComponent(customStore)).toBeEmptyRender();
    });

    it('если отчёт есть, но у него невалидный для показа в попапе статус', () => {
        const vinReport = {
            data: {
                report: {
                    ...cardVinReportFree.report,
                    status: Status.IN_PROGRESS,
                },
            },
        };
        const customStore = createStore({ user: userWithoutAuthMock, vinReport });

        expect(renderComponent(customStore)).toBeEmptyRender();
    });

    it('если отчёт только грузится', () => {
        const customStore = createStore({ user: userWithoutAuthMock, vinReport: { data: { fetching: true } } });
...
it('должен отправить событие paid_report_view_event во фронтлог', () => {
    ...
    (wrapper
        ...
        .find('CardVinReportCompactContent').dive()
...
describe('кнопка покупки отчета', () => {
    it('должен отрендерить CardVinReportButtonsUnauthorized для неавторизованного юзера', () => {
        ...
        const button = wrapper.find('ForwardRef(CardVinReportTemplate)').dive()
            .find('CardVinReportCompactContent').dive()
            .find('CardVinReportButtonsUnauthorized');
...
describe('распродажа отчетов', () => {
    it('не рисует плашку о распродаже', () => {
        const wrapper = renderComponent(REQUIRED_PROPS, createStore({ user: userWithoutAuthMock }));

        expect(wrapper.find('Connect(VinReportSalePromo)')).not.toExist();
    });
});

[
    {
        label: 'в экспе из попапа с телефоном',
        metrika_mini: 'exp_history_report_mini',
        metrika_full: 'exp_history_report_full',
        place: 'seller_popup' as OwnProps['place'],
    },
    {
        label: 'не в экспе',
        metrika_mini: 'history_report_mini',
        metrika_full: 'history_report_full',
        place: 'fullscreen_gallery' as OwnProps['place'],
    },
].forEach((testCase) => {
    describe('метрики ' + testCase.label, () => {
        it('если в адресе нет action=showVinReport, отправит метрику о показе мини', () => {
            renderComponent({ place: testCase.place, offer: cloneOfferWithHelpers(offer).withIsOwner(false).value() });
            expect(context.metrika.sendPageEvent).toHaveBeenCalled();
            expect(context.metrika.sendPageEvent.mock.calls[0][0]).toStrictEqual([ testCase.metrika_mini, 'view', 'free', 'not_owner' ]);
        });
        ...
        it('если в адресе есть action=showVinReport, отправит метрику о показе', () => {
            ...
            renderComponent({ place: testCase.place, offer: cloneOfferWithHelpers(offer).withIsOwner(false).value() }, customStore);

            expect(context.metrika.sendPageEvent).toHaveBeenCalled();
            expect(context.metrika.sendPageEvent.mock.calls[0][0]).toStrictEqual([ testCase.metrika_full, 'view', 'free', 'not_owner' ]);
        });
        ...
        describe('клик "показать бесплатный отчет"', () => {
            ...
            describe('на десктопе', () => {
                it('незалогин', () => {
                    renderWrapper({ ...REQUIRED_PROPS, place: testCase.place }, createStore({ user: userWithoutAuthMock }));

                    expect(context.metrika.sendPageEvent).toHaveBeenCalledTimes(2);
                    expect(context.metrika.sendPageEvent.mock.calls[1][0]).toStrictEqual([ testCase.metrika_mini, 'click_show_free_report', 'no_login' ]);
                });

                it('не владелец', () => {
                    renderWrapper({ place: testCase.place, offer: cloneOfferWithHelpers(offer).withIsOwner(false).value() });

                    expect(context.metrika.sendPageEvent).toHaveBeenCalledTimes(3);
                    expect(context.metrika.sendPageEvent.mock.calls[2][0]).toStrictEqual([ testCase.metrika_mini, 'click_show_free_report', 'not_owner' ]);
                });

                it('владелец', () => {
                    renderWrapper({ place: testCase.place, offer: cloneOfferWithHelpers(offer).withIsOwner(true).value() });

                    expect(context.metrika.sendPageEvent).toHaveBeenCalledTimes(3);
                    expect(context.metrika.sendPageEvent.mock.calls[2][0]).toStrictEqual([ testCase.metrika_mini, 'click_show_free_report', 'owner' ]);
                });
            });
        });
    });
