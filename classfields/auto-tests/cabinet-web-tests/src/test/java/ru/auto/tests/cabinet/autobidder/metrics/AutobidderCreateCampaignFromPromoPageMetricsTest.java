...
public class AutobidderCreateCampaignFromPromoPageMetricsTest {

    private static final String CREATE_CAMPAIGN = "{\"auction-used-autobidder\":{\"create_campaign\":{}}}";
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("«create_campaign» при создании кампании из блока описания на промо странице")
    public void shouldSeeCreateCampaignMetricFromDescriptionBlockPromoPage() {
        updateMockWithCampaignCreation();

        steps.onAutobidderPage().promoDescription().button(CREATE_FIRST_CAMPAIGN).click();

        browserMockSteps.assertWithWaiting(onlyOneCabinetMetricsRequest(hasSiteInfo(CREATE_CAMPAIGN)));
    }

    @Test
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("«create_campaign» при создании кампании из блока преимуществ на промо странице")
    public void shouldSeeCreateCampaignMetricFromAdvantageBlockPromoPage() {
        updateMockWithCampaignCreation();
