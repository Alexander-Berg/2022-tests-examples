...
@DisplayName("Создаем кампанию из таблицы")
@GuiceModules(CabinetTestsModule.class)
@RunWith(GuiceTestRunner.class)
public class AutobidderCreateCampaignFromTableTest {

    String NEW_CAMPAIGN_ID = String.valueOf(getRandomShortInt());
                ...
                stub().withPostDeepEquals(DEALER_AUCTION_CARS_USED_PROMO_CAMPAIGN)
                        .withResponseBody(getJsonObject(
                                promoCampaign().setCampaign(
                                        getBaseCampaign().setId(NEW_CAMPAIGN_ID)))),
    ...
    public void shouldCreateCampaign() {
        steps.onAutobidderPage().button(AutobidderPage.CREATE_CAMPAIGN).click();

        steps.onAutobidderPage().extendedRow().should(isDisplayed());
        steps.onAutobidderPage().rows().should(hasSize(2));
        steps.onAutobidderPage().rows().get(0).title().should(hasText(
                format("#%s:", NEW_CAMPAIGN_ID)));
    }
