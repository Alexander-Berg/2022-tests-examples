...
@Feature(AUTOBIDDER)
@Story(METRICS)
@DisplayName("campaign_delete")
@GuiceModules(DesktopDevToolsTestsModule.class)
@RunWith(GuiceTestRunner.class)
public class AutobidderDeleteCampaignMetricsTest {

    private static final String DELETE_CAMPAIGN = "{\"auction-used-autobidder\":{\"campaign\":{\"delete\":{}}}}";
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("«campaign_delete» при удалении кампании")
    public void shouldSeeCampaignDeleteMetricFromPause() {
        steps.onAutobidderPage().rows().get(0).delete().click();

        browserMockSteps.assertWithWaiting(onlyOneCabinetMetricsRequest(hasSiteInfo(DELETE_CAMPAIGN)));
    }
