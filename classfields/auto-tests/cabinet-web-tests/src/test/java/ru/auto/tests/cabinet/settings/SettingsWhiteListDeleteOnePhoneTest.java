...
public class SettingsWhiteListDeleteOnePhoneTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/SessionAuthDealer",
                ...
                "cabinet/DealerPhonesWhitelistDeleteOnePhone").post();
    ...
    @Owner(TIMONDL)
    @DisplayName("Удаляем один конкретный телефон")
    public void shouldDeleteOnePhone() {
        mockRule.overwriteStub(3, "cabinet/DealerPhonesWhitelistGetEmpty");

        steps.onSettingsWhiteListPage().getPhone(0).hover();
        steps.onSettingsWhiteListPage().getPhone(0).button("Удалить").should(isDisplayed()).click();
        steps.onSettingsWhiteListPage().popup().should(isDisplayed())
                .should(hasText("Вы действительно хотите удалить номер +7 111 000-11-11\nДа\nНет"));
