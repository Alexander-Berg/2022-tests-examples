...
public class CalculatorKomTCBlockWithTariffOnTest {
    ...
    @Owner(JENKL)
    @DisplayName("Блок «Коммерческий транспорт». Выключение тарифа")
    public void shouldSeeTurnOffTariff() {
        ...
        mockRule.newMock().with("cabinet/Session/DirectDealerRegions",
                ...
                "cabinet/ApiServiceAutoruAdsRequestCommercialClientDelete").post();

        steps.onNewCalculatorPage().komTCBlock().button("Отключить тариф").click();
    ...
    public void shouldSeeServicesByActiveTariff() {
        ...
        int prevComTc = steps.extractSum(steps.onNewCalculatorPage().balance().summary("Коммерческий транспорт"));
        ...
        steps.shouldSeeAmountIncrease(prevServInCat, steps.onNewCalculatorPage().balance()
                .summary(Balance.SERVICES_IN_CATEGORIES_SUMMARY));
        steps.shouldSeeAmountIncrease(prevComTc, steps.onNewCalculatorPage().balance().summary("С\u00a0пробегом"));
