...
public class ShivaTest {
    ...
    private static final String URL_TEMPLATE = "https://cabinet.%s/release";
    ...
    public void shouldSeeDifferentPackages() {
        urlSteps.fromUri(format(URL_TEMPLATE, urlSteps.getConfig().getBaseDomain())).open();

        String testingPackage = basePageSteps.onBasePage().body().should(hasText(startsWith("af-cabinet="))).getText();

        cookieSteps.setCookieForBaseDomain(urlSteps.getConfig().getBranchCookieName(), "production");
        urlSteps.refresh();

        String prodPackage = basePageSteps.onBasePage().body().should(hasText(startsWith("af-cabinet="))).getText();
    ...
    public void shouldSeeSamePackages() {
        ...
        cookieSteps.setCookie("_branch", "autotest",
                format(".%s", urlSteps.getConfig().getAutoruProdDomain()));
        urlSteps.refresh();

        String autotestPackage = basePageSteps.onBasePage().body().should(hasText(startsWith("af-cabinet="))).getText();
