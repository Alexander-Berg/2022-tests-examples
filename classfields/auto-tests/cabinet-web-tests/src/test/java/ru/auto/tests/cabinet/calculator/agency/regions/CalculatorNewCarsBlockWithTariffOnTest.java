...
public class CalculatorNewCarsBlockWithTariffOnTest {
    ...
    @Category({Regression.class, Testing.class})
    @Owner(JENKL)
    @DisplayName("Блок «Легковые новые». Отключить тариф")
    public void shouldSeeTurnOffTariff() {
        ...
        steps.onNewCalculatorPage().newCarsBlock().activeTariff().cancel().click();
        assertThat(steps.extractSum(steps.onNewCalculatorPage().balance().summary("Легковые новые")))
                .isZero().describedAs("Проверяем что сводка «Легковые новые» равна нулю");
        steps.onNewCalculatorPage().balance().replenish().should(not(isDisplayed()));
        assertThat(steps.extractSum(steps.onNewCalculatorPage().newCarsBlock().amount()))
