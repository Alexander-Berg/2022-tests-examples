...
@Feature(AUTOBIDDER)
@Story(METRICS)
@DisplayName("campaign_refresh")
@GuiceModules(DesktopDevToolsTestsModule.class)
@RunWith(GuiceTestRunner.class)
public class AutobidderRefreshCampaignMetricsTest {

    private static final String REFRESH_CAMPAIGN = "{\"auction-used-autobidder\":{\"campaign\":{\"refresh\":{}}}}";
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("«campaign_refresh» при обновлении кампании")
    public void shouldSeeCampaignRefreshMetric() {
        steps.onAutobidderPage().rows().get(0).click();
        steps.onAutobidderPage().extendedRow().filters().hover();
        steps.onAutobidderPage().extendedRow().button(SAVE_CAMPAIGN).click();

        browserMockSteps.assertWithWaiting(onlyOneCabinetMetricsRequest(hasSiteInfo(REFRESH_CAMPAIGN)));
    }
