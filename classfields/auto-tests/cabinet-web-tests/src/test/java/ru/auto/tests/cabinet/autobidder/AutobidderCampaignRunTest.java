...
import static ru.auto.tests.desktop.element.cabinet.autobidder.Filters.IN_STOCK;
...
@DisplayName("Запуск кампании")
@GuiceModules(CabinetTestsModule.class)
@RunWith(GuiceTestRunner.class)
public class AutobidderCampaignRunTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Запускаем базовую кампанию, без фильтров")
    public void shouldRunBaseCampaign() {
    ...
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Добавляем фильтры в кампанию, запускаем")
    public void shouldRunChangedCampaign() {
        ...
        steps.onAutobidderPage().extendedRow().filters().radioButton(IN_STOCK).click();
