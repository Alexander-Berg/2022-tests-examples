...
@DisplayName("Кабинет дилера. Калькулятор. Агентский дилер(Москва). Мото")
...
public class CalculatorMotoBlockWithTariffOnTest {
    ...
    public void shouldSeeTurnOffTariff() {
        ...
        steps.onNewCalculatorPage().motoBlock().activeTariff().cancel().click();
        assertThat(steps.extractSum(steps.onNewCalculatorPage().balance().summary("Мото")))
                .isZero().describedAs("Проверяем что сводка «Мото» равна нулю");
        steps.onNewCalculatorPage().balance().replenish().should(not(isDisplayed()));
        assertThat(steps.extractSum(steps.onNewCalculatorPage().motoBlock().amount()))
