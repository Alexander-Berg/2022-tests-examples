...
@DisplayName("Кабинет дилера. Снова в продаже. Стартовая страница")
@Epic(CABINET_DEALER)
...
public class StartPageTest {
    ...
    @Category({Regression.class, Testing.class})
    @Owner(SUCHKOVDENIS)
    @DisplayName("Клик по кнопке «Перейти в раздел»")
    public void shouldClickGoToSectionButton() {
        basePageSteps.onCabinetOnSaleAgainPage().button("Перейти в раздел").click();

        basePageSteps.onCabinetOnSaleAgainPage().filters().should(isDisplayed());
        basePageSteps.onCabinetOnSaleAgainPage().sortsBlock().should(isDisplayed());
        basePageSteps.onCabinetOnSaleAgainPage().listing().should(isDisplayed());
    }
