...
import ru.auto.tests.desktop.mock.MockDealerPriceOffer;
...
public class PriceReportOfferSalesTest {
    ...
    private static final int INITIAL_DISCOUNT = getRandomBetween(3000, 5000);
    private static final int CHANGED_DISCOUNT = getRandomBetween(3000, 5000);
    private static final int DELETED_DISCOUNT = 0;
    ...
    public MockDealerPriceOffer mockDealerPriceOffer;

    @Parameterized.Parameter(2)
    public DiscountOptions changedDiscountOption;

    @Parameterized.Parameter(3)
    public DiscountOptions deletedDiscountOption;

    @Parameterized.Parameters(name = "{index}: {0}")
    public static Collection<Object[]> getParameters() {
        return asList(new Object[][]{
                {MAXIMUM, dealerPriceOffer().setMaxDiscount(INITIAL_DISCOUNT),
                        discountOptions().setMaxDiscount(CHANGED_DISCOUNT),
                        discountOptions().setMaxDiscount(DELETED_DISCOUNT)
                },
                {TRADE_IN, dealerPriceOffer().setTradein(INITIAL_DISCOUNT),
                        discountOptions().setTradein(CHANGED_DISCOUNT),
                        discountOptions().setTradein(DELETED_DISCOUNT),
                },
                {CREDIT, dealerPriceOffer().setCredit(INITIAL_DISCOUNT),
                        discountOptions().setCredit(CHANGED_DISCOUNT),
                        discountOptions().setCredit(DELETED_DISCOUNT),
                },
                {KASKO, dealerPriceOffer().setInsurance(INITIAL_DISCOUNT),
                        discountOptions().setInsurance(CHANGED_DISCOUNT),
                        discountOptions().setInsurance(DELETED_DISCOUNT),
                }
        });
    }
    ...
    public void shouldSeeDiscount() {
        ...
        steps.onPriceReportPage().tableRows().get(0).offersTable().offers().get(0).inputBlock(blockName).input().should(
                hasValue(formatNumber(INITIAL_DISCOUNT)));
    }

    @Test
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Устанавливаем скидку")
    public void shouldSetDiscount() {
        ...
        steps.onPriceReportPage().notifier(SALE_SUCCESSFULLY_UPDATED).should(isDisplayed());
        steps.onPriceReportPage().tableRows().get(0).offersTable().offers().get(0).inputBlock(blockName).input().should(
                hasValue(formatNumber(CHANGED_DISCOUNT)));
    }

    @Test
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Меняем скидку")
    public void shouldChangeDiscount() {
        mockRule.setStubs(
                stub().withGetDeepEquals(DEALER_REPORT_PRICE_LISTING)
                        .withRequestQuery(
                                getPriceListingQuery())
        ...
        steps.onPriceReportPage().tableRows().get(0).offersTable().offers().get(0).inputBlock(blockName).clearInput();
        steps.onPriceReportPage().tableRows().get(0).offersTable().offers().get(0).inputBlock(blockName).input().sendKeys(
                String.valueOf(CHANGED_DISCOUNT));
        steps.onPriceReportPage().tableRows().get(0).offersTable().offers().get(0).inputBlock(WITH_DISCOUNT).click();
    ...
    public void shouldDeleteDiscount() {
        ...
        mockRule.setStubs(
                stub().withPostDeepEquals(format("%s/%s/attribute", USER_OFFERS_CARS, ID))
                        .withRequestBody(getJsonObject(
                                discountOptionsRequest().setDiscountOptions(deletedDiscountOption)))
                        .withStatusSuccessResponse()
        ).update();
