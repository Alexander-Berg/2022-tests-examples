...
@Feature(SALES_WIGET)
@DisplayName("EMBED: мобильный виджет карусели с объявлениями")
@Owner(NATAGOLOVKINA)
@RunWith(GuiceTestRunner.class)
@GuiceModules(MobileTestsModule.class)

public class WidgetListingCarouselTestMobile {
    ...
    public void before() {
        ...
        urlSteps.subdomain(SUBDOMAIN_PROMO).path(EMBED).path(LISTING_CAROUSEL)
                ...
                .addParam("deviceType", "mobile").open();
    }
    ...
    public void shouldClickSale() {
        ...
        urlSteps.mobileURI().path(CARS).path(USED).path(SALE).path(MARK.toLowerCase()).path(MODEL.toLowerCase())
                .path(OFFER)
    ...
    public void shouldClickListingLink() {
        ...
        basePageSteps.switchToNextTab();
        urlSteps.mobileURI().path(MOSKVA).path(CARS).path(MARK.toLowerCase()).path(MODEL.toLowerCase()).path(ALL)
                .addParam("deviceType", "mobile")
