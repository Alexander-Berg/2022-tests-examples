...
public class MotorcyclesVasColorSpecialTopTest {
    ...
    @DisplayName("Добавление объявления с услугами «Выделение цветом», «Спецпредложение», «Поднятие в ТОП»")
    public void shouldAddSaleWithColorSpecialTop() {
        formsSteps.onFormsPage().userVas().getSnippet(2).option("Выделение цветом").checkbox().click();
        formsSteps.onFormsPage().userVas().getSnippet(2).option("Спецпредложение").checkbox().click();
        formsSteps.onFormsPage().userVas().getSnippet(2).option("Поднятие в ТОП").checkbox().click();
        ...
        formsSteps.onFormsPage().billingPopup().header().waitUntil(hasText("Наборный пакет"));
        formsSteps.onFormsPage().billingPopup().priceHeader().waitUntil(hasText("2 733 ₽"));
        ...
        createOfferSteps.setOfferTemplate("offers/motoColorSpecialTopOffer.json");
