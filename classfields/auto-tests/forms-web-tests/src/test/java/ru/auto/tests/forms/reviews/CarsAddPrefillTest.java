...
public class CarsAddPrefillTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                "desktop/SearchCarsBreadcrumbsFordFocus",
                "desktop/ReferenceCatalogCarsSuggestFordFocus",
                "forms/ReviewsAutoCarsOffer",
                "forms/UserReviewsReviewIdCars").post();

        formsSteps.setWindowHeight(3000);
        urlSteps.testing().path(CARS).path(REVIEWS).path(ADD).path(OFFER_ID)
    ...
    @DisplayName("Предзаполнение отзыва")
