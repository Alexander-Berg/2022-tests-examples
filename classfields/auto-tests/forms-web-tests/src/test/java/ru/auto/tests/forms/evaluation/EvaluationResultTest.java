...
public class EvaluationResultTest {
    ...
    public void shouldSeeHelp() {
        formsSteps.onFormsEvaluationPage().evaluationResult().snippet("Дилеру в трейд-ин").helpIcon().hover();
        ...
        formsSteps.onFormsEvaluationPage().evaluationResult().snippet("Продажа на Авто.ру").helpIcon().hover();
        ...
        urlSteps.testing().path(MOSKVA).path(CARS).path("/hyundai/solaris/2020-year/20162370/20162385/20162567/")
    ...
    public void shouldClickSellButton() {
        ...
        urlSteps.testing().path(CARS).path(USED).path(ADD).addParam("from", "evaluation")
                .shouldNotSeeDiff();
        formsSteps.onPofferPage().blockMMM()
                .waitUntil(hasText(startsWith("Hyundai/Solaris/2020 Седан II Рестайлинг /1.4 AT (100 л.с.) Очистить")));
    }
    ...
    public void shouldClickSale() {
        ...
        urlSteps.switchToNextTab();
        urlSteps.testing().path(CARS).path(USED).path(SALE).path(MARK).path(MODEL).path("/1114848283-bebe3d0d/")
                .shouldNotSeeDiff();
    }
    ...
    public void shouldClickAllSalesButton() {
        formsSteps.onFormsEvaluationPage().evaluationResult().button("Смотреть все объявления")
                .waitUntil(isDisplayed()).click();
        urlSteps.switchToNextTab();
        urlSteps.testing().path(MOSKVA).path(CARS).path(MARK).path(MODEL).path("/2020-year/20162370/20162385/20162567/")
    ...
    public void shouldSwitchLists() {
        ...
        formsSteps.onFormsEvaluationPage().evaluationResult().radioButton("15\u00a0объявлений").click();
