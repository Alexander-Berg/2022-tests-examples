...
@DisplayName("Частник, снегоходы - Отображение формы")
@Feature(FORMS)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class SnowmobilesScreenshotTest {
    ...
    @Before
    public void before() {
        formsSteps.createSnowmobilesForm();
    ...
    public void shouldSeeForm() throws IOException {
        ...
        urlSteps.setProduction().testing().path(MOTO).path(ADD).addXRealIP(MOSCOW_IP).open();
        formsSteps.fillForm("Номер телефона");
        formsSteps.onFormsPage().footer().copyright().click();
