...
public class MotorcyclesVasTurboTest {
    ...
    @DisplayName("Добавление объявления с пакетом услуг «Турбо-продажа»")
    public void shouldAddSaleWithTurboPackage() {
        formsSteps.onFormsPage().userVas().getSnippet(1).submitButton().click();
        ...
        formsSteps.onFormsPage().billingPopup().header().waitUntil(hasText("Турбо продажа"));
        formsSteps.onFormsPage().billingPopup().priceHeader().waitUntil(hasText("2 297 ₽"));
        ...
        createOfferSteps.setOfferTemplate("offers/motoTurboOffer.json");
