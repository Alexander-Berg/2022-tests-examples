...
@DisplayName("Дилер, мотоциклы - добавление объявлений")
@Feature(FORMS)
...
public class MotorcyclesTest {
    ...
    public void shouldAddNewSale() throws IOException {
        ...
        createOfferSteps.setOfferTemplate("offers/motoDealerNewOffer.json");
    ...
    @Owner(DSVICHIHIN)
    @Category({Regression.class})
    @DisplayName("Добавление объявления о продаже нового ТС без фото")
    public void shouldAddNewSaleWithoutPhoto() throws IOException {
        formsSteps.createMotorcyclesDealerNewForm();
        formsSteps.fillForm(formsSteps.getDescription().getBlock());
        formsSteps.submitForm();

        formsSteps.onCardPage().cardHeader().title()
                .waitUntil("Объявление не добавилось", hasText("ABM Alpha 110"), 10);

        createOfferSteps.setOfferTemplate("offers/motoDealerNewOfferWithoutPhoto.json");
        createOfferSteps.getUserOffer(account,
                AutoApiOffer.CategoryEnum.MOTO, formsSteps.getOfferId());
        createOfferSteps.shouldSeeSameOffer(
                createOfferSteps.getUserOffer(account,
    ...
    public void shouldAddUsedSale() throws IOException {
        ...
        createOfferSteps.setOfferTemplate("offers/motoDealerUsedOffer.json");
        createOfferSteps.getUserOffer(account,
                AutoApiOffer.CategoryEnum.MOTO, formsSteps.getOfferId());
        createOfferSteps.shouldSeeSameOffer(
                createOfferSteps.getUserOffer(account,
    ...
    public void shouldAddNewSaleWithServices() throws IOException, InterruptedException {
        ...
        seleniumMockSteps.assertWithWaiting(onlyOneRequest(
                ...
                hasJsonBody("drafts/motoDealerNewOfferWithServices.json")
        ));
        urlSteps.shouldUrl(containsString("/motorcycle/new/sale/"));
        formsSteps.onCardPage().cardHeader().title()
                .waitUntil("Объявление не добавилось", hasText("ABM Alpha 110"), 10);
    }
    ...
    @After
    public void after() {
        formsSteps.deleteOffers(AutoApiOffer.CategoryEnum.MOTO.toString().toLowerCase());
    }
}
