...
@DisplayName("Избранное на карточке дилера под зарегом")
@Feature(FAVORITES)
@Story(DEALERS)
...
public class FavoritesDealersRegTest {
    ...
    public void shouldDeleteFromFavorites() {
        mockRule.setStubs(stub("desktop/UserFavoritesCarsDelete")).update();

        basePageSteps.onDealerCardPage().getSale(1).hover();
        basePageSteps.onDealerCardPage().getSale(0).addToFavoritesIcon().click();
        basePageSteps.onDealerCardPage().getSale(0).deleteFromFavoritesIcon().waitUntil(isDisplayed()).click();

        basePageSteps.onDealerCardPage().notifier().waitUntil(isDisplayed()).should(hasText(DELETED_FROM_FAV));
        basePageSteps.onDealerCardPage().getSale(0).addToFavoritesIcon().should(isDisplayed());
    }
}
