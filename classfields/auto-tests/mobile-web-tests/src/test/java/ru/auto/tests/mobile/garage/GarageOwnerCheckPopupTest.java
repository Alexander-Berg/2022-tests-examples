...
import static ru.auto.tests.desktop.mobile.page.GarageCardPage.BECOME_CHECKED_OWNER;
import static ru.auto.tests.desktop.mobile.page.GaragePage.OWNER_CHECK_POPUP_TEXT;
...
public class GarageOwnerCheckPopupTest {
    ...
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Попап проверенного собственника, по клику на «Станьте проверенным собственником» с карточки гаража")
    public void shouldSeeOwnerCheckPopup() {
        ...
        urlSteps.open();
        basePageSteps.onGarageCardPage().button(BECOME_CHECKED_OWNER).waitUntil(isDisplayed()).hover().click();
