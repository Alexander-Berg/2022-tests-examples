...
public class CatalogSpecificationBannersTest {
    ...
    public void before() {
        urlSteps.testing().path(CATALOG).path(CARS).path("bmw").path("5er").path("22212692").path("22213464")
                .path(SPECIFICATIONS).path("22213464_22219056_22213547").open();
    }

    @Test
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру С2 на странице характеристик в каталоге")
    public void shouldClickBannerSpecificC2() {
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру C3 на странице характеристик в каталоге")
    public void shouldClickBannerSpecificC3() {
        basePageSteps.onAdsPage().shouldSeeAds(greaterThan(0));
