...
@Story("Сделки в ЛК под продавцом")
@RunWith(GuiceTestRunner.class)
@GuiceModules(MobileEmulationTestsModule.class)
public class LkDealsForSellerTest {
    ...
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Текст в попапе «Отменить запрос»")
    public void shouldSeeTextInCancelPopup() {
        basePageSteps.onLkDealPage().getDeal(2).button(CANCEL_REQUEST).click();

        basePageSteps.onLkDealPage().popup().should(hasText("Почему решили отменить?\n" +
                "Договорился с другим покупателем\nНе устроила предложенная цена\n" +
                "Покупатель бездействует\nПередумал продавать\nДругое"));
    }
    ...
    public void shouldClickCancelRequestButtonInCancelPopup() {
        ...
        basePageSteps.onLkDealPage().getDeal(2).button(CANCEL_REQUEST).click();
        basePageSteps.onLkDealPage().popup().reasonItem("Договорился с другим покупателем").click();
    ...
    public void shouldClickGoToOfferButtonWhenDealDeclinedBuyer() {
        mockRule.with("mobile/OfferCarsUsedUser1113993894").update();

        basePageSteps.onLkDealPage().getDeal(3).button(GO_TO_OFFER).click();
    ...
    public void shouldApproveDeal() {
        ...
        basePageSteps.onLkDealPage().getDeal(4).button("Подтвердить").click();
    ...
    public void shouldDeclineDealWithAnotherBuyerReason() {
        ...
        basePageSteps.onLkDealPage().getDeal(4).button("Отклонить").click();
        basePageSteps.onLkDealPage().popup().reasonItem("Договорился с другим покупателем").click();
    ...
    public void shouldClickGoToOfferButtonWhenDealCanceledSeller() {
        mockRule.with("mobile/OfferCarsUsedUser1113961017").update();

        basePageSteps.onLkDealPage().getDeal(5).button(GO_TO_OFFER).click();
