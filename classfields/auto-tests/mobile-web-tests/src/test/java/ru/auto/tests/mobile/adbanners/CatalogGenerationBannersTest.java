...
public class CatalogGenerationBannersTest {
    ...
    @Before
    public void before() {
        urlSteps.testing().path(CATALOG).path(CARS).path("bmw").path("5er").path("22212692").open();
    }

    @Test
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру ТОП на странице поколения в каталоге")
    public void shouldClickBannerGenTop() {
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру С2 на странице поколения в каталоге")
    public void shouldClickBannerGenC2() {
        basePageSteps.onAdsPage().shouldSeeAds(greaterThan(0));
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру С3 на странице поколения в каталоге")
    public void shouldClickBannerGenC3() {
        basePageSteps.onAdsPage().shouldSeeAds(greaterThan(0));
