...
public class SearchModelSelectorsTest {
    ...
    public void shouldSearchModelSelector() {
        ...
        basePageSteps.onListingPage().filters().mmm().button("Модель").waitUntil(isDisplayed()).click();
        ...
        basePageSteps.onListingPage().mmmPopup().modelsList().should(hasSize(1));
        basePageSteps.onListingPage().mmmPopup().getModel(0).should(hasText(model));
    }
