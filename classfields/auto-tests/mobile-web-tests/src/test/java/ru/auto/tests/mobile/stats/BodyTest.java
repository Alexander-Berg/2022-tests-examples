...
@DisplayName("Статистика по кузову")
...
public class BodyTest {
    ...
    public void shouldSelectBody() {
        ...
        urlSteps.testing().path(STATS).path(CARS).path(MARK).path(MODEL).path(GENERATION_ID)
                .path(OTHER_BODY_ID).path(SLASH).shouldNotSeeDiff();
    }
    ...
    public void shouldSelectComplectation() {
        //basePageSteps.selectOption(basePageSteps.onCatalogPage().filter().select("Выбрать комплектацию"), COMPLECTATION);
        basePageSteps.onCatalogPage().filter().select("Выбрать комплектацию").should(isDisplayed()).click();
        basePageSteps.onCatalogPage().dropdown().item(COMPLECTATION).waitUntil(isDisplayed()).click();
        urlSteps.testing().path(STATS).path(CARS).path(MARK).path(MODEL).path(GENERATION_ID)
                .path(BODY_ID).path(COMPLECTATION_ID).path(SLASH).shouldNotSeeDiff();
    }
}
