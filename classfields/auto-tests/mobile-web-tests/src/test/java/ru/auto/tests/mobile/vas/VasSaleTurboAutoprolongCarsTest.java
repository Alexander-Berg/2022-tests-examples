...
public class VasSaleTurboAutoprolongCarsTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/OfferCarsUsedUserOwner",
                ...
                "mobile/BillingAutoruPaymentProcessSaleCarsTiedCard",
                "desktop/BillingAutoruPayment",
                "desktop/UserTransactionProlongablePut",
                "desktop/UserTransactionProlongableDelete").post();

        urlSteps.testing().path(CARS).path(USED).path(SALE).path(SALE_ID).open();

        mockRule.overwriteStub(0, "desktop/OfferCarsUsedUserOwnerWithAutoprolongServices");
    }
