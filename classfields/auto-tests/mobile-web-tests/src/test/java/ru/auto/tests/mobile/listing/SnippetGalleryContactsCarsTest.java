...
public class SnippetGalleryContactsCarsTest {
    ...
    public void before() {
        ...
        basePageSteps.onListingPage().getSale(TOP_SALES_COUNT).gallery().contacts()
                .should(hasText("БорисХоф BMW Восток\nОфициальный дилер")).click();
    }
    ...
    public void shouldSeeContacts() {
        basePageSteps.onListingPage().popup().waitUntil(hasText(matchesPattern("БорисХоф BMW Восток\n" +
                "Официальный дилер\n• На Авто.ру \\d+ (год|года|лет)\nБалашиха, г. Москва мкрн ЦОВБ 21. " +
    ...
    public void shouldClickInStockButton() {
        basePageSteps.onListingPage().popup().button("118 авто в наличии").click();
        urlSteps.switchToNextTab();
        urlSteps.testing().path(DILER).path(CARS).path(NEW).path("/borishof_balashiha_bmw/").shouldNotSeeDiff();
    }
