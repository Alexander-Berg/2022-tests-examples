...
import static ru.auto.tests.desktop.mobile.page.GarageCardPage.SALE;
...
@Story("Карточка автомобиля в гараже")
...
public class GarageCardTest {
    ...
    public void before() {
        mockRule.setStubs(stub("desktop/SessionAuthUser"),
                ...
                stub("desktop/GarageUserCardVin")).create();

        urlSteps.testing().path(GARAGE).path(VIN_CARD_ID).addXRealIP(MOSCOW_IP).open();
    }
    ...
    @Category({Regression.class, Testing.class})
    @Owner(DSVICHIHIN)
    @DisplayName("Клик по селектору автомобиля")
    public void shouldClickCarSelector() {
        basePageSteps.onGarageCardPage().carSelector().click();
        urlSteps.testing().path(GARAGE).path(VIN_CARD_ID).shouldNotSeeDiff();
    }
    ...
    @Category({Regression.class, Testing.class})
    @Owner(DSVICHIHIN)
    @DisplayName("Клик по кнопке «Изменить» и редактирование")
    public void shouldClickChangeButtonAndEdit() {
        ...
        basePageSteps.onGarageCardPage().form().block(format("%s%s", REGION_FIELD, "Москва")).click();
        ...
        basePageSteps.onGarageCardPage().button(CHANGE).should(isDisplayed());
    }
    ...
    public void shouldClickDeleteButton() {
        basePageSteps.onGarageCardPage().button("Изменить").click();
    ...
    public void shouldClickSellButton() {
        ...
        basePageSteps.onGarageCardPage().button(SALE).click();
        urlSteps.testing().path(PROMO).path(FROM_WEB_TO_APP).shouldNotSeeDiff();

        basePageSteps.onPromoPage().closeButton().click(); // проверяем, что после закрытия промки оставляет в гараже
        basePageSteps.onGarageCardPage().button(SALE).should(isDisplayed());
    }

    @Test
    @Category({Regression.class, Testing.class})
    @Owner(NATAGOLOVKINA)
    @DisplayName("Клик по кнопке «Добавить машину мечты»")
    public void shouldClickDreamCar() {
        mockRule.setStubs(stub("desktop/ReferenceCatalogCarsSuggest")).update();

        basePageSteps.onGarageCardPage().carSelector().button("Добавитьмашину").click();
    ...
    @Category({Regression.class, Testing.class})
    @Owner(NATAGOLOVKINA)
    @DisplayName("Клик по кнопке «Добавить бывшую»")
    public void shouldClickExCar() {
        mockRule.setStubs(stub("desktop/ReferenceCatalogCarsSuggest")).update();

        basePageSteps.onGarageCardPage().carSelector().button("Добавитьбывшую").hover().click();
    ...
    @Category({Regression.class, Testing.class})
    @Owner(DSVICHIHIN)
    @DisplayName("Клик по плашке «Поделитесь опытом эксплуатации — напишите отзыв»")
    public void shouldClickReviewsGrid() {
        mockRule.setStubs(stub("desktop/UserFavoritesAllSubscriptionsEmpty"),
                stub("desktop/ProxyPublicApi")).update();

        basePageSteps.onGarageCardPage().grid("Поделитесь опытом эксплуатации\u00a0— напишите отзыв").click();
        urlSteps.shouldUrl(startsWith(urlSteps.desktopURI().path(CARS).path(REVIEWS).path(EDIT).toString()));
    }

    @Test
    @Category({Regression.class, Testing.class})
    @Owner(DSVICHIHIN)
    @DisplayName("Клик по плашке «Журнал»")
    public void shouldClickMagGrid() {
        long pageOffset = basePageSteps.getPageYOffset();
        basePageSteps.onGarageCardPage().magGrid().click();
