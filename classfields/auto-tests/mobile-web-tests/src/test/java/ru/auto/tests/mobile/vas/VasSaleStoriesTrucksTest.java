...
public class VasSaleStoriesTrucksTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/OfferTrucksUsedUserOwner",
                ...
                "mobile/BillingAutoruPaymentInitStories",
    ...
    public void shouldBuyVas() {
        basePageSteps.scrollAndClick(basePageSteps.onCardPage().vas().service("Показ в Историях").buyButton());
        buyVas();
    }
    ...
    public void shouldBuyVasInPopup() throws InterruptedException {
        basePageSteps.openVasPopup("Показ в Историях");
        buyVas();
    }
    ...
    private void buyVas() {
        ...
        basePageSteps.onCardPage().vas().activeService("Показ в Историях")
                .waitUntil(hasText("Показ в Историях\nДействует 3 дня\nдо окончания"));
    }
}
