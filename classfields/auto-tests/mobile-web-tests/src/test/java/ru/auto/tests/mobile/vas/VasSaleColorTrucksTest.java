...
public class VasSaleColorTrucksTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/OfferTrucksUsedUserOwner",
                ...
                "mobile/BillingAutoruPaymentInitColor",
    ...
    public void shouldBuyVas() {
        basePageSteps.scrollAndClick(basePageSteps.onCardPage().vas().service("Выделение цветом").buyButton());
        buyVas();
    }
    ...
    public void shouldBuyVasInPopup() throws InterruptedException {
        basePageSteps.openVasPopup("Выделение цветом");
        buyVas();
    }
    ...
    private void buyVas() {
        ...
        basePageSteps.onCardPage().vas().activeService("Выделение цветом")
                .waitUntil(hasText("Выделение цветом\nДействует 3 дня\nдо окончания"));
    }
}
