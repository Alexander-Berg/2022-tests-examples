...
public class OffersTabsUsedTest {
    ...
    public void before() {
        ...
        basePageSteps.onCatalogGenerationPage().offers().tab("С пробегом").click();
    ...
    public void shouldClickShowAllButton() {
        ...
        urlSteps.testing().path(MOSKVA).path(CARS).path(MARK).path(MODEL).path(GENERATION).path(USED).shouldNotSeeDiff();
    }
}
