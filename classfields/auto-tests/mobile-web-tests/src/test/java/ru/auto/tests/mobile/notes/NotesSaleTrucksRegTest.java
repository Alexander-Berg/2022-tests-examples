...
public class NotesSaleTrucksRegTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                "desktop/UserNotesTrucksDelete",
                "desktop/UserFavoritesTrucksPost").post();

        urlSteps.testing().path(TRUCK).path(USED).path(SALE).path(SALE_ID).open();

        mockRule.overwriteStub(1, "desktop/OfferTrucksUsedUserWithNote");
    }
