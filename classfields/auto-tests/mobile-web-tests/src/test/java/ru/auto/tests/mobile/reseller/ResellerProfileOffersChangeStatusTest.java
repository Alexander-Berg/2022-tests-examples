...
import static ru.auto.tests.desktop.mobile.page.ResellerPage.IN_STOCK;
import static ru.auto.tests.desktop.mobile.page.ResellerPage.SOLD;
...
public class ResellerProfileOffersChangeStatusTest {
    ...
    public void shouldGoToSoldOffers() {
        ...
        mockRule.overwriteStub(0,
                stub().withGetDeepEquals(format("%s/%s/offers/all", USER, USER_ID))
                        .withRequestQuery(
                                query().setStatus(INACTIVE).setSort(DATE_DESC.getAlias()))
                        .withResponseBody(
                                resellerOffersExample()
                                        .setFiltersStatus(INACTIVE)
                                        .setOffers(INACTIVE_COUNT).build())
        );

        basePageSteps.onResellerPage().status(SOLD).click();
