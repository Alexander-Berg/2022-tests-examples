...
public class CatalogEquipmentBannersTest {
    ...
    @Before
    public void before() {
        urlSteps.testing().path(CATALOG).path(CARS).path("bmw").path("5er").path("22212692").path("22213464")
                .path(EQUIPMENT).path("22213464_22219056_22213547")
                .addParam("tech_param_id", "22213547").open();
    }

    @Test
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру С2 на странице комплектации в каталоге")
    public void shouldClickBannerEquipC2() {
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Клик по баннеру C3 на странице комплектации в каталоге")
    public void shouldClickBannerEquipC3() {
        basePageSteps.onAdsPage().shouldSeeAds(greaterThan(0));
