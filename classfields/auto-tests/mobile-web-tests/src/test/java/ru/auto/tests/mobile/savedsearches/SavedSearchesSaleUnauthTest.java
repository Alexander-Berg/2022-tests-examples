...
public class SavedSearchesSaleUnauthTest {
    ...
    public void before() {
        ...
        mockRule.newMock().with("desktop/OfferCarsUsedDealer",
                "mobile/UserFavoritesCarsSubscriptionsDealerIdUsedPost",
    ...
    public void shouldSaveSearch() {
        ...
        basePageSteps.onCardPage().savedSearchesPopup().waitUntil(isDisplayed()).should(hasText("Укажите свою почту " +
                "для получения уведомлений о новых объявлениях\nУкажите почту\nОтправить"));
        basePageSteps.onCardPage().savedSearchesPopup().input("Укажите почту", "test@test.com");
        basePageSteps.onCardPage().savedSearchesPopup().sendButton().click();
        basePageSteps.onCardPage().savedSearchesPopup().waitUntil(not(isDisplayed()));
        basePageSteps.onCardPage().button("Вы подписаны").waitUntil(isDisplayed());
    }
