...
public class CardSnippetMotoTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/SearchMotoBreadcrumbs",
                ...
                "mobile/SearchMotoAllDealerId",
                "desktop/OfferMotoUsedUser").post();
    ...
    public void shouldClickSale() {
        basePageSteps.scrollAndClick(basePageSteps.onDealerCardPage().getSale(0).title());
        urlSteps.testing().path(MOTORCYCLE).path(USED).path(SALE)
                .path("/harley_davidson/dyna_super_glide/1076842087-f1e84/").shouldNotSeeDiff();
    }
}
