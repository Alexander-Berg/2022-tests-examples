...
import static ru.auto.tests.desktop.mobile.page.DealerCardPage.DEALERSHIP_NEW;
...
public class CardSortNewTest {
    ...
    @Before
    public void before() {
        urlSteps.testing().path(DILER_OFICIALNIY).path(CARS).path(section).path(DEALERSHIP_NEW).open();
    }
    ...
    public void shouldSort() {
        ...
        urlSteps.path(SLASH).path(DEALERSHIP_MARK.toLowerCase()).path(SLASH).addParam("sort", sort.getAlias()).shouldNotSeeDiff();
