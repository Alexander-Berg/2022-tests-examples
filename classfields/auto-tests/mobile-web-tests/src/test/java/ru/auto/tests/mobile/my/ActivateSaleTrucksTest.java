...
@DisplayName("ЛК - активация объявления комТС")
@Epic(LK)
@Feature(SALES)
...
public class ActivateSaleTrucksTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/User",
                ...
                "desktop/UserOffersTrucksInactive",
                "desktop/UserOffersTrucksActivate").post();

        urlSteps.testing().path(MY).path(TRUCKS).open();

        mockRule.overwriteStub(1, "desktop/UserOffersTrucksActive");
    }
