...
public class ResellerProfileSortTest {
    ...
    public void shouldApplySort() {
        ...
        mockRule.overwriteStub(0,
                stub().withGetDeepEquals(format("%s/%s/offers/all", USER, USER_ID))
                        .withRequestQuery(
                                query().setStatus(ACTIVE.toUpperCase()).setSort(sortBy.getAlias()))
                        .withResponseBody(
                                resellerOffersExample().setOffers(COUNT_AFTER_SORT).build())
        );

        basePageSteps.onResellerPage().sort().click();
        basePageSteps.onListingPage().sortPopup().sort(sortBy.getName().replace("По ", "")).click();
