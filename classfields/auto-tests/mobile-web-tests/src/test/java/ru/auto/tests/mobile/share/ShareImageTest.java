...
@Feature(SALES)
@DisplayName("Объявление - Фото в поделяшке")
...
public class ShareImageTest {
    ...
    @DisplayName("Фото в поделяшке")
    public void shouldSeeSaleImageInShare() {
        String firstImageSrc = basePageSteps.onCardPage().gallery().currentImage().getAttribute("src")
                .replaceFirst("n$", "").replaceAll("456x342", "1200x900");
        basePageSteps.onCardPage().metaOgImage().should(hasAttribute("content", equalTo(firstImageSrc)));
    }
}
