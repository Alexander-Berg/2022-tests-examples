...
public class VasAutoprolongOptionTest {
    ...
    public void shouldProlonOptionBeforeCheckout() {
        ...
        basePageSteps.onLkSalesPage().billingPopup().autoprolongCheckbox().waitUntil(isDisplayed())
                .waitUntil(hasClass(containsString("Checkbox_checked")));
        ...
        basePageSteps.onLkSalesPage().notifier().waitUntil(isDisplayed())
                .waitUntil(hasText("Опция «Поднятие в ТОП» подключена"));
