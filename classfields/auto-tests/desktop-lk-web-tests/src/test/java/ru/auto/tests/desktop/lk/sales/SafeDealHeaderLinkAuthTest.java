...
public class SafeDealHeaderLinkAuthTest {
    ...
    @Before
    public void before() {
        mockRule.setStubs(stub("desktop/SessionAuthUser")).create();
    ...
    @Owner(TIMONDL)
    @DisplayName("Открытие списка сделок при клике в шапку")
    public void shouldSeeSafeDealBlock() {
        mockRule.setStubs(stub("desktop/SafeDealDealListForBuyer")).update();
    ...
    @Category({Regression.class, Testing.class})
    @Owner(TIMONDL)
    @DisplayName("Открытие промо при клике в шапку когда нет сделок")
    public void shouldOpenPromoPage() {
        mockRule.setStubs(stub("desktop/SafeDealDealListEmpty")).update();
