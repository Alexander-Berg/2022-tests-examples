...
@DisplayName("Снятие с продажи. КомТС")
@Issue("AUTORUFRONT-19742")
@Epic(AutoruFeatures.LK)
@Feature(AutoruFeatures.MY_OFFERS_RESELLER)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class HideTrucksOfferTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                "desktop-lk/UserOffersTrucksActive").post();

        basePageSteps.setWideWindowSize();
        urlSteps.testing().path(MY).path(RESELLER).path(TRUCKS).open();
    }
    ...
    public void shouldDeactivateSale() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                "desktop-lk/UserFavoriteReseller",
                "desktop-lk/UserOffersTrucksInactive",
                "desktop-lk/UserOffersTrucksHide").post();
    ...
    public void shouldDeactivateSaleSoldPrice() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                "desktop-lk/UserFavoriteReseller",
                "desktop-lk/UserOffersTrucksInactive",
                "desktop-lk/UserOffersTrucksHideSoldPrice").post();
    ...
    public void shouldDeactivateSaleBuyersPhoneFromList() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                "desktop-lk/UserFavoriteReseller",
                "desktop-lk/UserOffersTrucksInactive",
                "desktop-lk/UserOffersTrucksPredictBuyers",
                "desktop-lk/UserOffersTrucksHideBuyerPhone").post();
    ...
    private void deactivate() {
        ...
        basePageSteps.onLkResellerSalesPage().getSale(0).datesColumn().createDate()
                .waitUntil(hasText("Добавлено 24 июня 2019"));
        basePageSteps.onLkResellerSalesPage().getSale(0).status().text()
                .waitUntil(hasText("Объявление готово к публикации"));
        basePageSteps.onLkResellerSalesPage().getSale(0).status().button("Опубликовать объявление")
