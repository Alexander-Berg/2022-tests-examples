...
public class VasWalletTest {
    ...
    @DisplayName("Покупка услуги «Экспресс-продажа», оплата кошельком")
    public void shouldBuyPackageExpressWalletPay() {
        basePageSteps.onLkSalesNewPage().getSale(0).vasItem("Экспресс-продажа").click();
        ...
        basePageSteps.onLkSalesNewPage().billingPopup().header().waitUntil(hasText("Экспресс продажа"));
        basePageSteps.onLkSalesNewPage().billingPopup().priceHeader().waitUntil(hasText("797 \u20BD"));
        basePageSteps.onLkSalesNewPage().billingPopup().getPaymentMethod(0).title().waitUntil(hasText("Кошелёк"));
        basePageSteps.onLkSalesNewPage().billingPopup().walletPayButton().waitUntil(isDisplayed()).click();
        ...
        basePageSteps.onLkSalesNewPage().getSale(0).getVas(1)
                .should(hasText(matchesPattern("×5 просмотров\nАктивно ещё\\d+ (день|дня|дней)")));
    }
