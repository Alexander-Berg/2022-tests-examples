...
public class FavoritesPopupMotoTest {
    ...
    public void before() {
        mockRule.setStubs(
                stub().withGetDeepEquals(USER_OFFERS_MOTO_COUNT)
                        .withRequestQuery(
                                query().setCategory(AutoApiOffer.CategoryEnum.MOTO.getValue()))
                        .withResponseBody(
                                offersCount().getBody()),
    ...
    public void shouldClickSale() {
        ...
        urlSteps.testing().path(MOTORCYCLE).path(USED).path(SALE).path("/honda/cbr_300/")
    ...
    public void shouldSeeCallsStats() {
        mockRule.setStubs(stub("desktop/OfferMotoCallsStats")).update();
