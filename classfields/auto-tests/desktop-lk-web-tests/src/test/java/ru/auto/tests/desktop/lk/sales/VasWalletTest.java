...
import static ru.auto.tests.desktop.element.BillingPopup.EXPRESS_SALE;
import static ru.auto.tests.desktop.element.BillingPopup.PAYMENT_SUCCESS;
import static ru.auto.tests.desktop.element.BillingPopup.WALLET;
...
@DisplayName("Оплата кошельком")
@Epic(AutoruFeatures.LK)
@Feature(AutoruFeatures.MY_OFFERS_PRIVATE)
@RunWith(Parameterized.class)
@GuiceModules(DesktopTestsModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class VasWalletTest {
    ...
    public void before() {
        ...
        mockRule.setStubs(
                sessionAuthUserStub(),
                stub("desktop/User"),
                stub("desktop-lk/BillingAutoruPaymentInitExpressWallet"),
                stub("desktop-lk/BillingAutoruPaymentProcessWallet"),
                stub("desktop-lk/BillingAutoruPayment"),
                stub().withGetDeepEquals(userOfferPath).withResponseBody(
                        userOffersResponse().setOffers(
                                mockUserOffer(userOfferPathToMock)).build()),
    ...
    @DisplayName("Оплата услуги «Экспресс-продажа» кошельком")
    public void shouldBuyVas() {
        ...
        basePageSteps.onLkSalesPage().billingPopup().header().waitUntil(hasText(EXPRESS_SALE));
        basePageSteps.onLkSalesPage().billingPopup().priceHeader().waitUntil(hasText("797 \u20BD"));
        basePageSteps.onLkSalesPage().billingPopup().getPaymentMethod(0).title().waitUntil(hasText(WALLET));
        basePageSteps.onLkSalesPage().billingPopup().walletPayButton().waitUntil(isDisplayed()).click();
        ...
        basePageSteps.onLkSalesPage().notifier(PACKAGE_EXPRESS_ACTIVATED).waitUntil(isDisplayed());
