...
public class VasAutoprolongPackageAfterCheckoutTest {
    ...
    public void shouldBuyVas() {
        basePageSteps.onLkSalesNewPage().getSale(0).vasItem("Турбо-продажа").hover();
        basePageSteps.onLkSalesNewPage().popup().buttonContains("Подключить").click();
        ...
        basePageSteps.onLkSalesNewPage().billingPopup().header().waitUntil(hasText(TURBO_SALE));
        ...
        basePageSteps.onLkSalesNewPage().billingPopup().successMessage().waitUntil(hasText(PAYMENT_SUCCESS));
        basePageSteps.onLkSalesNewPage().billingPopup().autoProlongButton().waitUntil(isDisplayed()).click(); //нет кнопки
        basePageSteps.switchToDefaultFrame();
        basePageSteps.onLkSalesNewPage().notifier(AUTOPROLONG_ACTIVATED).waitUntil(isDisplayed());
