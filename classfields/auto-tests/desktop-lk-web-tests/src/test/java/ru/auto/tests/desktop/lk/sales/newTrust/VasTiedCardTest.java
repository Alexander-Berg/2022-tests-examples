...
public class VasTiedCardTest {
                ...
                stub().withPostDeepEquals(BILLING_AUTORU_PAYMENT_INIT)
                        .withRequestBody(
                                paymentInitVasRequest()
                                        .setProducts(PACKAGE_TURBO).getBody())
                        .withResponseBody(
                                paymentInitVasResponse()
                                        .tieCard()
                                        .setAccountBalance(0)
                                        .setDetailedProductInfos(PACKAGE_TURBO)
                                        .setCost(COST)
                                        .getBody()),
    ...
    public void shouldBuyVas() {
        ...
        basePageSteps.onLkSalesPage().newTrust().title().waitUntil(hasText(TURBO_SALE));
        basePageSteps.onLkSalesPage().newTrust().price().waitUntil(hasText(formatPrice(COST)));

        basePageSteps.onLkSalesPage().newTrust().paymentMethods().get(0).title().waitUntil(hasText("Mastercard"));
        basePageSteps.onLkSalesPage().newTrust().paymentMethods().get(0).click();
