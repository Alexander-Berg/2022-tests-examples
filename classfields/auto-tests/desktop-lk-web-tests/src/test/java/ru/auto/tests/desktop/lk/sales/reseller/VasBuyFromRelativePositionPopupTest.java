...
@DisplayName("Покупка услуги «Поднятие в поиске» через попап относительной позиции")
@Epic(AutoruFeatures.LK)
@Feature(AutoruFeatures.MY_OFFERS_RESELLER)
@RunWith(Parameterized.class)
@GuiceModules(DesktopTestsModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class VasBuyFromRelativePositionPopupTest {
    ...
    public void before() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                salesMock,
                offerIdMock,
                "desktop-lk/reseller/BillingAutoruPaymentInitRelativePosition",
    ...
    public void shouldBuyVas() {
        basePageSteps.onLkResellerSalesPage().getSale(0).button("Ниже, чем 27% похожих").waitUntil(isDisplayed()).hover();
        basePageSteps.onLkResellerSalesPage().popup().buttonContains("Поднять в поиске за").waitUntil(isDisplayed()).click();
