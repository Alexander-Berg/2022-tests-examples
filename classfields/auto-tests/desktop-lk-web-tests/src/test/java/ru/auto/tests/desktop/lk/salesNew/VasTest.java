...
public class VasTest {
    ...
    private static final String VAS_BLOCK_PATTERN = "×20 просмотров\nТурбо-продажа • 3 дня\nСкидка 40%\nосталось\n" +
            "\\d{2}:\\d{2}:\\d{2}\n1 437 ₽\n2 395 ₽\n×5 просмотров\nЭкспресс-продажа • 6 дней\nСкидка 40%\nосталось\n" +
            "\\d{2}:\\d{2}:\\d{2}\n197 ₽\n328 ₽\n×40 просмотров\nПоказ в Историях • 3 дня\n834 ₽\n×15 просмотров\n" +
            "Поднятие в ТОП • 3 дня\n197 ₽\n×3 просмотров\nПоднятие в поиске • 1 день\n87 ₽";
        ...
        mockRule.setStubs(
                stub().withGetDeepEquals(userOfferPath).withResponseBody(
                        userOffersResponse().setOffers(
                                mockUserOffer(userOfferPathToMock).setBaseServicePrices()
                        ).build()),
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Текст блока услуг")
    public void shouldSeeVasBlockText() {
        basePageSteps.onLkSalesNewPage().getSale(0).vas().should(hasText(matchesPattern(VAS_BLOCK_PATTERN)));
    }
