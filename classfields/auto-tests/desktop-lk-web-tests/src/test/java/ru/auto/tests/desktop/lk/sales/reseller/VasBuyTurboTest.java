...
import static ru.auto.tests.desktop.consts.Notifications.OPTION_SUCCESSFULLY_ACTIVATED;
...
public class VasBuyTurboTest {
    ...
    public void before() {
        ...
        mockRule.setStubs(
                ...
                stub("desktop-lk/reseller/BillingAutoruPaymentInitTurbo"),
    ...
    public void shouldBuyTurboPackage() {
        basePageSteps.onLkResellerSalesPage().getSale(0).vas("Турбо-продажа").should(isDisplayed()).click();
        ...
        basePageSteps.onLkResellerSalesPage().billingPopup().header().waitUntil(hasText(TURBO_SALE));
        basePageSteps.onLkResellerSalesPage().billingPopup().priceHeader().waitUntil(hasText("537 \u20BD"));
        yaKassaSteps.payWithCard();
        yaKassaSteps.waitForSuccessMessage();
        basePageSteps.onLkResellerSalesPage().notifier(OPTION_SUCCESSFULLY_ACTIVATED).waitUntil(isDisplayed());
        ...
        basePageSteps.onLkResellerSalesPage().getSale(0).vas("Турбо-продажа")
                .should(hasText("×20 просмотров\nТурбо-продажа • 3 дня\nАвтопродление"));
    }
