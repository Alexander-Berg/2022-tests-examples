...
public class SoldSalesCarsTest {
    ...
    public void shouldClickSaleTitle() {
        ...
        urlSteps.testing().path(CARS).path(USED).path(SALE).path("/vaz/2121/").path(SALE_ID).path(SLASH)
                .shouldNotSeeDiff();
    }
    ...
    public void shouldClickEditButton() {
        ...
        urlSteps.testing().path(CARS).path(USED).path(EDIT).path(SALE_ID).path(SLASH).shouldNotSeeDiff();
    }
    ...
    public void shouldClickDeleteButton() {
        ...
        int size = basePageSteps.onLkSalesNewPage().salesList().size();

        basePageSteps.onLkSalesNewPage().getSale(0).hover();
        basePageSteps.onLkSalesNewPage().getSale(0).deleteButton().waitUntil(isDisplayed()).click();
        basePageSteps.acceptAlert();
        basePageSteps.onLkSalesNewPage().notifier().waitUntil(isDisplayed()).should(hasText("Объявление удалено"));

        basePageSteps.onLkSalesNewPage().salesList().should(hasSize(size - 1));
    }
