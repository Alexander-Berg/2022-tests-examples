...
public class VasSberbankTest {
    ...
    @DisplayName("Оплата услуги «Турбо-продажа» Сбербанком")
    public void shouldBuyVas() {
        basePageSteps.onLkSalesPage().getSale(0).vas().buyButton().click();
        ...
        basePageSteps.onLkSalesPage().billingPopup().priceHeader().waitUntil(hasText("550 \u20BD"));
        basePageSteps.onLkSalesPage().billingPopup().getPaymentMethod(1).title()
                .waitUntil(hasText("Сбербанк Онлайн")).click();
        basePageSteps.onLkSalesPage().billingPopup().sberbankPhoneInput().sendKeys("9111111111");
        basePageSteps.onLkSalesPage().billingPopup().sberbankPayButton().waitUntil(isDisplayed()).click();
        basePageSteps.onLkSalesPage().billingPopup().successMessage()
                .waitUntil("Не появилось сообщение об успешной оплате", hasText(PAYMENT_SUCCESS), 10);
