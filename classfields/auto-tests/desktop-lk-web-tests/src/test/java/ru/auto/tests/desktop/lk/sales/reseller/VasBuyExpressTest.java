...
public class VasBuyExpressTest {
    ...
    public void before() {
        ...
        mockRule.setStubs(
                ...
                stub("desktop-lk/reseller/BillingAutoruPaymentInitExpress"),
    ...
    public void shouldBuyVas() {
        basePageSteps.onLkResellerSalesPage().getSale(0).vas("Экспресс-продажа").should(isDisplayed()).click();
                ...
                stub().withGetDeepEquals(userOfferPath).withResponseBody(
                        userOffersResponse().setOffers(
                                mockUserOffer(userOfferPathToMock)
                                        .setServices(
                                                service(PACKAGE_EXPRESS).setProlongable(true),
                                                service(ALL_SALE_COLORS).setProlongable(true),
                                                service(ALL_SALE_SPECIAL).setProlongable(true))
                        ).build()));
        ...
        basePageSteps.onLkResellerSalesPage().billingPopup().header().waitUntil(hasText("Экспресс продажа"));
        basePageSteps.onLkResellerSalesPage().billingPopup().priceHeader().waitUntil(hasText("1 137 \u20BD"));
        ...
        basePageSteps.onLkResellerSalesPage().getSale(0).vas("Экспресс-продажа")
