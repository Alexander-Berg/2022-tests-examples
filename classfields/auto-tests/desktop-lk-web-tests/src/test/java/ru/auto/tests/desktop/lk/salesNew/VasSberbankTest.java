...
public class VasSberbankTest {
    ...
    @DisplayName("Покупка услуги «Турбо-продажа», оплата через Сбербанк Онлайн")
    public void shouldBuyTurboPackageBySberbankOnline() {
        ...
        basePageSteps.onLkSalesNewPage().billingPopup().sberbankPhoneInput().sendKeys("9111111111");
        basePageSteps.onLkSalesNewPage().billingPopup().sberbankPayButton().waitUntil(isDisplayed()).click();
        basePageSteps.onLkSalesNewPage().billingPopup().successMessage()
                .waitUntil("Не появилось сообщение об успешной оплате", hasText("Платёж совершён успешно"), 10);
