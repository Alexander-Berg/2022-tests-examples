...
@DisplayName("Автопродление опции при оплате")
...
public class VasAutoprolongOptionTest {
    ...
    public void before() {
        ...
        mockRule.setStubs(
                ...
                stub("desktop-lk/UserTransactionProlongable")).create();
    ...
    public void shouldProlonOptionBeforeCheckout() {
                ...
                stub().withGetDeepEquals(format("%s/%s", userOfferPath, ID))
                        .withResponseBody(
                                mockUserOffer(userOfferPathToMock).setServices(
                                                service(ALL_SALE_TOPLIST).setProlongable(true))
                                        .getOffer()));
