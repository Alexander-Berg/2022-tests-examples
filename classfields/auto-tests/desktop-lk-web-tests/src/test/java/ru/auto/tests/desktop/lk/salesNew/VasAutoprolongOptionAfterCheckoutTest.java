...
public class VasAutoprolongOptionAfterCheckoutTest {
    ...
    public void shouldProlongOptionAfterCheckout() {
        basePageSteps.onLkSalesNewPage().getSale(0).vasItem("Поднятие в ТОП").click();
        ...
        basePageSteps.onLkSalesNewPage().billingPopup().header().waitUntil(hasText("Поднятие в ТОП"));
        basePageSteps.onLkSalesNewPage().billingPopup().priceHeader().waitUntil(hasText("697 \u20BD"));
        basePageSteps.onLkSalesNewPage().billingPopup().autoprolongCheckbox().waitUntil(isDisplayed()).click();
        basePageSteps.onLkSalesNewPage().billingPopup().tiedCardPayButton().waitUntil(isDisplayed()).click();
        basePageSteps.onLkSalesNewPage().billingPopup().autoProlongButton().waitUntil(isDisplayed()).click();
        basePageSteps.switchToDefaultFrame();

        basePageSteps.onLkSalesNewPage().notifier().waitUntil(isDisplayed()).waitUntil(hasText(AUTOPROLONG_ACTIVATED));
        basePageSteps.onLkSalesNewPage().billingPopupFrame().waitUntil(not(isDisplayed()));
        basePageSteps.onLkSalesNewPage().getSale(0).getVas(3).should(hasText("×15 просмотров\nАвтопродление"));
    }
