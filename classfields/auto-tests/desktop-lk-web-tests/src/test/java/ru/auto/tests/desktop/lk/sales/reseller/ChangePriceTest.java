...
public class ChangePriceTest {
    ...
    public void shouldEditPrice() {
        basePageSteps.onLkResellerSalesPage().getSale(0).price().changePriceIcon().click();
        basePageSteps.onLkResellerSalesPage().editPricePopup().input("Цена", "700000");
        basePageSteps.onLkResellerSalesPage().editPricePopup().button("Применить").click();

        basePageSteps.onLkResellerSalesPage().notifier().should(hasText("Цена изменена"));
        basePageSteps.onLkResellerSalesPage().getSale(0).price().priceText().should(hasText("700 000 ₽"));
    }
}
