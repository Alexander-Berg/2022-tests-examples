...
@DisplayName("Покупка услуги «Выделение цветом»")
...
public class VasBuyColorTest {
                ...
                stub().withPostDeepEquals(BILLING_AUTORU_PAYMENT_INIT)
                        .withRequestBody(
                                paymentInitVasRequest()
                                        .setProducts(ALL_SALE_COLORS).getBody())
                        .withResponseBody(
                                paymentInitVasResponse()
                                        .setCost(COST)
                                        .setDetailedProductInfos(ALL_SALE_COLORS)
                                        .getBody()),
    ...
    public void shouldBuyVas() {
        basePageSteps.onLkSalesPage().getSale(0).colorIcon().should(isDisplayed()).click();
        ...
        newTrustSteps.payForVas(ALL_SALE_COLORS.getName(), COST, "Опция «Выделение цветом» подключена");

        basePageSteps.onLkSalesPage().getSale(0).activatedColorIcon().should(isDisplayed());
        basePageSteps.onLkSalesPage().getSale(0).colorIcon().should(not(isDisplayed()));
    }
