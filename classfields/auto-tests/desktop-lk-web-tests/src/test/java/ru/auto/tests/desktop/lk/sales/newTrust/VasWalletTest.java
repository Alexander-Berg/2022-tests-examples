...
public class VasWalletTest {
    ...
    private static final int ACCOUNT_BALANCE = getRandomBetween(2500, 5000);
    ...
    public void shouldBuyVas() {
        ...
        basePageSteps.onLkSalesPage().newTrust().title().waitUntil(hasText(EXPRESS_SALE));
        basePageSteps.onLkSalesPage().newTrust().price().waitUntil(hasText(formatPrice(COST)));

        basePageSteps.onLkSalesPage().newTrust().paymentMethods().get(0).title().waitUntil(hasText(
                format("Кошелёк %s", formatPrice(ACCOUNT_BALANCE))));
        newTrustSteps.onLkSalesPage().newTrust().payButton().waitUntil(isDisplayed()).click();

        basePageSteps.onLkSalesPage().newTrust().statusTitle().waitUntil(hasText(PAYMENT_SUCCESS));
