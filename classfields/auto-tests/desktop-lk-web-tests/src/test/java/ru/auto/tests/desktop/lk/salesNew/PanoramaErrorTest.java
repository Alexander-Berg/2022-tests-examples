...
@DisplayName("Ошибка панорамы")
@Epic(AutoruFeatures.LK_NEW)
@Feature(AutoruFeatures.MY_OFFERS_PRIVATE)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
@Ignore
public class PanoramaErrorTest {
    ...
    public void before() {
        ...
        mockRule.setStubs(
                ...
                stub("desktop/User"),
                stub("desktop/UserOffersCarsActivePanoramaError"),
    ...
    @DisplayName("Ошибка панорамы")
    public void shouldSeePanoramaError() {
        basePageSteps.onLkSalesNewPage().getSale(0).panoramaError().should(hasText("Ошибка панорамы")).click();

        urlSteps.testing().path(CARS).path(USED).path(EDIT).path(SALE_ID).fragment("panorama").shouldNotSeeDiff();
    }
