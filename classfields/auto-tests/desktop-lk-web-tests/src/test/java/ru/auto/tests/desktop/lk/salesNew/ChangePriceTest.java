...
import static ru.auto.tests.desktop.element.lk.PricePopup.APPLY;
...
public class ChangePriceTest {
    ...
    public void shouldChangePrice() {
        basePageSteps.onLkSalesNewPage().getSale(0).editPriceButton().waitUntil(isDisplayed()).click();
        basePageSteps.onLkSalesNewPage().pricePopup().waitUntil(isDisplayed());
        basePageSteps.onLkSalesNewPage().pricePopup().priceCleanButton().click();
        basePageSteps.onLkSalesNewPage().pricePopup().priceInput().sendKeys(PRICE);
        basePageSteps.onLkSalesNewPage().pricePopup().button(APPLY).waitUntil(isDisplayed()).click();

        basePageSteps.onLkSalesNewPage().pricePopup().button(APPLY).waitUntil(not(isDisplayed()));
        basePageSteps.onLkSalesNewPage().getSale(0).price().should(hasText("700 000 ₽"));
    }
