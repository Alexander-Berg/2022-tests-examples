...
public class ActivateSecondSaleCarsTest {
    ...
    public void shouldActivate() {
        basePageSteps.onLkSalesPage().getSale(1).button(ACTIVATE).click();
        basePageSteps.onLkSalesPage().popup().waitUntil(hasText("Платное размещение\nВы уже истратили все " +
                "бесплатные объявления и теперь можете размещаться только платно. Срок размещения — 60 дней.\n" +
                "Разместить за 99 \u20BD"));
        basePageSteps.onLkSalesPage().popup().buttonContains("Разместить за ").waitUntil(isDisplayed()).click();
        ...
        basePageSteps.onLkSalesPage().billingPopup().priceHeader().waitUntil(hasText("99 \u20BD"));
        ...
        basePageSteps.onLkSalesPage().getSale(1).paidIcon().waitUntil(isDisplayed());
