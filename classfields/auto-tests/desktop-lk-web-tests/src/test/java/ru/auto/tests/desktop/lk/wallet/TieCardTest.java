...
@DisplayName("Кошелёк - привязка карты")
@Feature(LK)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class TieCardTest {
    ...
    public void before() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                "desktop/User",
                "desktop-lk/BillingAutoruPaymentInitWalletTiedCard",
                "desktop-lk/BillingAutoruPaymentProcessWalletTiedCard",
                "desktop/BillingAutoruPayment").post();

        urlSteps.testing().path(MY).path(WALLET).open();

        mockRule.overwriteStub(1, "desktop-lk/UserWithTiedCards");
    }

    @Test
    @Category({Regression.class, Testing.class, Billing.class})
    @Owner(DSVICHIHIN)
    @DisplayName("Привязка карты")
    public void shouldTieCard() {
        basePageSteps.onWalletPage().tiedCards().addCardButton().click();
        basePageSteps.onWalletPage().switchToBillingFrame();
        basePageSteps.onWalletPage().billingPopup().waitUntil(isDisplayed());
        basePageSteps.onLkSalesPage().billingPopup().rememberCardCheckbox()
                .should(hasClass(containsString("Checkbox_disabled Checkbox_checked")));
        basePageSteps.onWalletPage().billingPopup().header().waitUntil(hasText("Привязка карты"));
        basePageSteps.onWalletPage().billingPopup().priceHeader().waitUntil(hasText("1 \u20BD"));
        ...
        basePageSteps.onWalletPage().tiedCards().select("4111 11** **** 1111").waitUntil(isDisplayed());
        basePageSteps.onWalletPage().tiedCards().removeCardButton().waitUntil(isDisplayed());
    }
}
