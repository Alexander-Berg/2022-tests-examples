...
@Story("Заполнение и отправка заявки для eCredit")
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class CreditWizardEcreditTest {
    ...
    public void before() {
        mockRule.setStubs(
                ...
                stub("desktop/SharkCreditApplicationActiveEcreditWithOffers"),
                stub("desktop/SharkCreditApplicationActiveEcreditWithOffersWithPersonalProfile"),
                stub("desktop-lk/SharkCreditProductList"),
                stub("desktop-lk/SharkCreditApplicationUpdateEcredit")
        ).create();
    ...
    @Owner(TIMONDL)
    @DisplayName("Заявка на кредит для eCredit")
    public void shouldFillCreditApplication() {
        ...
        mockRule.setStubs(
                ...
                stub("desktop-lk/SharkCreditApplicationUpdateEcreditFull")
        ).create();
    ...
    private void showNextButton() {
        basePageSteps.setElementAttribute(basePageSteps.onLkCreditsPage().creditsForm().floatingControls(),
                "style", "display:block");
    }
    ...
    private void fillToTheEnd() {
        ...
        basePageSteps.onLkCreditsPage().creditsForm().geoSuggest().getItem(0).click();
        basePageSteps.onLkCreditsPage().creditsForm().block(ADDRESS).input("Дата регистрации", "01.01.2010");
        ...
        basePageSteps.onLkCreditsPage().creditsForm().block(DRIVING_LICENSE)
                .input("Номер водительского удостоверения", "12 AA 567890");
        basePageSteps.onLkCreditsPage().creditsForm().block(DRIVING_LICENSE)
                .input("Дата выдачи", "30.03.2020");
        showNextButton();
        basePageSteps.onLkCreditsPage().creditsForm().button(NEXT).click();
        hideNextButton();
    }
