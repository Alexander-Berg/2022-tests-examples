...
public class VasBuyAutoFreshTest {
    ...
    public void shouldTurnAutoFreshOnAndOff() {
        ...
        basePageSteps.onLkSalesPage().getSale(0).autoFreshPopup().waitUntil(isDisplayed());
    ...
    public void shouldSelectTime() {
        basePageSteps.onLkSalesPage().getSale(0).freshButton().waitUntil(isDisplayed()).hover();
        basePageSteps.onLkSalesPage().getSale(0).openPopup().waitUntil(isDisplayed());
        basePageSteps.onLkSalesPage().getSale(0).openPopup().selectTime("в 16:00");
    ...
    private void turnAutoFreshOn() {
        basePageSteps.onLkSalesPage().getSale(0).autoFreshPopup().turnOnButton().waitUntil(isDisplayed())
                .waitUntil(hasText("Включить за 87 ₽ в день")).hover().click();
        basePageSteps.onLkSalesPage().notifier().waitUntil(isDisplayed())
                .waitUntil(hasText("Автоподнятие активировано"));
        basePageSteps.onLkSalesPage().getSale(0).freshButton()
                .waitUntil(hasText("Поднять за 87 ₽\nВключено автоподнятие"));
    }

    @Step("Выключаем автоподнятие")
    private void turnAutoFreshOff() {
        basePageSteps.onLkSalesPage().getSale(0).autoFreshPopup().turnOffButton()
                .waitUntil(hasText("Выключить автоподнятие"));
        basePageSteps.onLkSalesPage().getSale(0).autoFreshPopup().turnOffButton().waitUntil(isDisplayed()).hover()
                .click();
        basePageSteps.onLkSalesPage().notifier().waitUntil(isDisplayed())
                .waitUntil(hasText("Автоподнятие отключено"));
        basePageSteps.onLkSalesPage().getSale(0).freshButton()
                .waitUntil(hasText(containsString("Поднять за 87 ₽")));
    }
}
