...
public class VasBuyStoryTest {
    ...
    public void before() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                salesMock,
                offerIdMock,
                "desktop-lk/reseller/BillingAutoruPaymentInitStory",
    ...
    public void shouldBuyVas() {
        basePageSteps.onLkResellerSalesPage().getSale(0).vas("Показ в Историях").should(isDisplayed()).click();
        ...
        basePageSteps.onLkResellerSalesPage().billingPopup().header().waitUntil(hasText("Показ в Историях"));
        basePageSteps.onLkResellerSalesPage().billingPopup().priceHeader().waitUntil(hasText("834 \u20BD"));
        ...
        basePageSteps.onLkResellerSalesPage().getSale(0).vas("Показ в Историях")
                .should(hasText("×40 просмотров\nПоказ в Историях • 3 дня\nАвтопродление"));
    }
}
