...
public class ActiveUnfoldedSaleTrucksTest {
    ...
    public void shouldClickSaleTitle() {
        ...
        urlSteps.testing().path(LCV).path(USED).path(SALE).path("/hyundai/porter/").path(SALE_ID_HASH)
    ...
    public void shouldClickEditButton() {
        ...
        basePageSteps.switchToNextTab();
        urlSteps.testing().path(TRUCKS).path(EDIT).path(SALE_ID_HASH).path("/").shouldNotSeeDiff();
    }
