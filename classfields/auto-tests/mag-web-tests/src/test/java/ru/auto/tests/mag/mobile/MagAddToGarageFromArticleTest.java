...
public class MagAddToGarageFromArticleTest {
    ...
    @Owner(ALEKS_IVANOV)
    @DisplayName("Отображается бабл, проскролив экран")
    public void shouldSeeBubble() {
        ...
        urlSteps.open();
        basePageSteps.scrollDown(PXLS_FOR_BANNER);
    ...
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Не отображается бабл, без скрола экрана")
    public void shouldNotSeeBubbleWithoutScroll() {
        mockRule.setStubs(
                stub().withGetDeepEquals(format("%s/%s", GARAGE_USER_CARD_ARTICLE, ARTICLE_PATH))
                        .withResponseBody(
                                garageArticleCard().setStatusSuccess().getBody())
    ...
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Не отображается бабл, без карточки гаража в ответе ручки")
    ...
    public void shouldGoToGarageFromBubble() {
        ...
        urlSteps.mobileURI().path(GARAGE).path(GARAGE_CARD_ID).path(SLASH).shouldNotSeeDiff();
    }
