...
public class MagArticleSalesTest {
    ...
    public void shouldClickTitle() {
        basePageSteps.onMagPage().sales().title().click();
        urlSteps.switchToNextTab();

        urlSteps.testing().path(MOSKVA).path(CARS).path(ALL).addParam(FROM, MAG_AD_CAROUSEL).shouldNotSeeDiff();
    ...
    public void shouldClickSale() {
        basePageSteps.onMagPage().sales().getSale(0).waitUntil(isDisplayed()).click();
        urlSteps.switchToNextTab();
    ...
    @Category({Regression.class, Testing.class})
    @Owner(DSVICHIHIN)
    @DisplayName("Клик по ссылке «Все {mark} на Авто.р»")
    public void shouldClickMoreSalesUrl() {
        basePageSteps.onMagPage().sales().button(format("Все %s на Авто.ру", MARK.toUpperCase())).click();
        urlSteps.switchToNextTab();

        urlSteps.testing().path(MOSKVA).path(CARS).path(MARK).path(ALL).addParam(FROM, MAG_AD_CAROUSEL).shouldNotSeeDiff();
        basePageSteps.onCardPage().title()
                .should(hasAttribute("textContent", startsWith("Купить ")));
    }
}
