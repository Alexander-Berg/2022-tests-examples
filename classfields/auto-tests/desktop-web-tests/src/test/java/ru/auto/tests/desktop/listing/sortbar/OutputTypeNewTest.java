...
public class OutputTypeNewTest {
    ...
    public void shouldSelectTableOutput() {
        ...
        urlSteps.addParam("output_type", "table").shouldNotSeeDiff();
    ...
    public void shouldSelectListOutput() {
        ...
        urlSteps.addParam("output_type", "list").shouldNotSeeDiff();
        basePageSteps.onListingPage().salesList().waitUntil(hasSize(greaterThan(0)));
    }
    ...
    @Category({Regression.class})
    @Owner(DSVICHIHIN)
    @DisplayName("По моделям")
    public void shouldSelectModelsOutput() {
        urlSteps.testing().path(MOSKVA).path(category).path(section).addParam("output_type", "table").open();
        basePageSteps.onListingPage().sortBar().modelSwitcher().click();
        urlSteps.replaceQuery("").shouldNotSeeDiff();
        basePageSteps.onListingPage().groupSalesList().waitUntil(hasSize(greaterThan(0)));
    }
}