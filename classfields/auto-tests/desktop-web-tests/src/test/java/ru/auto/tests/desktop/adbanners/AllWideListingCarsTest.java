...
public class AllWideListingCarsTest {
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Баннеры CARS/ALL ->  Марка, Модель")
    public void shouldOpenBannerMarkModelSedan() {
        urlSteps.testing().path(CARS).path("hyundai").path("solaris").path(ALL).open();
        basePageSteps.onListingPage().getSale(snippetSequence).hover();
        basePageSteps.scrollDown(300);
