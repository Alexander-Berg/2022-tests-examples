...
@DisplayName("Цели на лэндинге новых авто модели")
@Epic(PROMO)
@Feature(LANDING_NEW_CARS)
@Story("Страница модели")
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopDevToolsTestsModule.class)
public class LandingNewCarsModelGoalsTest {
    ...
    @Before
    public void before() {
        urlSteps.testing().path(MOSKVA).path(CARS).path(KIA).path("/k5/").path(Pages.PROMO).addParam(DEBUG, TRUE).open();
    }
    ...
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Цель «CARD_VIEW_LANDING_NEWAUTO» по клику на большой снипет модели")
    public void shouldSeeCardViewLandingNewautoMajorSnippet() {
        basePageSteps.onPromoCarsLandingPage().majorSnippet().click();
    ...
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Цель «PHONE_LANDING_NEWAUTO_POPUP» по клику на «Получить лучшую цену» в попапе большого снипета модели")
    public void shouldSeePhoneLandingNewautoPopupMajorSnippet() {
        basePageSteps.onPromoCarsLandingPage().majorSnippet().click();
        basePageSteps.onPromoCarsLandingPage().popup().input(PHONE_NUMBER).sendKeys(PHONE);
        basePageSteps.onPromoCarsLandingPage().popup().button(GET_BEST_PRICE).click();
    ...
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Цель «PHONE_LANDING_NEWAUTO_ALL» по клику на «Получить лучшую цену» в попапе большого снипета модели")
    public void shouldSeePhoneLandingNewautoAllFromPopupMajorSnippet() {
        basePageSteps.onPromoCarsLandingPage().majorSnippet().click();
        basePageSteps.onPromoCarsLandingPage().popup().input(PHONE_NUMBER).sendKeys(PHONE);
        basePageSteps.onPromoCarsLandingPage().popup().button(GET_BEST_PRICE).click();
