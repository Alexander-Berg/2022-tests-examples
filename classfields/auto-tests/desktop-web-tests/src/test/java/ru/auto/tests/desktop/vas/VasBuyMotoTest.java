...
@DisplayName("Карточка объявления - покупка услуги")
@Feature(VAS)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class VasBuyMotoTest {
    ...
    public void before() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                "desktop/BillingAutoruPaymentInitExpress",
                "desktop/BillingAutoruPaymentProcessMotoSale",
    ...
    public void shouldBuyVas() {
        basePageSteps.onCardPage().cardVas().tab("Экспресс-продажа").should(isDisplayed()).hover();
        basePageSteps.onCardPage().cardVas().activeTab().waitUntil(hasText("Экспресс-продажа"));
        ...
        basePageSteps.onCardPage().billingPopup().header().waitUntil(hasText("Экспресс продажа"));
        basePageSteps.onCardPage().billingPopup().priceHeader().waitUntil(hasText("219 \u20BD397 \u20BD"));
        ...
        basePageSteps.onCardPage().notifier().waitUntil(isDisplayed())
                .waitUntil(hasText("Пакет «Экспресс-продажа» подключён"));
