...
@DisplayName("Баннеры на листинге «CARS / USED»")
@Epic(BANNERS)
@Feature(LISTING)
@Story("Узкий экран")
...
public class UsedNarrowListingTest {
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Баннеры CARS/USED ->  Марка, Модель")
    public void shouldOpenBannerMarkModelSedan() {
        urlSteps.testing().path(CARS).path("hyundai").path("solaris").path(USED).open();
        basePageSteps.onListingPage().getSale(snippetSequence).hover();
        basePageSteps.scrollDown(300);
