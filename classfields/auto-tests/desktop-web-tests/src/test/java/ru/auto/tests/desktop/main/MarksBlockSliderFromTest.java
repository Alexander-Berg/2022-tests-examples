...
@DisplayName("Главная - блок марок - слайдер цены от")
@Feature(MAIN)
@RunWith(Parameterized.class)
@GuiceModules(DesktopTestsModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class MarksBlockSliderFromTest {
    ...
    @Owner(DSVICHIHIN)
    @DisplayName("Слайдер от")
    public void shouldSlideFrom() {
        basePageSteps.dragAndDrop(basePageSteps.onMainPage().marksBlock().sliderFrom(), offset, 0);
        ...
        urlSteps.path(CARS).path(ALL).addParam("price_from", priceFrom).shouldNotSeeDiff();
    }
}