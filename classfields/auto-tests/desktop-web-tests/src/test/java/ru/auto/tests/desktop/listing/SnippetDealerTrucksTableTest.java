...
public class SnippetDealerTrucksTableTest {
    ...
    public void before() {
        ...
        urlSteps.testing().path(MOSKVA).path(TRUCK).path(ALL).addParam(OUTPUT_TYPE, TABLE).open();
    }

    @Test
    @Owner(DSVICHIHIN)
    @Category({Regression.class, Testing.class})
    @DisplayName("Отображение грузового сниппета диллера, тип листинга «Таблица»")
    ...
    @Owner(DSVICHIHIN)
    @Category({Regression.class, Testing.class})
    @DisplayName("Ссылка на страницу дилера, грузовой сниппет, тип листинга «Таблица»")
