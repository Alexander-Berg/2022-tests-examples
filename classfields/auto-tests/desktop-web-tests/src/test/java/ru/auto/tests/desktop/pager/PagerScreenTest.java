...
public class PagerScreenTest {
    ...
    public static Collection<Object[]> getParameters() {
        return asList(new Object[][]{
                {CARS, "desktop/SearchCarsOneSale", "desktop/SearchCarsBreadcrumbsEmpty"},
                {TRUCK, "desktop/SearchTrucksOneSale", "desktop/SearchTrucksBreadcrumbsEmpty"},
                {MOTORCYCLE, "desktop/SearchMotoOneSale", "desktop/SearchMotoBreadcrumbsEmpty"}
        });
    }
    ...
    @DisplayName("Отображение пагинатора")
    public void shouldSeePager() {
        basePageSteps.onListingPage().pager().waitUntil(isDisplayed()).should(hasText("Показать ещё\n1\n2\n← Ctrl, " +
                "Предыдущая\nСледующая, Ctrl →"));
    }
}