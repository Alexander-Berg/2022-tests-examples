...
public class SafeDealBlockUserTest {
    ...
    public void before() {
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                "desktop/SafeDealDealCreate").post();
    ...
    public void shouldScrollToDeal() {
        basePageSteps.onCardPage().cardHeader().dealLink().waitUntil(isDisplayed()).click();
    ...
    public void shouldSeeDealBlock() {
        basePageSteps.onCardPage().dealBlock().waitUntil(isDisplayed()).should(hasText("Купите этот автомобиль с " +
                "Безопасной сделкой\nАвто.ру бесплатно поможет с договором и передачей денег. Подробнее\nНачать " +
    ...
    public void shouldClickMoreButton() {
        basePageSteps.onCardPage().dealBlock().button("Подробнее").waitUntil(isDisplayed()).click();
    ...
    public void shouldClickCancelButton() {
        ...
        createSafeDeal();
        basePageSteps.onCardPage().existingDealBanner().button("Отменить запрос").waitUntil(isDisplayed()).click();
        basePageSteps.onCardPage().popup().waitUntil(isDisplayed()).should(hasText("Почему решили отменить?" +
                "\nВыберите причину\nОтправить"));
        basePageSteps.onCardPage().popup().selectItem("Выберите причину", "Договорился с другим продавцом");
        basePageSteps.onCardPage().popup().button("Отправить").click();
        basePageSteps.onCardPage().notifier().waitUntil(isDisplayed())
                .should(hasText("Спасибо! Вы помогаете стать Авто.ру ещё лучше"));
        basePageSteps.onCardPage().dealBlock().button("Начать сделку").waitUntil(isDisplayed());
    }

    @Test
    @Owner(KIRILL_PKR)
    @Category({Regression.class, Testing.class})
    @DisplayName("Клик по кнопке «Отменить заявку» с причиной отмены «Другое»")
    public void shouldClickCancelButtonWithAnotherReason() {
        ...
        basePageSteps.onCardPage().popup().waitUntil(isDisplayed()).should(hasText("Почему решили отменить?" +
                "\nВыберите причину\nОтправить"));
        basePageSteps.onCardPage().popup().selectItem("Выберите причину", "Другое");
        basePageSteps.onCardPage().popup().input("Опишите причину", "test test");
        basePageSteps.onCardPage().popup().button("Отправить").click();
        basePageSteps.onCardPage().notifier().waitUntil(isDisplayed())
                .should(hasText("Спасибо! Вы помогаете стать Авто.ру ещё лучше"));
        basePageSteps.onCardPage().dealBlock().button("Начать сделку").waitUntil(isDisplayed());
    ...
    @Category({Regression.class, Testing.class})
    @Owner(TIMONDL)
    @DisplayName("Кнопки «Безопасная сделка» в заголовке объявления не должно быть")
    public void shouldNotSeeSafeDealButtonInCardHeader() {
        mockRule.overwriteStub(1, "desktop/OfferCarsUsedUser");
        urlSteps.refresh();

        basePageSteps.onCardPage().cardHeader().dealLink().should(not(isDisplayed()));
    }
    ...
    public void createSafeDeal() {
        basePageSteps.onCardPage().dealBlock().button("Начать сделку").waitUntil(isDisplayed()).click();
        ...
        basePageSteps.onCardPage().existingDealBanner().waitUntil(isDisplayed()).should(hasText("Подождите, пока продавец" +
                " примет ваш запрос на сделку\n" +
                "Отменить запросПерейти к сделкам"));
    }
}