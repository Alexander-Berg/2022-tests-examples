...
public class HeaderBurgerUrlsTest {
    ...
    @DisplayName("Клик по ссылкам в мега-меню")
    public void shouldClickUrlsInMegaMenu() {
        basePageSteps.onMainPage().header().burgerButton().hover();
        basePageSteps.onMainPage().header().burger().button(title).click();

        urlSteps.fromUri(format(url, urlSteps.getConfig().getBaseDomain())).ignoreParam(GEO_ID).ignoreParam("rgid")
                .shouldNotSeeDiff();
    }
