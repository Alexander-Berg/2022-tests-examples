...
import static ru.auto.tests.desktop.consts.Notifications.ADDED_GO_TO_COMPARE;
...
import static ru.auto.tests.desktop.mock.Paths.USER_COMPARE_CARS;
import static ru.auto.tests.desktop.mock.beans.stub.Query.query;
import static ru.auto.tests.desktop.mock.beans.userCompare.UserCompareResponse.userCompareResponse;
import static ru.auto.tests.desktop.page.ElectroModelPage.COMPARE;
import static ru.auto.tests.desktop.page.ElectroModelPage.GO_TO_COMPARE;
...
public class ElectroModelTest {
    ...
    public void shouldGoToGarage() {
        ...
        basePageSteps.onGarageCardPage().gallery().badge(CardGallery.DREAM_CAR).should(isDisplayed());
    ...
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Жмем «Сравнить»")
    public void shouldCompareModel() {
        mockRule.setStubs(
                sessionAuthUserStub(),
                stub().withGetDeepEquals(REFERENCE_CATALOG_CARS)
                        .withRequestQuery(
        ...
        mockRule.setStubs(
                stub().withPostDeepEquals(format("%s/%s", USER_COMPARE_CARS, "21057314__23069875"))
                        .withStatusSuccessResponse(),
                stub().withGetDeepEquals(USER_COMPARE_CARS)
                        .withResponseBody(getJsonObject(
                                userCompareResponse().setCatalogCardIds(CATALOG_CARD_IDS)))
        ).update();

        basePageSteps.onElectroModelPage().toCompare().waitUntil(hasText(COMPARE)).click();

        basePageSteps.onElectroModelPage().notifier(ADDED_GO_TO_COMPARE).should(isDisplayed());
        basePageSteps.onElectroModelPage().toCompare().should(hasText(GO_TO_COMPARE));
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Жмем «К сравнению»")
    public void shouldGoToCompareModel() {
        mockRule.setStubs(
                sessionAuthUserStub(),
                stub().withGetDeepEquals(REFERENCE_CATALOG_CARS)
                        .withRequestQuery(
        ...
        basePageSteps.onElectroModelPage().toCompare().waitUntil(hasText(GO_TO_COMPARE)).click();

        basePageSteps.onComparePage().h1().should(hasText("Сравнение моделей"));
    ...
    public void shouldSeeCarOffer() {
        ...
        basePageSteps.onElectroModelPage().offers().waitUntil(hasSize(8)).get(0).should(
                hasText(format("%s\n%s %s, %d\n%s",
                REGION_NAME, MARK_NAME, MODEL_NAME, YEAR, formatPrice(PRICE))));
    }
