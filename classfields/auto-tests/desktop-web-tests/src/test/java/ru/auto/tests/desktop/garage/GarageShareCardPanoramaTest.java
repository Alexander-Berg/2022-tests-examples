...
@DisplayName("Гараж. Панорама на share карточке")
...
public class GarageShareCardPanoramaTest {
    ...
    public void shouldOpenGallery() {
        basePageSteps.onGarageCardPage().panorama().pointer().waitUntil(isDisplayed()).click();
    ...
    public void shouldHideAndShowSpots() {
        basePageSteps.onGarageCardPage().panorama().pointer().waitUntil(isDisplayed()).click();
        basePageSteps.onGarageCardPage().panorama().showHideSpotsButton().hover();
        basePageSteps.onGarageCardPage().panorama().showHideSpotsButton().should(hasText("Скрыть точки")).click();
        basePageSteps.onGarageCardPage().panorama().spotsList().waitUntil(hasSize(0));

        basePageSteps.onGarageCardPage().panorama().showHideSpotsButton().hover();
        basePageSteps.onGarageCardPage().panorama().showHideSpotsButton().should(hasText("Показать точки")).click();
        basePageSteps.onGarageCardPage().panorama().spotsList().waitUntil(hasSize(2));
    }
    ...
    public void shouldSeeSpots() {
        basePageSteps.onGarageCardPage().panorama().pointer().waitUntil(isDisplayed()).click();
        basePageSteps.onGarageCardPage().panorama().getSpot(0).hover();
        basePageSteps.onGarageCardPage().popup().waitUntil(isDisplayed()).should(hasText("AMG обвес"));
        basePageSteps.onGarageCardPage().popup().click();

        basePageSteps.onGarageCardPage().panorama().getSpot(1).hover();
        basePageSteps.onGarageCardPage().popup().waitUntil(isDisplayed()).should(hasText("Двухцветная отделка салона, " +
                "отделка сидений - натуральная кожа"));

        basePageSteps.onGarageCardPage().panorama().getSpot(1).click();
        basePageSteps.onGarageCardPage().panorama().spotPhotoPopup().waitUntil(isDisplayed())
                .should(hasText("Двухцветная отделка салона, отделка сидений - натуральная кожа"));
        basePageSteps.onGarageCardPage().panorama().spotPhotoPopup().photo()
                .waitUntil(hasAttribute("src", format("%s/%s", urlSteps.getConfig().getAvatarsURI(),
                        IMAGE_PATH)));
    }
}
