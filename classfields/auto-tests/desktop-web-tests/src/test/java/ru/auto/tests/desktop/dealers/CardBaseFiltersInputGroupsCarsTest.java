...
public class CardBaseFiltersInputGroupsCarsTest {
    ...
    public void shouldInputFrom() {
        basePageSteps.onDealerCardPage().filter().inputGroup(paramName).input("от").sendKeys(paramValue);

        urlSteps.path(CARS_OFFICIAL_DEALER_MARK).path(SLASH)
                .addParam(format("%s_from", paramQueryName), paramValue)
                .shouldNotSeeDiff();
    ...
    public void shouldInputTo() {
        basePageSteps.onDealerCardPage().filter().inputGroup(paramName).input("до").sendKeys(paramValue);

        urlSteps.path(CARS_OFFICIAL_DEALER_MARK).path(SLASH)
                .addParam(format("%s_to", paramQueryName), paramValue)
                .shouldNotSeeDiff();
        basePageSteps.onDealerCardPage().filter().resultsButton().waitUntil(isDisplayed()).click();
