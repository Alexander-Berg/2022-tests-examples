...
public class PricePopupExchangeForGarageCarTest {
    ...
    private static final String PRICE_POPUP_TEXT_TEMPLATE = "%s\nХорошая цена\n23 446 $\n · \n22 047 €\n" +
            ...
            "О скидках и акциях узнавайте по телефону\nСтоимость этого автомобиля соответствует средней рыночной " +
            "относительно похожих автомобилей\nПодробней про хорошую цену";
    ...
    public void shouldSeePricePopupTextDreamExAndOneCurrentCars() {
        ...
        basePageSteps.onCardPage().pricePopup().should(hasText(format(
                PRICE_POPUP_TEXT_TEMPLATE,
    ...
    public void shouldSeePricePopupTextDreamExAndTwoCurrentCars() {
        ...
        basePageSteps.onCardPage().pricePopup().should(hasText(format(PRICE_POPUP_TEXT_TEMPLATE,
                formatPrice(offerPrice),
                firstMarkName,
                firstModelName,
                formatPrice(tradeinPrice),
    ...
    public void shouldSeePricePopupTextDreamGarageCar() {
        ...
        basePageSteps.onCardPage().pricePopup().should(hasText(format(PRICE_POPUP_WITHOUT_CURRENT_CAR_TEMPLATE,
                formatPrice(offerPrice))));
    }
    ...
    public void shouldChangeCurrentGarageCar() {
        ...
        basePageSteps.onCardPage().pricePopup().button(format("%s %s", firstMarkName, firstModelName))
                .waitUntil(isDisplayed()).click();
        basePageSteps.onCardPage().exchangeCarPopup().button(format("%s %s", secondMarkName, secondModelName))
                .waitUntil(isDisplayed()).click();
