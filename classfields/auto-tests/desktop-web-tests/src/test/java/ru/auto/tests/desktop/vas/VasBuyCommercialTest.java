...
public class VasBuyCommercialTest {
    ...
    public void before() {
        ...
        mockRule.newMock().with("desktop/SessionAuthUser",
                ...
                "desktop/BillingAutoruPaymentInitFresh",
                "desktop/BillingAutoruPaymentProcessTrucksSale",
    ...
    public void shouldBuyVas() {
        basePageSteps.onCardPage().cardVas().tab("Поднятие в поиске").should(isDisplayed()).hover();
        basePageSteps.onCardPage().cardVas().activeTab().waitUntil(hasText("Поднятие в поиске"));
        ...
        basePageSteps.onCardPage().billingPopup().header().waitUntil(hasText("Поднятие в поиске"));
        basePageSteps.onCardPage().billingPopup().priceHeader().waitUntil(hasText("97 \u20BD"));
