...
public class FeaturesScootersTest {
    ...
    @Before
    public void before() {
        mockRule.newMock().with("desktop/OfferMotoUsedUserScooter").post();

        urlSteps.testing().path(SCOOTERS).path(USED).path(SALE).path(SALE_ID).open();
    }
