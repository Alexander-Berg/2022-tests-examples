...
public class SnippetDealerMotoCarouselTest {
    ...
    public void before() {
        ...
        urlSteps.testing().path(MOSKVA).path(MOTORCYCLE).path(ALL).addParam(OUTPUT_TYPE, CAROUSEL).open();
    }

    @Test
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Отображение мото сниппета диллера, тип листинга «Карусель»")
    public void shouldSeeSnippet() {
        basePageSteps.onListingPage().getCarouselSale(5).should(hasText("Triumph Street Triple\n675 см³\n106 л.с.\n" +
                "4 такта\n3 цилиндра\nрядное\n6 передач\nцепь\nзелёный\n32 550 км\n2010\n340 000 ₽\nПоказать телефон\n" +
                "Перезвоните мне\nТемп\nМосква"));
    }
    ...
    @Owner(ALEKS_IVANOV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Ссылка на страницу дилера, мото сниппет, тип листинга «Карусель»")
