...
@DisplayName("Блок «Кредит Ecredit» на карточке под незарегом")
@Feature(AutoruFeatures.CREDITS)
@Story(AutoruFeatures.DEALERS)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class CreditDealerEcreditUnauthTest {
    ...
    public void shouldSeeDealersCredit() {
        ...
        basePageSteps.onCardPage().cardCreditBlock().should(hasText("Подбор автокредита у дилера в разных банках" +
                ...
                "Срок кредита\n7 лет\nПервый взнос\n0 ₽\nПлатеж\n20 500 ₽ / мес.\nФИО\nЭлектронная почта\n" +
                "Номер телефона\nПодтвердить\nДаю согласие ООО «Яндекс.Вертикали», ООО «Кредитит» и «САЛОНА " +
                "НЕ СУЩЕСТВУЕТ СОВСЕМ!!!» на обработку данных в целях рассмотрения заявки и на условия " +
                "обработки данных. ООО «Кредитит» осуществляет содействие в подборе финансовых услуг."));
    }
    ...
    public void shouldFillCreditRequest() {
        ...
        basePageSteps.onCardPage().cardCreditBlock().input("Электронная почта", "sosediuser1@mail.ru");
        basePageSteps.onCardPage().cardCreditBlock().input("Номер телефона", "9111111111");
        basePageSteps.onCardPage().cardCreditBlock().button("Подтвердить").waitUntil(isDisplayed()).click();
        basePageSteps.onCardPage().cardCreditBlock().input("Код из SMS").waitUntil(isDisplayed())
                .sendKeys("1234");
