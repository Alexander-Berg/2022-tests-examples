...
public class GarageDreamCarTransportTaxTest {
    ...
    private static final String REGION_NAME = "Санкт-Петербурге";
    ...
    private static final String TRANSPORT_TAX_TEMPLATE = "Транспортный налог\n3 900 ₽\n" +
            "Транспортный налог по данным на 2022 год в Санкт-Петербурге для 1.4 AMT (151 л.с.) бензин.";

    private static final String TRANSPORT_TAX_NO_BODY_TEMPLATE = "Транспортный налог\n3 900 ₽\n" +
            "Транспортный налог по данным на 2022 год в Санкт-Петербурге для 1.4 AMT (151 л.с.) бензин. " +
            "Для уточнения укажите: кузов";
    ...
    public void before() {
        garageCard = garageCardOffer()
                ...
                .setCardType(MockGarageCard.DREAM_CAR)
                .setTaxAmount(TAX)
                .setTaxRegionInfoNamePrepositional(REGION_NAME)
    ...
    public void shouldChangeRegionTransportTax() {
        ...
        mockRule.overwriteStub(1,
                stub().withGetDeepEquals(format("%s/%s", GARAGE_USER_CARD, GARAGE_CARD_ID))
                        .withResponseBody(
                                garageCard.setTaxRegionInfoNamePrepositional(NEW_REGION_NAME).
                                        setTaxBlockState(OK).getBody()));

        basePageSteps.onGarageCardPage().popup().input().sendKeys(NEW_REGION_NAME);
        basePageSteps.onGarageCardPage().geoSuggest().regionContains(NEW_REGION_NAME).click();
    ...
    public void shouldChangeBodyTransportTax() {
        ...
        basePageSteps.onGarageCardPage().transportTax().button("кузов").click();
