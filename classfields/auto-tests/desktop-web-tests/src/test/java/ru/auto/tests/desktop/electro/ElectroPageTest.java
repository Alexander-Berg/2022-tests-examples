...
public class ElectroPageTest {
    ...
    public void before() {
        mockRule.setStubs(
                ...
                stub().withGetDeepEquals(REVIEWS_AUTO_LISTING)
                        .withRequestQuery(
                                query().setCategory("CARS").setEngineType("ELECTRO").setPhoto(true).setPageSize("4"))
                        .withResponseBody(
                ...
                stub().withGetDeepEquals(SEARCH_CARS)
                        .withRequestQuery(
                                getSearchPromoElectroQuery().setPageSize("4"))
                        .withResponseBody(
    ...
    public void shouldSeeCarOffer() {
        ...
        basePageSteps.onElectroPage().offersSection().offers().waitUntil(hasSize(8)).get(0).should(hasText(
                format("%s\n%s %s, %d\n%s",
    ...
    public void shouldGoToReview() {
        String reviewTitle = "Nissan Leaf Electro AT (80.0 кВт)\nИдеальная машина для москвы\n4,6";
    ...
    public void shouldGoToReviewMore() {
        basePageSteps.onElectroPage().reviewsSection().button().waitUntil(hasText(SHOW_MORE_REVIEW)).click();
