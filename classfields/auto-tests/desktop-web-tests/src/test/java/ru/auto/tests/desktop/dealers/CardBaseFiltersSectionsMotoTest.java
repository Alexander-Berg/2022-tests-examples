...
public class CardBaseFiltersSectionsMotoTest {
...
//                {NEW, "Все", ALL},
    ...
    public void before() {
        ...
        urlSteps.testing().path(DILER_OFICIALNIY).path(MOTORCYCLE).path(startSection)
                .path(DEALER_CODE).open();
    }
    ...
    public void shouldClickSection() {
        basePageSteps.onDealerCardPage().filter().radioButton(expectedSectionTitle).waitUntil(isDisplayed()).click();

        urlSteps.testing().path(DILER_OFICIALNIY).path(MOTORCYCLE).path(expectedSection)
                .path(DEALER_CODE)
                .path(SLASH)
                .shouldNotSeeDiff();
