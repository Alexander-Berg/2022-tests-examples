...
@Feature(SALES)
@DisplayName("Объявление - кнопка «Написать сообщение», когда пользователь запретил ему звонить")
@RunWith(Parameterized.class)
@GuiceModules(DesktopTestsModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class ContactsChatOnlyTest {
    ...
    @Test
    @DisplayName("Клик по кнопке «Написать сообщение» под незарегом")
    @Category({Regression.class, Testing.class})
    @Owner(DSVICHIHIN)
    public void shouldClickSendMessageButtonUnauth() {
        mockRule.setStubs(stub("desktop/SessionUnauth"),
                stub(saleMock)).create();

        urlSteps.testing().path(category).path(USED).path(SALE).path(SALE_ID).open();
        String saleUrl = urlSteps.getCurrentUrl();
        basePageSteps.onCardPage().contacts().showPhoneButton().should(not(isDisplayed()));
        basePageSteps.onCardPage().contacts().sendMessageButton().should(hasText("Написать сообщение")).click();
    ...
    @Owner(DSVICHIHIN)
    @Category({Regression.class, Testing.class})
    @DisplayName("Клик по кнопке «Написать сообщение» под зарегом")
    public void shouldClickSendMessageButtonAuth() throws IOException {
        ...
        basePageSteps.onCardPage().contacts().showPhoneButton().should(not(isDisplayed()));
        basePageSteps.onCardPage().contacts().sendMessageButton().waitUntil(isEnabled())
                .should(hasText("Написать сообщение")).click();
