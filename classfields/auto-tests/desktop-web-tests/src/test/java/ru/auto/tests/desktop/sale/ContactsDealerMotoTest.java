...
public class ContactsDealerMotoTest {
    ...
    public void shouldClickDealerName() {
        ...
        urlSteps.testing().path(DILER).path(MOTORCYCLE).path(USED).path("/megamoto_moskva/").shouldNotSeeDiff();
    }
    ...
    public void shouldSeeContactsPopup() {
        ...
        basePageSteps.onCardPage().contactsPopup().waitUntil(isDisplayed())
                .should(hasText("МЕГАМОТО Москва\nМотосалон\n+7 916 039-84-27\nРоман\nc 10:00 до 23:00\n+7 916 039-84-28\n" +
                        "Дмитрий\nc 12:00 до 20:00\n АвиамоторнаяМосквам. Авиамоторная, ул.5-я Кабельная, дом 2 " +
                        "(ТРК \"СпортЕХ\", 3 уровень)\nHonda VT 750 • 260 000 ₽\n750 см³ / 4 такта\n2 цилиндра / " +
