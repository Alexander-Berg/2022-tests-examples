...
public class CardLoyaltyTest {
    ...
    public void shouldSeeLoyaltyPopup() {
        basePageSteps.onDealerCardPage().info().button("Проверенный дилер").click();
        basePageSteps.onDealerCardPage().activePopup().waitUntil(isDisplayed())
                .should(hasText("Статус «Проверенный дилер» выдается автосалонам, которые:\n" +
