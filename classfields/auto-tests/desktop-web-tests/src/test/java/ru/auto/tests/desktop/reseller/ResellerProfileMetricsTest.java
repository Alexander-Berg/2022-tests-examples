...
public class ResellerProfileMetricsTest {
    ...
    private static final String PHONE_SHOW = "{\"all\":{\"reseller-public-page\":{\"show-phone\":{\"user\":{\"phonePopup\":{}}}}}}";
    private static final String LINK_SHOW = "{\"all\":{\"reseller-public-page\":{\"reseller_public\":{\"link-show\":{\"sellerPopup\":{}}}}}}";
    ...
    public void shouldSeeResellerPublicShowPhone() {
        ...
        basePageSteps.onResellerPage().salesList().get(0).showPhonesButton().click();
    ...
    @Category({Regression.class, Testing.class})
    @Owner(ALEKS_IVANOV)
    @DisplayName("Метрика «link-show» при показе ссылки на профиль продавца в попапе телефона")
    public void shouldSeeResellerPublicLinkShowPhonePopup() {
        mockRule.setStubs(
                stub("cabinet/OfferCarsPhones").withPredicateType(PredicateType.EQUALS)
                        .withPath(format("%s/%s/phones", OFFER_CARS, ID))
        ).update();
        ...
        seleniumMockSteps.assertWithWaiting(onlyOneMetricsRequest(hasSiteInfo(LINK_SHOW)));
    }
