...
public class PresetsTest {
    ...
    public static Collection<Object[]> getParameters() {
        return asList(new Object[][]{
                {CARS, "Полезные", LIKE.getAlias()},
                {CARS, "Обсуждаемые", COMMENTS.getAlias()},
                {MOTO, "Полезные", LIKE.getAlias()},
                {MOTO, "Обсуждаемые", COMMENTS.getAlias()},
                {TRUCKS, "Полезные", LIKE.getAlias()},
                {TRUCKS, "Обсуждаемые", COMMENTS.getAlias()}
        });
    }
    ...
    public void shouldClickShowAllButton() {
        basePageSteps.onReviewsMainPage().presets().preset(preset).should(isDisplayed()).click();
        basePageSteps.onReviewsMainPage().presets().activePreset().waitUntil(hasText(preset));
        basePageSteps.onReviewsMainPage().reviewsList().waitUntil(hasSize(REVIEWS_LISTING_SIZE));
        basePageSteps.onReviewsMainPage().showAllButton().click();
        urlSteps.path(ALL).addParam("sort", sortBy).shouldNotSeeDiff();
