...
public class ReviewClickWideTest {
    ...
    public void shouldOpenBannerMotoReview() {
        urlSteps.testing().path(REVIEW).path(MOTO).path(CATEGORY1).path(MARK1).path(MODEL1).path(REVIEW1).open();
    ...
    public void shouldOpenBannerMotoReviewScooter() {
        urlSteps.testing().path(REVIEW).path(MOTO).path(CATEGORY2).path(MARK2).path(MODEL2).path(REVIEW2).open();
        basePageSteps.onAdsPage().clickAtBanner(bannerNumber);
