...
public class ListingMarkModelClickNarrowTest {
    ...
    public static Collection<Object[]> getData() {
        return asList(new Object[][]{
                {CATEGORY1, MARK1, MODEL1}, //мотоцикл
                {CATEGORY2, MARK2, MODEL2}  //скутер
        });
    }
    ...
    @DisplayName("Узкий экран + Мотоцикл. Клик по баннеру")
    public void shouldOpenBannerMotoMarkModel() {
        urlSteps.testing().path(REVIEWS).path(MOTO).path(category).path(mark).path(model).open();
