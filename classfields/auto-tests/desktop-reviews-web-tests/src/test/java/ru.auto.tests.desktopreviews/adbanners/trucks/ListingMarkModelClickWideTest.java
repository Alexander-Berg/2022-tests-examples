...
public class ListingMarkModelClickWideTest {
    ...
    public void shouldOpenBannerTrucksMarkModel() {
        urlSteps.testing().path(REVIEWS).path(TRUCKS).path(CATEGORY1).path(MARK1).path(MODEL1).open();
    ...
    public void shouldOpenBannerTrucksMarkModelLcv() {
        urlSteps.testing().path(REVIEWS).path(TRUCKS).path(CATEGORY2).path(MARK2).path(MODEL2).open();
        basePageSteps.onAdsPage().clickAtBanner(bannerNumber);
