...
public class ReviewClickWideTest {
    ...
    public void shouldOpenBannerCarsReviewSedan() {
        urlSteps.testing().path(REVIEW).path(CARS).path(MARK1).path(MODEL1).path(REVIEW_1).open();
    ...
    public void shouldOpenBannerCarsReviewAllroad() {
        urlSteps.testing().path(REVIEW).path(CARS).path(MARK2).path(MODEL2).path(REVIEW_2).open();
        basePageSteps.onAdsPage().clickAtBanner(bannerNumber);
