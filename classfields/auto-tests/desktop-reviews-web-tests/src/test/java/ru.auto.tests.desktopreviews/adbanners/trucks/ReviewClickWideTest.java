...
public class ReviewClickWideTest {
    ...
    @DisplayName("Широкий экран + Грузовик. Клик по банеру")
    public void shouldOpenBannerTrucksReview() {
        urlSteps.testing().path(REVIEW).path(TRUCKS).path(CATEGORY1).path(MARK1).path(MODEL1).path(REVIEW1).open();
    ...
    @Owner(NATAGOLOVKINA)
    @Category({Regression.class})
    @DisplayName("Широкий экран + Легкий коммерческий транспорт. Клик по банеру")
    public void shouldOpenBannerTrucksReviewLcv() {
        urlSteps.testing().path(REVIEW).path(TRUCKS).path(CATEGORY2).path(MARK2).path(MODEL2).path(REVIEW2).open();
        basePageSteps.onAdsPage().clickAtBanner(bannerNumber);
