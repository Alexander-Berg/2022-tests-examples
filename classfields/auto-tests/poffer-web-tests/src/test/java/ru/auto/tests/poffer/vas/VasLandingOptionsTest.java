...
@DisplayName("Частник - покупка опций на лендинге VAS")
@Feature(POFFER_VAS_LANDING)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopTestsModule.class)
public class VasLandingOptionsTest {

    private static final String OFFER_ID = "/1076842087-f1e84/";
    private static final String VAS_TITLE = "Наборный пакет";
    private static final String VAS_PRICE = "2 120 ₽";
    private static final String VAS_PRICE_UNFORMATTED = "2120 ₽";
    ...
    public void before() {
        ...
        mockRule.setStubs(
                ...
                stub("poffer/BillingAutoruPaymentInitAllLanding"),
    ...
    @DisplayName("Покупка опций")
    public void shouldBuy() {
        pofferSteps.onPofferPage().vasLanding().option("Поднятие в ТОП").click();
        pofferSteps.onPofferPage().vasLanding().option("Спецпредложение").click();
        pofferSteps.onPofferPage().vasLanding().option("Выделение цветом").click();
        pofferSteps.onPofferPage().vasLanding().option("Поднятие в\u00a0поиске").click();
        pofferSteps.onPofferPage().vasLanding().button(format("Подключить за %s", VAS_PRICE_UNFORMATTED)).hover();
        pofferSteps.scrollDown(200);
        pofferSteps.onPofferPage().vasLanding().button(format("Подключить за %s", VAS_PRICE_UNFORMATTED)).click();
