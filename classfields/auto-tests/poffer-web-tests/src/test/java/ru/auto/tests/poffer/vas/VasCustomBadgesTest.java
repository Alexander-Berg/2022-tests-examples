...
@DisplayName("Частник - добавление объявления с кастомными бейджами")
@Feature(BETA_POFFER)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopDevToolsTestsModule.class)
public class VasCustomBadgesTest {
    ...
    public void before() {
        ...
        mockRule.setStubs(
                ...
                stub("poffer/BillingAutoruPaymentInitBadges"),
    ...
    @DisplayName("Добавление объявления с кастомными бейджами")
    public void shouldAddCustomBadges() {
        pofferSteps.addCustomBadges("Бейдж 1", "Бейдж 2", "Бейдж 3");

        pofferSteps.onBetaPofferPage().badges().input(CUSTOM_TEXT_INPUT).waitUntil(not(isEnabled()));
        pofferSteps.onBetaPofferPage().badges().button(ADD_BUTTON).waitUntil(not(isEnabled()));
        pofferSteps.onBetaPofferPage().badges().summary()
                .waitUntil(hasText("Итого: 3 стикера за 3 ₽ — максимальное количество"));
        waitSomething(3, TimeUnit.SECONDS);

        seleniumMockSteps.assertWithWaiting(onlyOneRequest(
                "/-/ajax/poffer/saveDraftFormsToPublicApi/",
                pofferHasJsonBody("drafts/user_badges.json")
        ));

        pofferSteps.onBetaPofferPage().userVas().getSnippet(2).waitUntil(hasText("Опции отдельно\n0 ₽/ день\n" +
                "Комбинируйте опции по своему вкусу.\nПоказ в Историях\nНа 3 дня\n3 505 ₽\nВыделение цветом\n" +
                "На 3 дня\n597 ₽\nСпецпредложение\nНа 3 дня\n879 ₽\nПоднятие в ТОП\nНа 3 дня\n2 157 ₽\n" +
                "Стикеры быстрой продажи\nНа 60 дней\n3 ₽\nРазместить за 3 ₽ на 60 дней"));
        ...
        pofferSteps.onBetaPofferPage().billingPopup().priceHeader().waitUntil(hasText("297 ₽"));
