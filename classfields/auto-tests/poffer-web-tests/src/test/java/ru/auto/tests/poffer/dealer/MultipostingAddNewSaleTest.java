...
@DisplayName("Мультипостинг - добавление нового объявления")
@Feature(POFFER)
@Story(MULTIPOSTING)
@RunWith(GuiceTestRunner.class)
@GuiceModules(DesktopDevToolsTestsModule.class)
public class MultipostingAddNewSaleTest {
    ...
    @Owner(DSVICHIHIN)
    @Category({Regression.class, Testing.class})
    @DisplayName("Добавление нового объявления - Авто.ру")
    public void shouldAddAutoruSale() {
        ...
        seleniumMockSteps.assertWithWaiting(onlyOneRequest(
                ...
                pofferHasJsonBody("offers/cars_new_dealer_multiposting_autoru_offer.json")
        ));
    ...
    @Owner(DSVICHIHIN)
    @Category({Regression.class, Testing.class})
    @DisplayName("Добавление нового объявления - Авито")
    public void shouldAddAvitoSale() {
        ...
        seleniumMockSteps.assertWithWaiting(onlyOneRequest(
                "/-/ajax/poffer/saveFormAndPay/",
                pofferHasJsonBody("offers/cars_new_dealer_multiposting_avito_offer.json")
        ));
        urlSteps.testing().path(CARS).path(NEW).path(SALE).path(OFFER_ID)
                .addParam(PAGE_FROM, ADD_PAGE).shouldNotSeeDiff();
    }
    ...
    @Owner(DSVICHIHIN)
    @Category({Regression.class, Testing.class})
    @DisplayName("Добавление нового объявления - Дром")
    public void shouldAddDromSale() {
        ...
        seleniumMockSteps.assertWithWaiting(onlyOneRequest(
                "/-/ajax/poffer/saveFormAndPay/",
                pofferHasJsonBody("offers/cars_new_dealer_multiposting_drom_offer.json")
        ));
        urlSteps.testing().path(CARS).path(NEW).path(SALE).path(OFFER_ID)
                .addParam(PAGE_FROM, ADD_PAGE).shouldNotSeeDiff();
    }
