...
@Feature(FILTERS)
@DisplayName("Фильтры в мото")
@RunWith(GuiceTestRunner.class)
@GuiceModules(MobileEmulationTestsModule.class)
public class FiltersMotoTest {

    private static final String CATEGORY = "atv";
    ...
    public void shouldClickSelectMarkButton() {
        urlSteps.testing().path(REVIEWS).path(MOTO).path(CATEGORY).open();

        steps.onReviewPage().filters().selectMarkButton().click();
        urlSteps.testing().path(REVIEWS).path(MARKS).path(MOTO).path(CATEGORY).path("/")
                .shouldNotSeeDiff();
    ...
    public void shouldClickSelectModelButton() {
        urlSteps.testing().path(REVIEWS).path(MOTO).path(CATEGORY).path(MARK).open();

        steps.onReviewPage().filters().selectModelButton().click();
        urlSteps.testing().path(REVIEWS).path(MODELS).path(MOTO).path(CATEGORY).path("/")
