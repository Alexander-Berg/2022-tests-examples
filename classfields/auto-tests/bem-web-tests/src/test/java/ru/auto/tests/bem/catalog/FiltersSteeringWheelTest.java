...
@DisplayName("Каталог - фильтры - руль")
@Feature(AutoruFeatures.CATALOG)
@RunWith(Parameterized.class)
@GuiceModules(DesktopTestsModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class FiltersSteeringWheelTest {
    ...
    public static Collection<Object[]> getParameters() {
        return asList(new Object[][]{
                ...
                {"/toyota/", "Левый", "LEFT"},
                {"/toyota/", "Правый", "RIGHT"},
                {"/toyota/corolla/", "Левый", "LEFT"},
                {"/toyota/corolla/", "Правый", "RIGHT"},
                {"/toyota/corolla/20807898/", "Левый", "LEFT"},
                {"/toyota/corolla/20807898/", "Правый", "RIGHT"}
        });
    }
    ...
    @DisplayName("Параметр 'Руль'")
    public void shouldSeeSteeringWheelInUrl() {
        user.onCatalogPage().filter().steeringWheel().should(isDisplayed()).click();
        ...
        urlSteps.addParam("steering_wheel", paramValue.toLowerCase()).ignoreParam("view_type").shouldNotSeeDiff();
    }
}