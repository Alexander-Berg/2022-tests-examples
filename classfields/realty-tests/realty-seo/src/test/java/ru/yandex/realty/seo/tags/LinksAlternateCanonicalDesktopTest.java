...
public class LinksAlternateCanonicalDesktopTest {
    ...
    public static Collection<String[]> testParameters() throws Exception {
        CSVReader reader = new CSVReader(new FileReader("target/test-classes/alternate_canonical_desktop.csv"));
    ...
    @Test
    @DisplayName("Смотрим на «href» на десктопе")
    @Owner(KANTEMIROV)
    public void shouldSeeDesktopTags() {
        jSoupSteps.connectTo(host + path).get();
        String canonicalExpected = host + canonicalPath;
        String alternateExpected = alternatePath.isEmpty() ? "" : mHost + alternatePath;
        ...
        String alternateActual = jSoupSteps.select(ALTERNATE_LOCATOR).attr(HREF);
        assertThat(canonicalActual).isEqualTo(canonicalExpected);
        assertThat(alternateActual).isEqualTo(alternateExpected);
    }
}
