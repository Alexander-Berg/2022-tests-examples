...
@DisplayName("Показ телефона. Листинг поселков")
@Feature(VILLAGES)
...
public class ListingShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона листинг новостроек. Бесплатный поселок")
    public void shouldSeePhoneFreeVillageNormalCase() {
        ...
        basePageSteps.onVillageListing().offer(FIRST).showPhoneButton()
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона листинг новостроек. Платный поселок")
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ 500ки листинг поселков.")
    public void shouldSeePhone500() {
        ...
        urlSteps.testing().path(MOSKVA_I_MO).path(KUPIT).path(KOTTEDZHNYE_POSELKI).open();
        basePageSteps.onVillageListing().offer(FIRST).showPhoneButton().click();
        basePageSteps.onVillageListing().offer(FIRST).showPhoneButton().should(isDisplayed());
    }

    @Ignore("МОЖЕТ ЛИ БЫТЬ ДВА ТЕЛЕФОНА?")
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ двух телефонов листинг поселков.")
    public void shouldSeePhoneFreeVillageTwoPhones() {
        ...
        urlSteps.testing().path(MOSKVA_I_MO).path(KUPIT).path(KOTTEDZHNYE_POSELKI).open();
        basePageSteps.onVillageListing().offer(FIRST).showPhoneButton().should(hasText(TEST_PHONE + SECOND_TEST_PHONE));
    }
}