...
public class ExtendedProfFiltersNearTest {
    ...
    public String nearParam;

    @Parameterized.Parameters(name = "{0}")
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                {"Рядом парк", "hasPark"},
                {"Рядом водоём", "hasPond"},
                {"Рядом строят станцию метро", "expectMetro"}
        });
    }
    ...
    public void shouldSeeNearInUrl() {
        ...
        urlSteps.queryParam(nearParam, "YES").shouldNotDiffWithWebDriverUrl();
    }
