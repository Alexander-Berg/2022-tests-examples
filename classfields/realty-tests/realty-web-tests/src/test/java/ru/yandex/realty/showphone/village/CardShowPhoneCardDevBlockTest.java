...
@DisplayName("Показ телефона. Телефон в карточке застройщика поселка")
...
public class CardShowPhoneCardDevBlockTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в карточке застройщика поселка. Бесплатный поселок")
    public void shouldSeePhoneFreeVillageNormalCase() {
        ...
        basePageSteps.onVillageSitePage().cardDev().showPhoneClick();
        basePageSteps.onVillageSitePage().cardDev().shouldSeePhone(TEST_PHONE);
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в карточке застройщика поселка. Платный поселок")
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ двух телефонов в карточке застройщика.")
    public void shouldSeePhoneFreeVillageTwoPhones() {
        ...
        urlSteps.testing().villageSite().replaceQueryParam(ID_VALUE, ID).open();
        basePageSteps.onVillageSitePage().cardDev().showPhoneClick();
        basePageSteps.onVillageSitePage().cardDev().phones().should(hasSize(2)).get(0)
                .should(hasText(makePhoneFormatted(TEST_PHONE, PHONE_PATTERN_BRACKETS)));
        basePageSteps.onVillageSitePage().cardDev().phones().should(hasSize(2)).get(1)
