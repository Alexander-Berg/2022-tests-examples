...
@DisplayName("Профпоиск. Расширенные фильтры поиска по объявлениям гаража.")
@Feature(PROFILTERS)
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebModuleWithoutDelete.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class ExtendedProfFiltersRentGarageTest {
    ...
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                {"Электроэнергия включена", "hasElectricityIncluded"},
                {"КУ включены", "hasUtilitiesIncluded"}
        });
    }
    ...
    public void before() {
        ...
        urlSteps.testing().path(MANAGEMENT_NEW).path(SEARCH).queryParam(RGID, MOSCOW_RGID)
                .queryParam("category", "GARAGE").queryParam("type", "RENT").open();
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Параметры удобств")
    public void shouldSeeNearInUrl() {
        user.onProfSearchPage().extendFilters().selectCheckBox(label);
        user.onProfSearchPage().extendFilters().applyFiltersButton().click();
        urlSteps.queryParam(checkbox, "YES").shouldNotDiffWithWebDriverUrl();
    }
