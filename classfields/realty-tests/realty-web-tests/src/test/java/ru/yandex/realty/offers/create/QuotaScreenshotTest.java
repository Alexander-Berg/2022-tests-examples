...
public class QuotaScreenshotTest {
    ...
    @Test
    @DisplayName("Скриншот блока публикации второго оффера (квота=1) c табом «Продать быстрее» в платном регионе с фото.")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeQuotaWithPhotoFasterSalePublishBlock() {
    ...
    @Test
    @DisplayName("Скриншот блока публикации второго оффера (квота=1) c табом «Обычная продажа» в платном регионе с фото c услугой")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeQuotaWithPhotoNormalSalePublishBlock() {
    ...
    @Test
    @DisplayName("Скриншот блока публикации второго оффера (квота=1) c табом «Обычная продажа» в платном регионе с фото без услуг")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeQuotaWithPhotoFreePublishBlock() {
    ...
    private void checkButtonHasTurboText() {
        ...
        offerAddSteps.onOfferAddPage().publishBlock().payButton()
                .should(hasText(findPattern(ACTIVATE_WITH_OPTIONS_FOR)));
    }
    ...
    private void checkButtonHasServiceText() {
        ...
        offerAddSteps.onOfferAddPage().publishBlock().payButton().should(hasText(findPattern(ACTIVATE_WITH_OPTIONS_FOR)));
    }
    ...
    private void checkButtonWithoutServicesText() {
        ...
        offerAddSteps.onOfferAddPage().publishBlock().payButton().waitUntil(hasText(findPattern(ACTIVATE_FOR)));
    }
}