...
public class VillageTypeVillagesBaseFilterTest {
    ...
    public void shouldSeeObjectTypeSelectedButton() {
        urlSteps.testing().path(SPB_I_LO).path(KUPIT).path(KOTTEDZHNYE_POSELKI).queryParam(VILLAGE_OFFER_TYPE, expected)
                .open();
        basePageSteps.onVillageListing().filters().button(name).should(isDisplayed());
    }
}
