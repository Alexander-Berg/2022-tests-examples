...
public class ExtendedProfFiltersRentComfortTest {
    ...
    public void before() {
        ...
        urlSteps.testing().path(MANAGEMENT_NEW).path(SEARCH).queryParam(RGID, MOSCOW_RGID).queryParam("type", "RENT")
                .open();
        user.onProfSearchPage().openExtFilter();
        user.scrollToElement(user.onProfSearchPage().extendFilters().button(COMFORT));
        user.onProfSearchPage().extendFilters().clickDropdown(COMFORT);
    }
    ...
    public void shouldSeeComfortFiltersInUrl() {
        user.onProfSearchPage().filterPopup().selectCheckBox(label);
