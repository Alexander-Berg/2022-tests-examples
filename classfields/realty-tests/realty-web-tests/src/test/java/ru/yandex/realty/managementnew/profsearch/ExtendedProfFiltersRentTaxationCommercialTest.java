...
public class ExtendedProfFiltersRentTaxationCommercialTest {
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Параметр налогообложения")
    public void shouldSeeTaxationFiltersInUrl() {
        user.onProfSearchPage().extendFilters().selectCheckBox(label);
        user.onProfSearchPage().extendFilters().applyFiltersButton().click();
        urlSteps.queryParam("taxationForm", expected).shouldNotDiffWithWebDriverUrl();
    }
}
