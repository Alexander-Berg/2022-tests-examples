...
public class ExtendedFiltersDealTypeTest {
    ...
    public static Collection<Object[]> types() {
        return asList(new Object[][]{
                {"ЖК с закрытыми продажами", "showOutdated"},
                {"Скидки", "hasSpecialProposal"},
                {"Ипотека", "hasSiteMortgage"},
                {"Мат. капитал", "hasSiteMaternityFunds"},
                {"Военная ипотека", "hasMilitarySiteMortgage"},
                {"Рассрочка", "hasInstallment"}
        });
    }
    ...
    public void shouldSeeExtendedFilters() {
        ...
        basePageSteps.onMapPage().extendFilters().selectCheckBox(label);
        urlSteps.queryParam(expected, YES_VALUE).shouldNotDiffWithWebDriverUrl();
    }
