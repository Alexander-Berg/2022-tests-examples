...
@DisplayName("Главная. Геоселектор. Локация")
@Feature(MAIN)
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class GeoSelectorLocationsTest {
    ...
    public void suggestTest() {
        ...
        user.onBasePage().regionSelectorPopup().input().sendKeys(location.getName());
        user.onBasePage().regionSelectorPopup().suggestItem(location.getName()).click();
        user.onBasePage().regionSelectorPopup().button(SAVE).click();
        user.onBasePage().paranja().waitUntil(not(isDisplayed()));
        urlSteps.testing().path(location.getPath())
