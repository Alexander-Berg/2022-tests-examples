...
public class DeleteAgentOfferTest {
    ...
    public void before() {
        ...
        managementSteps.onManagementNewPage().agencyOffer(FIRST).button("Удалить")
    ...
    @DisplayName("Кликаем на «Удалить», проверяем что удалено")
    public void shouldDeleteAgentOffer() {
        apiSteps.waitOfferDeleted(account.getId(), offerId);
    }

    @Test
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Отменяем удаление")
    public void shouldUndeleteAgentOffer() {
        apiSteps.waitOfferDeleted(account.getId(), offerId);
        managementSteps.moveCursor(managementSteps.onManagementNewPage().agencyOffer(FIRST));
        managementSteps.onManagementNewPage().agencyOffer(FIRST).button("Отменить удаление").should(isDisplayed())
                .click();
