...
import static ru.yandex.realty.element.developer.Slide.CHOOSE_FLAT;
...
public class SliderTest {
    ...
    public void before() {
        mockRuleConfigurable
                .developerStub(ENHANCED_DEV_ID, mockEnhancedDeveloper().setSlides(slideTemplate().withSiteId(ID)
                        .withSiteName(NAME).withSiteRgid(SITE_RGID)).build())
    ...
    public void shouldSeeUrlInSlide() {
        basePageSteps.onDeveloperPage().slider().slides().get(FIRST).link().should(hasHref(equalTo(
                urlSteps.testing().path(MOSKVA).path(KUPIT).path(NOVOSTROJKA)
                        .path(format("%s-%s/", NAME_TRANSLIT, ID)).ignoreParam(RGID).toString())));
    }
    ...
    public void shouldSeeUrlInSlideButton() {
        basePageSteps.onDeveloperPage().slider().slides().get(FIRST).button(CHOOSE_FLAT).should(hasHref(equalTo(
                urlSteps.testing().path(MOSKVA).path(KUPIT).path(NOVOSTROJKA)
                        .path(format("%s-%s/", NAME_TRANSLIT, ID)).ignoreParam(RGID).toString())));
    }
