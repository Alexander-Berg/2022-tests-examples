...
@DisplayName("Показ телефона. Карточка оффера. После описания")
@Feature(OFFER_CARD)
@Link("https://st.yandex-team.ru/VERTISTEST-1599")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebModule.class)
public class OfferCardDescriptionShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона после описания")
    public void shouldSeePhoneNormalCaseAfterDescription() {
        ...
        basePageSteps.onOfferCardPage().authorBlock().phones().should(hasSize(1)).get(0)
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("При 500-ке показ заглушки после описания")
    public void shouldSeePhone500AfterDescription() {
        mockRuleConfigurable
                .cardStub(cardTemplate().offers(asList(offer)).build())
                .cardWithViewsStub(cardWithViewsTemplate().offer(offer).build())
        ...
        urlSteps.testing().path(OFFER).path(offerId).open();
        basePageSteps.onOfferCardPage().authorBlock().showPhoneButton().click();
        basePageSteps.onOfferCardPage().authorBlock().showPhoneButton().should(hasText(MESSAGE_500));
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона с подменным шильдиком после описания")
    public void shouldSeePhoneRedirectAfterDescription() {
        ...
        urlSteps.testing().path(OFFER).path(offerId).open();
        basePageSteps.onOfferCardPage().authorBlock().showPhoneButton().click();
        basePageSteps.onOfferCardPage().authorBlock().phoneProtected().should(isDisplayed());
        basePageSteps.onOfferCardPage().authorBlock().phone()
                .should(hasText(makePhoneFormatted(TEST_PHONE, PHONE_PATTERN_BRACKETS)));
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ двух телефонов после описания")
    public void shouldSeeTwoPhoneAfterDescription() {
        ...
        urlSteps.testing().path(OFFER).path(offerId).open();
        basePageSteps.onOfferCardPage().authorBlock().showPhoneButton().click();
        basePageSteps.onOfferCardPage().authorBlock().phones().get(0)
                .should(allOf(hasText(containsString(makePhoneFormatted(TEST_PHONE, PHONE_PATTERN_BRACKETS))),
                        hasHref(equalTo(format(TEL_HREF_PATTERN, TEST_PHONE)))));
        basePageSteps.onOfferCardPage().authorBlock().phones().get(1)
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона после описания открывает другие телефоны")
    public void shouldNotSeeOtherPhonesWhenRevealAfterDescription() {
        offersPhonesTemplate = offersPhonesTemplate().addPhone(TEST_PHONE);
        mockRuleConfigurable
                .cardStub(cardTemplate().offers(asList(offer)).build())
                .cardWithViewsStub(cardWithViewsTemplate().offer(offer).build())
