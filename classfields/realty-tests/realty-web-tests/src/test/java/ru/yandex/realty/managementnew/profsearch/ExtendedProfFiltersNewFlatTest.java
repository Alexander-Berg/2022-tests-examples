...
public class ExtendedProfFiltersNewFlatTest {
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Параметр «Вторичный рынок/Новостройки»")
    public void shouldSeeNewFlatInUrl() {
        user.onProfSearchPage().extendFilters().radio(label).click();
        user.onProfSearchPage().extendFilters().radio(label).should(isChecked());
