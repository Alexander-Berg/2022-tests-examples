...
@DisplayName("Подписки. Скриншот страницы c подпиской")
@Link("https://st.yandex-team.ru/VERTISTEST-1475")
@Feature(SUBSCRIPTION)
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebModule.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class WithSubscriptionScreenshotTest {
    ...
    public BackRtAdaptor.SubscriptionQualifier subscriptionQualifier;
    ...
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                {MY_SEARCHES_TAB, SEARCH, SEARCH_TAB_VALUE},
                {NEWBUILDING_TAB, NEWBUILDING, NEWBUILDING_TAB_VALUE},
                {PRICE_CHANGE_TAB, PRICE, PRICE_TAB_VALUE}
        });
    }
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Видим скриншот с подпиской")
    public void shouldSeeWithSubscriptionsScreenshot() {
        api.createVos2Account(account, AccountType.OWNER);
        api.createSubscription(account.getId(), subscriptionQualifier);
