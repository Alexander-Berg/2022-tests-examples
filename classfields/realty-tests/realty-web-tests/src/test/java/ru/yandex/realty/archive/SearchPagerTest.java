...
public class SearchPagerTest {
    ...
    @Owner(KANTEMIROV)
    @Description("Проверяем переход по страницам")
    public void shouldSeeNextPage() {
        urlSteps.testing().path(OTSENKA_KVARTIRY).path(PATH_ADDRESS).path(KUPIT).path(KVARTIRA).open();
        basePageSteps.onArchivePage().pagerButton(PAGE_NUMBER).waitUntil(isDisplayed()).click();
        basePageSteps.onArchivePage().pagerButtonChecked(PAGE_NUMBER).should(isDisplayed());
        basePageSteps.onArchivePage().paranja().waitUntil(not(isDisplayed()));
        urlSteps.path(PAGE_NUMBER + "/").shouldNotDiffWithWebDriverUrl();
    }
}
