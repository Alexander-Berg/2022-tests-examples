...
public class FavoriteListingShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в избранном")
    public void shouldSeePhoneNormalCaseFavListing() {
        ...
        basePageSteps.onFavoritesPage().offer(FIRST).button(SHOW_PHONE).click();
        basePageSteps.onFavoritesPage().showPhonePopup().phones()
    ...
    public void shouldSeePhone500FavListing() {
        ...
        urlSteps.testing().path(FAVORITES).open();
        basePageSteps.onFavoritesPage().offer(FIRST).button(SHOW_PHONE).click();
        basePageSteps.onFavoritesPage().showPhonePopup().phoneError()
    ...
    public void shouldSeePhoneRedirectFavListing() {
        ...
        urlSteps.testing().path(FAVORITES).open();
        basePageSteps.onFavoritesPage().offer(FIRST).button(SHOW_PHONE).click();
        basePageSteps.onFavoritesPage().showPhonePopup().phoneProtectSign().should(isDisplayed());
        basePageSteps.onFavoritesPage().showPhonePopup().phones()
                .should(hasSize(1)).get(0).should(hasText(makePhoneFormatted(TEST_PHONE, PHONE_PATTERN_DASHES)));
    }
    ...
    public void shouldSeeTwoPhoneFavListing() {
        ...
        urlSteps.testing().path(FAVORITES).open();
        basePageSteps.onFavoritesPage().offer(FIRST).button(SHOW_PHONE).click();
        basePageSteps.onFavoritesPage().showPhonePopup().phones().should(hasSize(2));
        basePageSteps.onFavoritesPage().showPhonePopup().phones().get(0)
                .should(hasText(makePhoneFormatted(TEST_PHONE, PHONE_PATTERN_DASHES)));
        basePageSteps.onFavoritesPage().showPhonePopup().phones().get(1)
