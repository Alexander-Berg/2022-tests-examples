...
public class ExtendedProfFiltersBalconyTest {
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Параметр «Балкон»")
    public void shouldSeeBalconyInUrl() {
        user.onProfSearchPage().extendFilters().clickDropdown("Балкон неважен");
        user.onProfSearchPage().selectPopup().item(label).click();
        user.onProfSearchPage().extendFilters().applyFiltersButton().click();
        urlSteps.queryParam("balcony", expected).shouldNotDiffWithWebDriverUrl();
    }
}
