...
@DisplayName("Профпоиск. Фильтры поиска для коммерческой аренды.")
@Feature(PROFILTERS)
@RunWith(Parameterized.class)
@GuiceModules(RealtyWebModuleWithoutDelete.class)
@Parameterized.UseParametersRunnerFactory(GuiceParametersRunnerFactory.class)
public class ExtendedProfFiltersRentDealTypeCommercialTest {
    ...
    public void before() {
        ...
        urlSteps.testing().path(MANAGEMENT_NEW).path(SEARCH).queryParam(RGID, MOSCOW_RGID)
                .queryParam("type", "RENT").queryParam("category", "COMMERCIAL").open();
        user.onProfSearchPage().openExtFilter();
        user.scrollToElement(user.onProfSearchPage().extendFilters().button("Тип сделки неважен"));
    }

    @Test
    @Category({Regression.class, Production.class})
    @Owner(KANTEMIROV)
    @DisplayName("Параметр «Тип сделки»")
    public void shouldSeeDealTypeFiltersInUrl() {
        user.onProfSearchPage().extendFilters().select("Тип сделки неважен", label);
        user.onProfSearchPage().extendFilters().applyFiltersButton().click();
        urlSteps.queryParam("dealStatus", expected).shouldNotDiffWithWebDriverUrl();
    }
}
