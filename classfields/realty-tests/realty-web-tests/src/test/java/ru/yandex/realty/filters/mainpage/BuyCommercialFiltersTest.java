...
public class BuyCommercialFiltersTest {
    ...
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                ...
                {"Торговое помещение", "/torgovoe-pomeshchenie/"},
                {"Помещение свободного назначения", "/pomeshchenie-svobodnogo-naznacheniya/"},
                {"Складское помещение", "/skladskoe-pomeshchenie/"},
                {"Производственное помещение", "/proizvodstvennoe-pomeshchenie/"},
                {"Земельный участок", "/uchastok-kommercheskogo-naznacheniya/"},
                {"Общепит", "/obshchepit/"},
                {"Автосервис", "/avtoservis/"},
                {"Гостиница", "/gostinica/"},
                {"Готовый бизнес", "/gotovyj-biznes/"},
                {"Юридический адрес", "/yuridicheskij-adres/"},
        });
    }
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Главная страница. Параметр типа коммерческой недвижимости")
    public void shouldSeeBuyCommercialType() {
        ...
        urlSteps.testing().path(MOSKVA).path(KUPIT).path(COMMERCIAL).path(expected).shouldNotDiffWithWebDriverUrl();
    }
}
