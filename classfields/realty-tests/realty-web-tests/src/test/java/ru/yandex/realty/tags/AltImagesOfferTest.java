...
public class AltImagesOfferTest {
    ...
    public void before() {
        ...
        jSoupSteps.connectTo(format("%s/offer/%s/", host, offerId)).cookie("mockritsa_imposter", port).get();
    }

    @Test
    @Owner(KANTEMIROV)
    public void shouldSeeAltInImgInOffer() {
        final String elementSelector = "img[class*=GalleryV2Slide__wrappedImage]";
        final String altExpected = "85 м², 3-комнатная квартира 15 000 000 ₽ - изображение 1";
        String altActual = jSoupSteps.select(elementSelector).first().attr("alt");
