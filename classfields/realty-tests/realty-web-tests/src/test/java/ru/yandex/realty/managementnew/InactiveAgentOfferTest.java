...
public class InactiveAgentOfferTest {
    ...
    @DisplayName("Кликаем на «Снять с публикации», проверяем что объявление снято с пубилкации")
    ...
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    @DisplayName("Активируем снятое с публикации объявление")
    public void shouldActivateOffer() {
        managementSteps.onManagementNewPage().publishControlPopup().item(OTHER_REASON).click();
        apiSteps.waitSearcherOfferStatusInactive(offerId);
        managementSteps.onManagementNewPage().agencyOffer(FIRST).button(ACTIVATE).click();
