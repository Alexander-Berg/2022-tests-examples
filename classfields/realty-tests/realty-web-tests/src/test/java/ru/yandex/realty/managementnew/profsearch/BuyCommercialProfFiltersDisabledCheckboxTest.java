...
public class BuyCommercialProfFiltersDisabledCheckboxTest {
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Профпоиск. При выборе чекбокса, остальные становятся неактивными")
    public void shouldSeeDisabledCheckboxes() {
        ...
        shouldSeeDisabledSheckBoxes(user.onProfSearchPage().filters().selectPopup().checkBoxList(),
                user.onProfSearchPage().filters().selectPopup().checkBoxList().size());

    }

    @Step("Всего {size} чекбоксов, неактивных на 1 меньше")
    private void shouldSeeDisabledSheckBoxes(ElementsCollection<? extends AtlasWebElement> checkBoxes, int size) {
        assertThat(checkBoxes.stream()
                .filter(c -> c.getAttribute("class").contains("Checkbox_disabled")).collect(Collectors.toList()).size())
                .isEqualTo(size - 1);
    }
}
