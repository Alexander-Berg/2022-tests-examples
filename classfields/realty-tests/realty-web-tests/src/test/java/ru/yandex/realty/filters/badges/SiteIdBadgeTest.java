...
@DisplayName("Гео Фильтры: название ЖК")
@Feature(FILTERS)
...
public class SiteIdBadgeTest {
    ...
    public void shouldSeeAddressInUrl() {
        ...
        user.onOffersSearchPage().filters().geoInput().sendKeys(SITE_NAME);
        ...
        urlSteps.shouldNotDiffWith(urlSteps.testing().path(MOSKVA_I_MO).path(KUPIT).path(KVARTIRA)
                .path(format("zhk-alye-parusa-%s/", address.getData().getParams().getSiteId().get(0))).toString());
    }
