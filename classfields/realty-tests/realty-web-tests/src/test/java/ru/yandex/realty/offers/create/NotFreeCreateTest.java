...
public class NotFreeCreateTest {
    ...
    public void openManagementAddPage() {
        ...
        offerAddSteps.fillRequiredFieldsForSellFlat(OfferAddSteps.MOSCOW_LOCATION);
    }

    @Test
    @DisplayName("Создание оффера с фото и без васов - показывается попап")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeNotFreeWithPhotoToLk() {
    ...
    @Test
    @DisplayName("Создание оффера с фото и с турбо - показывается попап")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeNotFreeWithPhotoWithTurboToLk() {
        offerAddSteps.onOfferAddPage().publishBlock().payButton().click();
        offerAddSteps.waitPublish();
    ...
    @Test
    @DisplayName("Создание оффера с фото и с премиумом - показывается попап")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeNotFreeWithPhotoWithPremiumToLk() {
        offerAddSteps.onOfferAddPage().publishBlock().sellTab(NORMAL_SALE).clickWhile(hasClass(containsString(ACTIVE)));
        offerAddSteps.onOfferAddPage().publishBlock().payButton().click();
        offerAddSteps.waitPublish();
        managementSteps.onWalletPage().cardsPopup().should(isDisplayed());
