...
public class ExtendedFiltersBuyLotTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Параметр «Садоводство/ИЖС»")
    public void shouldSeeLotTypeInUrl() {
        urlSteps.testing().path(SANKT_PETERBURG).path(KUPIT).path(UCHASTOK).path(KARTA).open();
        basePageSteps.onMapPage().openExtFilter();
        basePageSteps.onMapPage().extendFilters().checkButton("В садоводстве");
