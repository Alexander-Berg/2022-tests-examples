...
public class BaseProfFiltersTypeOfRealltyTest {
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Профпоиск. Параметр типа недвижимости")
    public void shouldSeeBuyTypeOfRealtyUrl() {
        user.onProfSearchPage().filters().dropDownButton("Купить квартиру").click();
        user.onProfSearchPage().filters().selectPopup().item(label).click();
        user.onProfSearchPage().filters().submitButton().click();
        urlSteps.queryParam("category", expected).shouldNotDiffWithWebDriverUrl();
    }
}