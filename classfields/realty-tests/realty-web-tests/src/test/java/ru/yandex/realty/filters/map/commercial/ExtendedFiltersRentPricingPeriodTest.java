...
public class ExtendedFiltersRentPricingPeriodTest {
    ...
    @Before
    public void before() {
        urlSteps.testing().path(MOSKVA).path(SNYAT).path(COMMERCIAL).path(KARTA).open();
    ...
    public void shouldSeePricingPeriodInUrl() {
        basePageSteps.onMapPage().extendFilters().checkButton("За год");
    ...
    public void shouldSeeDefaultPricingPeriodInUrl() {
        basePageSteps.onMapPage().extendFilters().button("За месяц").should(isChecked());
    }
}
