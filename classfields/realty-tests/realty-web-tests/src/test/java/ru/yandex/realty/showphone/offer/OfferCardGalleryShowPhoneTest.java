...
@DisplayName("Показ телефона. Карточка оффера. Галерея")
@Feature(OFFER_CARD)
@Link("https://st.yandex-team.ru/VERTISTEST-1599")
@RunWith(GuiceTestRunner.class)
@GuiceModules(RealtyWebModule.class)
public class OfferCardGalleryShowPhoneTest {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в галерее")
    public void shouldSeePhoneNormalCaseGallery() {
        ...
        basePageSteps.onOfferCardPage().fsGallery().phones().should(hasSize(1)).get(0)
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("При 500-ке показ заглушки в галерее")
    public void shouldSeePhone500Gallery() {
        mockRuleConfigurable
                .cardStub(cardTemplate().offers(asList(offer)).build())
                .cardWithViewsStub(cardWithViewsTemplate().offer(offer).build())
        ...
        basePageSteps.onOfferCardPage().openGallery();
        basePageSteps.onOfferCardPage().fsGallery().showPhoneButton().click();
        basePageSteps.onOfferCardPage().fsGallery().showPhoneButton().should(hasText(MESSAGE_500));
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона с подменным шильдиком в галерее")
    public void shouldSeePhoneRedirectGallery() {
        ...
        basePageSteps.onOfferCardPage().openGallery();
        basePageSteps.onOfferCardPage().fsGallery().showPhoneButton().click();
        basePageSteps.onOfferCardPage().fsGallery().phoneProtected().should(isDisplayed());
        basePageSteps.onOfferCardPage().fsGallery().phone()
                .should(hasText(makePhoneFormatted(TEST_PHONE, PHONE_PATTERN_BRACKETS)));
    }

    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ двух телефонов в галерее")
    public void shouldSeeTwoPhoneGallery() {
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Показ телефона в галерее не открывает других телефонов")
    public void shouldNotSeeOtherPhonesWhenRevealGallery() {
        offersPhonesTemplate = offersPhonesTemplate().addPhone(TEST_PHONE);
        mockRuleConfigurable
                .cardStub(cardTemplate().offers(asList(offer)).build())
                .cardWithViewsStub(cardWithViewsTemplate().offer(offer).build())
        ...
        basePageSteps.onOfferCardPage().openGallery();
        basePageSteps.onOfferCardPage().fsGallery().showPhoneButton().click();
        basePageSteps.onOfferCardPage().fsGallery().phones().should(hasSize(1));
        basePageSteps.onOfferCardPage().fsGallery().closeButton().click();
