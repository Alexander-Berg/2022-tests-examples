...
public class ExtendedProfFiltersOnlyTest {
    ...
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                {"От собственников", "agents", "NO"},
                {"Только с фото", "hasPhoto", "YES"},
                {"Только уникальные", "exclusive", "YES"}
        });
    }
    ...
    @Owner(KANTEMIROV)
    @DisplayName("Параметр «Только...»")
    public void shouldSeeOnlyInUrl() {
        user.onProfSearchPage().extendFilters().selectCheckBox(label);
        user.onProfSearchPage().extendFilters().applyFiltersButton().click();
        urlSteps.queryParam(onlyParam, onlyValue).shouldNotDiffWithWebDriverUrl();
    }
