...
public class CommunicationVillagesFilterTest {
    ...
    public static Collection<Object[]> testParameters() {
        return asList(new Object[][]{
                {"Электричество", "hasElectricitySupply"},
                {"Вода", "hasWaterSupply"},
                {"Отопление", "hasHeatingSupply"},
                {"Газ", "hasGasSupply"},
    ...
    @Test
    @Owner(KANTEMIROV)
    @DisplayName("Параметр «Коммуникации»")
    public void shouldSeeCommunications() {
        urlSteps.testing().path(SPB_I_LO).path(KUPIT).path(KOTTEDZHNYE_POSELKI).path(KARTA).open();
        basePageSteps.onMapPage().openExtFilter();
        basePageSteps.onMapPage().extendFilters().clickDropdown("Коммуникации");
        basePageSteps.onMapPage().selectPopup().selectCheckBox(name);
        basePageSteps.onMapPage().extendFilters().button(name).click();
