...
public class NotFreeRegionEditTest {
    ...
    public void openManagementAddPage() {
        ...
        offerId = offerBuildingSteps.addNewOffer(account)
                .withBody(OfferBuildingSteps.getDefaultOffer(OfferType.GARAGE_SELL)
                        .withLocation(getLocationForRegion(SUPER_HIGH).withApartment(getRandomApartment())))
                .withSearcherWait().create().getId();
        urlSteps.testing().path(MANAGEMENT_NEW_EDIT).path(offerId).open();
    }

    @Test
    @DisplayName("Редактирование активного оффера в платном регионе без услуг. Редирект в ЛК")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeNotFreeRegionEditOfferWithoutServiceRedirectLk() {
    ...
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeNotFreeRegionEditOfferWithServiceRedirectLk() {
    ...
    @Test
    @DisplayName("Редактирование активного оффера в платном регионе. С блока Турбо редирект в форму оплаты")
    @Owner(KANTEMIROV)
    @Category({Regression.class, Testing.class})
    public void shouldSeeNotFreeRegionEditOfferWithServiceRedirectToPay() {
