...
public class RaisingOnPublishOfferTest {
    ...
    public void openWallet() {
        ...
        offerAddSteps.onOfferAddPage().publishBlock().paySelector(RISING).click();
    ...
    @Owner(KURAU)
    @DisplayName("Включаем «поднятие» после публикации с оплатой")
    public void shouldSeeRisingAfterPayment() {
    ...
    @Category({Regression.class, Smoke.class, Testing.class})
    @Owner(KURAU)
    @DisplayName("Не включаем «поднятие» после отмены оплаты")
    public void shouldNotSeeRisingWithoutPay() {
