...
import static ru.yandex.arenda.steps.CalculatorSteps.TEST_ROOMS_TOUCH;
...
public class CalculatorPageFilterTouchTest {
    ...
    public void before() {
        ...
        calculatorSteps.fillTestCalculatorTouch(TEST_ADDRESS, TEST_ROOMS_TOUCH, TEST_AREA, TEST_FLOOR, TEST_RENOVATION);
        calculatorSteps.onCalculatorCostTouchPage().button(DONE_BUTTON).click();
        calculatorSteps.shouldSeeTestCalculatorUrl(DVUHKOMNATNAYA, TEST_ADDRESS, TEST_AREA, TEST_FLOOR, TEST_RENOVATION_VALUE);

    }

    @Test
    @DisplayName("Тач. Заполнение калькулятора -> полный цикл")
    public void shouldSeeFullFillingCalculator() {

        calculatorSteps.onCalculatorCostTouchPage().priceContainer().button(RENT_WITH_BUTTON).click();

        calculatorSteps.onCalculatorCostTouchPage().callbackPopup().inputId(INPUT_1_ID).sendKeys(TEST_PHONE);
        calculatorSteps.onCalculatorCostTouchPage().callbackPopup().button(CALL_ME).click();
        calculatorSteps.onCalculatorCostTouchPage().successToast();
    }

    @Test
    @DisplayName("Тач. Заполнение калькулятора -> клик «Оценить другую»")
    public void shouldSeeEmptyForm() {
        calculatorSteps.onCalculatorCostTouchPage().priceContainer().estimateOther().click();

        calculatorSteps.onCalculatorCostTouchPage().inputId(ADDRESS_ID).should(hasValue(EMPTY));
        calculatorSteps.onCalculatorCostTouchPage().inputId(AREA_ID).should(hasValue(EMPTY));
        calculatorSteps.onCalculatorCostTouchPage().inputId(FLOOR_ID).should(hasValue(EMPTY));
    }
}
