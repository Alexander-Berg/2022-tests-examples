...
public class RolesOpenPageUserTest {

    private static final String TEST_USER_ID = "/53a82cc2a1dc/";
    ...
    @Test
    @DisplayName("Видим открытие страницы пользователя под ролью")
    public void shouldSeeOpenedPage() {
        urlSteps.testing().path(MANAGEMENT).path(MANAGER).path(USER).path(TEST_USER_ID).path(OTHER)
                .queryParam(DEGRADATION_ROLE_PARAM, ADMIN_ROLE_VALUE).queryParam(ROLE_PARAM, role).open();
        lkSteps.onBasePage().h2().should(hasText(equalTo("Комментарий о человеке")));
    }
}
