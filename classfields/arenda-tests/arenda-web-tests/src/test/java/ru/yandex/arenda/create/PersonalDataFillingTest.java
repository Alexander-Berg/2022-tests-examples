...
import static ru.yandex.arenda.constants.UriPath.LK_FLATS_SEARCH;
...
public class PersonalDataFillingTest {
    ...
    @Test
    @DisplayName("Отправляем заявку с данными -> в персональных данных появляется возможность редактирования")
    public void shouldSeeSuccessDataEdit() {
    }

    @Test
    @DisplayName("Клик на «Анкета арендатора» -> ведет на расширенную анкету")
    ...
    @Test
    @DisplayName("Заполняем «Анкета арендатора» -> видим урл «/lk/tenant/flats-search/» и кнопку «Предложения на Я.Недвижимости»")
    public void shouldSeeFillAnketa() {
        ...
        urlSteps.testing().path(LK_FLATS_SEARCH).shouldNotDiffWithWebDriverUrl();
    }
}
