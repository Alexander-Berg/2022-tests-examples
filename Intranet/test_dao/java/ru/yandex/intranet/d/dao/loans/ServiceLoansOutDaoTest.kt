...
/**
 * Service loans outgoing DAO test.
 *
 * @author Dmitriy Timashov <dm-tim@yandex-team.ru>
 */
@IntegrationTest
class ServiceLoansOutDaoTest(@Autowired private val dao: ServiceLoansOutDao,
                             @Autowired private val tableClient: YdbTableClient) {

    @Test
    fun testUpsert(): Unit = runBlocking {
        val model = ServiceLoanOutModel(
    ...
    fun testUpsertMany(): Unit = runBlocking {
        val modelOne = ServiceLoanOutModel(
            tenantId = Tenants.DEFAULT_TENANT_ID,
            serviceId = 42L,
            status = LoanStatus.PENDING,
            dueAt = Instant.now().truncatedTo(ChronoUnit.MICROS),
        ...
        val modelTwo = ServiceLoanOutModel(
            tenantId = Tenants.DEFAULT_TENANT_ID,
            serviceId = 69L,
            status = LoanStatus.PENDING,
            dueAt = Instant.now().truncatedTo(ChronoUnit.MICROS),
        ...
        val resultFirstPage = dbSessionRetryable(tableClient) {
            ...
            rwTxRetryable {
                dao.getByServiceOrderByDueFirstPage(
                    txSession, Tenants.DEFAULT_TENANT_ID, serviceId = 42L, limit = 1
                )
            }
        }
    ...
    @Test
    fun testGetByServiceStatus(): Unit = runBlocking {
        val modelOne = ServiceLoanOutModel(
            tenantId = Tenants.DEFAULT_TENANT_ID,
            serviceId = 42L,
            status = LoanStatus.PENDING,
    ...
    fun testFilterServiceIdsByStatus(): Unit = runBlocking {
        ...
        val modelThree = ServiceLoanOutModel(
            tenantId = Tenants.DEFAULT_TENANT_ID,
            serviceId = 123L,
            status = LoanStatus.SETTLED,
            dueAt = Instant.now().truncatedTo(ChronoUnit.MICROS),
