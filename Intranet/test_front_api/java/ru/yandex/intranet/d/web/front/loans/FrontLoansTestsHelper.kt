...
class FrontLoansTestsHelper(
    ...
    suspend fun searchLoans(
        ...
        from: String? = null,
        limit: Int? = null
    ): FrontSearchLoansResponseDto = webClient
        .mutateWith(MockUser.uid(USER_1_UID))
        .post()
        .uri("/front/loans/_search")
        .bodyValue(FrontSearchLoansRequestDto(serviceId, status, direction, from, limit))
        .accept(MediaType.APPLICATION_JSON)
        .exchange()
        .expectStatus()
        .isOk
        .returnResult(FrontSearchLoansResponseDto::class.java)
        .responseBody
        .awaitSingle()!!

    suspend fun getLoansHistory(loanId: LoanId, limit: Int = 100): FrontGetLoansHistoryResponseDto = webClient
        .mutateWith(MockUser.uid(USER_1_UID))
        .get()
        .uri("/front/loans/$loanId/_history?&limit=$limit")
        .accept(MediaType.APPLICATION_JSON)
        .exchange()
        .expectStatus()
        .isOk
        .returnResult(FrontGetLoansHistoryResponseDto::class.java)
        .responseBody
        .awaitSingle()!!
    ...
    suspend fun getLoansHistoryNextPage(
        ...
        pageToken: String,
        limit: Int = 100
    ): FrontGetLoansHistoryResponseDto = webClient
        .mutateWith(MockUser.uid(USER_1_UID))
        .get()
        .uri("/front/loans/$loanId/_history?&limit=$limit&pageToken=$pageToken")
        .accept(MediaType.APPLICATION_JSON)
        .exchange()
        .expectStatus()
        .isOk
