...
/**
 * Test for cron job to sync providers accounts and quotas.
 *
 * @author Dmitriy Timashov <dm-tim@yandex-team.ru>
 */
@IntegrationTest
public class SyncAccountsJobTest {
    ...
    @Autowired
    private SyncAccountsParametersSource syncAccountsParametersSource;
    @Autowired
    private ProvidersSyncStatusDao providersSyncStatusDao;
    ...
    public void testSync() {
        ...
        tableClient.usingSessionMonoRetryable(session -> session.usingTxMonoRetryable(
                TransactionMode.SERIALIZABLE_READ_WRITE, txSession -> providersDao.upsertProviderRetryable(txSession,
                                provider))).block();
        tableClient.usingSessionMonoRetryable(session -> session.usingTxMonoRetryable(
        ...
        stubProviderService.setListAccountsResponses(List.of(GrpcResponse
                .success(ListAccountsResponse.newBuilder()
                        .addAccounts(Account.newBuilder()
                                ...
                                .build())
                        .build())));
        Optional<String> syncParam = syncAccountsParametersSource.getAllParamValues().stream()
                .filter(s -> s.equals(provider.getId())).findFirst();
        Assertions.assertTrue(syncParam.isPresent(), "Provider must be in sync params list");
        SyncAccountsJob spy = Mockito.spy(syncAccountsJob);
        Mockito.when(spy.getParam()).thenReturn(syncParam.get());
        spy.execute();
        ...
        Optional<ProvidersSyncStatusModel> syncStatus = tableClient.usingSessionMonoRetryable(session -> session
                .usingTxMonoRetryable(TransactionMode.SERIALIZABLE_READ_WRITE, txSession ->
                        providersSyncStatusDao.getById(txSession, provider.getId(), provider.getTenantId())
                )).block();
        Assertions.assertNotNull(syncStatus);
        Assertions.assertTrue(syncStatus.isPresent());
        Assertions.assertEquals(SyncStatuses.DONE_OK, syncStatus.get().getLastSyncStatus());
        Assertions.assertNotNull(syncStatus.get().getLastSyncStats());
        Assertions.assertEquals(1L, syncStatus.get().getLastSyncStats().getAccountsCount());
        Assertions.assertEquals(1L, syncStatus.get().getLastSyncStats().getQuotasCount());
