...
@patch('ok.utils.abc_repo.AbcWorker._iter_abc', lambda x, y, z: iter(DUTY_DATA))
def test_get_duty():
...
def test_extend_approvers(client, merge_to_right):
    flow1 = FlowFactory(
        ...
        code=(
            ...
            'add_approvers_group('
            '["flow1_group_approver1", "flow1_group_approver2"], is_with_deputies=True'
            ')\n'
            'flow2_result = execute_flow("flow2")[\'stages\']\n'
            f'extend_approvers(flow2_result, merge_to_right={merge_to_right})\n'
        ),
    )
    FlowFactory(
        name='flow2',
        code=(
            'add_approver("flow2_approver")\n'
            'add_approver("flow1_approver")\n'
            'add_approvers_group(['
    ...
    if not merge_to_right:
        assert resp_content == [
            {'approver': 'flow1_approver'},
            {
                ...
                'stages': [
                    {'approver': 'flow1_group_approver1'},
            ...
            {'approver': 'flow2_approver'},
            {
                ...
                'stages': [
                    {'approver': 'flow2_group_approver1'},
