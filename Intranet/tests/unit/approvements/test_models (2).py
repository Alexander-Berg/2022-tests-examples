...
def _update_and_check_current_stages(approvement, stages):
    ...
    assert list(approvement.stages.current().leaves()) == stages


# TODO: выпилить фикстуру и поменять все тесты на тестирование next_stages
@pytest.fixture
def check_current_stages():
    yield _update_and_check_current_stages
...
def test_next_stages_with_deputies():
    approvement = f.ApprovementFactory(is_parallel=False)
    complex_stage = f.create_parent_stage(
        approvement=approvement,
        need_approvals=1,
        is_with_deputies=True,
    )
    main_child_stage = f.create_child_stage(complex_stage)
    deputy_child_stage = f.create_child_stage(complex_stage)  # noqa
    f.ApprovementStageFactory(approvement=approvement)
    f.create_complex_stage(['c1', 'c2'], need_approvals=2)

    assert list(approvement.next_stages) == [complex_stage, main_child_stage]


def test_next_stages_parallel_with_deputies():
    approvement = f.ApprovementFactory(is_parallel=True)
    simple_stage = f.ApprovementStageFactory(approvement=approvement)
    complex_stage = f.create_parent_stage(
        approvement=approvement,
        need_approvals=2,
        is_with_deputies=True,
    )
    main_child_stage_1 = f.create_child_stage(complex_stage)
    main_child_stage_2 = f.create_child_stage(complex_stage)
    deputy_child_stage = f.create_child_stage(complex_stage)  # noqa

    next_stages = [simple_stage, complex_stage, main_child_stage_1, main_child_stage_2]
    assert list(approvement.next_stages) == next_stages
