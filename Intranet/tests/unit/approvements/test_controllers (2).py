...
from ok.approvements.controllers import (
    ...
    _get_user_data_by_login,
    enrich_stages_approvers,
)
...
from tests.utils.assertions import assert_history_exists, assert_not_raises
...
def test_approvement_create(mocked_ping_approvers, mocked_update_issue,
    ...
    assert approvement.groups == group_urls
...
def test_close(mocked_notification, mocked_update_issue, stage_status):
    ...
    mocked_update_issue.assert_called_once_with(
        approvement.id,
        [IssueFieldEnum.APPROVEMENT_STATUS, IssueFieldEnum.APPROVEMENT_ID],
    )
...
def test_cancelled_notification_enabled_by_switch(mocked_notification):
    ...
    current_stages = list(approvement.stages.current().leaves())
...
def test_get_user_data_by_login():
    f.UserFactory(username='a', first_name_en='Andrew', last_name_en='Smith')
    f.UserFactory(username='b', affiliation='external')

    result = _get_user_data_by_login(['a', 'b', 'x'])
    ...
    assert result['b']['affiliation'] == 'external'
    assert 'x' not in result


@pytest.mark.parametrize('strict, create_x, raises', (
    pytest.param(False, False, False, id='not-strict'),
    pytest.param(True, True, False, id='strict-ok'),
    pytest.param(True, False, True, id='strict-fail'),
))
def test_enrich_stages_approvers(strict, create_x, raises):
    defined_stages = _('a', _('b1', 'x', 'b2', n=1), 'x', 'c')['stages']
    f.UserFactory(username='a', first_name_en='Andrew', last_name_en='Smith')
    f.UserFactory(username='b1', first_name_en='Billy', last_name_en='Jean', affiliation='external')
    f.UserFactory(username='b2', first_name_en='Billy', last_name_en='Joe')
    f.UserFactory(username='c', first_name_en='Curtis', last_name_en='Jones')
    if create_x:
        f.UserFactory(username='x')

    # Здесь используем вспомогательную функцию – выше мы её саму уже протестили
    data_by_login = _get_user_data_by_login(['a', 'b1', 'b2', 'c', 'x'])

    expected_stages = [
        {'approver': data_by_login['a']},
        {
            'stages': [{'approver': data_by_login['b1']}, {'approver': data_by_login['b2']}],
            'need_approvals': 1,
            'need_all': False,
            'is_with_deputies': False,
        },
        {'approver': data_by_login['c']},
    ]
    if create_x:
        x = {'approver': data_by_login['x']}
        expected_stages[1]['stages'].insert(1, x)
        expected_stages.insert(2, x)

    raises_exception = pytest.raises if raises else assert_not_raises
    with raises_exception(AssertionError):
        result = enrich_stages_approvers(defined_stages, strict=strict)
    assert raises or result == expected_stages
