...
import static ru.yandex.partner.core.validation.defects.ids.TypeDefectIds.DATA_MUST_BE_BOOLEAN;

@CoreTest
public class BlockWithCustomBkDataFlagValidationTest extends BaseValidationTest {
    @Autowired
    BlockTypedRepository repository;

    @Test
    void validateIncorrectIsCustomBkData() {
        RtbBlock block = (RtbBlock) repository.getBlockByCompositeId(347649081345L);

        block.setIsCustomBkData(null);
        ...
        assertThat(defect.getDefect().defectId()).isEqualTo(DATA_MUST_BE_BOOLEAN);
    }
}
